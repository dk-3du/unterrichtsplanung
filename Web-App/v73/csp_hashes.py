...
aus** — der Hinweis steht allein in der Entwicklerkonsole.
  ...
  python3 csp_hashes.py --selbsttest    prüft den Wächter selbst: jede Aufweichung fällt auf

Geprüft werden nicht nur die Hashes, sondern die ganze Policy gegen ihr Soll
(SOLL_POLICY): genau diese Direktiven mit genau diesen Quellen, keine zweimal,
keine dazu — ein `connect-src *` oder ein `script-src-elem` fiele sonst durch.
...
Nur Standardbibliothek; wie alle Prüfskripte dieses Ordners für Python 3.10
oder neuer (geprüft mit 3.14.7, auch python3.14t).
"""
...
#: Die Policy, wie sie sein soll — jede Direktive mit genau diesen Quellen;
#: None heißt: nur Hashes der eigenen Blöcke ('sha256-…').
SOLL_POLICY = {
    'default-src': ["'none'"],
    'script-src': None,
    'style-src': None,
...
def policy_abweichungen(policy):
    """Was an der Policy vom Soll abweicht — leer, wenn nichts. Geprüft werden
    Direktiven und Quellen; ob die Hashwerte zu den Blöcken passen, prüft
    `pruefen` daneben."""
    gesehen = {}
    abweichungen = []
    for teil in policy.split(';'):
        ...
        name = worte[0].lower()
        if name in gesehen:
            abweichungen.append('Direktive %s steht zweimal' % name)
            continue
        gesehen[name] = worte[1:]
    for name in gesehen:
        if name not in SOLL_POLICY:
            abweichungen.append('Direktive %s ist nicht vorgesehen' % name)
    for name, quellen in SOLL_POLICY.items():
        if name not in gesehen:
            abweichungen.append('Direktive %s fehlt' % name)
            continue
        ist = gesehen[name]
        if quellen is None:
            fremde = [q for q in ist if not (q.startswith("'sha256-") and q.endswith("'"))]
            if fremde or not ist:
                abweichungen.append('%s darf nur Hashes tragen, hat: %s'
                                    % (name, ' '.join(fremde) if fremde else 'nichts'))
        elif sorted(q.lower() for q in ist) != sorted(quellen):
            abweichungen.append('%s soll %s sein, ist %s'
                                % (name, ' '.join(quellen), ' '.join(ist) if ist else 'leer'))
    return abweichungen


def selbsttest():
    """Der Wächter selbst: Die Soll-Policy geht durch, jede Aufweichung fällt auf."""
    hashes = "script-src 'sha256-AAAA'; style-src 'sha256-BBBB'"
    gut = ("default-src 'none'; " + hashes + "; img-src 'self' data:; connect-src 'none'; "
           "form-action 'none'; base-uri 'none'; frame-src 'none'; object-src 'none'; "
           "manifest-src 'self'")
    faelle = [
        ('Soll-Policy', gut, False),
        ('connect-src aufgeweicht', gut.replace("connect-src 'none'", 'connect-src *'), True),
        ('Direktive dazu (script-src-elem)', gut + "; script-src-elem 'unsafe-inline'", True),
        ('Direktive doppelt', gut + "; default-src *", True),
        ('Direktive fehlt', gut.replace("; object-src 'none'", ''), True),
        ('fremde Quelle in script-src',
         gut.replace("script-src 'sha256-AAAA'", "script-src 'sha256-AAAA' 'unsafe-inline'"), True),
        ('style-src ohne Hash', gut.replace("style-src 'sha256-BBBB'", "style-src 'self'"), True),
        ('img-src erweitert', gut.replace("img-src 'self' data:", 'img-src *'), True),
        ('base-uri aufgeweicht', gut.replace("base-uri 'none'", "base-uri 'self'"), True),
        ('Gross-/Kleinschreibung gleichgueltig', gut.replace('default-src', 'Default-Src'), False),
    ]
    fehler = 0
    for name, policy, soll_auffallen in faelle:
        auffaellig = bool(policy_abweichungen(policy))
        if auffaellig != soll_auffallen:
            fehler += 1
            print('  FEHLER  %s' % name)
        else:
            print('  stimmt  %s' % name)
    print('Selbsttest: %s' % ('bestanden' if not fehler else '%d Fehler' % fehler))
...
def pruefen(beschriftung, quelltext, nach_tag, fundstellen, policy):
    ...
    for abweichung in policy_abweichungen(policy):
        print('  >>> Policy weicht vom Soll ab: %s <<<' % abweichung)
        heil = False
...
def main():
    ...
    zerleger.add_argument('--selbsttest', action='store_true',
                          help='den Waechter selbst pruefen: jede Aufweichung der Policy faellt auf')
