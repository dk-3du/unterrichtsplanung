...
"""
    ...
    gesamt = (len(FAELLE) + len(STATUSFAELLE) + len(OEFFNENFAELLE) + len(MERKFAELLE) + len(SCHREIBFAELLE)
              + len(WIEDERAUFNAHMEFAELLE))
    if fehler:
    ...
    print(f"\n{GELUNGEN} ({len(FAELLE)} Faelle, dazu {len(STATUSFAELLE)} zur Fassung der Statusdatei, "
          f"{len(OEFFNENFAELLE)} zur Schranke vor dem Lesen, {len(MERKFAELLE)} zum Merken und "
