...
def statusFaelleLaufen(skript: str, deklarationen: str) -> int:
    ...
    for name in ("statusstandLesen", "statusstandDeuten", "statusFassungGilt", "istBehaelter",
                 "statusstandOeffnen"):
        # Die Klammerzaehlung beginnt bei `function` — das `async` davor kommt mit.
        vorsatz = "async " if f"async function {name}(" in skript else ""
        probe += vorsatz + funktion(skript, name) + "\n"
...
"""
    ...
    if fehler:
        print(f"\n{fehler} von {len(FAELLE) + len(STATUSFAELLE) + len(OEFFNENFAELLE)} Faellen "
              "weichen vom Leser der App ab.")
        return 1
    print(f"\n{GELUNGEN} ({len(FAELLE)} Faelle, dazu {len(STATUSFAELLE)} zur Fassung der Statusdatei "
          f"und {len(OEFFNENFAELLE)} zur Schranke vor dem Lesen)")
    return 0
