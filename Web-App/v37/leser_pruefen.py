...
def funktion(quelle: str, name: str) -> str:
    """Eine Funktion samt Rumpf, per Klammerzählung ab `function <name>(`."""
    anfang = quelle.index(f"function {name}(")
    return block(quelle, f"function {name}(", f"function {name}(")[len(quelle[anfang:anfang]):]


# Die Fassung der Statusdatei: dieselben Faelle wie „Die Fassung wird geprüft“
# in DateiPruefungen.swift — fehlend gilt, 1 gilt, alles andere nicht.
STATUSFAELLE = [
    ("Fassung 1", '{"version":1,"eintraege":{}}', "gelesen"),
    ("Fassung 1.0", '{"version":1.0,"eintraege":{}}', "gelesen"),
    ("ohne Fassung (Altbestand)", '{"eintraege":{}}', "gelesen"),
    ("Fassung 0", '{"version":0,"eintraege":{}}', "fehler"),
    ("Fassung 2", '{"version":2,"eintraege":{}}', "fehler"),
    ("Fassung 9999", '{"version":9999,"eintraege":{}}', "fehler"),
    ("Fassung als Zeichenkette", '{"version":"1","eintraege":{}}', "fehler"),
    ("Fassung als Wahrheitswert", '{"version":true,"eintraege":{}}', "fehler"),
    ("Fassung null", '{"version":null,"eintraege":{}}', "fehler"),
    ("Fassung 1.5", '{"version":1.5,"eintraege":{}}', "fehler"),
    ("Fassung 1e100", '{"version":1e100,"eintraege":{}}', "fehler"),
    ("kein Statusstand", '{"x":1}', "null"),
    ("kein JSON", 'kein JSON', "null"),
    ("leer", '', "null"),
]
...
def statusFaelleLaufen(skript: str, deklarationen: str) -> int:
    ...
    if not konstante:
        ...
        return 1
    probe = deklarationen + "\n" + konstante.group(0) + "\n"
    probe += funktion(skript, "statusstandLesen") + "\n" + funktion(skript, "statusFassungGilt") + "\n"
    probe += f"""
...
for (const [name, text] of STATUSFAELLE) {{
  ...
  try {{
    ...
    ergebnis = fehler instanceof Planungsfehler ? "fehler" : "Ausnahme: " + String(fehler);
  }}
...
"""
    ...
    fehler = 0
    for name, _text, erwartet in STATUSFAELLE:
...
"""
    ...
    fehler += statusFaelleLaufen(skript.group(1), teil)

    if fehler:
        print(f"\n{fehler} von {len(FAELLE) + len(STATUSFAELLE)} Faellen weichen vom Leser der App ab.")
        return 1
