...
def eintrag(i, ids, wochen):
    ...
    # Korrektur: nie neben einer Prüfung — trägt eine Datei beides, gilt die Prüfung.
    if random.random() < 0.4:
        e["korrektur"] = wahl([True, False, "", 0, 2, "x", BOM, None])
    if random.random() < 0.4:
...
func abzug(_ daten: Data) -> [String] {
    ...
    for e in planung.eintraege {
        ...
        zeilen.append("eintrag=\(norm(e.id))|\(norm(e.klasseId))|\(e.woche)|\(esc(e.titel))|"
                      ...
                      + "\(e.datum?.iso ?? "")|\(e.wochentag?.rawValue ?? 0)|\(e.dringend)|\(e.korrektur)|"
...
function abzug(text) {
  ...
  for (const e of p.eintraege) {
    ...
    z.push(`eintrag=${norm(e.id)}|${norm(e.klasseId)}|${e.woche}|${esc(e.titel)}|${esc(e.text)}|${e.erledigt}|${e.pruefung}|${e.pruefungstagISO}|${e.datumISO}|${tag}|${e.dringend}|${e.korrektur}|${esc(e.kommentar)}|${esc(e.statusGeaendert)}|${e.hausaufgabe}|${esc(e.hausaufgabenText)}|${material}`);
  }
...
DOPPELT = [
    ...
    ("doppelt-eintraege",
     ...
     "eintrag=e1|k1|0|A||false|false|||0|false|false|||", "eintrag=e2|k1|1|B||false|false|||0|false|false|||"),
    ("doppelt-version",
...
def laufen(befehl: list, frist: int) -> subprocess.CompletedProcess:
    """Ein Kindprozess mit Frist in Sekunden — ein haengender Lauf haelt die Pruefung nicht ewig (B15)."""
    try:
        return subprocess.run(befehl, capture_output=True, text=True, timeout=frist)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as ordner:
        ...
        bau = laufen(["swiftc", "-O", "-o", str(ordner / "abzug_swift"),
                      str(ordner / "main.swift")] + quellen, frist=600)
        if bau.returncode != 0:
        ...
        sw = laufen([str(ordner / "abzug_swift")] + dateien, frist=300)
        if sw.returncode != 0:
        ...
        js = laufen([str(JSC), str(ordner / "probe.js")], frist=300)
        if js.returncode != 0:
