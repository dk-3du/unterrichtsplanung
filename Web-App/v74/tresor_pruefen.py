...
* die App-Seite als kleines Programm — `Tresor.swift`, `Ablage.swift` und
  `Modell/Pruefuhr.swift` der
...
def cli_bauen(arbeitsordner: Path) -> Path:
    """Übersetzt Tresor.swift, Ablage.swift und Modell/Pruefuhr.swift samt Hauptprogramm."""
    ziel = arbeitsordner / "tresor_cli"
    if ziel.exists():
        return ziel
    # `Ablage` fragt die Modellschicht, ob dieser Prozess ein Prüfziel ist
    # (`Pruefziel.ja`) — die eine Regel, die auch die `Pruefuhr` braucht.
    quellen = [DIENSTE / "Tresor.swift", DIENSTE / "Ablage.swift",
               DIENSTE.parent / "Modell" / "Pruefuhr.swift"]
    for q in quellen:
        if not q.exists():
...
def probeplanung() -> bytes:
    """Eine kleine, gültige Planung — mit Umlaut, einem Link und einer Korrektur."""
    p = {
        ...
        "klassen": [{"id": "k-1", "name": "G6a", "fach": "Informatik", "notiz": "",
                     ...
                     "unterrichtstage": [1, 3]},
                    # Eine Sonderzeile (v69, E159): so sieht jede Runde die Zeile am Paket.
                    {"id": "k-2", "name": "Klassenleitung", "art": "klassenleitung", "fach": "G6a",
                     "notiz": "", "farbe": 0, "farbeManuell": False, "verwaltung": "",
                     "curriculum": "", "unterrichtstage": []}],
        "zellenfrei": [],
        "eintraege": [{"id": "e-1", "klasseId": "k-1", "woche": 1, "titel": "Bits und Bytes",
                       ...
                       "korrektur": False, "kommentar": "", "statusGeaendert": ""},
                      # Eine Korrektur: so liest jede Runde das Merkmal am Paket — aus
                      # dem Klartext (Lauf klick). Der Behälter führt sie auch; nach
                      # dem Entsperren liest derselbe Dateileser.
                      {"id": "e-2", "klasseId": "k-1", "woche": 2, "titel": "Test korrigieren",
                       "text": "", "erledigt": False, "materialien": [], "links": [],
                       "pruefung": False, "pruefungstag": "", "datum": "", "dringend": False,
                       "korrektur": True, "kommentar": "", "statusGeaendert": ""}],
    }
