...
--schreiben sichert die bisherige Fassung in den Temporaerordner und setzt die
neue ueber eine Nachbardatei per os.replace an ihre Stelle; ein Abbruch
mittendrin laesst die alte Datei unversehrt.
...
def hashwert(text):
    ...
    roh = hashlib.sha256(text.encode('utf-8')).digest()
...
def direktive_setzen(policy, name, hashes):
    """Ersetzt nur die genannte Direktive und lässt die übrige Policy unberührt."""
    neu = name + ' ' + ' '.join("'%s'" % h for h in hashes)
    teile = [t.strip() for t in policy.split(';') if t.strip()]
    for i, teil in enumerate(teile):
        if teil.split()[0].lower() == name:
            teile[i] = neu
            break
    else:
        teile.append(neu)
...
def ersetzend_schreiben(pfad, text):
    ...
    Die Sicherung liegt im Temporaerordner, nicht daneben: Dieser Ordner wird
    als Ganzes ins Netz geladen, eine .bak-Kopie der Seite ginge mit hoch."""
    sicherung = Path(tempfile.gettempdir()) / (pfad.name + '.bak')
    shutil.copy2(pfad, sicherung)

    neben = pfad.with_name(pfad.name + '.neu')
    with open(neben, 'w', encoding='utf-8', newline='') as datei:
        datei.write(text)
        datei.flush()
