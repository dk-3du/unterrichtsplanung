...
def funktion_ziehen(quelle: str, name: str) -> str:
    anfang = quelle.index(f"function {name}(")
    tiefe = 0
    start = quelle.index("{", anfang)
    for stelle in range(start, len(quelle)):
        if quelle[stelle] == "{":
            ...
            tiefe -= 1
            if tiefe == 0:
                return quelle[anfang:stelle + 1]
    raise SystemExit(f"Funktion {name} endet nicht — Klammern unausgeglichen.")
...
"""
    ablage = HIER / ".schulwochen_probe.js"
    ablage.write_text(probe, encoding="utf-8")
    try:
