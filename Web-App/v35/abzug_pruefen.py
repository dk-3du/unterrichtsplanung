...
behält den ersten, `JSON.parse` den letzten; ein Erzeuger kann so etwas nicht
bauen, weil ein Python-Wörterbuch keine doppelten Schlüssel kennt) und kanonisch
äquivalente Schlüssel (é gegen e + U+0301: Swift hält beide für einen).
...
def text():
    return wahl(SONDER) if random.random() < 0.4 else wahl(["Titel", "5a", "Bio", "Info", "G6a"])


def klasse(i):
    k = {"id": wahl(["k1", "k2", "k3", "k1\n", "\tk1 ", "", None, 7, "k%d" % i])
...
def eintrag(i, ids, wochen):
    ...
    if random.random() < 0.4:
        e["kommentar"] = wahl(["Kommentar", "", "k" * 25000, None])
    if random.random() < 0.3:
...
def planung(i):
    ...
    if random.random() < 0.5:
        p["ferien"] = [{"id": wahl(["f1", "", None]), "name": text(),
                        "von": wahl(DATEN), "bis": wahl(DATEN)}
                       for _ in range(random.randint(0, 3))]
    if random.random() < 0.5:
        p["sperrzeiten"] = [{"id": wahl(["s1", ""]), "name": text(),
                             "von": wahl(DATEN), "bis": wahl(DATEN),
                            ...
                            for _ in range(random.randint(0, 3))]
    if random.random() < 0.6:
        p["fachfarben"] = {wahl(SONDER + ["mathe", "Mathe", "bio"]): wahl(TOENE)
                           for _ in range(random.randint(0, 5))}
    if random.random() < 0.4:
        p["zellenfrei"] = [wahl([{"klasseId": wahl(ids + ["k9"]), "woche": wahl(DATEN)}, "x", {}])
                           for _ in range(random.randint(0, 4))]
    return p
...
func abzug(_ daten: Data) -> [String] {
    ...
    for e in planung.eintraege {
        ...
        zeilen.append("eintrag=\(norm(e.id))|\(norm(e.klasseId))|\(e.woche)|\(esc(e.titel))|"
                      ...
                      + "\(esc(e.kommentar))|\(esc(e.statusGeaendert))|\(material)")
    }
...
function abzug(roh) {
  ...
  for (const e of p.eintraege) {
    ...
    z.push(`eintrag=${norm(e.id)}|${norm(e.klasseId)}|${e.woche}|${esc(e.titel)}|${esc(e.text)}|${e.erledigt}|${e.pruefung}|${e.pruefungstagISO}|${e.datumISO}|${tag}|${e.dringend}|${esc(e.kommentar)}|${esc(e.statusGeaendert)}|${material}`);
  }
  z.push("bilanz=" + esc(p.uebergangen));
  return z;
}
const AUS = [];
ALLE.forEach((roh, i) => { AUS.push("### " + String(i).padStart(4, "0") + ".json"); AUS.push(...abzug(roh)); });
print(AUS.join("\n"));
'''
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as ordner:
        ...
        for i, p in enumerate(alle):
            (fallordner / ("%04d.json" % i)).write_text(json.dumps(p, ensure_ascii=False),
                                                        encoding="utf-8")
        ...
        probe = (teil + "\nconst ALLE = JSON.parse("
                 + json.dumps(json.dumps(alle, ensure_ascii=False), ensure_ascii=False)
                 + ");\n" + JS_ABZUG)
    ...
    if abweichend:
        print(f"\n{abweichend} von {len(app)} Faellen weichen ab (Saat {args.saat}).")
        print("Erste abweichende Zeile, nach Feld:")
        for k, v in sorted(ursachen.items(), key=lambda kv: -kv[1]):
            print(f"  {v:5d}  {k}")
        return 1
    print(f"\n{GELUNGEN} ({len(app)} Faelle, Saat {args.saat})")
    return 0
