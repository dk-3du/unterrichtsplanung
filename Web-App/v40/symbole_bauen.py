...
`Beiwerk/AppIcon.icon` der macOS-App — die SVG-Ebenen und `icon.json`. Gesucht
wird es in der höchsten Fassung unter `../../macOS-App/v<N>/`, damit der Pfad
einen Versionssprung übersteht.
...
def hoechstesSymbolbuendel(wurzel=APPWURZEL):
    """Das Symbolbündel der höchsten macOS-Fassung, die eines mitbringt.

    Fest verdrahtet zeigte der Pfad nach jedem Versionssprung ins Leere; gesucht
    wird deshalb unter `macOS-App/v<N>/`. Verglichen wird die **Zahl**, nicht der
    Name — sonst stünde v9 über v10.
    """
    gefunden = []
    for ordner in wurzel.glob("v*"):
        treffer = re.fullmatch(r"v(\d+)", ordner.name)
        buendel = ordner / "Beiwerk" / "AppIcon.icon"
        if treffer and buendel.is_dir():
            gefunden.append((int(treffer.group(1)), buendel))
    if not gefunden:
        return None
    return max(gefunden, key=lambda eintrag: eintrag[0])[1]


SYMBOLBUENDEL = hoechstesSymbolbuendel()
...
def main():
    ...
    if SYMBOLBUENDEL is None:
        raise SystemExit(
            "FEHLER: unter %s liegt keine Fassung mit Beiwerk/AppIcon.icon." % APPWURZEL)
