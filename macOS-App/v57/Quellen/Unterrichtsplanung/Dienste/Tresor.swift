...
final class Tresor: @unchecked Sendable {
    ...
    /// Für den Formwächter (E72), der zuerst nur den Kopf liest: Kündigen die
    /// ersten Byte einen Behälter an, wie diese App ihn schreibt? Ein „ja“ ist
    /// belegt; ein „nein“ heißt nur, dass der Kopf nicht entscheidet — dann
    /// deutet der Wächter weiter (N55-01).
    static func kopfIstBehaelter(_ kopf: Data) -> Bool { kopf.starts(with: kopfvorsatz) }

    /// Beginnt hier überhaupt ein JSON-Objekt? Dieselbe Vorprüfung, die
    /// `istBehaelter` anstellt — eine Regel an einer Stelle, damit Wächter und
    /// Leser nicht auseinanderlaufen (E77). Was sie verneint, ist auch für den
    /// Leser kein Behälter.
    static func kopfKoennteBehaelter(_ kopf: Data) -> Bool {
        kopf.prefix(64).first(where: { $0 != 0x20 && $0 != 0x0a && $0 != 0x0d && $0 != 0x09 })
            == UInt8(ascii: "{")
    }

    /// Die Schlüsselkennung **an ihrer Stelle** im eigenen Kopf — `nil`, wenn
    /// der Kopf nicht genau so anfängt, wie `behaelterSchreiben` ihn setzt.
    /// `nil` heißt hier nicht „kein Behälter“, sondern „der Kopf beantwortet es
    /// nicht“; dann deutet der Aufrufer die ganze Datei wie der Leser (E75).
    ///
    /// Gelesen wird der Anfang Feld für Feld — `typ`, `version`, `inhalt`,
    /// `schluesselkennung` —, nicht nach der Zeichenfolge gesucht. Nur so steht
    /// die gefundene Kennung beweisbar im obersten Objekt und ist die erste
    /// ihres Namens, die auch `JSONSerialization` nähme. Eine Suche fände eine
    /// Kennung, die in einem eingebetteten Objekt davorsteht, und der Wächter
    /// spräche dann über einen anderen Schlüssel als der Leser (N56-01, am Lauf
    /// nachgestellt: der fremde Schlüssel durfte schreiben, der richtige nicht).
    static func kennungImKanonischenKopf(_ kopf: Data) -> String? {
        var lauf = kopf[...]

        func abtragen(_ wortlaut: String) -> Bool {
            let byte = Data(wortlaut.utf8)
            guard lauf.starts(with: byte) else { return false }
            lauf = lauf.dropFirst(byte.count)
            return true
        }

        func abtragen(solange erlaubt: (UInt8) -> Bool) -> Data {
            let genommen = lauf.prefix(while: erlaubt)
            lauf = lauf.dropFirst(genommen.count)
            return Data(genommen)
        }

        guard abtragen("{\"typ\":\"\(typ)\",\"version\":"),
              !abtragen(solange: { $0 >= UInt8(ascii: "0") && $0 <= UInt8(ascii: "9") }).isEmpty,
              abtragen(",\"inhalt\":\""),
              !abtragen(solange: { $0 >= UInt8(ascii: "a") && $0 <= UInt8(ascii: "z") }).isEmpty,
              abtragen("\",\"schluesselkennung\":\"")
        else { return nil }
        let roh = abtragen(solange: { $0 >= UInt8(ascii: "0") && $0 <= UInt8(ascii: "9")
                                      || $0 >= UInt8(ascii: "a") && $0 <= UInt8(ascii: "f") })
        guard roh.count == kennungLaenge * 2, abtragen("\""),
              let text = String(data: roh, encoding: .utf8)
        else { return nil }
        // Steht das Feld ein zweites Mal im Kopf, entscheidet der Kopf nicht:
        // Welche von zwei gleichnamigen Angaben gilt, ist eine Frage an den
        // Deuter, nicht an eine Abkürzung. (Gemessen nimmt `JSONSerialization`
        // die erste — aber darauf baut hier nichts; der Umweg über `kopfLesen`
        // gibt dieselbe Antwort wie der Leser, welche es auch sei.)
        guard lauf.range(of: kennungsfeld) == nil else { return nil }
        return text
    }

    private static let kennungsfeld = Data("\"schluesselkennung\":\"".utf8)
    ...
    static func istBehaelter(_ roh: Data) -> Bool {
        if kopfIstBehaelter(roh) { return true }
        guard kopfKoennteBehaelter(roh),
