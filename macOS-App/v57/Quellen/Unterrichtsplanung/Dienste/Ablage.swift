...
final class Ablage: @unchecked Sendable {
    ...
    /// des Lesers an der ganzen Datei. „Allein belegt“ heißt: der Anfang ist
    /// Wort für Wort der eigene (`kennungImKanonischenKopf`), oder er ist gar
    /// kein Objekt. Alles dazwischen — ein umformatierter Behälter, ein Kopf
    /// mit fremden Feldern davor — ist damit nicht beantwortet; ihn deshalb zu
    /// sperren, hieße die Abkürzung zur Regel machen und spräche die Sitzung
    /// mit dem richtigen Schlüssel schuldig (nachgemessen).
    private static func ausDemKopf(_ kopf: Data) -> Form? {
        ...
        return Tresor.kennungImKanonischenKopf(kopf).map(Form.behaelter)
    }
