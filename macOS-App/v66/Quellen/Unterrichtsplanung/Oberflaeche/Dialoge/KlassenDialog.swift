...
private struct Sitzplanmenue: View {
    ...
    var body: some View {
        Menu {
            ...
            Image(systemName: plan != nil ? Zeichen.sitzplanGesetzt : Zeichen.sitzplan)
                .foregroundStyle(plan != nil ? Color.accentColor : Color.secondary)
        }
        .menuStyle(.borderlessButton)
        .menuIndicator(.hidden)
        .frame(width: Kursspalten.knopf)
        .disabled(!speicher.sitzplaene.schreibbar)
        .help(speicher.sitzplaene.sperrhinweis
              ?? plan.map { "Sitzplan: \($0.tische.count) " + ($0.tische.count == 1 ? "Platz" : "Plätze") }
              ?? "Sitzplan anlegen")
        .accessibilityLabel(plan != nil ? "Sitzplan von \(klasse.name)"
