...
final class Rasterkoordinator: NSObject, NSCollectionViewDataSource, NSCollectionViewDelegate {
    ...
    private func touranker(_ schritt: Planungsspeicher.Tourschritt) -> Tourfuehrer.Anker? {
        ...
        if schritt == .kurszelle {
            // Die Kurszelle liegt außerhalb des Rollbereichs — kein Element des
            // Sammelblatts; die Karte hängt an der Kursspalte der Bühne.
            tourzelleFreigeben()
            buehne?.layoutSubtreeIfNeeded()
            speicher.tourZelle = zeile.klasse.name
            return buehne?.klassenkopfAnker(hoehe: zeile.hoehe)
        }
    ...
    private func kachelmenue(_ id: String, kannHoch: Bool, kannRunter: Bool) -> NSMenu? {
        ...
        if betroffen == 1 {
            ...
            for zeile in Rasterkoordinator.merkmalzeilen(vorhanden) {
                menue.addItem(schalter(zeile.gesetzt, ein: zeile.ein, aus: zeile.aus,
                                       gesperrt: zeile.gesperrt) { [weak self] in
                    switch zeile.merkmal {
                    case .pruefung: self?.speicher.pruefungUmschalten(id)
                    case .dringlich: self?.speicher.dringlichUmschalten(id)
                    case .korrektur: self?.speicher.korrekturUmschalten(id)
                    }
                })
            }
            menue.addItem(schalter(vorhanden?.hausaufgabe == true,
                                   ein: "Hausaufgabe entfernen",
                                   aus: "Hausaufgabe hinzufügen") { [weak self] in
                self?.speicher.hausaufgabeUmschalten(id)
            })
            menue.addItem(.separator())
            if kannHoch {
    ...
    /// Die Schalter für Prüfung, Dringlichkeit und Korrektur im Rechtsklickmenü
    /// — als Werte, damit sich die Regel prüfen lässt: Ein Vorhaben ist
    /// entweder Prüfung oder Korrektur; der Eintrag des jeweils anderen ist
    /// ausgegraut, statt still etwas wegzunehmen.
    enum Merkmal { case pruefung, dringlich, korrektur }

    struct Merkmalzeile: Equatable {
        let merkmal: Merkmal
        let gesetzt: Bool
        let ein: String
        let aus: String
    ...
    static func merkmalzeilen(_ vorhaben: Vorhaben?) -> [Merkmalzeile] {
        // Die Reihenfolge des Vorhaben-Blatts, von oben nach unten — wer dort
        // etwas umstellt, stellt es hier mit um.
        [
            Merkmalzeile(merkmal: .dringlich, gesetzt: vorhaben?.dringend == true,
                         ein: "Nicht mehr als dringlich führen", aus: "Als dringlich kennzeichnen",
                         gesperrt: false),
            Merkmalzeile(merkmal: .korrektur, gesetzt: vorhaben?.korrektur == true,
                         ein: "Nicht mehr als Korrektur führen", aus: "Als Korrektur kennzeichnen",
                         gesperrt: vorhaben?.pruefung == true),
            Merkmalzeile(merkmal: .pruefung, gesetzt: vorhaben?.pruefung == true,
                         ein: "Nicht mehr als Prüfung führen", aus: "Als Prüfung führen",
                         gesperrt: vorhaben?.korrektur == true),
        ]
    }

    private func schalter(_ gesetzt: Bool, ein: String, aus: String, gesperrt: Bool = false,
                          _ handlung: @escaping () -> Void) -> NSMenuItem {
        let punkt = eintrag(gesetzt ? ein : aus, handlung)
        punkt.state = gesetzt ? .on : .off
        // Ohne Handlung graut AppKit den Eintrag aus.
        if gesperrt {
            punkt.action = nil
            punkt.target = nil
        }
