...
private struct Tageszeile: View {
    ...
    /// Prüfung, Hausaufgabe und Kommentar tragen ein Zeichen. „Dringlich“ steht
    ...
    private var marken: some View {
        HStack(spacing: 6) {
            ...
            if vorhaben.hausaufgabe {
                Image(systemName: Zeichen.hausaufgabe)
                    .font(.caption)
                    .foregroundStyle(Systemfarben.hausaufgabe)
                    .help(vorhaben.hausaufgabenBeschriftung)
            }
    ...
    private var nebenzeile: Text {
        ...
        if vorhaben.hausaufgabe { teile.append(vorhaben.hausaufgabenBeschriftung) }
        ...
        switch (vorhaben.dringend, vorhaben.korrektur) {
        case (false, false): return rest
        case (true, false): return Text("\(Dringlichkeitsmarke.text) · \(rest)")
        case (false, true): return Text("\(Korrekturmarke.text) · \(rest)")
        case (true, true): return Text("\(Dringlichkeitsmarke.text) · \(Korrekturmarke.text) · \(rest)")
        }
    }
    ...
    private var bedienungshilfentext: String {
        ...
        if vorhaben.korrektur { text += ", " + Korrekturmarke.wort }
        if vorhaben.hausaufgabe { text += ", " + vorhaben.hausaufgabenBeschriftung }
