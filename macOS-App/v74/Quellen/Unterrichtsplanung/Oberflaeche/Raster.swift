...
/// Was eine Sonderzeile nicht kann (E161): dieselbe Zeile wie das Angebot,
/// abgeblendet und ohne Klick — die Höhe bleibt, das Raster springt nicht.
private struct Gesperrtezeile: View {
    let text: String
    let klasse: Klasse
    let was: String
    ...
    var body: some View {
        Text(text)
            ...
            .padding(.horizontal, 5)
            .frame(height: Zellenmass.hoeheVerwaltungszeile)
            .help("Für „\(klasse.name)“ ist kein \(was) vorgesehen")
            .accessibilityLabel("\(was) für \(klasse.name) nicht vorgesehen")
    }
}
...
struct Klassenkopf: View {
    let klasse: Klasse
    /// Über der ersten Sonderzeile die kräftigere Linie (E158).
    var sonderzeilenBeginn = false
    @Environment(Planungsspeicher.self) private var speicher
    @State private var ueberfahren = false
    ...
    var body: some View {
        ...
        HStack(alignment: .top, spacing: 8) {
            VStack(alignment: .leading, spacing: 1) {
                ...
                // Drei Zeilen, immer — gesetzt, als Angebot oder, was die
                // Sonderzeile nicht kann, abgeblendet ohne Klick (E34, E161);
                // `Zellenmass.kopfhoehe` zählt dieselben.
                ForEach(Kursdateiart.allCases) { art in
                    if !klasse.erlaubt(art) {
                        Gesperrtezeile(text: art.angebot, klasse: klasse, was: art.dateibeschriftung)
                    } else if klasse.hat(art) {
                        Kursdateiverweis(klasse: klasse, art: art)
                    } else {
                        Kursdateiangebot(klasse: klasse, art: art)
                    }
                }
                if klasse.erlaubtSitzplan {
                    Sitzplanverweis(klasse: klasse)
                } else {
                    Gesperrtezeile(text: "+ Sitzplan", klasse: klasse, was: "Sitzplan")
                }
            }
            Spacer(minLength: 0)
        ...
        .overlay(alignment: .top) {
            if sonderzeilenBeginn { Systemfarben.sonderzeilenlinie.frame(height: 2) }
        }
        .onHover { ueberfahren = $0 }
        .animation(.easeOut(duration: 0.15), value: ueberfahren)
    }
