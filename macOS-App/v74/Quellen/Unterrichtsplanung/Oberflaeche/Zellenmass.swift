...
enum Zellenmass {
    ...
    private static let hausaufgabenbreite: CGFloat = 20 // Ranzen neben dem Titel samt Abstand
    ...
    /// Drei Verwaltungszeilen je Klasse, immer: Verwaltungsdatei, Curriculum,
    /// Sitzplan — gesetzt oder als Angebot (E34). Die Höhe hängt damit weder
    /// an der Planung noch an der Sitzplandatei, und das Raster springt nicht.
    static let verwaltungszeilen = Kursdateiart.allCases.count + 1
    ...
    static func kopfhoehe(_ klasse: Klasse) -> CGFloat {
        ...
        hoehe += CGFloat(verwaltungszeilen) * (1 + verwaltungszeile)
        return hoehe
    }
    ...
    static var breiteHausaufgabe: CGFloat { hausaufgabenbreite }

    /// Was der Titel an Breite verliert: den Haken, dazu das Zeichen der
    /// Hausaufgabe, wenn eines steht. Messung und Zeichnung nehmen dieselbe.
    static func titelbreite(innen: CGFloat, hausaufgabe: Bool) -> CGFloat {
        max(20, innen - hakenbreite - (hausaufgabe ? hausaufgabenbreite : 0))
    }
    static var hoeheVerweiszeile: CGFloat { verweiszeile }
    /// Verweis und Angebot in der Kurszelle sind genau so hoch — sonst
    /// wichen Messung und Zeichnung voneinander ab.
    static var hoeheVerwaltungszeile: CGFloat { verwaltungszeile }
    ...
    static func kachelhoehe(_ vorhaben: Vorhaben, breite: CGFloat) -> CGFloat {
        ...
        if vorhaben.traegtMarkenzeile { hoehe += pruefungszeile + pruefungsabstand }
        ...
        hoehe += max(hakenhoehe,
                     texthoehe(vorhaben.anzeigeTitel, stil: .callout, gewicht: .medium,
                               breite: titelbreite(innen: innen, hausaufgabe: vorhaben.hausaufgabe),
                               hoechstens: titelZeilen))
