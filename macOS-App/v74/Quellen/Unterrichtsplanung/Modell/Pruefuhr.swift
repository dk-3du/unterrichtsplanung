...
enum Pruefuhr {
    ...
    /// `isolation` hält die Funktion beim Akteur des Aufrufers — so darf die
    /// Handlung dessen Zustand berühren, ohne `Sendable` zu sein. An
    /// `withValue` wird sie nicht mehr weitergereicht: Seit Swift 6.4 läuft
    /// `withValue(_:operation:)` als `nonisolated(nonsending)` ohnehin beim
    /// Aufrufer, und die Überladung mit `isolation:` ist veraltet (Xcode 27).
    static func angehalten<T>(_ zeitpunkt: Date = Date(),
                              isolation: isolated (any Actor)? = #isolation,
                              _ waehrenddessen: () async throws -> T) async rethrows -> T {
        guard Pruefziel.ja else { return try await waehrenddessen() }
        return try await $festgehalten.withValue(zeitpunkt) { try await waehrenddessen() }
    }
}
