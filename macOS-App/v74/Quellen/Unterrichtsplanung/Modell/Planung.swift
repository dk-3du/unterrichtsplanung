...
enum Kennwerte {
    ...
    /// Die Fassung der Planungsdatei, die geschrieben wird — und die einzige,
    /// die gelesen wird: Ältere werden benannt statt mit falschen Farben
    /// geöffnet, neuere nicht gedeutet. Anheben nur bei unverträglichen
    /// Änderungen; ein zusätzliches Feld braucht keinen Sprung.
    static let dateiVersion = 2
    static let wochenStandard = 52
    static let wochenMax = 52
    /// Sitzplan je Klasse: Plätze und Zeichen je Namenseintrag — eigene
    /// Festwerte, die Ansicht kennt sie nicht (kein Fall in `masse_pruefen.py`).
    static let maxSitzplaetze = 35
    static let maxSitzplatzname = 100
...
struct Klasse: Identifiable, Hashable, Sendable {
    ...
    /// Unterricht — oder eine der drei Sonderzeilen (v69, E157). Eine
    /// Sonderzeile ist in allem eine Zeile wie jede; nur ihr Titel ist fest,
    /// und sie steht stets unter den Klassen/Kursen (`Planung.zeilenOrdnen`).
    var art: Zeilenart = .unterricht

    var istSonderzeile: Bool { art.istSonderzeile }
    /// Was diese Zeile kann — nach ihrer Art (E161).
    func erlaubt(_ dateiart: Kursdateiart) -> Bool { art.erlaubt(dateiart) }
    var erlaubtSitzplan: Bool { art.erlaubtSitzplan }
    var erlaubtFach: Bool { art.erlaubtFach }

    /// Eine neue Sonderzeile: fester Titel, kein Fach, noch keine Farbe.
    static func sonderzeile(_ art: Zeilenart) -> Klasse {
        Klasse(id: Kennung.neu("k"), name: art.titel, fach: "", notiz: "",
               farbe: Farbwelt.ohneFarbe, farbeManuell: false, art: art)
    }
...
enum Kursdateiart: String, CaseIterable, Identifiable, Sendable {
    ...
    /// Das Angebot in der Kurszelle, solange nichts hinterlegt ist — „+
    /// Verwaltungsdatei“, „+ Curriculum“ (E34, E35 (b)).
    var angebot: String {
        switch self {
        case .verwaltung: "+ Verwaltungsdatei"
        case .curriculum: "+ Curriculum"
...
struct Vorhaben: Identifiable, Hashable, Sendable {
    ...
    /// Das Vorhaben ist eine Korrektur — die Arbeit nach einer Prüfung.
    /// Gehandhabt wie die Dringlichkeit: ein Schalter, kein Pflichttermin (ein
    /// festes Datum trägt, wer will, in `datum`). Nie zugleich mit `pruefung`:
    /// Ein Vorhaben ist entweder Prüfung oder Korrektur — Leser, Speicher und
    /// Dialog halten das.
    var korrektur: Bool = false
    /// Prüfung und Korrektur tragen über dem Titel eine Zeile — dieselbe
    /// Stelle, dieselbe Höhe; zugleich kommen sie nie vor.
    var traegtMarkenzeile: Bool { pruefung || korrektur }
    ...
    /// Das Vorhaben trägt eine Hausaufgabe — ein Merkmal am Vorhaben, kein
    /// anderes Vorhaben. Der Schalter trägt das Zeichen; die Zeile darunter ist
    /// freiwillig und ohne Schalter nie gesetzt.
    var hausaufgabe: Bool = false
    /// Eine Zeile: Was aufgegeben ist („S. 42, Nr. 3–5“). Gemessen wie ein Titel.
    var hausaufgabenText: String = ""

    var anzeigeTitel: String { titel.isEmpty ? "Ohne Titel" : titel }

    /// „Hausaufgabe“ oder „Hausaufgabe: S. 42“ — ein Wortlaut für Tagesliste,
    /// Bedienungshilfen und Ansicht.
    var hausaufgabenBeschriftung: String {
        hausaufgabenText.isEmpty ? "Hausaufgabe" : "Hausaufgabe: " + hausaufgabenText
    }

    /// Für den Ausdruck, wie „Prüfung · Termin offen“: „Hausaufgabe · S. 42“.
    var hausaufgabenDruckmarke: String {
        hausaufgabenText.isEmpty ? "Hausaufgabe" : "Hausaufgabe · " + hausaufgabenText
    }
    ...
    func uebernommen(klasseId neueKlasse: String, woche neueWoche: Int,
        ...
        neu.kommentar = ""
        neu.statusGeaendert = ""
        // `hausaufgabe` und `hausaufgabenText` bleiben: Inhalt wie der Text, kein Verlauf.
...
struct Planung: Hashable, Sendable {
    ...
    /// Sonderzeilen stehen stets unter den Klassen/Kursen in fester Folge;
    /// innerhalb der Unterrichtszeilen — und innerhalb derselben Art — bleibt
    /// die Reihenfolge, wie sie war. Umgestellt wird, nie neu angelegt: Die
    /// Kennungen und damit Vorhaben, Sperren und Sitzpläne wandern mit.
    static func geordnet(_ zeilen: [Klasse]) -> [Klasse] {
        zeilen.enumerated()
            .sorted { ($0.element.art.rang, $0.offset) < ($1.element.art.rang, $1.offset) }
            .map(\.element)
    }

    /// Nach jeder Änderung an der Liste — der Leser ruft es ebenso.
    mutating func zeilenOrdnen() { klassen = Self.geordnet(klassen) }

    func hatSonderzeile(_ art: Zeilenart) -> Bool { klassen.contains { $0.art == art } }

    /// Wie viele Zeilen Unterricht sind — dort endet die freie Reihenfolge;
    /// eine neue Klasse wird an dieser Stelle eingefügt, nicht angehängt.
    var unterrichtszeilen: Int { klassen.count { !$0.istSonderzeile } }
    var sonderzeilenzahl: Int { klassen.count - unterrichtszeilen }
    ...
    struct Uebernahmebilanz: Hashable, Sendable {
        ...
        var uebergangen = 0
        /// Alte Klassen-Kennung → neue, für jede aufgenommene Zeile: Der
        /// Sitzplan folgt seiner Klasse in die neue Planung.
    ...
    static func mitUebernahme(titel: String, start: Tag, wochen: Int, basis: String,
        ...
        bilanz.zuordnung = zuordnung.filter { aufgenommen.contains($0.value) }
    ...
    var kennzahlen: String {
        ...
        return "KW \(erste.kw)/\(erste.jahr) – KW \(letzte.kw)/\(letzte.jahr)"
            + " · \(wochen) Wochen · \(unterrichtszeilen) Klassen/Kurse"
...
enum Kennung {
    ...
    /// Das Muster einer Kennung in Planung und Status: ASCII-Buchstaben und
    /// Ziffern, dazu Bindestrich, Unterstrich, Punkt und Doppelpunkt ab dem
    /// zweiten Zeichen; 1 bis `Planungsdatei.maxKennungslaenge` Zeichen. Eine
    /// Kennung ist Struktur, kein Text — was nicht passt, bekommt beim Lesen
    /// eine neue. Wortgleich in der Ansicht (`kennungGueltig`).
    static func istGueltig(_ wert: String) -> Bool {
        let zeichen = wert.unicodeScalars
        guard !zeichen.isEmpty, zeichen.count <= Planungsdatei.maxKennungslaenge else {
            return false
        }
        for (stelle, z) in zeichen.enumerated() {
            switch z.value {
            case 0x30...0x39, 0x41...0x5A, 0x61...0x7A: continue
            // Bindestrich, Unterstrich, Punkt, Doppelpunkt — nicht am Anfang.
            case 0x2D, 0x5F, 0x2E, 0x3A: if stelle > 0 { continue } else { return false }
    ...
    /// Ordnet nach Unicode-Codepunkten, wie `nachCodepunkten` in der Ansicht —
    /// Swifts `<` auf `String` ordnet nach kanonischer Form und träfe bei
    /// nicht normalisierten Schlüsseln eine andere Reihenfolge.
    static func nachCodepunkten(_ a: String, _ b: String) -> Bool {
...
/// Unterricht — eine Klasse oder ein Kurs — oder eine der drei Sonderzeilen
/// „Klassenleitung“, „Weiteres“, „Vertretungen“ (v69, E157). Bis v68 behalf
/// sich der Autor mit gewöhnlichen Zeilen als Ersatz; jetzt ist die Zeile durch
/// ihre Art eindeutig. In der Datei heißt das Feld `art`; **fehlend oder
/// unbekannt heißt Unterricht**, damit jede Datei bis v68 unverändert lädt und
/// eine v68-Ansicht eine v69-Datei weiter richtig zeigt. Die Ansicht liest
/// wortgleich (`leser_pruefen.py`).
enum Zeilenart: String, CaseIterable, Sendable {
    case unterricht, klassenleitung, weiteres, vertretungen

    /// Die drei Sonderzeilen in der Folge, in der sie im Raster stehen.
    static let sonderzeilen: [Zeilenart] = [.klassenleitung, .weiteres, .vertretungen]

    /// Der feste Titel einer Sonderzeile; Unterricht hat keinen.
    var titel: String {
        switch self {
        case .unterricht: ""
        case .klassenleitung: "Klassenleitung"
        case .weiteres: "Weiteres"
        case .vertretungen: "Vertretungen"
        }
    }

    var istSonderzeile: Bool { self != .unterricht }

    /// Was eine Zeile dieser Art kann (E161, Nutzer 17.09.2026): Klassenleitung
    /// und Weiteres tragen eine Verwaltungsdatei, kein Curriculum und keinen
    /// Sitzplan; Vertretungen nichts davon. Unterricht kann alles. Vorhaben,
    /// Termine, Sperren, Fach, Farbe und Notiz hat jede Zeile.
    func erlaubt(_ art: Kursdateiart) -> Bool {
        switch (self, art) {
        case (.unterricht, _): true
        case (.klassenleitung, .verwaltung), (.weiteres, .verwaltung): true
        default: false
        }
    }

    var erlaubtSitzplan: Bool { self == .unterricht }

    /// Ein Fach trägt nur, was eine Lerngruppe hat: Unterricht und die
    /// Klassenleitung (dort die Klasse, etwa „7a“). Weiteres und Vertretungen
    /// berühren verschiedene Lerngruppen — kein Fach (Nutzer, 17.09.2026).
    var erlaubtFach: Bool { self == .unterricht || self == .klassenleitung }

    /// Unterricht 0, dann die feste Folge — der Schlüssel der Ordnung.
    var rang: Int { Self.sonderzeilen.firstIndex(of: self).map { $0 + 1 } ?? 0 }

    /// Aus der Datei: genau der Schlüssel, sonst Unterricht.
    static func lesen(_ roh: Any?) -> Zeilenart {
        (roh as? String).flatMap(Zeilenart.init(rawValue:)) ?? .unterricht
    }
}
