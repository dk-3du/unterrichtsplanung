...
enum Drucken {
    ...
    static func drucken(_ planung: Planung, speicher: Planungsspeicher) {
        ...
        // Aus dem Hauptfenster (⌘P) frei wie bisher; aus einem Blatt als Bogen (E114/E115).
    ...
    static func alsPDFSichern(_ planung: Planung, speicher: Planungsspeicher) {
        ...
        // Aus dem Hauptfenster (⇧⌘P) bleibt der Dialog frei; `Dialogort` gäbe
        // ihm ein Blatt, stünde eines vorn (E114/E115).
...
struct Druckraster: View {
    ...
    /// Auf Papier gibt es kein Überfahren und keine Farbtiefe: Prüfung,
    /// Korrektur, Datum, Dringlichkeit und Hausaufgabe stehen deshalb als Wort
    /// da, nicht nur als Rahmen oder Zeichen.
    @ViewBuilder
    private func druckmarken(_ eintrag: Vorhaben) -> some View {
        if eintrag.traegtMarkenzeile || eintrag.datum != nil || eintrag.dringend || eintrag.hausaufgabe {
            HStack(spacing: 6) {
                if eintrag.pruefung {
                    Text("Prüfung · " + (eintrag.pruefungstag?.mitWochentag ?? "Termin offen"))
                        .foregroundStyle(Color(nsColor: Rasterfarben.pruefung))
                }
                if eintrag.korrektur {
                    Korrekturmarke.text
                }
                ...
                if eintrag.hausaufgabe {
                    Text(eintrag.hausaufgabenDruckmarke).foregroundStyle(Systemfarben.hausaufgabe)
                }
            }
    ...
    private func druckkachel(_ eintrag: Vorhaben, ton: Farbton) -> some View {
        ...
        .background {
            RoundedRectangle(cornerRadius: 5)
                ...
                .overlay {
                    // Wie im Raster: dringlich geht vor; die Korrektur ist
                    // gestrichelt im Rot der Prüfung — das trägt auch schwarzweiß.
                    if eintrag.korrektur, !eintrag.dringend {
                        RoundedRectangle(cornerRadius: 5)
                            .strokeBorder(Color(nsColor: Rasterfarben.pruefung).opacity(0.8),
                                          style: StrokeStyle(lineWidth: 1, dash: [4, 2.5]))
                    } else {
                        RoundedRectangle(cornerRadius: 5)
