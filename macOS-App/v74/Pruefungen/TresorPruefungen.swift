...
struct TresorPruefungen {
    ...
    func erkennen() throws {
        ...
        // Die Regel der Ansicht (E107): Marke und Leerraum davor ändern nichts,
        // eine Ausweichschreibung im Typnamen auch nicht — deuten, dann `typ`.
        #expect(Tresor.istBehaelter(Data([0xEF, 0xBB, 0xBF]) + behaelter))
        #expect(Tresor.istBehaelter(Data(String(repeating: " ", count: 200).utf8) + behaelter))
        #expect(Tresor.istBehaelter(Data("{\"typ\":\"\\u0075nterrichtsplanung-tresor\"}".utf8)))
        // Und drei Antworten, nicht zwei: Was sich nicht deuten lässt, ist kein Klartext (E108).
        #expect(Tresor.deuten(behaelter) == .behaelter)
        #expect(Tresor.deuten(klartext) == .klartext)
        #expect(Tresor.deuten(Data()) == .klartext && Tresor.deuten(Data("\n\n".utf8)) == .klartext)
        #expect(Tresor.deuten(Data("[1,2]".utf8)) == .klartext, "deutbar, aber kein Behälter")
        #expect(Tresor.deuten(Data("kein JSON".utf8)) == .undeutbar)
        #expect(Tresor.deuten(behaelter.dropLast(3)) == .undeutbar, "abgeschnitten")
    }
    ...
    @Test("Zahlen außerhalb des Bereichs werden abgewiesen, nicht abgestürzt")
    func zahlenAusserhalb() throws {
        // `Int(z)` bräche bei diesen Werten den Prozess ab — die Zahl kommt aus einer fremden Datei.
        for wert in [1e100, -1e100, .greatestFiniteMagnitude, .infinity, -.infinity, .nan, 1.5] as [Double] {
            #expect(JSONWert.zahl(wert).ganzzahl == nil, "\(wert)")
        }
        #expect(JSONWert.zahl(0).ganzzahl == 0)
        #expect(JSONWert.zahl(100_000).ganzzahl == 100_000)
        #expect(JSONWert.zahl(Double(Int.min)).ganzzahl == Int.min)
        #expect(JSONWert.text("100000").ganzzahl == nil)

        // Die Schranke sitzt in der Ableitung selbst — kein Aufrufer kommt vorbei.
        for verboten in [-1, 0, Tresor.rundenMindestens - 1, Tresor.rundenHoechstens + 1, Int.max, Int.min] {
            #expect(throws: Tresorfehler.self, "\(verboten)") { try Tresor.gepruefteRundenzahl(verboten) }
            #expect(throws: Tresorfehler.self, "\(verboten)") {
                try Tresor.pbkdf2("Passphrase", salt: Data(count: 16), runden: verboten)
            }
        }
        #expect(try Tresor.gepruefteRundenzahl(Tresor.rundenMindestens) == UInt32(Tresor.rundenMindestens))
        #expect(try Tresor.gepruefteRundenzahl(Tresor.rundenHoechstens) == UInt32(Tresor.rundenHoechstens))

        let t = try tresor()
        let blatt = try t.wiederherstellungAnlegen()
        let behaelter = try t.versiegeln(try planung(), inhalt: .planung, ziel: .export)
        let text = String(decoding: behaelter, as: UTF8.self)
        #expect(text.contains("\"runden\":\(runden)") && text.contains("\"version\":1"))
        // Ein Wahrheitswert ist keine Fassung — wie in der Ansicht.
        #expect(throws: Tresorfehler.self) {
            try Tresor.kopfLesen(Data(text.replacingOccurrences(of: "\"version\":1", with: "\"version\":true").utf8))
        }
        for unsinn in ["1e100", "-1e100", "1e19", "1.5", "1e400"] {
            let daten = Data(text.replacingOccurrences(
                of: "\"runden\":\(runden)", with: "\"runden\":\(unsinn)").utf8)
            let kopf: Behaelterkopf
            do {
                kopf = try Tresor.kopfLesen(daten)
            } catch is Tresorfehler {
                continue   // 1e400 ist schon für den JSON-Leser keine Zahl — ein sauberes Nein
            }
            // Der Passphrase-Weg benennt den Kopf als beschädigt …
            #expect(throws: Tresorfehler.self, "\(unsinn)") { try Tresor.oeffnen(kopf: kopf, passphrase: wort) }
            // … und wer über einen anderen Weg hereinkam, bekommt für die Passphrase
            // ein Nein statt eines Absturzes oder einer endlosen Ableitung.
            let ueberBlatt = try Tresor.oeffnen(kopf: kopf, wiederherstellung: blatt)
            #expect(!ueberBlatt.passphraseStimmt(wort), "\(unsinn)")
        }
        // Dieselbe Zahl als Fassung des Behälters.
        #expect(throws: Tresorfehler.self) {
            try Tresor.kopfLesen(Data(text.replacingOccurrences(of: "\"version\":1", with: "\"version\":1e100").utf8))
        }
    }
    ...
    func ziele() throws {
        ...
        let fremd = Behaelterkopf(
            ...
            daten: Data(count: 16))
        // Beide hier direkt angehängt, wie ein Mac sie in seiner Ablage trüge —
        // von außen nimmt kein Weg mehr eine Wicklung an.
        let mitEnklave = Tresor(schluessel: t.schluessel, kennung: t.kennung,
                                wicklungen: t.wicklungen + fremd.wicklungen)
    ...
    func unbekannteWicklung() throws {
        ...
        let haltend = Tresor(schluessel: t.schluessel, kennung: t.kennung, wicklungen: t.wicklungen + [fremd])
        let behaelter = try haltend.versiegeln(try planung(), inhalt: .planung, ziel: .kopie)
    ...
    /// Der Kopf kommt aus einer fremden Datei: Zahl, Tiefe, Breite und
    /// Textlänge der Wicklungen sind begrenzt — dieselben Grenzen wie in der
    /// Ansicht (`kopfLesen`, `masse_pruefen.py`).
    @Test("Zu viele, zu tiefe oder zu breite Wicklungen heißen beschädigt")
    func wicklungsgrenzen() throws {
        let t = try tresor()
        let behaelter = try t.versiegeln(try planung(), inhalt: .planung, ziel: .kopie)
        var objekt = try #require(JSONSerialization.jsonObject(with: behaelter) as? [String: Any])
        let echte = try #require(objekt["wicklungen"] as? [[String: Any]])
        func mit(_ wicklungen: [[String: Any]]) throws -> Data {
            objekt["wicklungen"] = wicklungen
            return try JSONSerialization.data(withJSONObject: objekt)
        }
        // Vier gehen, fünf nicht.
        let fremd: [String: Any] = ["art": "fremd", "x": "y"]
        #expect(try Tresor.kopfLesen(try mit(echte + [fremd, fremd, fremd])).wicklungen.count == 4)
        #expect(throws: Tresorfehler.self) { try Tresor.kopfLesen(try mit(echte + [fremd, fremd, fremd, fremd])) }
        // Tiefe: Objekt in Objekt in Liste in Objekt geht, eine Stufe mehr nicht.
        let tief: [String: Any] = ["art": "fremd", "a": ["b": [["c": 1]]]]
        #expect(try Tresor.kopfLesen(try mit(echte + [tief])).wicklung("fremd") != nil)
        let zuTief: [String: Any] = ["art": "fremd", "a": ["b": [["c": ["d": 1]]]]]
        #expect(throws: Tresorfehler.self) { try Tresor.kopfLesen(try mit(echte + [zuTief])) }
        // Breite und Textlänge.
        var breit: [String: Any] = ["art": "fremd"]
        for i in 0..<Tresor.wicklungEintraegeHoechstens { breit["f\(i)"] = i }
        #expect(throws: Tresorfehler.self) { try Tresor.kopfLesen(try mit(echte + [breit])) }
        let lang: [String: Any] = ["art": "fremd",
                                   "x": String(repeating: "x", count: Tresor.wicklungTextHoechstens + 1)]
        #expect(throws: Tresorfehler.self) { try Tresor.kopfLesen(try mit(echte + [lang])) }
        let gerade: [String: Any] = ["art": "fremd",
                                     "x": String(repeating: "x", count: Tresor.wicklungTextHoechstens)]
        #expect(try Tresor.kopfLesen(try mit(echte + [gerade])).wicklung("fremd") != nil)
        // Der echte Behälter bleibt, wie er ist.
        #expect(try Planungsdatei.lesen(try t.oeffnen(behaelter)).titel == "Tresorprobe")
    }

    /// Die Zusatzdaten (AAD) tragen die Fassung aus dem Kopf, nicht die
    /// Konstante — wie die Ansicht. Heute byteweise dasselbe; ein Kopf, dessen
    /// Fassung nicht zum Chiffrat passt, geht nicht auf.
    @Test("Die Zusatzdaten tragen die Fassung des Kopfes")
    func zusatzdatenNachKopf() throws {
        let t = try tresor()
        let behaelter = try t.versiegeln(Data("x".utf8), inhalt: .status, ziel: .kopie)
        let kopf = try Tresor.kopfLesen(behaelter)
        #expect(Tresor.zusatz(inhalt: "status", kennung: t.kennung, version: kopf.version)
                == Data("unterrichtsplanung-tresor|1|status|\(t.kennungHex)".utf8))
        #expect(Tresor.zusatz(inhalt: "status", kennung: t.kennung, version: 2)
                != Tresor.zusatz(inhalt: "status", kennung: t.kennung, version: 1))
        let umgeschrieben = Behaelterkopf(version: 2, inhalt: kopf.inhalt, kennung: kopf.kennung,
                                          nonce: kopf.nonce, wicklungen: kopf.wicklungen, daten: kopf.daten)
        #expect(throws: Tresorfehler.self, "die Fassung im Kopf ist beglaubigt") {
            try Tresor.entsiegeln(umgeschrieben, mit: t.schluessel)
        }
        #expect(try Tresor.entsiegeln(kopf, mit: t.schluessel) == Data("x".utf8))
    }

    /// Ein Kopf, den das Öffnen als beschädigt abweist, kommt über das Blatt
    /// auf — das Belegen der Passphrase nennt ihn dann ebenso beschädigt, nicht
    /// „passt nicht“.
    @Test("Kurzes Salt oder fremdes kdf: Öffnen und Belegen antworten gleich — beschädigt, nicht falsch")
    func wicklungBeschaedigt() throws {
        let t = try tresor()
        let blatt = try t.wiederherstellungAnlegen()
        let behaelter = try t.versiegeln(try planung(), inhalt: .planung, ziel: .export)
        var objekt = try #require(JSONSerialization.jsonObject(with: behaelter) as? [String: Any])
        let echte = try #require(objekt["wicklungen"] as? [[String: Any]])
        func art(_ arbeit: () throws -> Void) -> Tresorfehler.Art? {
            do { try arbeit() } catch let fehler as Tresorfehler { return fehler.art } catch { return nil }
            return nil
        }
        for (feld, wert) in [("salz", Data(count: 4).base64EncodedString()), ("kdf", "fremd")] {
            var wicklungen = echte
            for i in wicklungen.indices where wicklungen[i]["art"] as? String == Wicklung.passphrase {
                wicklungen[i][feld] = wert
            }
            objekt["wicklungen"] = wicklungen
            let kopf = try Tresor.kopfLesen(try JSONSerialization.data(withJSONObject: objekt))
            #expect(art { _ = try Tresor.oeffnen(kopf: kopf, passphrase: wort) } == .beschaedigt, "\(feld)")
            let ueberBlatt = try Tresor.oeffnen(kopf: kopf, wiederherstellung: blatt)
            #expect(art { try ueberBlatt.passphraseBelegen(wort) } == .beschaedigt, "\(feld)")
            #expect(!ueberBlatt.passphraseStimmt(wort), "\(feld)")
        }
        // Die falsche Passphrase heißt weiter falsch, die richtige geht durch.
        #expect(art { try t.passphraseBelegen("Ein Satz, den man vergisst") } == .falscherSchluessel)
        #expect(art { try t.passphraseBelegen(wort) } == nil)
    }
...
struct AblageVersiegeltPruefungen {
    ...
    func schreibenVersiegelt() throws {
        ...
        let t = try tresor()
        try ablage.schreiben(Data("{\"titel\":\"Geheimes\"}".utf8), tresor: t)
        ...
        guard case .daten(let gelesen) = ablage.lesen(hoechstens: Planungsdatei.hoechstgroesse) else { Issue.record("erwartet .daten"); return }
        #expect(try ablage.entsiegelt(gelesen, tresor: t) == Data("{\"titel\":\"Geheimes\"}".utf8))
    }
    ...
    func ohneTresor() throws {
        ...
        #expect(throws: Tresorfehler.self) { try ablage.entsiegelt(behaelter, tresor: nil) }
        #expect(try ablage.entsiegelt(Data("{}".utf8), tresor: nil) == Data("{}".utf8))
    }
    ...
    func altbestaende() throws {
        let (ablage, ordner) = try ablage()
        defer { try? FileManager.default.removeItem(at: ordner) }
        try ablage.schreiben(Data("{\"titel\":\"eins\"}".utf8), tresor: nil)
        try ablage.schreiben(Data("{\"titel\":\"zwei\"}".utf8), tresor: nil)   // → planung-vorher.json = eins
        ...
        let t = try tresor()
        let erster = ablage.altbestaendeVersiegeln(mit: t, hoechstens: Planungsdatei.hoechstgroesse)
        #expect(erster.umgestellt.count == 3 && erster.vollstaendig)
        ...
        #expect(try ablage.entsiegelt(try Data(contentsOf: kaputt), tresor: t) == Data("{kaputt".utf8))
        // Ein zweiter Lauf schreibt neu (frische Nonces), verliert aber nichts.
        #expect(ablage.altbestaendeVersiegeln(mit: t, hoechstens: Planungsdatei.hoechstgroesse).umgestellt.count == 3)

        // Der Rückweg als Generation des Aufhebens: Behälter kommen in den Klartext,
        // die Vorgängerfassung entsteht aus der Ablage (hier Klartext: nichts zu tun),
        // geschrieben wird nichts.
        let zurueck = ablage.generationErzeugen(neu: nil, alter: t, hoechstens: Planungsdatei.hoechstgroesse)
        #expect(zurueck.bilanz.umgestellt == [kaputt.lastPathComponent, gestempelt.lastPathComponent] && zurueck.bilanz.vollstaendig)
        #expect(zurueck.dateien[kaputt.lastPathComponent] == Data("{kaputt".utf8))
        #expect(zurueck.dateien[gestempelt.lastPathComponent] == Data("{\"titel\":\"alt\"}".utf8))
        #expect(zurueck.dateien["planung-vorher.json"] == nil, "die Ablage liegt im Klartext — die Vorgängerfassung braucht nichts")
        #expect(Tresor.istBehaelter(try Data(contentsOf: ablage.vorherigeFassung)), "die Platte ist unberührt")
    ...
    @Test("Ein neuer Schlüssel: die Generation trägt die Vorgängerfassung aus der Ablage unter dem neuen Schlüssel — ohne den alten bleibt sie übrig")
    func schluesselwechsel() throws {
        let (ablage, ordner) = try ablage()
        defer { try? FileManager.default.removeItem(at: ordner) }
        let alter = try tresor()
        try ablage.schreiben(Data("{\"titel\":\"eins\"}".utf8), tresor: alter)
        try ablage.schreiben(Data("{\"titel\":\"zwei\"}".utf8), tresor: alter)
        let neuer = try tresor()
        let generation = ablage.generationErzeugen(neu: neuer, alter: alter, hoechstens: Planungsdatei.hoechstgroesse)
        #expect(generation.bilanz.umgestellt == ["planung-vorher.json"] && generation.bilanz.vollstaendig)
        let vorher = try #require(generation.dateien["planung-vorher.json"])
        #expect(try Tresor.kopfLesen(vorher).kennung == neuer.kennung)
        #expect(try ablage.entsiegelt(vorher, tresor: neuer) == Data("{\"titel\":\"zwei\"}".utf8),
                "die Vorgängerfassung ist der Stand der Ablage vor dem Übergang")
        #expect(try ablage.entsiegelt(try Data(contentsOf: ablage.vorherigeFassung), tresor: alter)
                == Data("{\"titel\":\"eins\"}".utf8), "die Platte ist unberührt")
        // Ohne den alten Schlüssel bleibt ein fremder Behälter außerhalb der Generation —
        // und wird benannt, nicht verschwiegen.
        let ohneAlten = ablage.generationErzeugen(neu: neuer, alter: nil, hoechstens: Planungsdatei.hoechstgroesse)
        #expect(ohneAlten.dateien.isEmpty && ohneAlten.bilanz.uebrig.map(\.name) == ["planung-vorher.json"])
        #expect(ohneAlten.bilanz.beschreibung.contains("anderen Schlüssel"))
    }
}
