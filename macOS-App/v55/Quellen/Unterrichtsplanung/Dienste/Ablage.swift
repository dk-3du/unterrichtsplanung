...
final class Ablage: @unchecked Sendable {
    ...
    func schreiben(_ daten: Data, tresor: Tresor?, hoechstens grenze: Int = .max) throws -> Int {
        try sperre.withLock {
            ...
            if let grund = Ablage.formwaechter(datei, tresor: tresor) { throw Formsperre(grund: grund) }
    ...
    /// So viel vom Kopf, wie der Wächter braucht: Typ und Schlüsselkennung
    /// stehen am Anfang des Behälters. Die ganze Datei zu lesen wäre bei jeder
    /// Autosicherung ein zweites Lesen der Planung.
    static let formkopf = 4096

    /// Die Form auf der Platte — `nil` als Kennung heißt: ein Behälter, dessen
    /// Kennung nicht im Kopf steht. Dann wird nicht geschrieben.
    enum Form: Equatable {
        case nichts
        case klartext
    ...
    /// `gebundenLesen` — `nil`, wenn nichts liegt oder sich nichts lesen lässt.
    static func kopf(_ url: URL, byte: Int = formkopf) -> Data? {
        let deskriptor = Darwin.open(url.path, O_RDONLY | O_NONBLOCK | O_CLOEXEC)
        guard deskriptor >= 0 else { return nil }
        let griff = FileHandle(fileDescriptor: deskriptor, closeOnDealloc: false)
        defer { try? griff.close() }
        var status = stat()
        guard fstat(deskriptor, &status) == 0, (status.st_mode & S_IFMT) == S_IFREG else { return nil }
        return (try? griff.read(upToCount: byte)) ?? Data()
    }

    static func formAufDerPlatte(_ url: URL) -> Form {
        guard let kopf = kopf(url) else { return .nichts }
        guard Tresor.kopfIstBehaelter(kopf) else { return .klartext }
        return .behaelter(Tresor.kennungImKopf(kopf))
    }

    /// `nil`, wenn geschrieben werden darf — sonst der Grund (E72).
    static func formwaechter(_ ziel: URL, tresor: Tresor?) -> String? {
        guard case .behaelter(let kennung) = formAufDerPlatte(ziel) else { return nil }
        let name = ziel.lastPathComponent
        guard let tresor else {
            return "„\(name)“ liegt verschlüsselt, und diese Sitzung hat keinen Schlüssel dafür — "
                + "geschrieben wird nichts"
        }
        guard let kennung, kennung == tresor.kennungHex else {
            return "„\(name)“ liegt unter einem anderen Schlüssel als dem der Sitzung — "
                + "geschrieben wird nichts"
        }
        return nil
    }
