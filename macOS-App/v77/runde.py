...
def packziel(ziel, name):
    ...
    rein = os.path.normpath(ziel)
    letzter = os.path.basename(rein)
    if letzter.startswith("Rundenpaket-"):
        darueber = os.path.dirname(rein) or "."
        return None, (f"ZIEL „{letzter}“ benennt schon ein Rundenpaket. ZIEL ist der Ordner "
                      f"darüber — den Namen hängt der Treiber an, sonst läge das Rundenpaket "
                      f"im Rundenpaket und --laufen fände im Gast kein Manifest neben sich. "
                      f"Gemeint war vermutlich: --packen {darueber}")
...
def selbsttest():
    ...
    syntax()
    P = "Paket: org.3ducation.Unterrichtsplanung.probe 1.9.6 (77), Quellenstand abcabcabcabc…, Werkzeugstand d840422ba290…"
    F = "Paket: org.3ducation.Unterrichtsplanung 1.9.6 (77), Quellenstand abcabcabcabc…, Werkzeugstand d840422ba290…"
    ...
    for ziel, sollGrund in (("Paket", False), ("Paket/", False), (".", False),
                            ...
                            ("Paket/Rundenpaket-1.9.6-77", True),
                            ("Paket/Rundenpaket-1.9.6-77/", True),
                            ("/tmp/Rundenpaket-0.0.0-0", True)):
        ordner, grund = packziel(ziel, "Rundenpaket-1.9.6-77")
