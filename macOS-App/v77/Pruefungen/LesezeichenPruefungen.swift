...
struct LesezeichenPruefungen {
    ...
    /// Der Übergang am Zugriff, wie der Übergabestand ihn ausführt: den
    /// Behälter liefern, hinlegen, umschalten.
    private func versiegeln(_ zugriff: Ordnerzugriff, _ ablage: Ablage, unter t: Tresor) throws {
        try ablage.lesezeichenSchreiben(try zugriff.behaelter(unter: t))
        zugriff.umschalten(auf: t)
    }

    /// Das Aufheben am Zugriff: Einstellungen vorbereiten, Behälter fort, umschalten.
    private func entsiegeln(_ zugriff: Ordnerzugriff, _ ablage: Ablage) {
        zugriff.entsiegelnVorbereiten()
        ablage.lesezeichenEntfernen()
        zugriff.umschalten(auf: nil)
    }
    ...
    func hinUndZurueck() throws {
        ...
        try versiegeln(zugriff, ablage, unter: t)
    ...
    func erneuern() throws {
        ...
        try versiegeln(zugriff, ablage, unter: alter)
        let vorher = zugriff.eintraege

        try versiegeln(zugriff, ablage, unter: neuer)
        let kopf = try behaelterKopf(ablage)
    ...
    @Test("Der Übergang am Zugriff (E47): der Behälter entsteht ohne Schreiben, zu oder gesperrt liefert keinen; Umschalten folgt der Platte")
    func behaelterUndUmschalten() throws {
        let ablageOrdner = try ordner("ablage")
        let ziel = try ordner("ziel")
        let material = try ordner("material")
        defer { aufraeumen(ablageOrdner, ziel, material) }
        ...
        // Der Behälter unter dem neuen Schlüssel — nichts liegt auf der Platte.
        let behaelter = try zugriff.behaelter(unter: neuer)
        #expect(!FileManager.default.fileExists(atPath: ablage.lesezeichen.path))
        #expect(zugriff.quelle == .einstellungen, "die Sitzung bleibt, bis die Marke liegt")
        let kopf = try Tresor.kopfLesen(behaelter)
        #expect(kopf.inhalt == "lesezeichen" && kopf.kennung == neuer.kennung)
        let nutzlast = try Ordnerzugriff.nutzlastLesen(try neuer.oeffnen(kopf: kopf))
        #expect(nutzlast.eintraege == zugriff.eintraege && nutzlast.zielordner == ziel.path)

        // Umschalten nach dem Einsetzen: Quelle und Schlüssel folgen, nichts wird geschrieben.
        try ablage.lesezeichenSchreiben(behaelter)
        zugriff.umschalten(auf: neuer)
        #expect(zugriff.quelle == .behaelter && zugriff.ungesichert == nil)
        #expect(zugriff.zustaendig(fuer: material.path) == eintrag && zugriff.zielordner == ziel.path)
        #expect(try Data(contentsOf: ablage.lesezeichen) == behaelter, "unverändert")
        // Ein Merken schreibt jetzt unter dem neuen Schlüssel.
        try zugriff.merken(ziel)
        #expect(try behaelterKopf(ablage).kennung == neuer.kennung)
        #expect(try Tresor.kopfLesen(try zugriff.behaelter(unter: alter)).kennung == alter.kennung,
                "unter dem alten ebenso lieferbar (Rücknahme gibt es nicht mehr — die Marke entscheidet)")

        // Zu oder gesperrt: kein Behälter — die Datei gehört nicht in die Generation.
        let zu = Ordnerzugriff(ablage: ablage)
        zu.schliessen()
        #expect(throws: Uebergangsfehler.self) { try zu.behaelter(unter: neuer) }
        zu.umschalten(auf: neuer)
        #expect(zu.quelle == .zu, "zu bleibt zu")
        zu.umschalten(auf: nil)
        #expect(zu.quelle == .zu, "auch das Aufheben entlässt einen zuen Vorrat nicht")

        // Aufheben: Vorbereiten und Verwerfen sind ohne Einstellungsspeicher still; Umschalten auf nil geht in die Einstellungen.
        zugriff.entsiegelnVorbereiten()
        zugriff.entsiegelnVerwerfen()
        #expect(zugriff.quelle == .behaelter)
        ablage.lesezeichenEntfernen()
        zugriff.umschalten(auf: nil)
    ...
    // ── Zugang und Auflösen, getrennt (R76-02, E237, v77) ────────────────

    @Test("Zugangsdaten holen berührt keine Datei; aufgelöst wird damit ohne Vorrat und ohne Sitzung")
    func zugangsdatenUndAufloesen() throws {
        let ablageOrdner = try ordner("zugang")
        let material = try ordner("zugang-material")
        defer { aufraeumen(ablageOrdner, material) }
        let zugriff = Ordnerzugriff(ablage: Ablage(ordner: ablageOrdner))
        _ = try zugriff.merken(material)

        let z = try zugriff.zugangsdaten(material.path)
        #expect(z.eintrag == Ordnerzugriff.kanonisch(material.path))
        #expect(!z.lesezeichen.isEmpty, "die Bytes des Lesezeichens, mehr nicht")

        // `aufloesen` ist `nonisolated` — im Betrieb läuft es im Arbeiter.
        let (bereich, erneuert) = try Ordnerzugriff.aufloesen(z)
        #expect(bereich.ordner.standardizedFileURL == material.standardizedFileURL)
        #expect(erneuert == nil, "ein frisches Lesezeichen ist nicht veraltet")
        // Und der Bereich trägt, was er tragen muss: Lesen gelingt darin.
        try Data("x".utf8).write(to: material.appending(component: "probe.txt"))
        let gelesen = try bereich.mit { ordner in
            try Data(contentsOf: ordner.appending(component: "probe.txt"))
        }
        #expect(gelesen == Data("x".utf8))
    }

    @Test("Ein verschobener Ordner: Das Lesezeichen wird im Arbeiter erneuert und erst danach abgelegt")
    func lesezeichenErneuernGetrennt() throws {
        let ablageOrdner = try ordner("erneuern")
        let material = try ordner("erneuern-material")
        defer { aufraeumen(ablageOrdner, material) }
        let zugriff = Ordnerzugriff(ablage: Ablage(ordner: ablageOrdner))
        let eintrag = try zugriff.merken(material)
        let vorher = try #require(zugriff.eintraege[eintrag])

        let neuerOrt = material.deletingLastPathComponent()
            .appending(component: "erneuern-verlegt-\(UUID().uuidString)", directoryHint: .isDirectory)
        try FileManager.default.moveItem(at: material, to: neuerOrt)
        defer { aufraeumen(neuerOrt) }

        let z = try zugriff.zugangsdaten(material.path)
        let (bereich, erneuert) = try Ordnerzugriff.aufloesen(z)
        #expect(bereich.ordner.lastPathComponent == neuerOrt.lastPathComponent, "nachgeführt")
        let e = try #require(erneuert, "das veraltete Lesezeichen wurde erneuert")
        #expect(zugriff.eintraege[eintrag] == vorher,
                "im Vorrat steht noch das alte — das Ablegen ist Sache des Hauptakteurs")

        zugriff.lesezeichenErneuern(e)
        #expect(zugriff.eintraege[eintrag] != vorher, "jetzt liegt das neue")
        #expect(zugriff.eintraege[Ordnerzugriff.kanonisch(neuerOrt.path)] == zugriff.eintraege[eintrag],
                "der neue Ort bekommt seinen eigenen Schlüssel, der alte bleibt als Zweitname")
    }

    @Test("Ein Lesezeichen, das sich nicht auflösen lässt, wird beim Auflösen benannt — nicht beim Zugang")
    func unaufloesbarErstBeimAufloesen() throws {
        let ablageOrdner = try ordner("unaufloesbar")
        let material = try ordner("unaufloesbar-material")
        defer { aufraeumen(ablageOrdner) }
        let zugriff = Ordnerzugriff(ablage: Ablage(ordner: ablageOrdner))
        _ = try zugriff.merken(material)
        let z = try zugriff.zugangsdaten(material.path)   // wirft nicht: keine Datei berührt
        aufraeumen(material)
        #expect(throws: Ordnerzugriff.Fehler.self) { try Ordnerzugriff.aufloesen(z) }
    }
    ...
    func gesperrt() throws {
        ...
        #expect(zugriff.zielordner.isEmpty)
        #expect(throws: Uebergangsfehler.self) { try zugriff.behaelter(unter: t) }
    ...
    func schreibenScheitert() throws {
        ...
        // Der Behälter entsteht, hinlegen geht nicht — der Übergang bricht vor der Marke ab, nichts ändert sich.
        #expect(throws: (any Error).self) { try ablage.lesezeichenSchreiben(try zugriff.behaelter(unter: t)) }
        #expect(zugriff.quelle == .einstellungen && zugriff.zustaendig(fuer: material.path) == eintrag)
        #expect(!FileManager.default.fileExists(atPath: ablage.lesezeichen.path))
        ...
        try FileManager.default.setAttributes([.posixPermissions: 0o700], ofItemAtPath: ablageOrdner.path)
        zugriff.zielordner = material.path
        #expect(zugriff.ungesichert == nil, "der nächste Anlass holt nach")
        try FileManager.default.setAttributes([.posixPermissions: 0o500], ofItemAtPath: ablageOrdner.path)
        var gemeldet: [String] = []
    ...
    func einstellungen() throws {
        ...
        // Aufheben: Vorbereiten legt den Vorrat in die Einstellungen, solange der Behälter noch gilt; Verwerfen leert sie wieder.
        zugriff.entsiegelnVorbereiten()
        #expect(einstellungen.dictionary(forKey: "unterrichtsplanung.lesezeichen")?.count == 2 && zugriff.quelle == .behaelter)
        zugriff.entsiegelnVerwerfen()
        #expect(einstellungen.dictionary(forKey: "unterrichtsplanung.lesezeichen") == nil)
        entsiegeln(zugriff, ablage)
    ...
    @Test("Ein Ort, den die Wahl schon gemerkt hat, bekommt kein zweites Lesezeichen")
    func lesezeichenNurEinmal() throws {
        let ablageOrdner = try ordner("ablage")
        let ziel = try ordner("ziel")
        let weiterer = try ordner("weiterer")
        defer { aufraeumen(ablageOrdner, ziel, weiterer) }
        ...
        let behaelter = Ablage(ordner: ablageOrdner).lesezeichen

        // Die Wahl über den Dialog merkt den Ort — nachgestellt; danach legt
        // niemand ein zweites Lesezeichen an, und der Behälter bleibt Byte für Byte.
        try s.zugriff.merken(ziel)
        s.autoexportZielSetzen(ziel.path)
        let vorher = try Data(contentsOf: behaelter)
        let eintraege = s.zugriff.eintraege
        s.sicherung.lesezeichenAblegen(ziel)
        #expect(s.zugriff.eintraege == eintraege)
        #expect(try Data(contentsOf: behaelter) == vorher, "kein zweites Versiegeln")

        // Ein Ort ohne Lesezeichen bekommt seines.
        s.sicherung.lesezeichenAblegen(weiterer)
        #expect(s.zugriff.zustaendig(fuer: weiterer.path) != nil)
        #expect(try Data(contentsOf: behaelter) != vorher)
    }
