...
final class Kachelansicht: NSView {
    ...
    /// Der Ranzen rechts neben dem Titel — nur das Zeichen, der Text steht im
    /// Dialog, in der Tagesliste und auf dem Blatt (E18). Eine Ansicht statt
    /// einer Zeichnung: `contentTintColor` färbt das Vorlagenbild.
    private let hausaufgabenzeichen = NSImageView()
    ...
    override init(frame: NSRect) {
        ...
        hausaufgabenzeichen.image = Symbolvorrat.bild(Zeichen.hausaufgabe)
        hausaufgabenzeichen.imageScaling = .scaleProportionallyUpOrDown
        hausaufgabenzeichen.contentTintColor = Rasterfarben.hausaufgabe
        hausaufgabenzeichen.isHidden = true
        // Kein eigenes Element: Die Beschriftung der Kachel nennt die Hausaufgabe.
        hausaufgabenzeichen.setAccessibilityElement(false)
        addSubview(hausaufgabenzeichen)
    ...
    private func knoepfeJetzt() {
        ...
        hausaufgabenzeichen.isHidden = !vorhaben.hausaufgabe
        hausaufgabenzeichen.toolTip = vorhaben.hausaufgabe ? vorhaben.hausaufgabenBeschriftung : nil
        ...
        if vorhaben.korrektur { beschriftung += ", " + Korrekturmarke.wort }
        if vorhaben.hausaufgabe { beschriftung += ", " + vorhaben.hausaufgabenBeschriftung }
    ...
    override func layout() {
        ...
        if stand.istPruefung || stand.istKorrektur { y += Zellenmass.hoehePruefungszeile + Zellenmass.abstandPruefung }
        if stand.gesetztesDatum != nil {
            y += Zellenmass.hoeheDatumszeile + Zellenmass.abstandDatum
        }

        let titelhoehe = stand.titelhoehe
        let titelbreite = Zellenmass.titelbreite(innen: innen, hausaufgabe: stand.istHausaufgabe)
        hakenknopf.frame = NSRect(x: bounds.width - Zellenmass.kachelRechts - 16,
                                  y: y, width: 16, height: 16)
        // Rechts neben dem Titel, auf der ersten Zeile, links vom Haken.
        hausaufgabenzeichen.frame = NSRect(x: Zellenmass.kachelLinks + titelbreite + 3,
                                           y: y, width: 16, height: 16)
    ...
    private func zeichnenJetzt() {
        ...
        if angewaehlt {
            ...
            karte.lineWidth = 1.5
        } else if stand.istKorrektur {
            // Das Rot der Prüfung, gestrichelt: verwandt, aber nicht dasselbe.
            Rasterfarben.pruefung.withAlphaComponent(ueberfahren ? 0.95 : 0.7).setStroke()
            karte.lineWidth = 1.5
            karte.setLineDash(Korrekturmarke.strichelung, count: Korrekturmarke.strichelung.count, phase: 0)
        } else {
        ...
        if let hinweis = stand.gesetzteKorrektur {
            // Am Platz der Prüfungszeile — umrandet statt gefüllt.
            let zeile = NSRect(x: Zellenmass.kachelLinks, y: y, width: innen,
                               height: Zellenmass.hoehePruefungszeile)
            let rand = NSBezierPath(roundedRect: zeile.insetBy(dx: 0.5, dy: 0.5), xRadius: 5, yRadius: 5)
            rand.lineWidth = 1
            Rasterfarben.pruefung.withAlphaComponent(0.7).setStroke()
            rand.stroke()
            hinweis.draw(with: zeile.insetBy(dx: 6, dy: 1),
                         options: [.usesLineFragmentOrigin, .truncatesLastVisibleLine])
            y += Zellenmass.hoehePruefungszeile + Zellenmass.abstandPruefung
