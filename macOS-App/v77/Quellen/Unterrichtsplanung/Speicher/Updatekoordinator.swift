...
final class Updatekoordinator {
    ...
    /// Über welche Sitzung geprüft wird — für Prüfungen ersetzbar.
    @ObservationIgnored var sitzung: URLSession = Updatepruefer.standardSitzung
    /// Wie lange die Anfrage auf Daten wartet — für Prüfungen ersetzbar (B66, v76).
    @ObservationIgnored var zeitgrenze: TimeInterval = Updatepruefer.zeitueberschreitung
    /// Die laufende Prüfung und die Epoche der Einwilligung (E199, B46, v73):
    /// Wer sie entzieht, bricht eine Prüfung ab, die nicht von Hand kam — der
    /// Abbruch erreicht die Anfrage —, und danach wird nichts übernommen.
    @ObservationIgnored private var laufend: (aufgabe: Task<Void, Never>, erzwungen: Bool)?
    @ObservationIgnored private var epoche = 0
    /// Die zuletzt gestartete Prüfung, nur lesend und nie geleert (E207, R73-01,
    /// v74): Eine Prüfung wartet auf ihr Ende, statt ihr Zeit zu lassen —
    /// `laeuft` fällt beim Entzug sofort und sagt darüber nichts.
    @ObservationIgnored private(set) var letztePruefung: Task<Void, Never>?
    ...
    /// wer ausschaltet, bricht eine laufende ab, die nicht von Hand kam.
    func erlauben(_ erlaubt: Bool) {
        ...
        epoche += 1
        if let laufend, !laufend.erzwungen {
            laufend.aufgabe.cancel()
            self.laufend = nil
            laeuft = false
        }
    }
    ...
    func pruefen(erzwungen: Bool = false) {
        ...
        let pruefer = Updatepruefer(quelle: quelle, installiert: Updates.installierterBuild,
                                    ...
                                    etag: stand.etag, gemerkt: stand.antwort, sitzung: sitzung,
                                    zeitgrenze: zeitgrenze)
        let meine = epoche
        let aufgabe = Task { @MainActor [weak self] in
            let ergebnis = await pruefer.pruefen()
            guard let self else { return }
            // Nach einem Entzug gilt das Ergebnis nicht mehr — außer bei einer
            // Prüfung von Hand, die an der Einwilligung nicht hängt.
            guard erzwungen || (epoche == meine && erlaubt) else { return }
            laufend = nil
            ergebnisUebernehmen(ergebnis, erzwungen: erzwungen)
        }
        laufend = (aufgabe, erzwungen)
        letztePruefung = aufgabe
    }
