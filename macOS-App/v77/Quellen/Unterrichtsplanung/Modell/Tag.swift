...
enum Zeitrechnung {
    ...
    ///
    /// Der Stempel sagt, *wann* zuletzt geändert wurde. Er ist **kein**
    /// Kennzeichen einer Fassung: Zwei Änderungen in derselben Millisekunde
    /// tragen denselben (N56-02, nachgemessen — 2 000 Abrufe in dichter Folge
    /// ergaben drei verschiedene Werte). Was auf der Platte liegt, entscheidet
    /// die Sicherung darum am Abdruck der Byte, nicht hieran.
    static func jetztAlsZeitstempel() -> String {
        Formatvorrat.gemeinsam.zeitstempel(Pruefuhr.jetzt)
    }
    ...
    /// in Ortszeit wie jede Anzeige, damit der Name zur Uhr des Nutzers passt.
    static func dateistempel(_ datum: Date = Date()) -> String {
        Formatvorrat.gemeinsam.dateistempel(datum)
    }
}
...
private final class Formatvorrat: @unchecked Sendable {
    ...
    private let stempelUhr: DateFormatter
    ...
    private init() {
        ...
        stempelUhr = Formatvorrat.uhr("yyyy-MM-dd-HHmmss")
    }
    ...
    func dateistempel(_ datum: Date) -> String { sperre.withLock { stempelUhr.string(from: datum) } }
