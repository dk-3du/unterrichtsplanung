...
enum Statusdatei {
    ...
    static func lesen(_ daten: Data) throws -> Statusstand {
        ...
        // Die Fassung wird geprüft, nicht geraten: Eine spätere Ansicht könnte
        // dieselben Felder anders meinen, und jede unterstützte Ansicht schreibt
        // sie. Dieselbe Regel gilt in der Ansicht (`statusstandDeuten`): nur die
        // Zahl 1, keine Zeichenkette, kein Wahrheitswert, kein Bruch, nicht fehlend.
        guard let rohVersion = d["version"] else {
            throw Planungsfehler(text: "Die Statusdatei trägt keine Fassungsnummer — sie stammt aus "
                                 + "einer nicht mehr unterstützten Fassung der Web App und wurde nicht eingelesen.")
        }
        guard let gelesen = Planungsdatei.ganzzahl(rohVersion) else {
            throw Planungsfehler(text: "Die Statusdatei trägt keine gültige Fassungsnummer "
                                 + "und wurde nicht eingelesen.")
        }
        guard gelesen == version else {
            throw Planungsfehler(text: "Die Statusdatei stammt aus einer anderen Fassung der "
                                 + "Web App (\(gelesen)) und wurde nicht eingelesen — bitte "
        ...
        for (kennung, wert) in rohEintraege {
            // Nur Kennungen nach dem Muster der Planung: Eine andere kann zu
            // keinem Vorhaben gehören, das der Planungsleser hergibt.
            guard Kennung.istGueltig(kennung), let e = wert as? [String: Any] else { continue }
            // Dieselben Regeln wie im Planungsleser (Steuerzeichen, Grenze):
            // Sonst nähme die Planung über den Statusweg einen Kommentar auf,
            // den sie beim nächsten Start wieder bereinigt oder kappt —
            // derselbe Stand bedeutete vor und nach einem Neustart Verschiedenes.
            // Zugeschnitten wie an der Schranke (E208, B52, v74): Was das Kappen
            // freilegt, fällt gleich weg — wortgleich `kommentarKuerzen` der Ansicht.
            let kommentar = Planungsdatei.zugeschnitten(text(e["kommentar"]),
                                                        grenze: Planungsdatei.maxTextlaenge).text
