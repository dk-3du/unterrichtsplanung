...
final class Ordnerzugriff {
    ...
    nonisolated static var imSandbox: Bool { Ablage.container != nil }
    ...
    /// Vor jedem Schreiben des Behälters gefragt: Ein offener Übergang wird
    /// zuerst nachgeholt; bleibt er offen, nennt die Sperre den Grund, und der
    /// Behälter bleibt, wie er ist (E54). Gesetzt vom Sicherungsdienst.
    @ObservationIgnored var schreibsperre: (@MainActor () -> String?)?
    ...
    private func behaelterSchreiben() -> Schreibbefund {
        ...
        if let grund = schreibsperre?() { return .nichtGeschrieben(grund) }
        if let sperre = Ablage.formwaechter(ablage.lesezeichen, tresor: tresor) {
            return .nichtGeschrieben(sperre.grund)
        }
        var abgelegt = false
        do {
    ...
    nonisolated static func kanonisch(_ pfad: String) -> String {
        var teile: [Substring] = []
    ...
    nonisolated private static var erzeugen: URL.BookmarkCreationOptions {
        imSandbox ? [.withSecurityScope] : []
    }

    nonisolated private static var lesen: URL.BookmarkResolutionOptions {
    ...
    // ── Der Zugang, getrennt vom Auflösen (R76-02, E237, v77) ────────────
    // Bis 1.9.5 lag beides in `finden`: Nachschlagen im Vorrat und das
    // Auflösen des Lesezeichens. Das Nachschlagen braucht den Hauptakteur
    // (der Vorrat gehört ihm), das Auflösen fasst den Zielordner an und
    // dauert so lange, wie der Speicher braucht. Seit 1.9.6 sind es zwei
    // Schritte: `zugangsdaten` holt die Bytes, `aufloesen` macht daraus einen
    // Bereich — ohne Sitzung, ohne Vorrat, also überall zu rufen.

    /// Alles, was zum Auflösen nötig ist: unveränderliche Bytes und Pfade.
    struct Zugangsdaten: Sendable {
        /// Der Schlüssel im Vorrat, der zuständig war — an ihm wird der Rest
        /// des Pfades abgetrennt, auch wenn das Ziel inzwischen anders heißt.
        let eintrag: String
        let lesezeichen: Data
        /// Der kanonische Pfad, nach dem gesucht wurde.
        let gesucht: String
        /// Der Pfad, wie er hereinkam — für die Fehlertexte.
        let pfad: String
    }

    /// Ein erneuertes Lesezeichen, das noch abzulegen ist. Das Ablegen bleibt
    /// beim Hauptakteur: Der Behälter ist versiegelt, geht durch den Tresor
    /// und liegt im eigenen Container — dort ist nichts langsam (gemessen
    /// v77: 0,7 ms bei 11, 4,7 ms bei 201 Lesezeichen).
    struct Erneuerung: Sendable {
        let eintrag: String
        /// Kanonischer Pfad des neuen Orts — sein eigener Schlüssel.
        let neuerEintrag: String
        let lesezeichen: Data
    }

    /// Das Lesezeichen zum Pfad, noch nicht aufgelöst. Nur Nachschlagen im
    /// Vorrat und die Schreibsperre — keine Datei wird berührt.
    func zugangsdaten(_ pfad: String) throws -> Zugangsdaten {
        ...
        return Zugangsdaten(eintrag: eintrag, lesezeichen: daten, gesucht: gesucht, pfad: pfad)
    }

    /// Das Lesezeichen auflösen — ein umbenannter oder verschobener Ordner
    /// wird erkannt (`bookmarkDataIsStale`) und ein neues Lesezeichen gleich
    /// hier erzeugt, im Bereich des alten. Abgelegt wird es nicht: Das tut der
    /// Hauptakteur mit `lesezeichenErneuern`, und nur, wenn die Lage dann noch
    /// gilt. Misslingt das Erzeugen, bleibt das alte Lesezeichen; es löst
    /// weiterhin auf.
    ///
    /// `nonisolated`: Beide Schritte fassen den Zielordner an und gehören
    /// dorthin, wo gearbeitet wird — im Betrieb also in den Arbeiter, der
    /// auch liest (R76-02).
    nonisolated static func aufloesen(_ z: Zugangsdaten) throws -> (bereich: Bereich, erneuert: Erneuerung?) {
        var veraltet = false
        let ziel: URL
        do {
            ziel = try URL(resolvingBookmarkData: z.lesezeichen, options: Ordnerzugriff.lesen, relativeTo: nil,
                           bookmarkDataIsStale: &veraltet)
        } catch {
            throw Fehler(.unaufloesbar, pfad: z.gesucht, grund: error.localizedDescription)
        }
        let bereich = Bereich(ziel: ziel,
                              ordner: Ordnerzugriff.verlegt(z.gesucht, eintrag: z.eintrag, ziel: ziel),
                              pfad: z.pfad)
        return (bereich, veraltet ? erneuertes(z.eintrag, ziel: ziel) : nil)
    }

    nonisolated private static func erneuertes(_ eintrag: String, ziel: URL) -> Erneuerung? {
        ...
        return Erneuerung(eintrag: eintrag, neuerEintrag: kanonisch(ziel.path), lesezeichen: neu)
    }

    /// Ein erneuertes Lesezeichen ablegen: Der neue Ort bekommt seinen
    /// Schlüssel, der alte bleibt als Zweitname stehen — die Planungsdatei
    /// kennt weiterhin den alten Pfad.
    func lesezeichenErneuern(_ e: Erneuerung) {
        guard schreibbar else { return }
        eintraege[e.eintrag] = e.lesezeichen
        eintraege[e.neuerEintrag] = e.lesezeichen
        ablegen()
    }

    /// Das Lesezeichen zum Pfad und sein aufgelöstes Ziel, in einem Zug —
    /// für alle Wege, die gleichlaufend arbeiten.
    private func finden(_ gesucht: String) throws -> (eintrag: String, ziel: URL) {
        let z = try zugangsdaten(gesucht)
        let (bereich, erneuert) = try Ordnerzugriff.aufloesen(z)
        if let erneuert { lesezeichenErneuern(erneuert) }
        return (z.eintrag, bereich.ziel)
    }
    ...
    nonisolated private static func verlegt(_ gesucht: String, eintrag: String, ziel: URL) -> URL {
    ...
    func mit<T>(_ pfad: String, _ arbeit: (URL) throws -> T) throws -> T {
        try bereich(pfad).mit(arbeit)
    }

    /// Der Bereich zu einem Pfad: über das zuständige Lesezeichen aufgelöst,
    /// noch nicht geöffnet. Gleichlaufend — wer den Hauptstrang frei halten
    /// muss, nimmt `zugangsdaten` und `aufloesen` einzeln (R76-02, v77).
    func bereich(_ pfad: String) throws -> Bereich {
        let z = try zugangsdaten(pfad)
        let (bereich, erneuert) = try Ordnerzugriff.aufloesen(z)
        if let erneuert { lesezeichenErneuern(erneuert) }
        return bereich
    }

    /// Ein aufgelöster Sicherheitsbereich. Aufgelöst wird auf dem Hauptstrang;
    /// `mit` öffnet und schließt ihn in einem Stück — auch abseits des
    /// Hauptstrangs, wo langsam gelesen wird, aber nie über ein `await` hinweg
    /// (R75-01, v76).
    struct Bereich: Sendable {
        /// Das Ziel des Lesezeichens — dessen Bereich wird geöffnet.
        let ziel: URL
        /// Der nachgeführte Pfad, den die Arbeit bekommt.
        let ordner: URL
        let pfad: String

        nonisolated func mit<T>(_ arbeit: (URL) throws -> T) throws -> T {
            let offen = ziel.startAccessingSecurityScopedResource()
            defer { if offen { ziel.stopAccessingSecurityScopedResource() } }
