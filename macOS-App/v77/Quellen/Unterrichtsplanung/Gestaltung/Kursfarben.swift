...
enum Rasterfarben {
    ...
    /// Die kräftigere Linie über der ersten Sonderzeile (E158).
    static let sonderzeilenlinie = abgeschwaecht(.secondaryLabelColor, 0.6)
    ...
    /// Indigo für die Hausaufgabe — ein eigener Ton neben Rot (Prüfung) und
    /// Gelb (Dringlichkeit): hsl(243 55% 46%) = #3b35b6 hell (Kontrast 8,9 zu
    /// Weiß), hsl(243 85% 76%) = #938ef6 dunkel (6,0 zu #1c1c1e). Die Ansicht
    /// trägt dieselben Werte (`--hausaufgabe`).
    static let hausaufgabe = NSColor(name: nil) { erscheinung in
        let dunkel = erscheinung.bestMatch(from: [.aqua, .darkAqua]) == .darkAqua
        return NSColor(hsl: 243, s: dunkel ? 85 : 55, l: dunkel ? 76 : 46)
    }
}
...
enum Systemfarben {
    ...
    /// Die kräftigere Linie über der ersten Sonderzeile (E158) — dieselbe wie in den Zellen.
    static let sonderzeilenlinie = Color(nsColor: Rasterfarben.sonderzeilenlinie)
    ...
    static let hausaufgabe = Color(nsColor: Rasterfarben.hausaufgabe)
}
