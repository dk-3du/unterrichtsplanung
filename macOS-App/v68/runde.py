...
Jeder Prüfstand beginnt mit seiner Paketzeile (E128, E144) und endet mit „ENDE
...
# (Exit 3) druckt stattdessen die Zeile des Dienstes und kein ENDE.
ENDE = "ENDE"
...
def manifestDateien(ordner):
    ...
    __pycache__ und ohne .DS_Store (den legt der Finder beim Kopieren an)."""
    treffer = []
...
def bewerten(zeilen, code, erwartet, paketanfang, auftrag):
    ...
    die Zeile des Dienstes und *kein* ENDE. Bis v67 fehlten (4) und (5): Ein
    Prüfstand, der nach der Paketzeile still endete, galt als bestanden (N67-01).
    Liefert (ok, gruen, rot, paketOk, endeOk)."""
    ...
    hatAbbruch = any(z.startswith(ABBRUCHZEILE) for z in zeilen)
    ...
    return ok, gruen, rot, paketOk, endeOk
...
def runde(probe, saat, bilanz):
    ...
    def lauf(name, argumente, umgebung=None, ordnerpfad=None, erwartet=0, frist=420):
        ...
        erste = zeilen[0] if zeilen else ""
        ok, gruen, rot, paketOk, endeOk = bewerten(zeilen, code, erwartet, paketanfang, auftragsname(argumente))
        ...
        bilanz.sag(f"{nummer:02d} {name:28s} {'✓' if ok else '✗'}  Rückgabe {code} (erwartet {erwartet}), "
                   ...
                   f"Ende {'✓' if endeOk else '✗'}, {dauer:.0f} s")
        for z in rot[:6]:
            bilanz.sag("      ", z[:160])
        time.sleep(ABSTAND)
        return ok
...
darf; wie das ausbleibt, steht in {ANLEITUNG} —, fährt dann die 26 Läufe (etwa fünf
...
def selbsttest():
    ...
    syntax()
    P = "Paket: org.3ducation.Unterrichtsplanung.probe 1.7.9 (68), Quellenstand abcabcabcabc…, Werkzeugstand d840422ba290…"
    F = "Paket: org.3ducation.Unterrichtsplanung 1.7.9 (68), Quellenstand abcabcabcabc…, Werkzeugstand d840422ba290…"
    abbruch = ["UEBERGANGSTEST (einschalten)", "  ✓ Planung geladen — Stand aus",
               "  UEBERGANG_ABBRUCH=vorbereitet: der Prozess endet nach diesem Schritt",
               "UEBERGANG_ABBRUCH: harter Abbruch nach „vorbereitet“"]
    # (Beschreibung, Zeilen, Rückgabe, erwartet, Auftrag, soll bestehen)
    faelle = [
        ("nur Paketzeile und Rückgabe 0 besteht nicht", [P], 0, 0, "KLICKTEST", False),
        ("Zusicherungen ohne Ende bestehen nicht", [P, "KLICKTEST a ✓", "KLICKTEST b ✓", "KLICKTEST c ✓"], 0, 0, "KLICKTEST", False),
        ("stiller Frühausstieg (kein ✓, kein ✗, kein Ende) besteht nicht", [P, "KLICKTEST keine Planung"], 0, 0, "KLICKTEST", False),
        ("Ende ohne eine einzige Zusicherung besteht nicht", [P, f"{ENDE} KLICKTEST"], 0, 0, "KLICKTEST", False),
        ("ein ✗ vor dem Ende besteht nicht", [P, "KLICKTEST a ✓", "KLICKTEST ✗ b", f"{ENDE} KLICKTEST"], 0, 0, "KLICKTEST", False),
        ("falsche Paketzeile besteht nicht", [F, "KLICKTEST a ✓", f"{ENDE} KLICKTEST"], 0, 0, "KLICKTEST", False),
        ("das Ende eines anderen Prüfstands zählt nicht", [P, "KLICKTEST a ✓", f"{ENDE} TOURTEST"], 0, 0, "KLICKTEST", False),
        ("falsche Rückgabe besteht nicht", [P, "KLICKTEST a ✓", f"{ENDE} KLICKTEST"], 1, 0, "KLICKTEST", False),
        ("Frist besteht nicht", [P, "KLICKTEST a ✓"], "FRIST", 0, "KLICKTEST", False),
        ("vollständiger Lauf besteht", [P, "KLICKTEST a ✓", "KLICKTEST b ✓", f"{ENDE} KLICKTEST"], 0, 0, "KLICKTEST", True),
        ("Ende darf vor Zeilen der Fehlerausgabe stehen", [P, "KLICKTEST a ✓", f"{ENDE} KLICKTEST", "2026-09-17 … NSWindow warning"], 0, 0, "KLICKTEST", True),
        ("gewollter Abbruch (Exit 3) mit der Zeile des Dienstes besteht", [P] + abbruch, 3, 3, "UEBERGANGSTEST", True),
        ("Rückgabe 3 ohne die Abbruchzeile besteht nicht", [P, "UEBERGANGSTEST (einschalten)", "  ✓ Planung geladen"], 3, 3, "UEBERGANGSTEST", False),
        ("Abbruch mit Ende-Zeile besteht nicht — regulär beendet statt abgebrochen", [P] + abbruch + [f"{ENDE} UEBERGANGSTEST"], 3, 3, "UEBERGANGSTEST", False),
        ("Abbruch ohne eine einzige Zusicherung besteht nicht", [P, abbruch[0], abbruch[2], abbruch[3]], 3, 3, "UEBERGANGSTEST", False),
    ]
    alle = True
    for text, zeilen, code, erwartet, auftrag, soll in faelle:
        ist = bewerten(zeilen, code, erwartet, P, auftrag)[0]
        gilt = ist == soll
        alle = alle and gilt
