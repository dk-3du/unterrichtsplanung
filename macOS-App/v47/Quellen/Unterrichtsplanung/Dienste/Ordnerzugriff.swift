...
final class Ordnerzugriff {
    ...
    private static func nutzlastAus(_ roh: Data, tresor: Tresor) throws -> (eintraege: [String: Data], zielordner: String) {
        ...
        return try nutzlastLesen(try tresor.oeffnen(kopf: kopf))
    }
    ...
    /// Den Behälter im Speicher bauen und prüfen, atomar ablegen, über
    /// denselben Weg wie das Öffnen zurücklesen und vergleichen.
    private func behaelterSchreiben() -> Schreibbefund {
        ...
        do {
            ...
            // Vor dem Ablegen: Die Nutzlast muss sich lesen lassen und der Behälter
            // unter der Grenze bleiben, die das Öffnen anlegt — sonst schriebe die
            // App etwas hin, das ihr nächster Start beiseitelegt.
            ...
            guard gedeutet.eintraege == eintraege, gedeutet.zielordner == gemerkterZielordner else {
                return .nichtGeschrieben("die Nutzlast liest sich anders, als sie geschrieben wurde")
            }
            ...
            guard behaelter.count <= Ordnerzugriff.hoechstgroesse else {
                return .nichtGeschrieben("der Behälter wäre mit \(behaelter.count / 1024 / 1024) MB größer als die Lesegrenze")
            }
    ...
    /// Den Vorrat unter `neu` versiegeln, wenn `neu` schon gilt — die Ablage
    /// trägt ihn (Passphrase geändert) oder ist auf ihn zurückgenommen. Aus
    /// den Einstellungen in den Behälter, oder den Behälter unter die neue
    /// Hülle. Zurückgelesen. Misslingt es, geht ein Vorrat aus den
    /// Einstellungen dorthin zurück; ein Behälter behält `neu` und gilt als
    /// ungesichert, bis der nächste Schreibvorgang gelingt. Der Grund kommt zurück.
    func versiegeln(unter neu: Tresor) -> String? {
        versiegeln(neu, bisherigerGiltWeiter: false)
    }

    /// Den Vorrat unter `neu` versiegeln, bevor die Ablage wechselt
    /// (Einschalten, Erneuern): Misslingt es, findet der Wechsel nicht statt —
    /// der bisherige Schlüssel gilt weiter, und ein Behälter, der schon unter
    /// `neu` abgelegt wurde, wird einmal unter dem bisherigen nachgeschrieben.
    ...
    private func versiegeln(_ neu: Tresor, bisherigerGiltWeiter: Bool) -> String? {
        ...
        if bisherigerGiltWeiter {
            tresor = bisheriger
            if case .ungeprueft = befund, behaelterSchreiben() == .gelungen {
                ungesichert = nil
                return grund
            }
        }
