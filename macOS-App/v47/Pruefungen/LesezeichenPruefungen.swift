...
struct LesezeichenPruefungen {
    ...
    @Test("Versiegeln vorab: scheitert es, gilt der bisherige Schlüssel weiter — versiegeln unter: der neue")
    func versiegelnVorabOderUnter() throws {
        let ablageOrdner = try ordner("ablage")
        let ziel = try ordner("ziel")
        let material = try ordner("material")
        let zweites = try ordner("zweites")
        defer { aufraeumen(ablageOrdner, ziel, material, zweites) }
        let ablage = Ablage(ordner: ablageOrdner)
        let (zugriff, _) = try vorrat(ablage, material: material, ziel: ziel)
        let alter = try tresor()
        let neuer = try tresor()
        #expect(zugriff.versiegeln(unter: alter) == nil)

        func verbauen() throws {
            try? FileManager.default.removeItem(at: ablage.lesezeichen)
            try FileManager.default.createDirectory(at: ablage.lesezeichen, withIntermediateDirectories: true)
        }
        // Vor dem Wechsel der Ablage: Der Fehlschlag lässt den bisherigen Schlüssel gelten.
        try verbauen()
        #expect(zugriff.versiegeln(vorab: neuer) != nil)
        #expect(zugriff.quelle == .behaelter && zugriff.ungesichert != nil)
        try FileManager.default.removeItem(at: ablage.lesezeichen)
        try zugriff.merken(zweites)
        let unterAltem = try behaelterKopf(ablage)
        #expect(zugriff.ungesichert == nil && unterAltem.kennung == alter.kennung,
                "nachgeschrieben unter dem bisherigen Schlüssel")

        // Nach dem Wechsel der Ablage: Der neue Schlüssel gilt, auch wenn der Behälter noch nicht liegt.
        try verbauen()
        #expect(zugriff.versiegeln(unter: neuer) != nil)
        #expect(zugriff.ungesichert != nil)
        try FileManager.default.removeItem(at: ablage.lesezeichen)
        zugriff.vergessen(zweites.path)
        let unterNeuem = try behaelterKopf(ablage)
        #expect(zugriff.ungesichert == nil && unterNeuem.kennung == neuer.kennung,
                "nachgeschrieben unter dem neuen Schlüssel")
    }
