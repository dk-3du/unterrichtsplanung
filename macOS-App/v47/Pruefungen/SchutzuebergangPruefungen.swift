...
struct SchutzuebergangPruefungen {
    ...
    @Test("Erneuern scheitert am Behälter der Lesezeichen: zurückgenommen, der Behälter bleibt beim geltenden Schlüssel")
    func erneuernScheitertAmBehaelter() throws {
        ...
        // Ein Ordner an der Stelle des Behälters: Das Schreiben scheitert, die Ablage nicht.
        ...
        _ = try s.schluesselErneuernVorbereiten(alt: passphrase, neu: "Ein anderer Satz, den man behält")
        let ergebnis = s.verschluesselungEinschalten()
        guard case .zurueckgenommen(let grund) = ergebnis.ablage else {
            Issue.record("erwartet: zurückgenommen, war \(ergebnis.ablage)")
            return
        }
        #expect(grund.contains("Lesezeichen"), "\(grund)")
        #expect(s.tresor?.kennung == alter.kennung, "die Sitzung bleibt auf dem alten Schlüssel")
        #expect(s.zugriff.quelle == .behaelter && s.zugriff.ungesichert != nil, "der Vorrat gilt, ungesichert")
    ...
    @Test("Erneuern scheitert an der Ablage: zurückgenommen, der Behälter der Lesezeichen kehrt unter den alten Schlüssel zurück")
    func erneuernScheitertAnDerAblage() throws {
        ...
        let alter = try #require(s.tresor)
        let behaelter = Ablage(ordner: ablage).lesezeichen
        // Die Ablage unveränderbar: Der Behälter geht unter den neuen Schlüssel, die Ablage nicht.
