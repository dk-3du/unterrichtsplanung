...
final class Sitzplandienst {
    ...
    /// Vor jedem Schreiben gefragt: Ein offener Übergang wird zuerst
    /// nachgeholt; bleibt er offen, nennt die Sperre den Grund, und die Datei
    /// bleibt, wie sie ist (E54). Gesetzt vom Sicherungsdienst.
    ...
    private func schreiben() -> Schreibbefund {
        ...
        // Vor dem Bewahren wie vor dem Schreiben: Was versiegelt liegt, wird
        // nicht durch Klartext ersetzt und nicht entfernt (E72).
        if let sperre = Ablage.formwaechter(ablage.sitzplaene, tresor: tresor) {
    ...
    /// nicht in die Generation), wenn Klartext noch auf die Antwort des
    /// Nutzers wartet (E43 — kein Übergang versiegelt, was er nicht bestätigt
    /// hat, F04) oder das Original einer verlustbehaftet gelesenen Datei noch
    /// nicht bewahrt ist (E46) — die Kopie wird zuvor noch einmal versucht.
    func inhalt(unter neu: Tresor?) throws -> Data? {
        ...
        guard !klartextUnbestaetigt else {
            throw Uebergangsfehler(text: "die Sitzpläne warten auf die Antwort — versiegeln oder beiseitelegen")
        }
    ...
    /// Nichts wird geschrieben. Eine offene Frage zu Klartext (E43) bleibt
    /// offen — `inhalt(unter:)` lässt keinen Übergang über sie hinweg. Zu oder
    /// gesperrt bleibt, was es war: Die Datei war nicht in der Generation.
