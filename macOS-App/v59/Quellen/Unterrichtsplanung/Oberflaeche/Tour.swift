...
struct TourDialog: View {
    ...
    var body: some View {
        Dialograhmen(titel: "Kurze Tour durch die Oberfläche",
            ...
            Section {
                Text("Die Tour zeigt die Werkzeugleiste am oberen Fensterrand, die Kurszelle "
                     + "am linken Rand und an einer Zelle des Rasters, wie ein Vorhaben "
                     + "entsteht und wie eine Zelle unterrichtsfrei wird.")
...
final class Tourfuehrer: NSObject, NSPopoverDelegate {
    ...
    private func anker(fuer schritt: Planungsspeicher.Tourschritt) -> Anker? {
        let gefunden: Anker? = switch schritt.anker {
        case .werkzeug(let namen): werkzeuganker(namen)
        case .kurszelle, .zelle: zellenanker?(schritt)
        }
        if gefunden == nil, case .werkzeug(let namen)? = schritt.ersatzanker {
            return werkzeuganker(namen)
        }
        return gefunden
    }
