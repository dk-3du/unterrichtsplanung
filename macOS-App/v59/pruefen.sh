...
schritt "swift test (frisch übersetzt, PLANUNGSORDNER=$PRUEFORDNER)" \
    env PLANUNGSORDNER="$PRUEFORDNER" swift test --scratch-path "$BAUORDNER"
warnungenZaehlen "Programm und Prüfungen"
schritt "swift build --configuration release (frisch übersetzt)" \
    swift build --configuration release --arch arm64 --scratch-path "$BAUORDNER"
