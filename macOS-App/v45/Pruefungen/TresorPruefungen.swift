...
struct AblageVersiegeltPruefungen {
    ...
    func altbestaende() throws {
        ...
        let zurueck = ablage.altbestaendeEntsiegeln(t, hoechstens: Planungsdatei.hoechstgroesse)
        #expect(zurueck.umgestellt.count == 3 && zurueck.vollstaendig)
    ...
    func schluesselwechsel() throws {
        ...
        #expect(ablage.altbestaendeVersiegeln(mit: neuer, alter: alter, hoechstens: Planungsdatei.hoechstgroesse).umgestellt == ["planung-vorher.json"])
        #expect(try ablage.entsiegelt(try Data(contentsOf: ablage.vorherigeFassung), tresor: neuer)
        ...
        try ablage.schreiben(Data("{\"titel\":\"drei\"}".utf8), tresor: alter)
        let ohneAlten = ablage.altbestaendeVersiegeln(mit: neuer, alter: nil, hoechstens: Planungsdatei.hoechstgroesse)
        #expect(ohneAlten.umgestellt.isEmpty && ohneAlten.uebrig.map(\.name) == ["planung-vorher.json"])
    }
}
