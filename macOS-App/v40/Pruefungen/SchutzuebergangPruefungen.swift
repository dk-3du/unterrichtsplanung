...
struct SchutzuebergangPruefungen {
    ...
    func erneuern() throws {
        ...
        _ = try s.schluesselErneuernVorbereiten(passphrase: passphrase)
        let ergebnis = s.verschluesselungEinschalten()
    ...
    @Test("Passphrase ändern: die Kopie außer Haus trägt die neue Wicklung sofort")
    func passphraseAendern() throws {
        ...
        #expect(ergebnis.lesezeichen == .nichtNoetig)
        #expect(try Data(contentsOf: Ablage(ordner: ablage).lesezeichen) == lesezeichenVorher,
                "derselbe Datenschlüssel — der Behälter der Lesezeichen bleibt unangetastet")
        let kopf = try Tresor.kopfLesen(try Data(contentsOf: kopie))
        #expect(throws: Tresorfehler.self) { try Tresor.oeffnen(kopf: kopf, passphrase: passphrase) }
        #expect(try Tresor.oeffnen(kopf: kopf, passphrase: neue).kennung == kopf.kennung)
