...
struct LesezeichenPruefungen {
    ...
    func beschaedigt() throws {
        ...
        for (nummer, (name, roh, erwartet)) in faelle.enumerated() {
            ...
            guard case .beiseitegelegt(let grund) = befund else {
                Issue.record("\(name): erwartet beiseitegelegt, war \(befund)")
                continue
            }
            #expect(grund.contains(erwartet), "\(name): \(grund)")
            let rettung = ablageOrdner.appending(component: "lesezeichen-beschaedigt-fall-\(nummer).json")
            #expect(try Data(contentsOf: rettung) == roh, "\(name): die Rettungskopie ist das Original")
        ...
        // Die Rettungskopien zählen nicht zu den Nebendateien der Ablage.
        #expect(try ablage.nebendateien().isEmpty)
    }
    ...
    func nachruesten() throws {
        ...
        let leer = Ordnerzugriff(ablage: Ablage(ordner: try ordner("leer")))
        leer.schliessen()
        #expect(leer.oeffnen(mit: t, stempel: "s") == .angelegt(0, zielordner: false))
    }

    @Test("Lässt sich der Behälter nicht schreiben, bleibt der Klartext — und der Grund kommt zurück")
    func schreibenScheitert() throws {
        ...
        zugriff.schliessen()
        guard case .klartextGeblieben = zugriff.oeffnen(mit: t, stempel: "s") else {
            Issue.record("erwartet: klartextGeblieben")
            return
        }
        #expect(zugriff.quelle == .einstellungen && zugriff.zustaendig(fuer: material.path) == eintrag)
        ...
        try FileManager.default.setAttributes([.posixPermissions: 0o500], ofItemAtPath: ablageOrdner.path)
        var gemeldet: [String] = []
        zugriff.beiStoerung = { gemeldet.append($0) }
        let zweites = try ordner("zweites")
        defer { aufraeumen(zweites) }
        let zweiterEintrag = try zugriff.merken(zweites)
    ...
    private final class Einstellungsattrappe: Einstellungsspeicher {
    ...
    func startMitBeschaedigtemBehaelter() async throws {
        ...
        #expect(warnung.art == .warnung && warnung.text.contains("Rettungskopie"))
