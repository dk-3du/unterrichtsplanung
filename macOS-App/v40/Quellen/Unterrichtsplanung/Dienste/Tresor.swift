...
final class Tresor: @unchecked Sendable {
    ...
    /// Ablage — nur die App liest ihn, die Ansicht weist ihn wie `rohdaten` ab.
    enum Inhalt: String, Sendable { case planung, status, rohdaten, lesezeichen }
    ...
    func passphraseStimmt(_ passphrase: String) -> Bool {
        // Dieselbe Schranke wie beim Öffnen: Die Wicklung kann aus einer Datei
        // stammen, die über die Enklave aufging, ohne dass `runden` je geprüft wurde.
        guard let w = wicklungen.first(where: { $0.art == Wicklung.passphrase }),
              let runden = w.zahl("runden"), runden >= Tresor.rundenMindestens,
              runden <= Tresor.rundenHoechstens, let salt = w.daten("salz"),
              let kek = try? Tresor.pbkdf2(passphrase, salt: salt, runden: runden),
    ...
    static func kopfLesen(_ roh: Data) throws -> Behaelterkopf {
        ...
        // Nur eine Zahl — `JSONSerialization` liefert auch `true` als `NSNumber`;
        // die Ansicht weist den Wahrheitswert ab, also hier ebenso.
        guard let rohVersion = objekt["version"] as? NSNumber, String(cString: rohVersion.objCType) != "c",
              let version = Int(exactly: rohVersion.doubleValue), version >= 1
        else { throw beschaedigt }
