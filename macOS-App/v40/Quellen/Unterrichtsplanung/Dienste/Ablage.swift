...
final class Ablage: @unchecked Sendable {
    ...
    /// Verschlüsselung; `nebendateien()` erfasst ihn nicht.
    let lesezeichen: URL
    ...
    func lesezeichenLesen() -> Bestand {
        sperre.withLock {
            guard FileManager.default.fileExists(atPath: lesezeichen.path) else { return .keine }
            do { return .daten(try Data(contentsOf: lesezeichen)) }
            catch { return .unlesbar(error) }
        }
    }
    ...
    /// `true`, wenn danach kein Behälter mehr im Weg liegt.
    @discardableResult
    func lesezeichenBeiseitelegen(stempel: String) -> Bool {
        sperre.withLock {
            let verwaltung = FileManager.default
            guard verwaltung.fileExists(atPath: lesezeichen.path) else { return true }
            let ziel = ordner.appendingPathComponent("lesezeichen-beschaedigt-\(stempel).json")
            try? verwaltung.removeItem(at: ziel)
            try? verwaltung.moveItem(at: lesezeichen, to: ziel)
            return !verwaltung.fileExists(atPath: lesezeichen.path)
        }
    }
    ...
    /// Versiegelt jede Nebendatei unter `tresor` — Klartext an Ort
