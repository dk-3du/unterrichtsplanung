...
enum Systemzugriff {
    ...
    static func dateienWaehlen(start: String, zugriff: Ordnerzugriff) -> [String] {
        auswahl(ordner: false, mehrere: true, start: start,
                titel: "Material auswählen", knopf: "Verknüpfen", zugriff: zugriff)
    }

    static func dateiWaehlen(start: String, titel: String, zugriff: Ordnerzugriff) -> String? {
        auswahl(ordner: false, mehrere: false, start: start,
                titel: titel, knopf: "Zuweisen", zugriff: zugriff).first
    }
    ...
    private static func auswahl(ordner: Bool, mehrere: Bool, start: String,
                                titel: String, knopf: String, zugriff: Ordnerzugriff) -> [String] {
        ...
        return gemerkt(dialog.urls, in: zugriff).map(\.path)
    }
    ...
    private static func gemerkt(_ urls: [URL], in zugriff: Ordnerzugriff) -> [URL] {
        for url in urls { _ = try? zugriff.merken(url) }
        return urls
    }
    ...
    static func zugriffWaehlen(fuer pfad: String, zugriff: Ordnerzugriff) -> URL? {
        let dialog = NSOpenPanel()
        ...
        return gemerkt([url], in: zugriff).first
    }
