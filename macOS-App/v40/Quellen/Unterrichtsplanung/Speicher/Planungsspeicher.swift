...
final class Planungsspeicher {
    ...
    init(ablage: Ablage, vorschau: Bool = false) {
        ...
        sicherung.beiStoerung = { [weak self] in
            self?.melden("Die Autosicherung schlägt fehl. Bitte über „Export“ als Datei sichern.", .warnung)
        }
        sicherung.zugriff.beiStoerung = { [weak self] text in self?.melden(text, .warnung) }
        updates.melden = { [weak self] text, art in self?.melden(text, art) }
    ...
    func unlesbarenStandBehandeln(_ fehler: any Error) {
        ...
        if let vorige = sicherung.ablage.vorigeFassungLesen() {
