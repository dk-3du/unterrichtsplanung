...
final class Updatekoordinator {
    ...
    init(pruefstand: Bool, umgebung: [String: String] = ProcessInfo.processInfo.environment) {
        ...
        // Ein Prüflauf erbt die Einwilligung nicht — er fragt ohnehin nie.
        stand = pruefstand ? Updatestand() : Updatekoordinator.standLesen()
    }

    private static func standLesen() -> Updatestand {
        let einstellungen = UserDefaults.standard
        return Updatestand(
            erlaubt: einstellungen.object(forKey: Einstellungen.Schluessel.updatesErlaubt) as? Bool,
            zuletzt: einstellungen.object(forKey: Einstellungen.Schluessel.updateZuletzt) as? Date,
            uebersprungen: einstellungen.object(forKey: Einstellungen.Schluessel.updateUebersprungen) as? Int,
            etag: einstellungen.string(forKey: Einstellungen.Schluessel.updateEtag),
            antwort: einstellungen.data(forKey: Einstellungen.Schluessel.updateAntwort))
    }
