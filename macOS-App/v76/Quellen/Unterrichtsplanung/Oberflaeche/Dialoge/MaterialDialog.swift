...
struct MaterialDialog: View {
    ...
    /// Zählt die Ladeversuche. Geladen wird allein über `.task(id:)` — an das
    /// Blatt gebunden: Schließt es, endet die Aufgabe; „Erneut laden“ zählt nur
    /// hoch und startet damit dieselbe Aufgabe neu.
    @State private var versuch = 0
    ...
    private func laden() async {
        ...
        let befund = await speicher.materialkatalogLaden()
        // Das Blatt ist zu oder ein neuer Versuch läuft: Dieses Ergebnis zählt nicht mehr.
    ...
    private var koerper: some View {
        ...
        case .unlesbar(let grund):
            hinweis("Die Materialliste ließ sich nicht lesen — vermutlich ist die Website gerade im "
                    ...
                    erneut: true, einzelheit: grund)
        case .geladen(let katalog):
            liste(katalog)
        }
    }

    /// Der Grund in einer Zeile, begrenzt — er hilft dem, der die Website pflegt.
    static func einzelheit(_ grund: String) -> String {
        let zeile = Materialkatalog.zeile(grund)
        return "Einzelheit: " + (zeile.count > 200 ? String(zeile.prefix(199)) + "…" : zeile)
    }

    private func hinweis(_ text: String, symbol: String?, erneut: Bool = false,
                         einzelheit: String? = nil) -> some View {
        VStack(spacing: 14) {
            Spacer(minLength: 0)
            if let symbol {
            ...
            Text(text)
                ...
                .frame(maxWidth: 480)
                .fixedSize(horizontal: false, vertical: true)
            if let einzelheit, !einzelheit.isEmpty {
                Text(MaterialDialog.einzelheit(einzelheit))
                    .font(.caption)
                    .multilineTextAlignment(.center)
                    .foregroundStyle(.secondary)
