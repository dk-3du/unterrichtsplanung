...
enum Statusabgleich {
    ...
    enum Hindernis: Error, Equatable {
        ...
        var text: String {
            switch self {
            case .klartextNichtErlaubt:
                "Die Statusdatei der Web App ist unverschlüsselt, die Ablage auf "
                + "diesem Mac ist versiegelt — sie wurde nicht eingelesen. Bitte in der Web App "
                + "die aktuelle Kopie öffnen; ihr Status kommt dann versiegelt."
            case .verschluesseltOhneTresor:
                "Die Statusdatei der Web App ist verschlüsselt, die Ablage "
                + "auf diesem Mac ist es nicht — sie wurde nicht eingelesen."
            case .keinStatus(let inhalt):
                "Die Statusdatei der Web App ist ein Behälter mit Inhalt "
                + "„\(inhalt)“, kein Status — sie wurde nicht eingelesen."
            case .fremderSchluessel:
                "Die Statusdatei der Web App ist unter einem anderen Schlüssel "
                + "versiegelt und wurde nicht eingelesen — die Web App hat eine ältere "
                + "Kopie geöffnet."
            case .nichtEntsiegelt(let grund):
                "Die Statusdatei der Web App ließ sich nicht entsiegeln: "
                + "\(grund) Die Planung bleibt unverändert."
            case .unlesbar(let grund):
                "Die Statusdatei der Web App ließ sich nicht lesen: "
                + "\(grund) Die Planung bleibt unverändert."
            }
        }
    }
    ...
    /// **Widerrufen legt Inhalt zurück, keine Autorität (N58-01, E109).** Eine
    /// Rücknahme setzt eine ganze Momentaufnahme ein — und mit ihr die alten
    /// Stempel `statusGeaendert`. Gegen genau die misst `anwenden`: Ein
    /// liegengebliebener iPad-Stand von T1 gewänne gegen ein Widerrufen bei T3,
    /// obwohl das Widerrufen die jüngere Entscheidung ist — und `statusAnwenden`
    /// schriebe das. Darum bekommt jedes Vorhaben, dessen `erledigt` oder
    /// `kommentar` sich durch das Zurücklegen gegenüber dem bisherigen Stand
    /// ändert, einen frischen Stempel: `jetzt`, nie unter dem, was es bisher
    /// trug oder die Momentaufnahme trägt (eine zurückgestellte Uhr darf die
    /// Schranke nicht senken). Nicht alle stempeln (das sperrte berechtigte
    /// Stände aus), nicht keines (das ist der Befund).
    ///
    /// **Und kein Zurücklegen senkt einen Stempel (N58-01, Rest; E120).** Ein
    /// Vorhaben, das gleich bleibt, kam bis 1.7.2 mit dem Stempel der
    /// Momentaufnahme zurück — auch wenn es inzwischen einen jüngeren trug:
    /// Haken bei T3, ⌘Z bei T4 (frisch: T4), dann ⌘Z auf den Titel bei T5 →
    /// die Momentaufnahme von vor T2 brachte T0 mit, und der iPad-Stand von T1
    /// gewann wieder (elfte Review). Darum behält, was gleich bleibt, den
    /// **jüngeren seiner beiden Stempel** — den der Momentaufnahme oder den
    /// bisherigen —, ohne als neu entschieden zu gelten (kein `jetzt`): Ein
    /// iPad-Stand, jünger als seine letzte Entscheidung, bleibt berechtigt.
    /// Nur ein Vorhaben, das die Rücknahme wiederherstellt (nur nachher da),
    /// behält den Stempel der Momentaufnahme: Es wurde nicht entschieden, nur
    /// zurückgeholt. Liefert, wie viele Vorhaben frisch gestempelt wurden.
    @discardableResult
    static func nachRuecknahme(_ neu: inout Planung, gegen bisher: Planung, jetzt: String) -> Int {
        let bisherige = Dictionary(bisher.eintraege.map { ($0.id, $0) }, uniquingKeysWith: { erster, _ in erster })
        var gestempelt = 0
        for stelle in neu.eintraege.indices {
            let eintrag = neu.eintraege[stelle]
            // Nur nachher da — wiederhergestellt, nicht entschieden.
            guard let alt = bisherige[eintrag.id] else { continue }
            if eintrag.erledigt != alt.erledigt || eintrag.kommentar != alt.kommentar {
                neu.eintraege[stelle].statusGeaendert = max(jetzt, eintrag.statusGeaendert, alt.statusGeaendert)
                gestempelt += 1
            } else {
                // Gleich geblieben: der jüngere der beiden Stempel, kein `jetzt` (E120).
                neu.eintraege[stelle].statusGeaendert = max(eintrag.statusGeaendert, alt.statusGeaendert)
            }
        }
        return gestempelt
    }
