...
struct SchutzuebergangPruefungen {
    ...
    func erneuern() throws {
        ...
        // Seit 1.7.6 folgt der Meldung des Übergangs die zur Wicklung dieses Macs
        // aus dem Ergebnis der Vorbereitung (E139) — im Prüfziel „nicht
        // verfügbar“; die Meldung des Übergangs ist darum nicht mehr die letzte.
        #expect(s.meldungen.contains { $0.text.contains("Schlüssel erneuert") })
    }
    ...
    func zielordnerFehlt() throws {
        ...
        #expect(s.tresor != nil)
        // Die letzte Warnung — nach ihr kommt seit 1.7.6 noch der Satz zur
        // Wicklung dieses Macs (E139, im Prüfziel „nicht verfügbar“), kein Befund.
        let meldung = try #require(s.meldungen.last { $0.art == .warnung })
        #expect(meldung.text.contains("Offen:") && meldung.text.contains("Kopie außer Haus"))
    }
