...
struct KalenderPruefungen {
    ...
    /// Rettungskopien heißen nach der Uhr des Nutzers, nicht nach UTC — wie
    /// jede Anzeige der App.
    @Test("Der Dateistempel steht in Ortszeit")
    func dateistempelOrtszeit() {
        let zeitpunkt = Date(timeIntervalSince1970: 1_789_000_000)
        let erwartet = DateFormatter()
        erwartet.locale = Locale(identifier: "de_DE")
        erwartet.timeZone = .current
        erwartet.dateFormat = "yyyy-MM-dd-HHmmss"
        #expect(Zeitrechnung.dateistempel(zeitpunkt) == erwartet.string(from: zeitpunkt))
        #expect(Zeitrechnung.dateistempel().count == "2026-08-27-231205".count)
    }
...
@Suite("Kennungen")
struct KennungPruefungen {

    @Test("Erzeugte Kennungen passen auf das Muster")
    func erzeugt() {
        for vorsatz in ["k", "e", "f", "s"] {
            let id = Kennung.neu(vorsatz)
            #expect(Kennung.istGueltig(id), "\(id)")
            #expect(id.hasPrefix(vorsatz + "-"))
        }
    }

    /// Dieselben Fälle prüft `leser_pruefen.py` an `kennungGueltig` der Ansicht.
    @Test("Das Muster: ASCII, Buchstabe oder Ziffer vorn, begrenzt")
    func muster() {
        for gut in ["k1", "e-mf3x2a1b-ab12c", "K_1.a:b", "0", "A",
                    String(repeating: "x", count: Planungsdatei.maxKennungslaenge)] {
            #expect(Kennung.istGueltig(gut), "\(gut)")
        }
        for schlecht in ["", "-k1", "_k1", ".k", ":k", "k 1", "k1\n", "k1 ", "\tk1",
                         "kä", "k/1", "k1;", "__proto__", "\u{FEFF}k1",
                         String(repeating: "x", count: Planungsdatei.maxKennungslaenge + 1)] {
            #expect(!Kennung.istGueltig(schlecht), "\(schlecht.debugDescription)")
        }
    }

    @Test("Die Ordnung nach Codepunkten stellt e + U+0301 vor é und nichts vor U+FFFF um")
    func ordnung() {
        #expect(Kennung.nachCodepunkten("e\u{301}", "\u{E9}"))
        #expect(!Kennung.nachCodepunkten("\u{E9}", "e\u{301}"))
        // Swifts `<` hielte beide für gleich.
        #expect("e\u{301}" == "\u{E9}")
        // Ein Zeichen jenseits der Grundebene steht hinter U+FFFF — nach
        // UTF-16-Einheiten stünde es davor.
        #expect(Kennung.nachCodepunkten("\u{FFFF}", "\u{1F600}"))
        #expect(!Kennung.nachCodepunkten("b", "a") && Kennung.nachCodepunkten("a", "ab"))
    }
}
...
@Suite("Sonderzeilen: Klassenleitung, Weiteres, Vertretungen (E157)")
struct SonderzeilenPruefungen {

    private func zeile(_ name: String, _ art: Zeilenart = .unterricht) -> Klasse {
        art.istSonderzeile ? Klasse.sonderzeile(art)
                           : Klasse(id: Kennung.neu("k"), name: name, fach: "", notiz: "",
                                    farbe: Farbwelt.ohneFarbe, farbeManuell: false)
    }

    @Test("Die Art aus der Datei: bekannt heißt die Art, alles andere heißt Unterricht")
    func artLesen() {
        #expect(Zeilenart.lesen("klassenleitung") == .klassenleitung)
        #expect(Zeilenart.lesen("weiteres") == .weiteres)
        #expect(Zeilenart.lesen("vertretungen") == .vertretungen)
        #expect(Zeilenart.lesen("unterricht") == .unterricht)
        #expect(Zeilenart.lesen(nil) == .unterricht, "jede Datei bis v68 kennt kein Feld art")
        #expect(Zeilenart.lesen("Klassenleitung") == .unterricht, "genau der Schlüssel, keine Deutung")
        #expect(Zeilenart.lesen(7) == .unterricht)
        #expect(Zeilenart.lesen("") == .unterricht)
        #expect(Zeilenart.sonderzeilen == [.klassenleitung, .weiteres, .vertretungen], "die feste Folge")
        #expect(Zeilenart.sonderzeilen.map(\.titel) == ["Klassenleitung", "Weiteres", "Vertretungen"])
        #expect(Zeilenart.unterricht.titel.isEmpty && !Zeilenart.unterricht.istSonderzeile)
        #expect(Zeilenart.allCases.map(\.rang) == [0, 1, 2, 3])
    }

    @Test("Was eine Sonderzeile kann (E161): Klassenleitung und Weiteres eine Verwaltungsdatei, sonst nichts; Vertretungen nichts davon")
    func funktionsumfang() {
        for art in Kursdateiart.allCases {
            #expect(Zeilenart.unterricht.erlaubt(art), "Unterricht kann alles")
        }
        #expect(Zeilenart.unterricht.erlaubtSitzplan)
        for zeile in [Zeilenart.klassenleitung, .weiteres] {
            #expect(zeile.erlaubt(.verwaltung), Comment(rawValue: zeile.titel))
            #expect(!zeile.erlaubt(.curriculum), Comment(rawValue: zeile.titel))
            #expect(!zeile.erlaubtSitzplan, Comment(rawValue: zeile.titel))
        }
        #expect(!Zeilenart.vertretungen.erlaubt(.verwaltung) && !Zeilenart.vertretungen.erlaubt(.curriculum)
                && !Zeilenart.vertretungen.erlaubtSitzplan)
        let k = Klasse.sonderzeile(.klassenleitung)
        #expect(k.erlaubt(.verwaltung) && !k.erlaubt(.curriculum) && !k.erlaubtSitzplan, "die Zeile fragt ihre Art")
        // Ein Fach trägt nur, was eine Lerngruppe hat: Unterricht und die Klassenleitung.
        #expect(Zeilenart.unterricht.erlaubtFach && Zeilenart.klassenleitung.erlaubtFach)
        #expect(!Zeilenart.weiteres.erlaubtFach && !Zeilenart.vertretungen.erlaubtFach)
        #expect(k.erlaubtFach && !Klasse.sonderzeile(.weiteres).erlaubtFach)
    }

    @Test("Die Kennzahlen zählen nur die Klassen/Kurse, nicht die Sonderzeilen (Nutzer, 17.09.2026)")
    func kennzahlenOhneSonderzeilen() {
        let planung = Planung.leer(titel: "P", start: .heute, wochen: 4, basis: "",
                                   klassen: [zeile("7a"), zeile("8b"), zeile("", .klassenleitung), zeile("", .vertretungen)],
                                   fachfarben: [:])
        #expect(planung.kennzahlen.contains("2 Klassen/Kurse"), Comment(rawValue: planung.kennzahlen))
        #expect(!planung.kennzahlen.contains("4 Klassen/Kurse"))
        #expect(planung.sonderzeilenzahl == 2)
    }

    @Test("Die Fabrik: fester Titel, kein Fach, eigene Kennung, noch keine Farbe")
    func fabrik() {
        let k = Klasse.sonderzeile(.weiteres)
        #expect(k.art == .weiteres && k.istSonderzeile)
        #expect(k.name == "Weiteres" && k.fach == "" && k.notiz == "")
        #expect(k.farbe == Farbwelt.ohneFarbe && !k.farbeManuell)
        #expect(k.id.hasPrefix("k") && k.id != Klasse.sonderzeile(.weiteres).id)
        #expect(!zeile("7a").istSonderzeile)
    }

    @Test("Sonderzeilen stehen stets unter den Klassen/Kursen, in fester Folge, sonst bleibt die Reihenfolge")
    func ordnung() {
        let roh = [zeile("", .vertretungen), zeile("7a"), zeile("", .klassenleitung), zeile("8b"),
                   zeile("", .weiteres), zeile("AG")]
        let planung = Planung.leer(titel: "P", start: .heute, wochen: 4, basis: "",
                                   klassen: roh, fachfarben: [:])
        #expect(planung.klassen.map(\.name) == ["7a", "8b", "AG", "Klassenleitung", "Weiteres", "Vertretungen"])
        #expect(planung.klassen.map(\.id) == [roh[1], roh[3], roh[5], roh[2], roh[4], roh[0]].map(\.id),
                "die Kennungen wandern mit — es wird umgestellt, nicht neu angelegt")
        #expect(planung.unterrichtszeilen == 3)
        #expect(planung.hatSonderzeile(.klassenleitung) && planung.hatSonderzeile(.weiteres)
                && planung.hatSonderzeile(.vertretungen))
        #expect(planung.klassen.allSatisfy { Farbwelt.istGueltig($0.farbe) }, "jede Zeile bekommt ihre Farbe")

        // Eine geordnete Liste bleibt, wie sie ist; eine Liste ohne Sonderzeilen auch.
        #expect(Planung.geordnet(planung.klassen) == planung.klassen)
        let ohne = [zeile("9c"), zeile("5a")]
        #expect(Planung.geordnet(ohne) == ohne)
        var p2 = planung
        p2.klassen.append(zeile("10d"))
        p2.zeilenOrdnen()
        #expect(p2.klassen.map(\.name) == ["7a", "8b", "AG", "10d", "Klassenleitung", "Weiteres", "Vertretungen"],
                "eine angehängte Unterrichtszeile rückt vor die Sonderzeilen")
        #expect(p2.unterrichtszeilen == 4)
        let leer = Planung.leer(titel: "P", start: .heute, wochen: 4, basis: "", klassen: [], fachfarben: [:])
        #expect(leer.unterrichtszeilen == 0 && !leer.hatSonderzeile(.weiteres))
    }

    @Test("Zwei Zeilen derselben Art bleiben beide — der Leser verwirft nichts, die Ordnung hält ihre Folge")
    func doppelteArt() {
        let a = zeile("", .weiteres), b = zeile("", .weiteres)
        let planung = Planung.leer(titel: "P", start: .heute, wochen: 4, basis: "",
                                   klassen: [a, zeile("7a"), b], fachfarben: [:])
        #expect(planung.klassen.map(\.id) == [planung.klassen[0].id, a.id, b.id])
        #expect(planung.klassen.count == 3)
    }

    @Test("Die Übernahme in eine neue Planung trägt die Art mit und ordnet")
    func uebernahme() {
        var alte = Planung.leer(titel: "Alt", start: .heute, wochen: 4, basis: "",
                                klassen: [zeile("7a"), zeile("", .klassenleitung)], fachfarben: [:])
        let leitung = alte.klassen[1]
        alte.eintraege.append(Vorhaben(id: "v1", klasseId: leitung.id, woche: 0, titel: "Elternabend",
                                       text: "", erledigt: false, materialien: [], links: []))
        let (neue, bilanz) = Planung.mitUebernahme(
            titel: "Neu", start: .heute, wochen: 4, basis: "", klassen: [zeile("", .vertretungen), zeile("9c")],
            fachfarben: [:], ersterSchultag: nil, von: alte,
            uebernahme: [Planung.Uebernahmewunsch(klasse: leitung, mitVorhaben: true)])
        #expect(neue.klassen.map(\.name) == ["9c", "Klassenleitung", "Vertretungen"])
        #expect(neue.klassen[1].art == .klassenleitung && neue.klassen[1].id != leitung.id)
        #expect(bilanz.klassen == 1 && bilanz.vorhaben == 1)
        #expect(neue.eintraege.first?.klasseId == neue.klassen[1].id)
    }

    @Test("Die Höchstzahl gilt beim Anlegen für Klassen/Kurse, Sonderzeilen und Übernommene zusammen (Empfehlung der fünfzehnten Review)")
    func hoechstzahlMitSonderzeilen() {
        var alte = Planung.leer(titel: "Alt", start: .heute, wochen: 4, basis: "",
                                klassen: [zeile("7a"), zeile("", .weiteres)], fachfarben: [:])
        let weiteres = alte.klassen[1]
        alte.eintraege.append(Vorhaben(id: "v1", klasseId: weiteres.id, woche: 0, titel: "AG",
                                       text: "", erledigt: false, materialien: [], links: []))
        // 73 Klassen/Kurse + Klassenleitung + Vertretungen = 75; das Übernommene passt nicht mehr.
        let neueZeilen = (1...(Kennwerte.maxKlassen - 2)).map { zeile("K\($0)") }
            + [zeile("", .klassenleitung), zeile("", .vertretungen)]
        let (neue, bilanz) = Planung.mitUebernahme(
            titel: "Neu", start: .heute, wochen: 4, basis: "", klassen: neueZeilen,
            fachfarben: [:], ersterSchultag: nil, von: alte,
            uebernahme: [Planung.Uebernahmewunsch(klasse: weiteres, mitVorhaben: true)])
        #expect(neue.klassen.count == Kennwerte.maxKlassen)
        #expect(neue.klassen.suffix(2).map(\.art) == [.klassenleitung, .vertretungen], "die gewählten Sonderzeilen stehen unten")
        #expect(!neue.hatSonderzeile(.weiteres) && bilanz.klassen == 0 && bilanz.vorhaben == 0,
                "was nicht mehr passt, wird nicht übernommen — die Bilanz sagt es")
        #expect(neue.eintraege.isEmpty)
    }
}
