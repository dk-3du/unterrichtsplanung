...
if [ "$NURDMG" = "0" ]; then
  ...
  # als UPQuellenstand in die Info.plist des Pakets — mitsigniert. Die
  # Beglaubigung verlangt, dass Paket, Prüfvermerk und Quellen denselben Stand
  # tragen: geprüft und gebaut ist dann dasselbe (N51-04).
  QUELLENSTAND="$("$HIER/pruefen.sh" --stand)" || { echo "Der Stand der Quellen ließ sich nicht bestimmen (./pruefen.sh --stand)."; exit 1; }
...
Verschlüsselung
  ...
  eingesetzt ist, ruht die Autosicherung.
