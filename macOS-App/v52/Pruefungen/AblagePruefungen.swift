...
struct AblagePruefungen {
    ...
    func nebendateienBilanz() throws {
        ...
        let nachgeholt = ablage.altbestaendeVersiegeln(mit: tresor, nurKlartext: true, hoechstens: Planungsdatei.hoechstgroesse)
        #expect(nachgeholt.umgestellt == ["planung-beschaedigt-1.json"])
        #expect(nachgeholt.uebrig.map(\.name) == ["planung-beschaedigt-2.json"])
        #expect(ablage.altbestaendeVersiegeln(mit: tresor, nurKlartext: true, hoechstens: Planungsdatei.hoechstgroesse).umgestellt.isEmpty, "nichts mehr im Klartext")
