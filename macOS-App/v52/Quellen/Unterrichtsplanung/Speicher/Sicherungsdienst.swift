...
final class Sicherungsfolge: @unchecked Sendable {
    ...
    /// Ein übergebener Übergang, dessen Einsetzen nicht zu Ende kam: Marke,
    /// Ordner und der Stand, den seine Generation trägt. Solange er liegt,
    /// schreibt nichts an ihm vorbei; jeder Anlass holt ihn zuerst nach (E54).
    private var offen: (marke: Uebergangsmarke, ordner: URL, stand: String)?
    ...
    func uebergeben(nummer: Int, stand: String, marke: Uebergangsmarke, ordner: URL,
    ...
    var uebergangOffen: Bool { sperre.withLock { offen != nil } }

    /// Das offene Einsetzen nachholen — von jedem Schreibanlass aus. Liefert
    /// den Grund, wenn es weiter offen bleibt; sonst trägt die Platte den
    /// Stand der Generation.
    ...
    private func nachholenGesperrt() -> String? {
        guard let offen else { return nil }
        switch Uebergangsdienst.einsetzenErneut(offen.marke, in: offen.ordner) {
        case .vollendet:
            self.offen = nil
            letzterStand = offen.stand
            return nil
    ...
    func schreiben(nummer: Int, stand: String, daten: Data, tresor: Tresor?) throws -> Ergebnis {
        try sperre.withLock {
            ...
            letzteNummer = nummer
            if let grund = nachholenGesperrt() { return .uebergangOffen(grund) }
...
final class Sicherungsdienst {
    ...
    func uebergangNachholen() -> String? {
        guard folge.uebergangOffen else { return nil }
    ...
    func generationErzeugen(neu: Tresor?, alter: Tresor?) throws(Schreibhindernis)
        ...
        return (dateien, bilanz, nummer, planung.geaendert)
    }
    ...
    func uebergangEinsetzen(nummer: Int, stand: String) -> Uebergangsdienst.Einsetzbefund {
        guard let marke = uebergang.laufend else { return .unvollendet("keine übergebene Generation") }
        let befund = folge.uebergeben(nummer: nummer, stand: stand, marke: marke, ordner: ablage.ordner) {
            uebergang.einsetzen()
        }
        switch befund {
        case .vollendet:
    ...
    /// Platte behielte die alte —, oder der vorige Übergang ist noch nicht
    /// eingesetzt. Das Blatt zeigt den Grund und sperrt die Knöpfe.
    var schreibsperrgrund: String? {
        guard !istVorschau else { return nil }
        switch stoerung {
        case .zuGross, .uebergangOffen: return stoerung?.kurz
    ...
    @discardableResult
    func altbestaendeVersiegeln(mit tresor: Tresor, nurKlartext: Bool = false) -> Ablage.Nebendateienbilanz {
        ablage.altbestaendeVersiegeln(mit: tresor, nurKlartext: nurKlartext, hoechstens: Sicherungsdienst.lesedecke)
    }
