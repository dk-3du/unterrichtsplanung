    ...
    private func uebergangWiederanlaufen() -> Bool {
        ...
        case .verworfen(let anzahl):
            melden("Ein Übergang des Schutzes kam beim letzten Beenden nicht bis zur Übergabe — "
                   ...
                   + "der bisherige Stand.")
        case .nichtVollendbar(let art, let grund, let rettung):
            melden("\(art?.satzanfang ?? "Ein Übergang des Schutzes") war beim letzten Beenden nicht abgeschlossen "
                   + "und ließ sich nicht vollenden (\(grund)) — es gilt der bisherige Stand; Marke und Zwillinge "
                   + "liegen als \(rettung.map { "„\($0)“" }.joined(separator: ", ")) neben der Planung. Bitte den "
                   + "Übergang erneut ausführen.", .warnung)
