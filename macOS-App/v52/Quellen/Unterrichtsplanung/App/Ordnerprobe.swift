    ...
    static func laufenUndBeenden(_ speicher: Planungsspeicher) {
        Task { @MainActor in
            ...
            do {
                ...
                // Der Schlüssel eines Ortes ist lexikalisch: Ein Symlink wird nicht
                // aufgelöst — nicht beim Merken, nicht beim Nachschlagen. Gemerkt wird
                // der wirkliche Ordner (der Auswahldialog liefert ihn aufgelöst; ein
                // Lesezeichen auf den Symlink selbst legt das Sandbox nicht an), und
                // der Weg über den Symlink ist ein anderer Schlüssel.
                let echt = wurzel.appendingPathComponent("echt", isDirectory: true)
                let verlinkt = wurzel.appendingPathComponent("verlinkt", isDirectory: true)
                try verwaltung.createDirectory(at: echt, withIntermediateDirectories: true)
                try verwaltung.createSymbolicLink(at: verlinkt, withDestinationURL: echt)
                let echtDatei = echt.appendingPathComponent("notiz.txt", isDirectory: false)
                try Data("hinter dem Symlink".utf8).write(to: echtDatei)
                let verlinktDatei = verlinkt.appendingPathComponent("notiz.txt", isDirectory: false)
                pruefen(Ordnerzugriff.kanonisch(verlinktDatei.path).hasSuffix("/verlinkt/notiz.txt"),
                        "kanonisch löst den Symlink nicht auf: \(Ordnerzugriff.kanonisch(verlinktDatei.path))")
                let echtEintrag = try zugriff.merken(echt)
                pruefen(zugriff.zustaendig(fuer: echtDatei.path) == echtEintrag
                        && zugriff.zustaendig(fuer: verlinktDatei.path) == nil,
                        "über den wirklichen Ort zuständig, über den Symlink ein anderer Schlüssel")
                let hinterLink = try zugriff.mit(echtDatei.path) { try String(contentsOf: $0, encoding: .utf8) }
                pruefen(hinterLink == "hinter dem Symlink", "im Bereich des wirklichen Ortes gelesen")
                if versiegelt, let tresor = speicher.tresor {
                    // Der Behälter auf der Platte trägt das neue Lesezeichen; im Klartext liegt nichts.
                    if case .daten(let roh) = ablage.lesezeichenLesen(hoechstens: Ordnerzugriff.hoechstgroesse),
                    ...
                    // Der Prüflauf hat keinen Einstellungsspeicher — ein Blick in die
                    // echten UserDefaults belegte hier nichts.
                    pruefen(!zugriff.hatEinstellungsspeicher, "Prüflauf ohne Einstellungsspeicher")
                    pruefen(zugriff.ungesichert == nil, "Behälter gesichert")
                    ...
                    pruefen(frisch.zustaendig(fuer: echtDatei.path) == echtEintrag
                            && frisch.zustaendig(fuer: verlinktDatei.path) == nil,
                            "Neustart: derselbe Schlüssel für den wirklichen Ort, keiner für den Symlink")
