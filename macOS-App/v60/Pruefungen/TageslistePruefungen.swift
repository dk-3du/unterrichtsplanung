...
struct TageslistePruefungen {
    ...
    @Test("Eine Hausaufgabe ändert nicht, was auf der Liste steht — nur, wie es dort aussieht")
    func hausaufgabe() throws {
        var mit = try planung()
        for stelle in mit.eintraege.indices {
            mit.eintraege[stelle].hausaufgabe = true
            mit.eintraege[stelle].hausaufgabenText = "S. 1"
        }
        let liste = mit.tagesliste(am: try pruefTag)
        let ohne = try planung().tagesliste(am: try pruefTag)
        #expect(liste.amTag.map(\.id) == ohne.amTag.map(\.id))
        #expect(liste.inDerWoche.map(\.id) == ohne.inDerWoche.map(\.id))
        #expect(liste.alle.allSatisfy { $0.vorhaben.hausaufgabenBeschriftung == "Hausaufgabe: S. 1" })
    }
