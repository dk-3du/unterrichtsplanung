...
struct SitzplanPruefungen {
    ...
    @Test("B30: ein anders geschriebener fremder Behälter wird als fremd beiseitegelegt — wie der kanonische; ein abgeschnittener bleibt bewahrt")
    func dienstAndersGeschrieben() throws {
        let ort = try ordner("b30")
        defer { aufraeumen(ort) }
        let ablage = Ablage(ordner: ort)
        let eigener = try tresor()
        let fremder = try tresor()
        let behaelter = try fremder.versiegeln(try Sitzplandatei.schreiben(["k-eins": plan(2)]),
                                               inhalt: .sitzplaene, ziel: .ablage)
        // Bis v59 galt das als „beschädigt“: Der Erkenner sah die Marke und keinen
        // Behälter — obwohl `kopfLesen` ihn annimmt und der Schlüssel entscheidet.
        try ablage.sitzplaeneSchreiben(Data([0xEF, 0xBB, 0xBF]) + behaelter)
        guard case .beiseitegelegt(let grund, let rettung) = Sitzplandienst(ablage: ablage).oeffnen(mit: eigener, stempel: "s1") else {
            Issue.record("erwartet beiseitegelegt")
            return
        }
        #expect(grund.contains("anderen Schlüssel") && rettung == "sitzplaene-fremd-s1.json", Comment(rawValue: grund))
        #expect(try Data(contentsOf: ort.appendingPathComponent(rettung)) == Data([0xEF, 0xBB, 0xBF]) + behaelter,
                "die Rettungskopie ist die Datei, Byte für Byte")

        // Abgeschnitten und umformatiert: kein Klartext, sondern beschädigt — und bewahrt.
        let objekt = try JSONSerialization.jsonObject(with: behaelter)
        let kaputt = try JSONSerialization.data(withJSONObject: objekt, options: [.prettyPrinted, .sortedKeys]).dropLast()
        try ablage.sitzplaeneSchreiben(Data(kaputt))
        guard case .beiseitegelegt(_, let rettung2) = Sitzplandienst(ablage: ablage).oeffnen(mit: eigener, stempel: "s2") else {
            Issue.record("erwartet beiseitegelegt")
            return
        }
        #expect(rettung2 == "sitzplaene-beschaedigt-s2.json")
        #expect(try Data(contentsOf: ort.appendingPathComponent(rettung2)) == Data(kaputt))
    }
    ...
    func dienstSonderfaelle() throws {
        ...
        // Bis zur Antwort geht der Klartext in keine Generation, und Umschalten
        // beantwortet die Frage nicht (F04).
        #expect(throws: Uebergangsfehler.self) { try nachholer.inhalt(unter: t) }
        nachholer.umschalten(auf: t)
        #expect(nachholer.klartextUnbestaetigt)
        nachholer.klartextVersiegeln()
        #expect(try kopf(ablage.sitzplaene).inhalt == "sitzplaene")
        #expect(try nachholer.inhalt(unter: t) != nil, "beantwortet: der Inhalt kommt")
    }
    ...
    func ansicht() {
        ...
        ansicht.beiAenderung = { neu, handgriff in
            gemeldet.append(neu)
            handgriffe.append(handgriff)
        }
        ...
        #expect(handgriffe.last?.name == Schrittname.tischeEntfernen, "zwei Tische, und das Menü sagt es")
        taste(36)
