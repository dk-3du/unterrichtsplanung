...
final class Ablage: @unchecked Sendable {
    ...
    /// Die Regel selbst steht im Modell (`Pruefziel`), weil die `Pruefuhr` sie
    /// dort auch braucht und die Modellschicht an nichts darüber hängen darf.
    static var imPruefziel: Bool { Pruefziel.ja }
    ...
    struct Formsperre: LocalizedError {
        enum Anlass: Equatable {
            ...
            case ungewiss
            /// Dort liegt ein Behälter, den der Leser abweist — was er nicht
            /// öffnet, ersetzt er auch nicht (N57-01).
            case unbrauchbar
        }
        let anlass: Anlass
        let grund: String
    ...
    /// So viel liest der Wächter zuerst. Passt die Datei ganz hinein, ist sie
    /// damit gedeutet; sonst liest er sie bis zur Grenze — der Kopf ist kein
    /// eigener Erkenner (N55-01, E107).
    ...
    enum Form: Equatable {
        case nichts
        case klartext
        /// Ein Behälter, den der Leser öffnen **würde**, mit der Kennung, unter
        /// der er liegt. Sie kommt aus `Tresor.kopfLesen`, nie aus einer
        /// Abkürzung: Erkennen ist nicht Erlauben (N57-01).
        case behaelter(String)
        /// Dort liegt ein Behälter, den der Leser **abweist** — eine Fassung,
        /// die diese App nicht kennt, ein unbekanntes Verfahren, ein
        /// abgeschnittener oder beschädigter Rumpf. Mit dem Wortlaut des
        /// Lesers: Er sagt zu, die Datei bleibe unangetastet (N57-01, E90).
    ...
    /// öffnen würde (E75, E107): `Tresor.deuten` an der ganzen Datei, bis zur
    /// Grenze gelesen. Der Kopf ist keine Abkürzung mehr, nur der erste Griff:
    /// Passt die Datei hinein, ist sie damit ganz gelesen. Ein zweites Lesen
    /// kostet weniger als ein zweiter Erkenner (N55-01, nachgemessen: 0,3 ms
    /// bei 1 MB).
    static func formAufDerPlatte(_ url: URL) -> Form {
        ...
        case .kopf(let kopf, let ganz):
            if ganz { return gedeutet(kopf, name: url.lastPathComponent) }
            switch gebundenLesen(url, hoechstens: formgrenze) {
            case .keine: return .nichts
            case .daten(let roh): return gedeutet(roh, name: url.lastPathComponent)
    ...
    /// **Erkennen ist nicht Erlauben (N57-01, E89).** Bis v57 sprach der Kopf
    /// die Freigabe aus, sobald er Wort für Wort der eigene war und die Kennung
    /// nannte — damit belegte er die *Herkunft*, nicht die *Brauchbarkeit*. Bis
    /// v59 sagte ein Erkenner, der an Byte schnüffelte, „kein Behälter“ — und
    /// daraus wurde „Klartext, darf ersetzt werden“, auch für einen
    /// umformatierten, abgeschnittenen Behälter (N57-01 Rest) und für
    /// unversehrte Behälter in anderer Schreibweise (B28). Die Freigabe hing an
    /// der Schreibweise, nicht an der Sache.
    ///
    /// Jetzt entscheidet **eine** Regel an der ganzen Datei (`Tresor.deuten`,
    /// E107), und sie kennt drei Antworten (E108): Klartext darf beschrieben
    /// werden; ein Behälter geht an `kopfLesen` — was es annimmt, trägt seine
    /// Kennung, was es abweist, ist unbrauchbar und bleibt liegen, mit dem
    /// Wortlaut des Lesers; **was sich nicht deuten lässt, ist ungewiss** und
    /// sperrt — wie ein Lesefehlschlag (E76). Nur die leere Datei ist ein
    /// Klartext, der noch nichts sagt.
    ///
    /// Kosten: Klartext wird jetzt ganz gedeutet statt nur durchsucht —
    /// gemessen (v60): 0,47 ms je 200 KB Klartext (vorher 0,05 ms), der ganze
    /// Wächter 0,52 ms; ein Behälter von 267 KB 0,76 ms (deuten 0,32 ms, dann
    /// `kopfLesen` 0,37 ms). Gegen 700 ms Entprellung und ≈ 5 ms für das
    /// Schreiben selbst. **Die einzige Stelle, die eine Freigabe belegt.**
    private static func gedeutet(_ roh: Data, name: String) -> Form {
        switch Tresor.deuten(roh) {
        case .klartext:
            return .klartext
        case .undeutbar:
            return .unklar("„\(name)“ lässt sich nicht deuten — weder als Planung noch als Behälter")
        case .behaelter:
            do { return .behaelter(try Tresor.kopfLesen(roh).kennungHex) }
            catch { return .unbrauchbar(error.localizedDescription) }
        }
    }
    ...
    static func formwaechter(_ ziel: URL, tresor: Tresor?) -> Formsperre? {
        ...
        case .unbrauchbar(let grund):
            return Formsperre(anlass: .unbrauchbar,
                              grund: "„\(name)“ trägt einen Behälter, den diese App nicht lesen kann: "
                                     + "\(grund) Geschrieben wird nichts")
        case .behaelter(let kennung):
            ...
            guard kennung == tresor.kennungHex else {
                return Formsperre(anlass: .fremderSchutz,
