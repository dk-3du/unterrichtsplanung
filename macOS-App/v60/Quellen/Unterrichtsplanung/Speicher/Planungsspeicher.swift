...
final class Planungsspeicher {
    ...
    var kannWiderrufen: Bool { geltenderVerlauf?.kannZurueck ?? verlauf.kannZurueck }
    var kannWiederholen: Bool { geltenderVerlauf?.kannVor ?? verlauf.kannVor }

    /// „Widerrufen: Vorhaben löschen“ — der Name des Schrittes steht im Menü.
    var widerrufenTitel: String {
        let name = geltenderVerlauf?.naechsterName ?? verlauf.naechsterName
        return name.map { "Widerrufen: \($0)" } ?? "Widerrufen"
    }

    var wiederholenTitel: String {
        let name = geltenderVerlauf?.wiederholungsName ?? verlauf.wiederholungsName
