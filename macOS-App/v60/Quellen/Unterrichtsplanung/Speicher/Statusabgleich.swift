...
enum Statusabgleich {
    ...
    /// Schranke nicht senken). Alle anderen behalten ihren — auch ein Vorhaben,
    /// das die Rücknahme wiederherstellt: Es wurde nicht entschieden, nur
    /// zurückgeholt, und ein anstehender iPad-Stand dazu ist berechtigt. Nicht
    /// alle stempeln (das sperrte berechtigte Stände aus), nicht keines (das
    /// ist der Befund). Liefert, wie viele Vorhaben frisch gestempelt wurden.
    @discardableResult
    static func nachRuecknahme(_ neu: inout Planung, gegen bisher: Planung, jetzt: String) -> Int {
        ...
        for stelle in neu.eintraege.indices {
            ...
            guard let alt = bisherige[eintrag.id],
                  eintrag.erledigt != alt.erledigt || eintrag.kommentar != alt.kommentar
            else { continue }
