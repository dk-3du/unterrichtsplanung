...
extension Planungsspeicher {
    ...
    private func huelleNachziehen(alter: Tresor?, neu: Tresor, _ ergebnis: inout Schutzergebnis) {
        ...
        ergebnis.sitzplaene = sitzplaeneVersiegeln(unter: neu)
    ...
    private func sitzplaeneVersiegeln(unter neu: Tresor) -> Schutzergebnis.Aussenstelle {
        if let grund = sitzplaene.versiegeln(unter: neu) {
            return .offen(grund.ohneSchlusspunkt
    ...
    func verschluesselungEinschalten() -> Schutzergebnis {
        ...
        if !sicherung.istVorschau {
            ...
            if let grund = sitzplaene.versiegeln(vorab: neu) {
    ...
    /// oder unter den alten Schlüssel.
    ...
    func verschluesselungAufheben() -> Schutzergebnis {
        ...
        if !sicherung.istVorschau {
            ...
            // Die Sitzpläne zurück in den Klartext — wie die Planung (E22).
            let hattePlaene = sitzplaene.hatPlaene
            let sitzplansperre = sitzplaene.sperrgrund
            ergebnis.sitzplaene = sitzplaene.entsiegeln()
                ? (hattePlaene ? .erledigt : .nichtNoetig)
                : sitzplansperre.map { .offen("die Datei bleibt liegen: \($0)") } ?? .nichtNoetig
