...
struct Sitzplan: Hashable, Sendable {
    ...
    static func namenLesen(_ text: String) -> Namensliste {
        ...
        for (stelle, roh) in text.components(separatedBy: "\n").enumerated() {
            let zeile = Planungsdatei.ohneSteuerzeichen(roh)
    ...
    /// Das Element unter einem Punkt — der zuletzt gezeichnete zuerst, wie er
    /// oben liegt.
    func element(bei punkt: CGPoint) -> String? {
        ...
        return tische.last { $0.rahmen.contains(punkt) }?.id
    }
    ...
    static func bereinigterName(_ roh: String) -> String? {
        let sauber = Planungsdatei.ohneSteuerzeichen(roh).trimmingCharacters(in: .whitespacesAndNewlines)
...
/// Kennungen ersetzt; geschrieben kanonisch.
enum Sitzplandatei {
    ...
    static func lesen(_ daten: Data) throws -> [String: Sitzplan] {
        ...
        for (klasseId, wert) in rohePlaene {
            guard Kennung.istGueltig(klasseId), let roh = wert as? [String: Any] else { continue }
            plaene[klasseId] = plan(aus: roh, klasseId: klasseId)
        }
        return plaene
    }
    ...
    private static func geklemmt(_ x: CGFloat?, _ y: CGFloat?, in bereich: CGRect, groesse: CGSize) -> CGPoint {
        CGPoint(x: min(max(x ?? 0, bereich.minX), bereich.maxX - groesse.width),
                y: min(max(y ?? 0, bereich.minY), bereich.maxY - groesse.height))
    }

    private static func plan(aus roh: [String: Any], klasseId: String) -> Sitzplan {
        var tische: [Tisch] = []
        var belegt = Set<String>()
        for eintrag in (roh["tische"] as? [Any] ?? []).prefix(Kennwerte.maxSitzplaetze) {
            guard let t = eintrag as? [String: Any],
                  let name = Sitzplan.bereinigterName(Planungsdatei.text(t["name"])) else { continue }
        ...
        var lehrertisch: CGPoint?
        if let l = roh["lehrertisch"] as? [String: Any] {
        ...
        return Sitzplan(klasseId: klasseId, tische: tische, lehrertisch: lehrertisch,
                        geaendert: Planungsdatei.ohneSteuerzeichen(Planungsdatei.text(roh["geaendert"])))
    }
