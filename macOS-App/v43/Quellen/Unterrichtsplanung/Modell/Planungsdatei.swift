...
enum Planungsdatei {
    ...
    /// Obergrenze der Datei selbst, geprüft vor dem Lesen — in der Ablage, beim
    /// Öffnen von außen und in der Ansicht (`MAX_DATEIGROESSE`). Das Gegenstück
    /// zu `Statusdatei.hoechstgroesse`, nur großzügiger: Auch die Planungsdatei
    /// kommt von außen, wird aber auf dem Hauptstrang gelesen.
    static let hoechstgroesse = 32 * 1024 * 1024
    /// Die Decke für die eigene Ablage beim Start: Bis v42 schrieb die
    /// Autosicherung jede Größe. Ein solcher Bestand wird bis hierhin noch
    /// geladen — die Sicherung steht dann, bis er unter `hoechstgroesse`
    /// liegt; darüber hat diese App nicht geschrieben.
    static let hoechstgroesseEigene = 4 * hoechstgroesse
    ...
    static func pruefenMitBilanz(_ roh: Any) throws
        ...
        // Datei sie bei der Zeile nannte; eine doppelte bleibt bei der ersten
        // Zeile, die sie trug. Wortgleich in der Ansicht (`ersetzt`).
        ...
        for (stelle, fall) in roheKlassen.prefix(Kennwerte.maxKlassen).enumerated() {
            ...
            let warDoppelt = idsBelegt.contains(genannt.trimmingCharacters(in: .whitespaces))
            let id = kennung(genannt, "k", belegt: &idsBelegt)
            if !warDoppelt, genannt != id, ersetzt[genannt] == nil { ersetzt[genannt] = id }
