...
final class Ablage: @unchecked Sendable {
    ...
    /// Größe und Art vor dem Lesen: Eine Datei jenseits der Grenze läge sonst
    /// ganz im Speicher, ehe jemand sie prüft; was keine reguläre Datei ist
    /// (ein Ordner, eine Pipe), wird gar nicht erst gelesen.
    private func bestand(_ url: URL, hoechstens grenze: Int) -> Bestand {
        ...
        do {
            let werte = try url.resourceValues(forKeys: [.fileSizeKey, .isRegularFileKey])
            guard werte.isRegularFile == true else {
                throw Lesefehler(text: "„\(url.lastPathComponent)“ ist keine reguläre Datei.")
            }
            guard let groesse = werte.fileSize else {
                throw Lesefehler(text: "Die Größe von „\(url.lastPathComponent)“ ließ sich nicht bestimmen.")
            }
            guard groesse <= grenze else { return .zuGross(groesse) }
            return .daten(try Data(contentsOf: url))
        } catch {
    ...
    func stand() -> (zeitpunkt: Date?, groesse: Int?) {
        sperre.withLock {
            let werte = try? datei.resourceValues(forKeys: [.contentModificationDateKey, .fileSizeKey])
