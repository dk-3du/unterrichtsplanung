...
final class Sicherungsdienst {
    ...
    enum Stoerung: Equatable {
        ...
        var text: String {
            ...
            case .zuGross(let groesse):
                "Die Planung ist mit \(groesse / 1024 / 1024) MB größer als die Lesegrenze von App "
                    + "und Ansicht (\(Planungsdatei.hoechstgroesse / 1024 / 1024) MB). Die Autosicherung "
                    + "schreibt sie nicht — auf der Platte bleibt der zuletzt gesicherte Stand. Bitte "
                    + "Beschreibungen und Kommentare kürzen oder Vorhaben entfernen; danach sichert "
                    + "die App wieder von selbst."
            }
        }
    }
    ...
    init(ablage: Ablage, vorschau: Bool = false) {
        ...
        folge = Sicherungsfolge(ablage: ablage, hoechstens: Planungsdatei.hoechstgroesse)
    ...
    func sofortSchreiben() throws(Schreibhindernis) {
        ...
        do {
            ...
            lageMelden(.schreiben)
            throw .schreiben(error.localizedDescription)
        }
        uebernehmen(ergebnis, stand: planung.geaendert)
        if case .zuGross(let groesse) = ergebnis { throw .schreiben(Stoerung.zuGross(groesse).text) }
    }
    ...
    func ablagelage() -> Ablagelage {
        switch ablage.lesen(hoechstens: Planungsdatei.hoechstgroesse) {
    ...
    func statusdateiNeuVersiegeln(alter: Tresor?, neu: Tresor) throws(Statusdateilage) -> Bool {
        do {
            return try imZielordner { ordner in
                ...
                guard let groesse = try Sicherungsdienst.statusgroesse(datei), groesse > 0 else { return false }
                let roh: Data
                do { roh = try Data(contentsOf: datei) } catch { throw Statusdateilage.schreiben(error.localizedDescription) }
                let klartext: Data
    ...
    /// Die Größe der Statusdatei vor dem Lesen — `nil`, wenn keine liegt. Eine
    /// gescheiterte Abfrage, ein Ordner, eine Pipe: ein Fehler, keine Null —
    /// sonst ginge etwas ungeprüft ganz in den Speicher. Über der Grenze: benannt.
    private static func statusgroesse(_ datei: URL) throws(Statusdateilage) -> Int? {
        guard FileManager.default.fileExists(atPath: datei.path) else { return nil }
        let werte: URLResourceValues
        do { werte = try datei.resourceValues(forKeys: [.fileSizeKey, .isRegularFileKey]) } catch {
            throw .schreiben("Größe nicht bestimmbar: " + error.localizedDescription)
        }
        guard werte.isRegularFile == true, let groesse = werte.fileSize else {
            throw .schreiben("keine reguläre Datei")
        }
        guard groesse <= Statusdatei.hoechstgroesse else { throw .zuGross(groesse) }
        return groesse
    }
    ...
    func statusdateiLesen() throws(Statusdateilage) -> Data? {
        do {
            return try imZielordner { ordner in
                let datei = ordner.appending(component: Statusdatei.name, directoryHint: .notDirectory)
                guard let groesse = try Sicherungsdienst.statusgroesse(datei), groesse > 0 else { return nil }
                do { return try Data(contentsOf: datei) } catch { throw Statusdateilage.schreiben(error.localizedDescription) }
            }
        } catch let lage as Statusdateilage {
            throw lage
        } catch {
