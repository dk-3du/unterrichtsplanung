...
final class Planungsspeicher {
    ...
    func starten() {
        ...
        switch sicherung.ablage.lesen(hoechstens: Planungsdatei.hoechstgroesseEigene) {
        case .keine:
        ...
        case .zuGross(let groesse):
            ...
            beschaedigtenStandRetten(beschreibung: "mit \(groesse / 1024 / 1024) MB größer als die "
                                     + "Decke der eigenen Ablage (\(Planungsdatei.hoechstgroesseEigene / 1024 / 1024) MB)")
        case .daten(let gelesen):
            // Zwischen Lesegrenze und Decke (ein Bestand aus ≤ v42): geladen —
            // die Sicherung steht, bis die Planung kürzer ist.
            if gelesen.count > Planungsdatei.hoechstgroesse { sicherung.lageMelden(.zuGross(gelesen.count)) }
    ...
    func unlesbarenStandBehandeln(_ fehler: any Error) {
        ...
        if let vorige = sicherung.ablage.vorigeFassungLesen(hoechstens: Planungsdatei.hoechstgroesseEigene) {
