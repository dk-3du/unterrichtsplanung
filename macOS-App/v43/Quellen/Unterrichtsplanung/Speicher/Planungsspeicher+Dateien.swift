...
extension Planungsspeicher {
    ...
    func importieren(von url: URL) {
        // Größe und Art vor dem Lesen — und eine gescheiterte Abfrage ist ein
        // Fehler, keine Null: Sonst ginge eine Datei, deren Größe niemand
        // kennt, ungeprüft ganz in den Speicher.
        let werte: URLResourceValues
        do { werte = try url.resourceValues(forKeys: [.fileSizeKey, .isRegularFileKey]) } catch {
            melden("Die Größe von „\(url.lastPathComponent)“ ließ sich nicht bestimmen "
                   + "(\(error.localizedDescription)) — die Datei wurde nicht geöffnet.", .warnung)
            return
        }
        guard werte.isRegularFile == true, let groesse = werte.fileSize else {
            melden("„\(url.lastPathComponent)“ ist keine reguläre Datei und wurde nicht geöffnet.", .warnung)
            return
        }
        guard groesse <= Planungsdatei.hoechstgroesse else {
