...
extension Planungsspeicher {
    ...
    nonisolated static func erneuernVorbereiten(_ alter: Tresor, alt: String, neu: String,
        ...
        guard alter.passphraseStimmt(alt) else {
            throw Tresorfehler(art: .falscherSchluessel, text: "Die bisherige Passphrase passt nicht.")
        }
        guard neu != alt else {
    ...
    private func ablageSchreibenGeprueft(alter: Tresor?, neu: Tresor?) -> Schutzergebnis.Ablagebefund {
        ...
        case alt, .keine:
            ...
            return .zurueckgenommen("die Ablage ließ sich nicht schreiben ("
                                    + (grund.isEmpty ? "die Platte trägt weiter den alten Stand" : grund) + ")")
    ...
    private func huelleNachziehen(alter: Tresor?, neu: Tresor, _ ergebnis: inout Schutzergebnis) {
        ...
        ergebnis.nebendateien = sicherung.ablage.altbestaendeVersiegeln(mit: neu, alter: alter, hoechstens: Planungsdatei.hoechstgroesseEigene)
    ...
    private func schutzMelden(_ ergebnis: Schutzergebnis, getan: String, nichtGetan: String) {
        ...
        case .zurueckgenommen(let grund):
            melden("\(nichtGetan): \(grund). Die Planung bleibt, wie sie war.", .warnung)
            return
    ...
    func verschluesselungEinschalten() -> Schutzergebnis {
        ...
        if !sicherung.istVorschau {
            if let grund = zugriff.versiegeln(unter: neu) {
                let ergebnis = Schutzergebnis(
                    ablage: .zurueckgenommen("die Lesezeichen ließen sich nicht versiegeln: \(grund)"))
        ...
        if case .zurueckgenommen = ergebnis.ablage {
            ...
            if let alter { _ = zugriff.versiegeln(unter: alter) } else { zugriff.entsiegeln() }
            schutzMelden(ergebnis, getan: getan, nichtGetan: nichtGetan)
            nachEinrichtung = nil
            return ergebnis
        }
    ...
    private func nebendateienNachholen() {
        ...
        let bilanz = sicherung.ablage.altbestaendeVersiegeln(mit: tresor, nurKlartext: true, hoechstens: Planungsdatei.hoechstgroesseEigene)
    ...
    /// Statusdatei tragen diese Wicklung nie.
    func neuVersiegeln() {
        sicherung.neuSchreibenErzwingen()
        jetztSichern()
        zugriff.neuVersiegeln()
        guard !sicherung.istVorschau, let tresor else { return }
        let bilanz = sicherung.ablage.altbestaendeVersiegeln(mit: tresor, hoechstens: Planungsdatei.hoechstgroesseEigene)
        if !bilanz.vollstaendig {
    ...
    func verschluesselungAufheben() -> Schutzergebnis {
        ...
        if !sicherung.istVorschau {
            ergebnis.nebendateien = sicherung.ablage.altbestaendeEntsiegeln(tresor, hoechstens: Planungsdatei.hoechstgroesseEigene)
    ...
    /// Die neue Wicklung in den Tresor der Sitzung, dann jede Datei unter die
    /// neue Hülle. Gerechnet wurde sie für den Schlüssel `kennung` — trägt die
    /// Sitzung inzwischen einen anderen, gehört sie nirgends hinein.
    func passphraseUebernehmen(_ wicklung: Wicklung, fuer kennung: Data) -> Schutzergebnis {
        ...
        huelleNachziehen(alter: tresor, neu: tresor, &ergebnis)
