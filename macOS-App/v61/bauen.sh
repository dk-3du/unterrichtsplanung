...
#  dass er der aktuelle ist (N51-04).
# ──────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
if [ "$NURDMG" = "0" ]; then
  ...
  # nichts davon steht hier zweimal.
  MINDESTSYSTEM="$(/usr/libexec/PlistBuddy -c 'Print :LSMinimumSystemVersion' "$HIER/Beiwerk/Info.plist" 2>/dev/null || true)"
  SDKVERSION="$(xcrun --show-sdk-version 2>/dev/null || true)"
  case "$MINDESTSYSTEM" in *[!0-9.]*|"") echo "Das Mindestsystem (LSMinimumSystemVersion) ließ sich nicht lesen: „$MINDESTSYSTEM“"; exit 1 ;; esac
  case "$SDKVERSION" in *[!0-9.]*|"") echo "Die SDK-Fassung ließ sich nicht bestimmen (xcrun --show-sdk-version): „$SDKVERSION“"; exit 1 ;; esac
  ...
  swift build --configuration "$KONFIGURATION" --arch arm64 \
    -Xlinker -platform_version -Xlinker macos -Xlinker "$MINDESTSYSTEM" -Xlinker "$SDKVERSION"
  ...
  BAUVERSION="$(otool -l "$PAKET/Contents/MacOS/$NAME" | awk '/LC_BUILD_VERSION/{f=1} f&&/minos/{m=$2} f&&/sdk/{s=$2; exit} END{print m" "s}')"
  if [ "$BAUVERSION" != "$MINDESTSYSTEM $SDKVERSION" ]; then
    echo "LC_BUILD_VERSION trägt „$BAUVERSION“ (minos sdk), erwartet „$MINDESTSYSTEM $SDKVERSION“ — es wird nichts geschnürt."
    exit 1
  fi
  echo "  Mindestsystem $MINDESTSYSTEM, Bau-SDK $SDKVERSION (LC_BUILD_VERSION geprüft)"
