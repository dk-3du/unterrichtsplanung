...
Abmeldung (N67-01 Rest, v69; vorher beendet hieße „ABGEBROCHEN <Name> ✗“) — mit „ENDE
...
def gegenprobeBewerten(zeilen, code, paketanfang, auftrag):
    """Die Gegenprobe (N67-01 Rest, v69): Ein Lauf, der mit PRUEFSTAND_FRUEHES_ENDE
    nach seiner ersten Zusicherung regulär endet, darf *nicht* bestehen — und er
    muss sagen, warum: „ABGEBROCHEN <Name>“ statt „ENDE <Name>“. Bestanden hat die
    Gegenprobe, wenn die Bewertung den Lauf abweist, die Abbruchmeldung steht und
    kein ENDE. Liefert (ok, bewertungOk, hatAbgebrochen, hatEnde)."""
    bewertungOk = bewerten(zeilen, code, 0, paketanfang, auftrag)[0]
    hatEnde = any(z.strip() == f"{ENDE} {auftrag}" for z in zeilen)
    hatAbgebrochen = any(z.strip().startswith(f"{ABGEBROCHEN} {auftrag}") for z in zeilen)
    return (not bewertungOk) and hatAbgebrochen and not hatEnde, bewertungOk, hatAbgebrochen, hatEnde
...
def runde(probe, saat, bilanz):
    ...
    def lauf(name, argumente, umgebung=None, ordnerpfad=None, erwartet=0, frist=420, abbruchstelle=None,
        ...
        if gegenprobe:
            ...
            ok, bewertungOk, hatAbgebrochen, hatEnde = gegenprobeBewerten(zeilen, code, paketanfang,
            ...
            bilanz.sag(f"{nummer:02d} {name:28s} {'✓' if ok else '✗'}  GEGENPROBE — Rückgabe {code}, {gruen} ✓, {len(rot)} ✗, "
                       f"Paketzeile {'✓' if paketOk else '✗'}; Bewertung "
                       f"{'✗ wie verlangt' if not bewertungOk else '✓ — DER BEFUND: ein früher regulärer Ausstieg besteht'}, "
                       f"{ABGEBROCHEN} {'steht ✓' if hatAbgebrochen else 'fehlt ✗'}, "
                       f"{ENDE} {'fehlt ✓' if not hatEnde else 'steht ✗'}, {dauer:.0f} s")
            time.sleep(ABSTAND)
            return ok
        ergebnisse.append((name, ok, gruen, len(rot), paketOk, endeOk))
...
def selbsttest():
    ...
    syntax()
    P = "Paket: org.3ducation.Unterrichtsplanung.probe 1.8.0 (69), Quellenstand abcabcabcabc…, Werkzeugstand d840422ba290…"
    F = "Paket: org.3ducation.Unterrichtsplanung 1.8.0 (69), Quellenstand abcabcabcabc…, Werkzeugstand d840422ba290…"
