...
final class Tresor: @unchecked Sendable {
    ...
    /// Die Schlüsselkennung aus dem Kopf — `nil`, wenn sie dort nicht steht
    /// oder nicht wie eine Kennung aussieht. Dann schreibt der Formwächter
    /// nicht (E72).
    static func kennungImKopf(_ kopf: Data) -> String? {
        guard let stelle = kopf.range(of: Data("\"schluesselkennung\":\"".utf8)) else { return nil }
        let rest = kopf[stelle.upperBound...]
        guard let ende = rest.firstIndex(of: UInt8(ascii: "\"")) else { return nil }
        let roh = Data(rest[rest.startIndex..<ende])
        guard roh.count == kennungLaenge * 2, let text = String(data: roh, encoding: .utf8),
