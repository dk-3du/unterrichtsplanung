...
struct UpdatePruefungen {
    ...
    @Test("Über der Schranke: unlesbar, nichts gemerkt — an der Bytezahl wie an Content-Length")
    func zuGross() async throws {
        Antwortstub.antworten["https://stub.test/gross"] = .init(
            status: 200, kopf: ["ETag": "W/\"g\""],
            daten: Data(repeating: UInt8(ascii: " "), count: Updatepruefer.antwortHoechstens + 1))
        let gross = await pruefer("gross", etag: "W/\"alt\"", gemerkt: antwortV33).pruefen()
        #expect(gross.befund == .unlesbar && gross.etag == nil && gross.antwort == nil)

        Antwortstub.antworten["https://stub.test/laenge"] = .init(
            status: 200, kopf: ["Content-Length": "\(Updatepruefer.antwortHoechstens + 1)"], daten: antwortV33)
        #expect(await pruefer("laenge").pruefen().befund == .unlesbar)

        // Genau an der Schranke wird noch gelesen.
        let knapp = antwortV33 + Data(repeating: UInt8(ascii: " "), count: Updatepruefer.antwortHoechstens - antwortV33.count)
        Antwortstub.antworten["https://stub.test/knapp"] = .init(status: 200, kopf: [:], daten: knapp)
        #expect(await pruefer("knapp").pruefen().befund == .neu(try lesen()))
    }
}
