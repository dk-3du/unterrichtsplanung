...
enum Uebergangsprobe {
    ...
    /// Findet der Start eine blockierte Ablage vor (E66), prüft der Lauf
    /// stattdessen die Blockade: kein Weg schreibt, und der Ausweg räumt
    /// gestempelt ins Register (E67).
    ...
    /// Mit `UEBERGANG_ABBRUCH=vorbereitet|uebergeben|einsetzen|aufgeraeumt` bricht die
        ...
        Task { @MainActor in
            ...
            func inventar() -> [String] {
                ((try? FileManager.default.contentsOfDirectory(atPath: ablage.ordner.path)) ?? [])
                    .filter { $0.hasSuffix(".json") || $0.hasSuffix(Uebergangsdienst.zwillingssuffix)
                              || $0.hasSuffix(Uebergangsdienst.vorgaengersuffix) }.sorted()
            }
            func einStand() {
                let namen = inventar()
                pruefen(!namen.contains(Uebergangsdienst.markenname)
                        && !namen.contains(Uebergangsdienst.zweitschriftname), "keine Marke")
                pruefen(!namen.contains { $0.hasSuffix(Uebergangsdienst.zwillingssuffix) }, "kein Zwilling")
                pruefen(!namen.contains { $0.hasSuffix(Uebergangsdienst.vorgaengersuffix) }, "kein Vorgänger")
                pruefen(!namen.contains { $0.contains("-fremd-") || $0.contains("-beschaedigt-")
                                          || $0.contains("-uebergang-") || $0.contains("-vorgaenger-") },
                        "keine Rettungskopie")
            ...
            // Ein blockierter Wiederanlauf lädt nichts — und lässt nichts mehr
            // schreiben. Das ist ein eigener Prüflauf (E66, E67).
            if speicher.blockadeOffen {
                // Ein blockierter Wiederanlauf lädt nichts — und lässt nichts
                // mehr schreiben (E66); der Ausweg räumt gestempelt ins
                // Register (E67).
                pruefen(speicher.blockadeOffen, "der Wiederanlauf blockiert")
                let vorher = inventar()
                if let klasse = speicher.planung?.klassen.first?.id {
                    pruefen(speicher.sitzplanUebernehmen(
                        Sitzplan.anordnen(klasseId: klasse, namen: ["Dana", "Emil"])) != nil,
                        "der Sitzplan wird abgewiesen")
                }
                speicher.jetztSichern()
                pruefen(!ablage.beschaedigtenStandBeiseitelegen(stempel: "probe"), "nichts beiseitegelegt")
                pruefen(ablage.sitzplaeneKopieren(als: "sitzplaene-bereinigt-probe.json") == nil,
                        "keine Rettungskopie")
                pruefen(inventar() == vorher, "der Ordner ist unberührt")
                zeilen.append("  Dateien: " + vorher.joined(separator: ", "))
                // Und der Ausweg: Nach der Rückfrage liegt alles im Register.
                speicher.blockadeRaeumenFragen()
                guard let frage = speicher.rueckfrage else {
                    pruefen(false, "die Rückfrage steht")
                    return
                }
                pruefen(frage.text.contains("Gelöscht wird nichts"), "die Rückfrage sagt, dass nichts gelöscht wird")
                frage.handlung()
                pruefen(!speicher.blockadeOffen, "die Sperre ist geräumt")
                let danach = inventar()
                pruefen(!danach.contains { $0 == Uebergangsdienst.markenname
                                           || $0 == Uebergangsdienst.zweitschriftname
                                           || $0.hasSuffix(Uebergangsdienst.zwillingssuffix)
                                           || $0.hasSuffix(Uebergangsdienst.vorgaengersuffix) },
                        "keine Spur mehr im Weg")
                pruefen(danach.contains { $0.contains("-uebergang-") || $0.contains("-vorgaenger-") },
                        "alles liegt gestempelt im Register")
                zeilen.append("  Register: " + danach.joined(separator: ", "))
                await ende()
                return
            }
            ...
            do {
                ...
                case "wicklung":
                    ...
                    // Seit 1.7.6 mit der Bedingung aus dem Feld der Wicklung (E141).
                    let bedingung = speicher.geraeteWicklungBedingung.map { $0 ? " (Touch ID oder Anmeldepasswort)" : " (Anmeldepasswort)" } ?? ""
                    zeilen.append("  Wicklung dieses Macs: \(vorher ? "entfernt" : "angelegt" + bedingung)")
                case "aufheben":
