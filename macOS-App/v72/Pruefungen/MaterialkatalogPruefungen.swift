...
private func katalog(_ text: String) throws -> Materialkatalog {
    try Materialkatalog.lesen(Data(text.utf8))
}
...
struct MaterialkatalogPruefungen {
    ...
    func literale() throws {
        let l = Materialkatalog.literal
        #expect(try l("[]") == .feld([]))
        #expect(try l(" const X = [ 1 , -2.5 , .5 , 1e3 , true , false , null ] ; ")
    ...
    func fehler() throws {
        ...
        func wirft(_ text: String, _ erwartet: (F) -> Bool, _ kommentar: String) {
        ...
        wirft("[@]", { if case .unerwartet(zeile: 1, spalte: 2, _) = $0 { true } else { false } }, "unbekanntes Zeichen mit Stelle")
    ...
    func grenzfaelle() throws {
        let json = try Data(contentsOf: vorlagenordner.appending(path: "literale-grenzfaelle.erwartet.json"))
        ...
        #expect(faelle.count >= 60, "die Vorlage ist da")
        ...
        for fall in faelle {
            ...
            let gelesen = try? Materialkatalog.literal("const K = " + quelle + ";")
            switch soll {
            case "gleich":
                let wert = try #require(jsc["wert"], Comment(rawValue: "\(name): jsc liefert einen Wert"))
                let literal = try #require(gelesen, Comment(rawValue: "\(name): der Leser nimmt es an"))
        ...
        #expect(gleich >= 20 && abgewiesen >= 35)
    }
