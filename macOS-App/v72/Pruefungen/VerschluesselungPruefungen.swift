...
// gehen über `Ablage.shared`. Den Schlüssel hält jeder Speicher in seiner
// Sitzung — ein zweiter Speicher kennt ihn nicht, wie nach einem Neustart.
...
extension SicherungswegPruefungen {
    ...
    private func ablageLeerenUndEntsperren() {
        ...
        where (name.hasPrefix("planung") || name.hasPrefix("lesezeichen")) && name.hasSuffix(".json") {
    ...
    func einschalten() async throws {
        ...
        // Ein zweiter Stand mit anderem Stempel — über die angehaltene Uhr,
        // nicht über drei Millisekunden Warten (E145).
        Pruefuhr.angehalten(Date().addingTimeInterval(60)) {
        ...
        #expect(speicher.tresor != nil)
        #expect(Tresor.istBehaelter(try rohAufDerPlatte()))
        #expect(Tresor.istBehaelter(try #require(Ablage.shared.vorigeFassungLesen(hoechstens: Planungsdatei.hoechstgroesse))),
    ...
    func startGesperrt() async throws {
        ...
        // Neuer Prozessstart nachgestellt: Der zweite Speicher kennt den Schlüssel nicht.
        let speicher = Planungsspeicher()
        speicher.starten()
        ...
        await speicher.entsperren(passphrase: "falsche Passphrase")
        #expect(speicher.planung == nil)
        ...
        await speicher.entsperren(passphrase: "Passphrase mit Umlaut a\u{0308}")
    ...
    func abbrechen() async throws {
        ...
        #expect(speicher.offenerDialog == .entsperren)
        await speicher.entsperren(passphrase: "Ein Satz, den man behält")
    ...
    func aufheben() throws {
        ...
        if let vorige = Ablage.shared.vorigeFassungLesen(hoechstens: Planungsdatei.hoechstgroesse) {
    ...
    func aendernUndErneuern() throws {
        ...
        let alteKennung = try #require(speicher.tresor?.kennung)
        ...
        #expect(speicher.tresor?.kennung == alteKennung, "nur die Wicklung wechselt")
        ...
        #expect(throws: Tresorfehler.self) {
            try speicher.schluesselErneuernVorbereiten(alt: "nicht die richtige", neu: "Noch ein neuer Satz")
        }
        #expect(throws: Tresorfehler.self, "dieselbe Passphrase erneuert nichts") {
            try speicher.schluesselErneuernVorbereiten(alt: "Ein ganz neuer Satz", neu: "Ein ganz neuer Satz")
        }
        let blatt = try speicher.schluesselErneuernVorbereiten(alt: "Ein ganz neuer Satz", neu: "Noch ein neuer Satz")
        #expect(speicher.tresor?.kennung == alteKennung, "vor der Bestätigung geschieht nichts")
        speicher.verschluesselungEinschalten()
        let neueKennung = try #require(speicher.tresor?.kennung)
        ...
        #expect(throws: Tresorfehler.self, "die bisherige Passphrase öffnet den neuen Schlüssel nicht") {
            try Tresor.oeffnen(kopf: kopf, passphrase: "Ein ganz neuer Satz")
        }
        #expect(try Tresor.oeffnen(kopf: kopf, passphrase: "Noch ein neuer Satz").kennung == neueKennung)
        #expect(speicher.planung?.titel == "Bleibt gleich")
        // Die Vorgängerfassung ist unter dem neuen Schlüssel versiegelt.
        let vorige = try #require(Ablage.shared.vorigeFassungLesen(hoechstens: Planungsdatei.hoechstgroesse))
    ...
    func oeffnenVonAussen() async throws {
        ablageLeerenUndEntsperren()
        defer { ablageLeerenUndEntsperren() }
        let speicher = Planungsspeicher()
        speicher.starten()
        ...
        speicher.jetztSichern()
        try speicher.pruefverschluesselung()
        let eigener = try #require(speicher.tresor)
        ...
        await speicher.entsperren(passphrase: "Fremde Passphrase 2026")
        speicher.rueckfrageBeantworten(true)
    ...
    func beschaedigterBehaelter() async throws {
        ablageLeerenUndEntsperren()
        defer { ablageLeerenUndEntsperren() }
        let speicher = Planungsspeicher()
        speicher.starten()
        ...
        await zweiter.entsperren(passphrase: "Ein Satz, den man behält")
        ...
        for name in liegt where (name.hasPrefix("planung") || name.hasPrefix("lesezeichen")) && name.hasSuffix(".json") {
    ...
    func haekchenRichtetEin() async throws {
        ablageLeerenUndEntsperren()
        defer { ablageLeerenUndEntsperren() }
        let einrichtung = Planungsspeicher()
        einrichtung.starten()
    ...
    func ohneHaekchen() async throws {
        ablageLeerenUndEntsperren()
        defer { ablageLeerenUndEntsperren() }
        let einrichtung = Planungsspeicher()
        einrichtung.starten()
    ...
    func toteWicklung() async throws {
        ...
        // Je Speicher, nicht prozessweit: Andere Reihen laufen derweil ohne Enklave.
        let speicher = Planungsspeicher()
        speicher.enklaveImPruefstandAnlegen = true
        speicher.starten()
        ...
        try await abwarten("das Blatt nach der Enklaven-Abfrage", versuche: 2000) { speicher.offenerDialog == .entsperren }
...
struct KopieVerschluesseltPruefungen {
    ...
    @Test("Eine Klartext-Statusdatei gilt bei versiegelter Ablage nicht — benannt, nicht übernommen, nicht angefasst")
    func statusKlartext() throws {
        ...
        let statusdatei = ziel.appending(component: Statusdatei.name)
        let roh = try Statusdatei.schreiben(stand)
        try roh.write(to: statusdatei, options: [.atomic])
        speicher.statusUebernehmen()
        let eins = try #require(speicher.planung?.eintraege.first { $0.id == "e-1" })
        #expect(!eins.erledigt && eins.kommentar.isEmpty)
        #expect(speicher.meldungen.contains { $0.art == .warnung && $0.text.contains("unverschlüsselt") })
        #expect(try Data(contentsOf: statusdatei) == roh, "die Datei bleibt, wie sie war")
    }

    @Test("Eine Wicklung aus der Statusdatei bleibt dort — der Schlüsselkopf der Ablage ändert sich nicht")
    func fremdeWicklungBleibtDraussen() throws {
        let ziel = try ordner()
        defer { try? FileManager.default.removeItem(at: ziel) }
        let speicher = Planungsspeicher(vorschau: try planung())
        speicher.autoexportZielSetzen(ziel.path)
        ...
        // Eine Statusdatei unter dem eigenen Schlüssel, mit einer angehängten Wicklung.
        ...
        try traeger.versiegeln(try Statusdatei.schreiben(stand), inhalt: .status, ziel: .kopie)
            .write(to: ziel.appending(component: Statusdatei.name), options: [.atomic])
        speicher.statusUebernehmen()
        // Der Stand kommt an, die Wicklung nicht.
        #expect(speicher.planung?.eintraege.first { $0.id == "e-1" }?.erledigt == true)
        #expect(!tresor.hat("passkey"))

        #expect(speicher.autoexportAusfuehren())
        let kopf = try Tresor.kopfLesen(try Data(contentsOf: ziel.appending(component: "Kopie_Tresor.json")))
        #expect(kopf.wicklung("passkey") == nil)
    }
}
