...
struct TourPruefungen {
    ...
    /// Wartet auf eine Meldung, die der Speicher verzögert ausspricht (300 ms
    /// nach dem Ende der Tour) — mit Frist und Meldung statt still (E145).
    private func meldungAbwarten(_ s: Planungsspeicher, _ teil: String) async throws {
        try await abwarten("Meldung „\(teil)“") { s.meldungen.contains { $0.text.contains(teil) } }
    }
