...
final class Rasterbuehne: NSView {
    ...
    /// Für die Tour (K3): der erste Klassenkopf der Kursspalte — die Karte
    /// hängt rechts daran. `hoehe` kommt aus den Rasterdaten; die Spalte
    /// selbst kennt ihre Zeilen nicht. Gilt bei Rollstand 0 der Zeilen.
    func klassenkopfAnker(hoehe: CGFloat) -> Tourfuehrer.Anker? {
        guard let kursspalte, hoehe > 0 else { return nil }
        let y = kursspalte.isFlipped ? 0 : kursspalte.bounds.height - hoehe
        return Tourfuehrer.Anker(
            ansicht: kursspalte,
            rahmen: NSRect(x: 0, y: y, width: Masse.spalteKlasse, height: hoehe),
            kante: .maxX)
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ForEach(zeilen, id: \.klasse.id) { zeile in
                Klassenkopf(klasse: zeile.klasse, sonderzeilenBeginn: zeile.sonderzeilenBeginn)
