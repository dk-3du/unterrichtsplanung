...
extension Planungsspeicher {
    ...
    func versetzen(_ vorhaben: [Vorhaben], nach ziel: Zellenort, verschieben: Bool,
        ...
        aendern(verschieben ? Schrittname.vorhabenVerschieben : Schrittname.vorhabenEinsetzen) { $0 = p }
        auswahl = neueAuswahl
        zielzelle = nil
    ...
    private func einsetzen(_ vorhaben: [Vorhaben], nach ziel: Zellenort,
        ...
        // Eine Gruppe geht als **ein** Schritt zurück — sie ist ein Handgriff.
        let name = if verschieben {
            ausDerselbenZelle ? Schrittname.reihenfolgeAendern : Schrittname.vorhabenVerschieben
        } else {
            Schrittname.vorhabenEinsetzen
        }
        aendern(name) { $0 = p }
    ...
    func auswahlLoeschen() {
        ...
        fragen("\(gewaehlt.count) Vorhaben endgültig entfernen?\n\n"
            ...
            aendern(Schrittname.vorhabenEntfernen) { $0.eintraege.removeAll { kennungen.contains($0.id) } }
            auswahl = []
            if let inhalt = ablage,
