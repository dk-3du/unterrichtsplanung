...
extension Planungsspeicher {
    ...
    func klasseHinzufuegen() -> String? {
        ...
        guard p.klassen.count < Kennwerte.maxKlassen else {
            melden("Mehr als \(Kennwerte.maxKlassen) Zeilen sind nicht vorgesehen.",
                   .warnung)
            return nil
        }
        let neu = Klasse(
        ...
        aendern(Schrittname.kursHinzufuegen) { p in
            // Vor die Sonderzeilen, nicht ans Ende (E157).
            p.klassen.insert(neu, at: p.unterrichtszeilen)
            p.farbenVervollstaendigen()
        }
        return neu.id
    }

    /// Eine Sonderzeile (E157): höchstens eine je Art; sie steht am Ende in
    /// fester Folge. Liefert die Kennung — oder nil, wenn es sie schon gibt,
    /// kein Platz ist oder die Art keine Sonderzeile ist.
    @discardableResult
    func sonderzeileHinzufuegen(_ art: Zeilenart) -> String? {
        guard let p = planung, art.istSonderzeile else { return nil }
        guard !p.hatSonderzeile(art) else {
            melden("„\(art.titel)“ gibt es in dieser Planung schon.")
            return nil
        }
        guard p.klassen.count < Kennwerte.maxKlassen else {
            melden("Mehr als \(Kennwerte.maxKlassen) Zeilen sind nicht vorgesehen.",
        ...
        let neu = Klasse.sonderzeile(art)
        aendern(Schrittname.sonderzeileHinzufuegen(art)) { p in
    ...
    func klasseAendern(id: String, name: String? = nil, fach: String? = nil, notiz: String? = nil) {
        ...
        // Der Titel einer Sonderzeile ist fest (E157) — der Speicher lässt ihn stehen,
        // auch wenn ein Aufrufer einen Namen reicht (B36, v70); das Blatt zeigt kein Feld.
        let neueBezeichnung = (planung?.klasse(id)?.istSonderzeile ?? false)
            ? nil : gekappt(name, "Die Bezeichnung")
        let neueNotiz = gekappt(notiz, "Die Notiz")
        // Weiteres und Vertretungen tragen kein Fach (E161) — das Feld ist abgeblendet.
        let neuesFach = (planung?.klasse(id)?.erlaubtFach ?? true) ? gekappt(fach, "Das Fach") : nil
    ...
    func kursdateiSetzen(klasse id: String, art: Kursdateiart, pfad: String?) {
        ...
        guard erlaubtAnZeile(id, art) else { return }
    ...
    /// Eine Sonderzeile kann nicht alles (E161): Was nicht vorgesehen ist,
    /// wird benannt und nicht gesetzt — die Oberfläche bietet es gar nicht an.
    func erlaubtAnZeile(_ id: String, _ art: Kursdateiart) -> Bool {
        guard let zeile = planung?.klasse(id) else { return false }
        guard zeile.erlaubt(art) else {
            melden("Für „\(zeile.name)“ ist keine \(art.dateibeschriftung) vorgesehen.", .warnung)
            return false
        }
        return true
    }
    ...
    func kursdateiWaehlen(klasse id: String, art: Kursdateiart) {
        guard erlaubtAnZeile(id, art) else { return }
        Systemzugriff.dateiWaehlen(start: planung?.basis ?? "",
                                   titel: art.auswahltitel, zugriff: zugriff) { [weak self] gewaehlt in
            guard let self, let gewaehlt else { return }
            kursdateiSetzen(klasse: id, art: art, pfad: gewaehlt)
        }
    }

    /// Nur unter den Klassen/Kursen: Sonderzeilen stehen fest (E157).
    func klassenTauschen(_ a: Int, _ b: Int) {
        aendern(Schrittname.kurseUmstellen) { p in
            guard p.klassen.indices.contains(a), p.klassen.indices.contains(b),
                  a < p.unterrichtszeilen, b < p.unterrichtszeilen else { return }
    ...
    func klasseEntfernen(_ klasse: Klasse) {
        ...
        fragen(frage, bestaetigung: "Entfernen", gefahr: true, ort: .klassen) { [weak self] in
            ...
            // Die Tasche vor dem Entfernen füllen: Danach ist er fort. Immer eine —
            // auch leer (B31): Ein Sitzplan, der nach ⌘Z angelegt wird, muss beim
            // ⇧⌘Z mitreisen können.
            let beiwerk = sitzplanBeiwerk(fuer: klasse.id)
    ...
    /// Das Beiwerk eines Löschschritts, der den Sitzplan einer Klasse mitnimmt
    /// — Kurs entfernen und Sitzplan löschen tragen dasselbe (E110). Die Tasche
    /// nimmt beim Anlegen den Sitzplan auf, der jetzt liegt; **`vor` greift beim
    /// Wiederholen den, der dann liegt**, und `zurueck` legt genau den zurück —
    /// nicht die Momentaufnahme vom ersten Löschen (N58-02). Richtungsweise
    /// gefragt: Zurücklegen braucht schreibbare Sitzpläne nur, wenn die Tasche
    /// etwas trägt; Entfernen braucht sie, um zu wissen, was da ist.
    func sitzplanBeiwerk(fuer klasseId: String) -> Ruecknahme<Planung>.Beiwerk {
        let tasche = Ruecknahme<Planung>.Tasche(sitzplaene.plan(fuer: klasseId))
        return Ruecknahme<Planung>.Beiwerk(
            moeglich: { [weak self] richtung in
                guard let self else { return "die Planung ist fort" }
                if richtung == .zurueck, tasche.inhalt == nil { return nil }
                return sitzplaene.schreibbar ? nil
                    : (sitzplaene.sperrhinweis ?? "die Sitzpläne sind gesperrt")
            },
            zurueck: { [weak self] in
                guard let self, let plan = tasche.inhalt else { return }
                tasche.inhalt = nil
                sitzplanZurueck(plan, fuer: klasseId)
            },
            vor: { [weak self] in
                guard let self else { return }
                tasche.inhalt = sitzplaene.plan(fuer: klasseId)
                sitzplaene.setzen(nil, fuer: klasseId)
            })
    }
    ...
    func pruefungUmschalten(_ id: String) {
        ...
        // Entweder Prüfung oder Korrektur — an einer Korrektur ohne Wirkung.
        guard !p.eintraege[stelle].korrektur else { return }
