...
extension Planungsspeicher {
    ...
    func vorhabenSichern(_ entwurf: VorhabenEntwurf) {
        ...
        var langeNamen = 0
        // Erst bereinigen, dann kappen — wie der Dateileser (`nichtLeer`): Was
        // hier bliebe, nähme er beim nächsten Start heraus und meldete es.
        func benennung(_ wert: String) -> String {
            let sauber = Planungsdatei.ohneSteuerzeichen(wert)
            guard sauber.count > Planungsdatei.maxNamenslaenge else { return sauber }
            langeNamen += 1
            return String(sauber.prefix(Planungsdatei.maxNamenslaenge))
        }
