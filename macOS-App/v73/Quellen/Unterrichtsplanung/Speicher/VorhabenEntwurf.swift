...
struct VorhabenEntwurf: Identifiable, Equatable {
    ...
    /// Bezeichnung zu lang war.
    static func bezeichnung(_ wert: String, ersatz: @autoclosure () -> String) -> (text: String, gekuerzt: Bool) {
        func sauber(_ text: String) -> String {
            Planungsdatei.ohneSteuerzeichen(text).trimmingCharacters(in: .whitespacesAndNewlines)
        }
        let eigene = sauber(wert)
        let voll = eigene.isEmpty ? sauber(ersatz()) : eigene
        guard voll.count > Planungsdatei.maxNamenslaenge else { return (voll, false) }
        return (String(voll.prefix(Planungsdatei.maxNamenslaenge)), !eigene.isEmpty)
    }
}
