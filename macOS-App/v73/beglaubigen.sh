...
PAKETSTAND=""
PAKETKONFIGURATION=""
# Ohne stimmiges Inventar gibt pruefen.sh keinen Stand aus und schreibt den
# Grund nach stderr (E197, R72-01) — er wird gezeigt, nicht verschluckt. Die
# beiden Ströme bleiben getrennt (E202, B50): Entschieden wird nach Rückgabe und
# Form der Kennung auf stdout; was sonst auf stderr steht (etwa eine Warnung von
# perl, das hinter shasum steckt), verwirft keinen gültigen Stand.
STANDFEHLER="$(/usr/bin/mktemp "${TMPDIR:-/tmp}/unterrichtsplanung-standgrund.XXXXXX")"
QUELLENSTAND="$("$HIER/pruefen.sh" --stand 2>"$STANDFEHLER")" || QUELLENSTAND=""
[[ "$QUELLENSTAND" =~ ^[0-9a-f]{64}$ ]] || QUELLENSTAND=""
STANDGRUND="$(cat "$STANDFEHLER")"
rm -f "$STANDFEHLER"
if [ -d "$PAKET" ]; then
