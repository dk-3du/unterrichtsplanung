...
final class Sicherungsdienst {
    ...
    /// Die Statusdatei der Ansicht im Zielordner unter den neuen Schlüssel
    /// bringen — Klartext oder Behälter unter `alter` → Behälter unter `neu`,
    /// Inhalt unverändert. Nur im Übergang des Schutzes, unmittelbar nach dem
    /// Übernehmen: Danach zählt bei versiegelter Ablage nur noch ein Behälter.
    /// `false`: keine Datei da, nichts zu tun. Nur ein lesbarer Status wird
    /// neu versiegelt; was nicht gilt, bleibt liegen und wird benannt.
    func statusdateiNeuVersiegeln(alter: Tresor?, neu: Tresor) throws(Statusdateilage) -> Bool {
        do {
            return try imZielordner { ordner in
                ...
                guard groesse > 0 else { return false }
                ...
                if Tresor.istBehaelter(roh) {
                    ...
                    if neu.passt(zu: kopf) { return false }
                    guard let alter, alter.passt(zu: kopf) else { throw Statusdateilage.fremderSchluessel }
                    do { klartext = try alter.oeffnen(kopf: kopf) } catch { throw Statusdateilage.keinStatus(error.localizedDescription) }
                } else {
                    klartext = roh
                }
