...
enum Selbstabbild {
    ...
    static func ordnertestUndBeenden(_ speicher: Planungsspeicher) {
        guard ProcessInfo.processInfo.arguments.contains("--ordnertest") else { return }
        guard Pruefstandsschranke.eigenerOrdnerVerlangt("ORDNERTEST") else { return }
        Task { @MainActor in
            var bestanden = true
            ...
            let wurzel = Ablage.shared.ordner.appendingPathComponent(
            ...
            print("ORDNERTEST Sandbox: \(Ordnerzugriff.imSandbox ? "an" : "aus"), Container: "
                  + "\(Ablage.container?.path ?? "—"), Prüfordner: \(Ablage.shared.ordner.path)")
            do {
    ...
    static func updatetestUndBeenden(_ speicher: Planungsspeicher) {
        guard ProcessInfo.processInfo.arguments.contains("--updatetest") else { return }
        guard Pruefstandsschranke.eigenerOrdnerVerlangt("UPDATETEST") else { return }
    ...
    /// nennt den Ort.
    static func abbildziel(_ gewuenscht: URL, container: URL? = Ablage.container,
                           pruefordner: URL = Ablage.shared.ordner) -> URL {
    ...
    private static func ablegen(_ fenster: NSWindow, nach gewuenscht: URL) {
        let ziel = abbildziel(gewuenscht)
        if ziel != gewuenscht {
