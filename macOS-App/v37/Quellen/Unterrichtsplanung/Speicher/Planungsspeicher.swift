...
final class Planungsspeicher {
    ...
    private(set) var planung: Planung? {
        didSet {
            ...
            auswahlNachfuehren()
            if planung?.basis != oldValue?.basis { freigabenNachfuehren() }
        }
    }
    ...
    private func imZielordner<T>(_ arbeit: (URL) throws -> T) throws -> T {
        ...
        do {
            return try Ordnerzugriff.mit(gemerkt.path) { ordner in
                ...
                if ordner.path != autoexportOrdner { autoexportOrdner = ordner.path }
    ...
    /// Nachwahl (`nachwahlPruefen`). Ein Lesezeichen ohne Sicherheitsbereich
    /// (Einstellung von vor dem Sandbox) ist gegenstandslos und wird entfernt.
    func lesezeichenNachruesten() {
        Einstellungen.entfernen(Schluessel.autoexportLesezeichenAlt)
        defer { freigabenNachfuehren() }
        guard !autoexportOrdner.isEmpty,
              Ordnerzugriff.zustaendig(fuer: autoexportOrdner) == nil else { return }
    ...
    func autoexportAusfuehren(vomNutzer: Bool = false) -> Bool {
        ...
        do {
            return try imZielordner { ordner in
                // Nur verschlüsselt. Gesperrt heißt gesperrt; ohne Verschlüsselung
                // sagt der Weg, warum nichts kommt.
        ...
        } catch Zielordnerlage.keinZugriff {
            autoexportFehlerVormerken(
                "Auf den Zielordner der Sicherungskopie darf die App noch nicht zugreifen: "
                + "\(autoexportOrdner). Bitte den Ordner einmal wählen — unter „Einstellungen“ "
    ...
    func verschluesselungEinschalten() {
        ...
        neuVersiegeln()
        let bilanz = Ablage.shared.altbestaendeVersiegeln(alter: alter)
    ...
    private func nebendateienNachholen() {
        guard !istVorschau, verschluesselungsstand == .an else { return }
        let bilanz = Ablage.shared.altbestaendeVersiegeln(nurKlartext: true)
    ...
    func verschluesselungAufheben() {
        ...
        neuVersiegeln()
        let bilanz = istVorschau ? Ablage.Nebendateienbilanz() : Ablage.shared.altbestaendeEntsiegeln(tresor)
        let alt = bilanz.umgestellt.count
    ...
    func passphraseAendern(alt: String, neu: String) throws {
        ...
        neuVersiegeln()
        let bilanz = Ablage.shared.altbestaendeVersiegeln()
    ...
    private(set) var ausstehendeFreigaben: [Ordnerfreigabe] = []
    /// „Später“ gilt für diese Sitzung; der nächste Start fragt wieder.
    private var nachwahlVerschoben = false
    /// Für Prüfstand und Abbild: das Blatt auch ohne Sandbox.
    private var nachwahlProbe = false

    private func freigabenNachfuehren() {
        guard Ordnerzugriff.imSandbox || nachwahlProbe else {
            if !ausstehendeFreigaben.isEmpty { ausstehendeFreigaben = [] }
            return
        }
        var liste: [Ordnerfreigabe] = []
        if !autoexportOrdner.isEmpty, Ordnerzugriff.zustaendig(fuer: autoexportOrdner) == nil {
            liste.append(Ordnerfreigabe(zweck: .zielordner, pfad: autoexportOrdner))
        }
        if let basis = planung?.basis, !basis.isEmpty, Ordnerzugriff.zustaendig(fuer: basis) == nil {
    ...
    func nachwahlWaehlen(_ freigabe: Ordnerfreigabe) {
        let titel = "Zugriff erlauben: „\(Pfade.dateiName(freigabe.pfad))“ wählen — oder einen "
        ...
        if Ordnerzugriff.zustaendig(fuer: freigabe.pfad) != nil {
    ...
    // ── Updates ───────────────────────────────────────────────────────────
    // Nur mit Einwilligung, höchstens einmal je Woche, nie im Prüfstand.
    // Der Prüfer (`Updatepruefer`) fragt und vergleicht; hier liegt, was zu
    // merken ist, und wann das Blatt kommen darf.
    ...
    private(set) var updatestand = Updatestand()
    /// Das Release, das das Blatt zeigt — `nil`, sobald es beantwortet ist.
    private(set) var update: Veroeffentlichung?
    private(set) var updateLaeuft = false
    /// Das Ergebnis der letzten Prüfung in einem Satz — für die Einstellungen,
    /// wo der Melder unter dem Blatt läge.
    private(set) var updateMeldung: String?
    private var updateBeimStartGeprueft = false

    var updatesErlaubt: Bool { updatestand.erlaubt ?? false }
    var updatesGefragt: Bool { updatestand.erlaubt != nil }

    private static func updatestandLesen() -> Updatestand {
        let einstellungen = UserDefaults.standard
        return Updatestand(
            erlaubt: einstellungen.object(forKey: Schluessel.updatesErlaubt) as? Bool,
            zuletzt: einstellungen.object(forKey: Schluessel.updateZuletzt) as? Date,
            uebersprungen: einstellungen.object(forKey: Schluessel.updateUebersprungen) as? Int,
            etag: einstellungen.string(forKey: Schluessel.updateEtag),
            antwort: einstellungen.data(forKey: Schluessel.updateAntwort))
    }

    private func updatestandSichern() {
        ...
        ablegen(updatestand.erlaubt, Schluessel.updatesErlaubt)
        ablegen(updatestand.zuletzt, Schluessel.updateZuletzt)
        ablegen(updatestand.uebersprungen, Schluessel.updateUebersprungen)
        ablegen(updatestand.etag, Schluessel.updateEtag)
        ablegen(updatestand.antwort, Schluessel.updateAntwort)
    }
    ...
    func updatesErlauben(_ erlaubt: Bool) {
        updatestand.erlaubt = erlaubt
        updatestandSichern()
        if erlaubt { updatesPruefen() }
    }
    ...
    func updatesBeimStartPruefen() -> Bool {
        guard hatPlanung, !updateBeimStartGeprueft else { return false }
        updateBeimStartGeprueft = true
    ...
    func updatesPruefen(erzwungen: Bool = false) {
        guard !updateLaeuft else {
            if erzwungen { melden("Die Prüfung läuft schon.") }
            return
        }
        guard Updatepruefer.faellig(erlaubt: updatesErlaubt, zuletzt: updatestand.zuletzt,
                                    erzwungen: erzwungen) else { return }
        guard let quelle = Updatepruefer.quelle(umgebung: ProcessInfo.processInfo.environment,
                                                pruefstand: Ablage.istPruefstand) else {
        ...
        let pruefer = Updatepruefer(quelle: quelle, installiert: Updates.installierterBuild,
                                    uebersprungen: updatestand.uebersprungen,
                                    etag: updatestand.etag, gemerkt: updatestand.antwort)
        Task { @MainActor [weak self] in
            let ergebnis = await pruefer.pruefen()
            self?.updateErgebnisUebernehmen(ergebnis, erzwungen: erzwungen)
        }
    }

    /// Der Zeitstempel wird in jedem Fall gesetzt; ETag und Antwort nur, wenn
    /// eine kam. Das Blatt nur für Neues — von Hand auch für Übersprungenes.
    func updateErgebnisUebernehmen(_ ergebnis: Updateergebnis, erzwungen: Bool) {
        updateLaeuft = false
        updatestand.zuletzt = .now
        if case .nichtErreichbar = ergebnis.befund {} else {
            updatestand.etag = ergebnis.etag
            updatestand.antwort = ergebnis.antwort
        }
        updatestandSichern()

        switch ergebnis.befund {
        case .neu(let release):
            updateMeldung = "Neu: \(release.titel)."
            update = release
            updateAnzeigenPruefen()
        case .uebersprungen(let release):
            updateMeldung = "\(release.titel) liegt vor — übersprungen."
            guard erzwungen else { return }
            update = release
            updateAnzeigenPruefen()
        case .aktuell:
            updateMeldung = "Auf dem neuesten Stand — Version \(Updates.installierteFassung)."
            if erzwungen { melden("Die App ist auf dem neuesten Stand — Version \(Updates.installierteFassung).") }
        case .nichtErreichbar:
            updateMeldung = "Keine Verbindung zu GitHub."
            if erzwungen {
                melden("Keine Verbindung zu GitHub — bitte später noch einmal versuchen.", .warnung)
            }
        case .unlesbar:
            updateMeldung = "Die Antwort von GitHub ließ sich nicht lesen."
    ...
    func updateUeberspringen() {
        if let build = update?.build {
            updatestand.uebersprungen = build
            updatestandSichern()
        }
        update = nil
    }
    ...
    func updateSeiteOeffnen() {
        if let seite = update?.seite { Systemzugriff.adresseOeffnen(seite.absoluteString) }
        update = nil
    }
    ...
    func statusUebernehmen() {
        ...
        // Gelesen im Sicherheitsbereich des Zielordners; ein Ordner, der gerade
        // fehlt oder noch nicht gewählt ist, meldet sich beim Schreiben der Kopie.
        let gelesen: Data?? = try? imZielordner { ordner in
        ...
        guard let roh = gelesen ?? nil else { return }
    ...
    init() {
        ...
        // Ein Prüflauf erbt auch die Einwilligung nicht — er fragt ohnehin nie.
        updatestand = Ablage.istPruefstand ? Updatestand() : Planungsspeicher.updatestandLesen()
    }
