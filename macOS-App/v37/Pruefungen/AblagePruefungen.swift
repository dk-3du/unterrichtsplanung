...
struct AblagePruefungen {
    ...
    func nebendateienBilanz() throws {
        ...
        let bilanz = ablage.altbestaendeVersiegeln()
        ...
        let nachgeholt = ablage.altbestaendeVersiegeln(nurKlartext: true)
        #expect(nachgeholt.umgestellt == ["planung-beschaedigt-1.json"])
        #expect(nachgeholt.uebrig.map(\.name) == ["planung-beschaedigt-2.json"])
        #expect(ablage.altbestaendeVersiegeln(nurKlartext: true).umgestellt.isEmpty, "nichts mehr im Klartext")
        ...
        weg.tresor = tresor
        let ohneOrdner = weg.altbestaendeVersiegeln()
