...
struct SandboxPruefungen {
    ...
    private func aufraeumen(_ orte: URL...) {
        for ort in orte {
            Ordnerzugriff.vergessen(ort.path)
    ...
    func ohneSandbox() {
        ...
        #expect(Ordnerzugriff.erreichbar("/nicht/da/\(UUID().uuidString).pdf"),
        ...
        if case .fehlt = Systemzugriff.dateiOeffnen("/nicht/da/\(UUID().uuidString).pdf") {} else {
    ...
    func zustaendig() throws {
        ...
        #expect(Ordnerzugriff.zustaendig(fuer: datei) == nil)

        let eintrag = try Ordnerzugriff.merken(wurzel)
        #expect(eintrag == Ordnerzugriff.kanonisch(wurzel.path))
        #expect(Ordnerzugriff.zustaendig(fuer: datei) == eintrag)
        #expect(Ordnerzugriff.zustaendig(fuer: wurzel.path + "/") == eintrag, "Endschrägstrich zählt nicht")
        #expect(Ordnerzugriff.zustaendig(fuer: wurzel.path + "-anderer/x.pdf") == nil,
                "ein gleicher Anfang im Namen ist kein Unterordner")
        #expect(Ordnerzugriff.zustaendig(fuer: unten.path + "/../oben.pdf") == eintrag,
                "`..` wird zusammengefasst")

        let untenEintrag = try Ordnerzugriff.merken(unten)
        #expect(Ordnerzugriff.zustaendig(fuer: datei) == untenEintrag, "das nächstliegende Lesezeichen")
        #expect(Ordnerzugriff.zustaendig(fuer: wurzel.path + "/oben.pdf") == eintrag)
        #expect(Ordnerzugriff.alle.contains(eintrag) && Ordnerzugriff.alle.contains(untenEintrag))
    }
    ...
    func aufloesenUndMit() throws {
        ...
        try Ordnerzugriff.merken(wurzel)
        #expect(try Ordnerzugriff.aufloesen(datei.path).path == Ordnerzugriff.kanonisch(datei.path))
        let gelesen = try Ordnerzugriff.mit(datei.path) { try String(contentsOf: $0, encoding: .utf8) }
    ...
    func verschobenerOrdner() throws {
        ...
        let alterEintrag = try Ordnerzugriff.merken(vorher)
        ...
        #expect(try Ordnerzugriff.mit(datei.path) { try String(contentsOf: $0, encoding: .utf8) }
                == "Inhalt")
        #expect(Ordnerzugriff.zustaendig(fuer: nachher.path) != nil, "der neue Ort hat sein Lesezeichen")
        #expect(Ordnerzugriff.alle.contains(alterEintrag),
    ...
    func benannteFehler() throws {
        ...
        #expect(throws: Ordnerzugriff.Fehler.self) { try Ordnerzugriff.aufloesen(fremd) }
        do {
            _ = try Ordnerzugriff.aufloesen(fremd)
        ...
        defer { Ordnerzugriff.vergessen(wurzel.path) }
        do {
            _ = try Ordnerzugriff.aufloesen(wurzel.path + "/x.pdf")
    ...
    func merkenScheitertBenannt() {
        ...
        #expect(throws: Ordnerzugriff.Fehler.self) { try Ordnerzugriff.merken(fremd) }
    }

    @Test("Im Prüflauf bleiben die Lesezeichen im Speicher, nicht in den Einstellungen")
    ...
    func zielordnerLesezeichen() throws {
        ...
        #expect(Ordnerzugriff.zustaendig(fuer: ziel.path) != nil)
        #expect(speicher.ausstehendeFreigaben.isEmpty)
    }
}
