...
struct SystemzugriffPruefungen {
    ...
    func aliasAufgeloest() throws {
        ...
        guard case .abgewiesen = Systemzugriff.dateiOeffnen(aufProgramm.path) else {
    ...
    func aliasUnaufloesbar() throws {
        ...
        guard case .abgewiesen(let text) = Systemzugriff.dateiOeffnen(verweis.path) else {
            Issue.record("erwartet war .abgewiesen")
            return
        }
        ...
        guard case .fehlt = Systemzugriff.dateiOeffnen(o.appending(component: "nie.txt").path) else {
