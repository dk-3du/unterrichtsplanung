...
if [ -d "$PAKET" ]; then
  ...
  # lief, keinen Befund trägt und zum Stand der Quellen passt (nicht zur Uhr:
  # gebaut wird nach dem Prüfen). Bei --probe ein Hinweis; --ja verlangt ihn.
  ...
  if [ -n "$LAEUFT" ] && { [ -z "$VERMERK" ] || [ "$LAEUFT" -nt "$VERMERK" ]; }; then
    ...
    ok "Prüfvermerk $(basename "$VERMERK") ist vollständig, passt zum Stand der Quellen, ohne Befund"
    VERMERK_GILT=1
  fi
