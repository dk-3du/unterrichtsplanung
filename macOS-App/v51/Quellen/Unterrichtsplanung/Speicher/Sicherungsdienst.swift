...
final class Sicherungsfolge: @unchecked Sendable {
    ...
    /// unvollständig, schreibt der nächste Anlass ihn erneut.
    func uebergeben(nummer: Int, stand: String,
        ...
        sperre.withLock {
            ...
            letzterStand = befund == .vollendet ? stand : nil
            return befund
        }
    }
...
final class Sicherungsdienst {
    ...
    /// Vollendet trägt die Platte den Stand der Sitzung; sonst schreibt der
    /// nächste Anlass ihn erneut, und der nächste Start vollendet.
    func uebergangEinsetzen(nummer: Int, stand: String) -> Uebergangsdienst.Einsetzbefund {
        let befund = folge.uebergeben(nummer: nummer, stand: stand) { uebergang.einsetzen() }
        if befund == .vollendet {
            gesicherterStand = stand
