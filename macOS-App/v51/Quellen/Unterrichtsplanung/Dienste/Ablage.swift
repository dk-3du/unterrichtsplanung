...
final class Ablage: @unchecked Sendable {
    ...
    /// (`…-uebergang-<Stempel>.json`). Jeder Wechsel der Hülle geht über diese
    /// Liste — nichts daneben wird von Hand nachgezogen.
    ...
    /// Behälter unter einer älteren Hülle. Mit `nurKlartext` bleibt liegen, was
    /// schon Schlüssel und Hülle der Sitzung trägt; ein Behälter unter einem
    /// fremden Schlüssel zählt als übrig. Kein Fehler wird verschluckt: Jede
    /// Datei, die nicht umgestellt wurde, steht in der Bilanz. Die Übergänge
    /// selbst schreiben nicht hier — sie nehmen die Nebendateien in ihre
    /// Generation (`generationErzeugen`).
    @discardableResult
    func altbestaendeVersiegeln(mit tresor: Tresor, nurKlartext: Bool = false,
        ...
        sperre.withLock {
            ...
            for url in dateien {
                ...
                do {
                    ...
                    if nurKlartext, Tresor.istBehaelter(roh), let kopf = try? Tresor.kopfLesen(roh),
