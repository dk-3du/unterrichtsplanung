...
struct MaterialladerPruefungen {
    ...
    func unlesbar() async {
        ...
        #expect(await mit(Data("const KATEGORIEN = {}".utf8)) == .unlesbar(Materialkatalog.Fehler.keineListe.description))
        // Ohne Deklaration ist es für die Startseite keine Liste — der Grund sagt es (E198, v73).
        guard case .unlesbar(let ohne) = await mit(Data("{}".utf8)) else {
            Issue.record("ohne Deklaration müsste unlesbar sein"); return
        }
        #expect(ohne.hasPrefix("Zeile 1, Spalte 1: die Datei deklariert keine Liste „KATEGORIEN“"), Comment(rawValue: ohne))
...
struct MaterialienSpeicherPruefungen {
    ...
    @Test("Bezeichnungen überstehen das Wiederlesen: bereinigt, getrimmt, bei leer der Name des Lesers, dann gekappt")
    func bezeichnungenRundreise() throws {
        let s = try speicher()
        var entwurf = VorhabenEntwurf(klasseId: try #require(s.planung?.klassen.first?.id), woche: 0)
        entwurf.titel = "Stunde"
        let voll = String(repeating: "a", count: Planungsdatei.maxNamenslaenge)
        // (Bezeichnung, Adresse oder Pfad, erwartet) — R72-04, B48 (E200, v73).
        let links: [(String, String, String)] = [
            ("\u{202E}", "https://example.org/eins", "example.org"),
            (" \u{202E} ", "https://www.example.org/zwei", "example.org"),
            ("", "https://example.org/drei", "example.org"),
            ("\u{7} x", "https://example.org/vier", "x"),
            ("   ", "https://example.org/fuenf", "example.org"),
            (voll + "\u{202E}b", "https://example.org/sechs", voll),
            ("مرحبا שלום", "https://example.org/sieben", "مرحبا שלום"),
        ]
        let materialien: [(String, String, String)] = [
            ("", "/tmp/blatt.pdf", "blatt.pdf"),
            ("\u{7}", "/tmp/zwei.pdf", "zwei.pdf"),
            ("   ", "/tmp/drei.pdf", "drei.pdf"),
            (" \u{7}Blatt ", "/tmp/vier.pdf", "Blatt"),
            ("", "/tmp/fu\u{202E}enf.pdf", "fuenf.pdf"),
        ]
        entwurf.links = links.map { Weblink(titel: $0.0, adresse: $0.1) }
        entwurf.materialien = materialien.map { Material(titel: $0.0, pfad: $0.1) }
        s.vorhabenSichern(entwurf)
        let gesichert = try #require(s.planung?.eintraege.first)
        #expect(gesichert.links.map(\.titel) == links.map(\.2))
        #expect(gesichert.materialien.map(\.titel) == materialien.map(\.2))
        // Die Rundreise: Was die App nach dem Sichern zeigt, liest sie beim nächsten Öffnen.
        let (gelesen, bilanz) = try Planungsdatei.lesenMitBilanz(try Planungsdatei.schreiben(try #require(s.planung)))
        let wieder = try #require(gelesen.eintraege.first)
        #expect(wieder.links.map(\.titel) == gesichert.links.map(\.titel), "Links: gesichert gleich wieder gelesen")
        #expect(wieder.materialien.map(\.titel) == gesichert.materialien.map(\.titel), "Materialien: gesichert gleich wieder gelesen")
