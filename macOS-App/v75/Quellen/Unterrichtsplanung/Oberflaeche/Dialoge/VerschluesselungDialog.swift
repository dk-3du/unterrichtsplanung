...
struct EntsperrenDialog: View {
    ...
    var body: some View {
        ...
        Dialograhmen(titel: fuerAblage ? "Passphrase eingeben" : "Datei entsperren",
            ...
            if mitHaekchen {
                Section {
                    Toggle(Tresor.schalterName, isOn: $speicher.enklaveMerken)
            ...
            Section {
                ...
                if speicher.enklaveMoeglich {
                    Button {
                        ...
                        Label("\(Tresor.freigabeNameGross(biometrie: speicher.geraeteBiometrie)) erneut versuchen",
                              systemImage: speicher.geraeteBiometrie ? Zeichen.touchID : Zeichen.schluessel)
                    }
    ...
    private var fusszeile: String {
        ...
        return fuerAblage ? "Dieselbe Passphrase öffnet die Kopie in der Web App."
    ...
    private var haekchentext: String {
        ...
        return speicher.enklaveMerken
            ? "Danach genügt \(Tresor.freigabeName) dieses Macs; die Passphrase "
              + "bleibt für die Kopie in der Web App und für andere Macs."
...
struct VerschluesselungDialog: View {
    ...
    /// Solange die Ablage nicht geschrieben werden kann, beginnt kein Übergang —
    /// der Grund steht im Blatt, bevor jemand eine Passphrase tippt.
    ...
    var body: some View {
        Dialograhmen(titel: "Verschlüsselung", unterzeile: unterzeile,
            ...
            case .blatt(let schluessel, let erneuern):
                // Der Abschnitt „Dieser Mac“ — womit beim Start geöffnet wird,
                // oder warum die Wicklung fehlt — steht im Schritt selbst (B32, B33).
                Wiederherstellungsschritt(schluessel: schluessel, erneuern: erneuern,
                                          verwahrt: $verwahrt, verstanden: $verstanden)
            ...
            if let sperrgrund {
                Section {
                    Label("Verschlüsselung lässt sich gerade nicht ändern: \(sperrgrund). Sobald die "
                          + "Sicherung wieder schreibt — Beschreibungen und Kommentare kürzen oder "
                          + "Vorhaben entfernen —, geht es weiter.", systemImage: Zeichen.warnung)
                        .foregroundStyle(.orange)
                        .fixedSize(horizontal: false, vertical: true)
                }
            }
    ...
    /// Was der Schalter „Dieser Mac“ beschreibt (E141): die liegende Wicklung —
    /// oder, ohne Wicklung, was „An“ jetzt anlegen würde (das Gerät der Stunde).
    private var dieserMacBiometrie: Bool {
        speicher.enklaveEingerichtet ? speicher.geraeteBiometrie : Tresor.biometrieEingerichtet
    }

    /// Der Fuß sagt, was ist — und den Weg, wenn die Wicklung hinter dem Gerät
    /// zurückbleibt (N64-01: seit 1.7.6 legt „An“ nach dem Gerät der Stunde an,
    /// nicht nach dem vom Start; der Satz war bis dahin nicht wahr).
    private var dieserMacFuss: String {
        var text = "An: Beim Start öffnet \(Tresor.freigabeName(biometrie: dieserMacBiometrie)) die Planung. "
            + "Aus: Beim Start ist die Passphrase fällig. "
        if speicher.geraetewicklungVeraltet {
            text += "Touch ID ist inzwischen eingerichtet — Aus und wieder An stellt die Wicklung darauf um. "
        } else if !Tresor.biometrieEingerichtet {
            text += "Wird Touch ID später eingerichtet, stellt Aus und wieder An die Wicklung darauf um. "
        }
        return text + "Die Kopie beim Beenden und der Export bleiben davon unberührt."
    }
    ...
    private var uebersicht: some View {
        ...
        if Tresor.enklaveVerfuegbar {
            Section {
                Toggle(Tresor.schalterName(biometrie: dieserMacBiometrie), isOn: Binding(
