            ...
            Section {
                if erneuern {
                    ...
                    Text("Ein zufälliger Datenschlüssel versiegelt die Planung mit AES-256. Auf "
                         + "diesem Mac öffnet ihn \(Tresor.freigabeName); die Passphrase "
                         + "öffnet ihn überall — in der Web App, auf einem neuen Rechner, "
...
/// Solange die Schlüsselableitung abseits des Hauptstrangs läuft: ein Kreisel
/// und ein Satz — die Knöpfe des Blatts sind derweil gesperrt.
struct Schluesselarbeitszeile: View {
    var body: some View {
        Section {
            HStack(spacing: 10) {
                ProgressView()
                    .controlSize(.small)
                Text("Der Schlüssel wird abgeleitet — das dauert einen Moment.")
...
/// Der Abschnitt „Dieser Mac“ (B33): womit dieser Mac beim Start öffnet — der
/// Kern fett, damit er ins Auge fällt; Symbol Touch ID oder Schlüssel. Der
/// Hinweis kommt aus dem Ergebnis der Vorbereitung (E139), nicht aus dem Gerät.
struct Starthinweiszeile: View {
    let hinweis: Tresor.Starthinweis
    let symbol: String

    var body: some View {
        Label {
            // Interpolation statt `+`: Das Zusammenfügen von `Text` ist unter
            // macOS 26 veraltet (der Wächter zählte zwei Warnungen).
            Text("\(hinweis.vorsatz.map { $0 + " " } ?? "")\(Text(hinweis.kern).bold()) \(hinweis.rest)")
...
struct Wiederherstellungsschritt: View {
    ...
    /// Der Hinweis „Dieser Mac“ ist gelesen und bestätigt (B33) — ohne beide
    /// Bestätigungen wird nicht eingeschaltet.
    @Binding var verstanden: Bool
    ...
    var body: some View {
        ...
        // Vor dem Einschalten, womit dieser Mac beim Start öffnet (B33) — aus
        // dem Ergebnis der Vorbereitung (E139, N64-02): angelegt → der
        // Starthinweis nach der Bedingung der Wicklung; nicht verfügbar → die
        // Passphrase ist fällig; gescheitert → warum und wie es weitergeht
        // (B32, E131). Bis 1.7.5 galt „kein Grund“ als „Wicklung da“ — im
        // Prüfstand, der die Enklave meidet, in jedem Abbild. Mit eigener
        // Bestätigung: Ein Missverständnis hier kostet viel Frust (Nutzer,
        // 16.09.2026). `nil` (keine Vorbereitung bekannt — hier nicht zu
        // erwarten) fällt auf das Gerät zurück.
        Section {
            switch speicher.vorbereitungGeraet {
            case .gescheitert(let grund)?:
                Label(grund, systemImage: Zeichen.warnung)
                    .foregroundStyle(.orange)
                    .fixedSize(horizontal: false, vertical: true)
            case let geraet?:
                if let hinweis = geraet.starthinweis {
                    Starthinweiszeile(hinweis: hinweis,
                                      symbol: geraet == .angelegt(biometrie: true) ? Zeichen.touchID : Zeichen.schluessel)
                }
            case nil:
                Starthinweiszeile(hinweis: Tresor.starthinweis,
                                  symbol: Tresor.biometrieEingerichtet ? Zeichen.touchID : Zeichen.schluessel)
            }
            Toggle("Ich habe den Hinweis zur Öffnung der Planung verstanden", isOn: $verstanden)
        } header: {
            Text("Dieser Mac")
        } footer: {
            Text(erneuern
                 ? "Erst mit beiden Bestätigungen wird der Schlüssel erneuert."
                 : "Erst mit beiden Bestätigungen wird die Verschlüsselung eingeschaltet. Bis "
