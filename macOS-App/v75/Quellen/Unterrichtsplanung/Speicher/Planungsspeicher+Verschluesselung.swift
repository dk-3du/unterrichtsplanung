...
extension Planungsspeicher {
    ...
    var sperrhinweis: String {
        if let grund = sicherung.wiederanlaufBlockade {
            // In einem blockierten Ordner wird nichts angelegt, was sich nicht
            // sichern ließe (R1): Der Weg führt über „Unterbrochenen Übergang
            // beiseitelegen“ (E67).
            return "Die Ablage ist gesperrt (\(grund)) — bitte den unterbrochenen Übergang unter "
                + "„Ablage → Unterbrochenen Übergang beiseitelegen …“ räumen; bis dahin wird nichts "
                + "angelegt oder geöffnet."
        }
        return sitzung.entsperrung != nil
    ...
    /// Die Bedingung der liegenden Wicklung dieses Macs aus ihrem Feld (E141):
    /// wahr = Touch ID oder Anmeldepasswort, falsch = Anmeldepasswort allein,
    /// `nil` = keine Wicklung oder kein Feld (Dateien bis 1.7.5).
    var geraeteWicklungBedingung: Bool? {
        (tresor?.wicklung(Wicklung.enklave) ?? gesperrterKopf?.wicklung(Wicklung.enklave))?.biometrieBedingung
    }

    /// Womit dieser Mac die Planung öffnet — nach der liegenden Wicklung, und
    /// erst ohne sie oder ohne Feld nach dem Gerät (N64-01: Bis 1.7.5 sagte die
    /// Oberfläche immer, was das Gerät kann, nie, was die Wicklung verlangt).
    var geraeteBiometrie: Bool { geraeteWicklungBedingung ?? Tresor.biometrieEingerichtet }

    /// Die Wicklung bleibt hinter dem Gerät zurück: Anmeldepasswort allein,
    /// obwohl Touch ID inzwischen eingerichtet ist — Aus und wieder An unter
    /// „Dieser Mac“ legt sie nach dem Gerät der Stunde neu an (E140).
    var geraetewicklungVeraltet: Bool { geraeteWicklungBedingung == false && Tresor.biometrieEingerichtet }
    ...
    ///
    /// **Die Wicklung dieses Macs ist eine Bequemlichkeit, keine Bedingung
    /// (B32, E131).** Bis 1.7.4 riss ihr Fehlschlag die ganze Vorbereitung mit:
    /// In einer VM ohne eingerichtete Touch ID war die Verschlüsselung damit
    /// nicht einzuschalten — und ebenso auf jedem Mac ohne Touch ID. Jetzt
    /// steht der Tresor mit Passphrase und Wiederherstellung auch dann, und
    /// das Ergebnis `geraet` sagt, was aus der Wicklung wurde (E139, N64-02):
    /// angelegt — mit der Bedingung `biometrie` —, nicht verfügbar (`enklave`
    /// falsch: nicht versucht) oder gescheitert mit Grund. `anleger` ist im
    /// Betrieb `enklaveAnlegen` mit der Bedingung des Augenblicks (E140); die
    /// Prüfungen spritzen Erfolg und Fehlschlag ein.
    nonisolated static func tresorVorbereiten(
        passphrase: String, enklave: Bool, biometrie: Bool = Tresor.biometrieEingerichtet,
        anleger: (Tresor, Bool) throws -> Void = { try $0.enklaveAnlegen(biometrie: $1) }
    ) throws -> (tresor: Tresor, blatt: String, geraet: Tresor.Geraetewicklung) {
        let tresor = Tresor.neu()
        try tresor.passphraseSetzen(passphrase)
        let blatt = try tresor.wiederherstellungAnlegen()
        guard enklave else { return (tresor, blatt, .nichtVerfuegbar) }
        do { try anleger(tresor, biometrie) } catch {
            return (tresor, blatt, .gescheitert(grund: enklaveHinweis(error, biometrie: biometrie)))
        }
        return (tresor, blatt, .angelegt(biometrie: biometrie))
    }

    /// Der Satz zu einer Wicklung dieses Macs, die nicht zustande kam — mit dem
    /// Grund und dem Weg (E131, Wortlaut des Nutzers vom 16.09.2026). Ohne
    /// Touch ID heißt der erste Satz, was der Mac nicht hat.
    nonisolated static func enklaveHinweis(_ fehler: any Error,
                                           biometrie: Bool = Tresor.biometrieEingerichtet) -> String {
        let grund = fehler.localizedDescription.trimmingCharacters(in: .whitespacesAndNewlines)
        let satz = grund.isEmpty ? "" : " " + grund + (grund.hasSuffix(".") ? "" : ".")
        let anfang = biometrie
            ? "Die Wicklung für diesen Mac (Touch ID oder Anmeldepasswort) ließ sich nicht anlegen:"
            : "Dieser Mac verfügt nicht über Touch ID. Die Wicklung mit dem Anmeldepasswort ließ sich nicht anlegen:"
        return anfang + satz + " Beim Start ist die Passphrase fällig; unter „Verschlüsselung → Dieser Mac“ "
            + "lässt sie sich später einschalten."
    }
    ...
    nonisolated static func erneuernVorbereiten(
        _ alter: Tresor, alt: String, neu: String, enklave: Bool, biometrie: Bool = Tresor.biometrieEingerichtet,
        anleger: (Tresor, Bool) throws -> Void = { try $0.enklaveAnlegen(biometrie: $1) }
    ) throws -> (tresor: Tresor, blatt: String, geraet: Tresor.Geraetewicklung) {
        ...
        return try tresorVorbereiten(passphrase: neu, enklave: enklave, biometrie: biometrie, anleger: anleger)
    }
    ...
    func verschluesselungVorbereiten(passphrase: String, enklave: Bool = Tresor.enklaveVerfuegbar,
                                     anleger: (Tresor, Bool) throws -> Void = { try $0.enklaveAnlegen(biometrie: $1) }) throws -> String {
        let vorbereitet = try Planungsspeicher.tresorVorbereiten(
            passphrase: passphrase, enklave: enklave, biometrie: biometrieQuelle(), anleger: anleger)
        vorbereitung = vorbereitet.tresor
        vorbereitungGeraet = vorbereitet.geraet
    ...
    func verschluesselungVorbereitenAsynchron(passphrase: String) async throws -> String? {
        ...
        let biometrie = biometrieQuelle()
        return try await schluesselarbeit({
            try Planungsspeicher.tresorVorbereiten(passphrase: passphrase, enklave: enklave, biometrie: biometrie)
        }) { vorbereitet in
            vorbereitung = vorbereitet.tresor
            vorbereitungGeraet = vorbereitet.geraet
            return vorbereitet.blatt
    ...
    struct Schutzergebnis: Equatable {
        enum Ablagebefund: Equatable {
            ...
            /// die Marke liegt, nichts schreibt an ihr vorbei — jeder Anlass
            /// holt das Einsetzen nach, der nächste Start vollendet es (E54).
        ...
        var offenes: [String] {
            ...
            if case .unvollendet(let grund) = ablage {
                teile.append("nicht jede Datei eingesetzt (\(grund.ohneSchlusspunkt)) — die Autosicherung ruht, bis der "
                             + "Übergang eingesetzt ist; jeder Schreibanlass versucht es erneut, der nächste Start vollendet ihn")
            }
            ...
            if case .offen(let grund) = statusdatei { teile.append("Statusdatei der Web App (\(grund))") }
            return teile
        }
    }

    /// Der eine Weg jedes Übergangs (E47, E48): die Generation erzeugen —
    /// zuerst die Sitzpläne von ihrem Dienst (mit der Bewahrungskopie, die er
    /// nachholt), dann Ablage, Vorgängerfassung und Register vom
    /// Sicherungsdienst, der Behälter der Lesezeichen vom Ordnerzugriff —,
    /// die Zwillinge legen (*vorbereiten*), die Marke schreiben
    ...
    private func uebergang(_ art: Uebergangsart, alter: Tresor?, neu: Tresor?, aussenstellen: Bool = true,
        ...
        // Zuerst die Sitzpläne: unter den neuen Schlüssel oder in den Klartext;
        // ohne Pläne entfällt die Datei. Ihr Dienst holt dabei eine ausstehende
        // Bewahrungskopie nach (E46) — sie muss liegen, bevor das Register
        // gezählt wird, sonst bliebe sie außerhalb der Generation (N51-03).
        // Gesperrt: aus dem Klartext kein Hindernis (B03); beim Aufheben
        // Abweisung — ohne Schlüssel wären sie für immer zu (E38).
        ...
        if case .unvollendet(let grund) = sicherung.uebergangEinsetzen(nummer: nummer, stand: stand, abdruck: abdruck) {
    ...
    func schutzMelden(_ ergebnis: Schutzergebnis, getan: String, nichtGetan: String) {
        ...
        if ergebnis.statusdatei == .erledigt { nachgezogen.append("die Statusdatei der Web App") }
        ...
        text += " Offen: " + offenes.joined(separator: "; ") + ". Nebendateien holt die App beim nächsten "
            ...
            + "nicht mehr gelesen — bitte in der Web App die neue Kopie öffnen."
        melden(text, .warnung)
    }
    ...
    func verschluesselungEinschalten() -> Schutzergebnis {
        ...
        vorbereitung = nil
        let geraet = vorbereitungGeraet
        vorbereitungGeraet = nil
        ...
        // Die Meldung zur Wicklung dieses Macs kommt aus dem Ergebnis der
        // Vorbereitung (E139, N64-02) — gescheitert: der Grund als Warnung
        // (B32); angelegt ohne Touch ID: dass ab jetzt das Anmeldepasswort
        // öffnet (B33); nicht verfügbar: dass die Passphrase fällig ist. Nicht
        // aus dem Gerät: Bis 1.7.5 versprach die Meldung das Anmeldepasswort,
        // sobald keine Biometrie da war — ob eine Wicklung entstand oder nicht.
        if let meldung = geraet?.meldung {
            melden(meldung.text, meldung.warnung ? .warnung : .hinweis)
        }
    ...
    private func nebendateienNachholen() {
        ...
        let bilanz = sicherung.altbestaendeVersiegeln(mit: tresor, nurAbweichende: true)
    ...
    func verschluesselungAufheben() -> Schutzergebnis {
        ...
        if case .unvollendet(let grund) = ergebnis.ablage {
            text += " Nicht jede Datei ist schon eingesetzt (\(grund.ohneSchlusspunkt)) — die Autosicherung ruht, bis der "
                + "Übergang eingesetzt ist; jeder Schreibanlass versucht es erneut, der nächste Start vollendet ihn."
        }
    ...
    func schluesselErneuernVorbereiten(alt: String, neu: String) throws -> String {
        ...
        let (tresor, blatt, geraet) = try Planungsspeicher.erneuernVorbereiten(
            alter, alt: alt, neu: neu, enklave: Tresor.enklaveVerfuegbar, biometrie: biometrieQuelle())
        vorbereitung = tresor
        vorbereitungGeraet = geraet
        return blatt
    }
    ...
    func schluesselErneuernVorbereitenAsynchron(alt: String, neu: String) async throws -> String? {
        ...
        let biometrie = biometrieQuelle()
        return try await schluesselarbeit({
            try Planungsspeicher.erneuernVorbereiten(alter, alt: alt, neu: neu, enklave: enklave, biometrie: biometrie)
        }) { vorbereitet in
            vorbereitung = vorbereitet.tresor
            vorbereitungGeraet = vorbereitet.geraet
            return vorbereitet.blatt
    ...
    /// Nach dem Entsperren: angleichen — oder warten, solange Klartext-Sitzpläne
    /// auf die Antwort des Nutzers warten (E43). Das Angleichen ist ein Übergang
    /// über alle Dateien; er liefe sonst über die unbestätigten Pläne, ehe die
    /// Frage beantwortet ist (F04). `sitzplanantwortFortsetzen` holt es nach.
    private func enklaveAngleichenNachSitzplanantwort(_ tresor: Tresor) {
        guard sitzplaene.klartextUnbestaetigt else { return enklaveAngleichen(tresor) }
        nachSitzplanantwort = { [weak self] in self?.enklaveAngleichen(tresor) }
    }
    ...
    private func enklaveAngleichen(_ tresor: Tresor) {
        ...
        if enklaveMerken {
            ...
            guard let biometrie = enklaveWicklungAnlegen(tresor) else { return }
            melden(enklaveWicklungPasstNicht
                   ? "Dieser Mac ist neu eingerichtet — die Planung öffnet sich wieder über "
                     + "\(Tresor.freigabeName(biometrie: biometrie))."
                   : "Ab jetzt öffnet sich die Planung auf diesem Mac über \(Tresor.freigabeName(biometrie: biometrie)).")
    ...
    func enklaveAufDiesemMac(_ an: Bool) {
        ...
        if an {
            guard !tresor.hat(Wicklung.enklave), let biometrie = enklaveWicklungAnlegen(tresor) else { return }
            melden("Ab jetzt öffnet sich die Planung auf diesem Mac über \(Tresor.freigabeName(biometrie: biometrie)).")
        } else {
    ...
    /// Die Wicklung dieses Macs anlegen — mit der Bedingung des Augenblicks
    /// (E140) — und alles damit neu versiegeln; liefert die Bedingung, mit der
    /// sie entstand, oder `nil`. Findet der Übergang nicht statt, gilt sie auch
    /// in der Sitzung nicht: Es bleibt, was vorher war — keine oder die alte,
    /// nicht mehr passende Wicklung.
    private func enklaveWicklungAnlegen(_ tresor: Tresor) -> Bool? {
        guard Tresor.enklaveVerfuegbar || enklaveImPruefstandAnlegen else { return nil }
        if let grund = schutzuebergangGesperrt {
        ...
        let biometrie = biometrieQuelle()
        do { try tresor.enklaveAnlegen(biometrie: biometrie, auchImPruefstand: enklaveImPruefstandAnlegen) }
        catch {
            melden(Planungsspeicher.enklaveHinweis(error, biometrie: biometrie), .warnung)
            return nil
        }
    ...
    private func entsperrt(mit tresor: Tresor, durchEnklave: Bool) {
        ...
        case .ablage(let daten):
            ...
            do {
                ...
                if !durchEnklave { enklaveAngleichenNachSitzplanantwort(tresor) }
