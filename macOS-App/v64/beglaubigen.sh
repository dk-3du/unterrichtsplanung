...
if [ -d "$PAKET" ]; then
  ...
  # gewechselt; der Vermerk unten muss denselben nennen.
  WERKZEUG_AKTUELL="$(werkzeugkennung 2>/dev/null || true)"
  ...
  if [ -z "$PAKETWERKZEUG" ]; then
    ...
    nein "das Paket stammt aus einer anderen Werkzeugkette ($(cut -c1-12 <<<"$PAKETWERKZEUG")… statt $(cut -c1-12 <<<"$WERKZEUG_AKTUELL")…) — seit dem Bau wurde Xcode, Swift oder das SDK gewechselt; erst ./bauen.sh"
  else
...
printf '%s  paket  Quellenstand %s  Werkzeugstand %s  CDHash %s  Vermerk %s\n' "$STEMPEL" "$PAKETSTAND" \
  "${PAKETWERKZEUG:-?}" \
