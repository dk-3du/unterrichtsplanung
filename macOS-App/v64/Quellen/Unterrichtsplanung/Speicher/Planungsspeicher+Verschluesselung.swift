...
extension Planungsspeicher {
    ...
    /// `enklaveGrund` sagt, warum die Wicklung fehlt. `anleger` ist im Betrieb
    /// `enklaveAnlegen`; die Prüfungen spritzen Erfolg und Fehlschlag ein.
    nonisolated static func tresorVorbereiten(
        passphrase: String, enklave: Bool,
        anleger: (Tresor) throws -> Void = { try $0.enklaveAnlegen() }
    ) throws -> (tresor: Tresor, blatt: String, enklaveGrund: String?) {
        let tresor = Tresor.neu()
        try tresor.passphraseSetzen(passphrase)
        let blatt = try tresor.wiederherstellungAnlegen()
        var grund: String?
        if enklave {
            do { try anleger(tresor) } catch { grund = enklaveHinweis(error) }
        }
        return (tresor, blatt, grund)
    }
    ...
    nonisolated static func erneuernVorbereiten(
        _ alter: Tresor, alt: String, neu: String, enklave: Bool,
        anleger: (Tresor) throws -> Void = { try $0.enklaveAnlegen() }
    ) throws -> (tresor: Tresor, blatt: String, enklaveGrund: String?) {
        ...
        return try tresorVorbereiten(passphrase: neu, enklave: enklave, anleger: anleger)
    }
    ...
    func verschluesselungVorbereiten(passphrase: String, enklave: Bool = Tresor.enklaveVerfuegbar,
                                     anleger: (Tresor) throws -> Void = { try $0.enklaveAnlegen() }) throws -> String {
        let vorbereitet = try Planungsspeicher.tresorVorbereiten(
            passphrase: passphrase, enklave: enklave, anleger: anleger)
        vorbereitung = vorbereitet.tresor
        vorbereitungHinweis = vorbereitet.enklaveGrund
    ...
    func verschluesselungEinschalten() -> Schutzergebnis {
        ...
        vorbereitung = nil
        let hinweis = vorbereitungHinweis
        vorbereitungHinweis = nil
        ...
        // Eingeschaltet ohne die Wicklung dieses Macs: Der Grund gehört
        // gesagt, nicht nur im Blatt gezeigt (B32). Mit Wicklung, aber ohne
        // Touch ID: dass ab jetzt das Anmeldepasswort öffnet (B33).
        if let hinweis {
            melden(hinweis, .warnung)
        } else if alter == nil, !Tresor.biometrieEingerichtet {
            melden("Beim Start öffnet das Anmeldepasswort dieses Macs die Planung — dasselbe "
                   + "Passwort wie bei der Anmeldung am Mac; die Passphrase gilt auf dem iPad und "
                   + "auf jedem anderen Rechner.")
        }
    ...
    func schluesselErneuernVorbereiten(alt: String, neu: String) throws -> String {
        ...
        let (tresor, blatt, grund) = try Planungsspeicher.erneuernVorbereiten(
            alter, alt: alt, neu: neu, enklave: Tresor.enklaveVerfuegbar)
        vorbereitung = tresor
        vorbereitungHinweis = grund
        return blatt
    }
        ...
        if enklaveMerken {
            ...
            melden(enklaveWicklungPasstNicht
                   ? "Dieser Mac ist neu eingerichtet — die Planung öffnet sich wieder über "
                     + "\(Tresor.freigabeName)."
                   : "Ab jetzt öffnet sich die Planung auf diesem Mac über \(Tresor.freigabeName).")
    ...
    func enklaveAufDiesemMac(_ an: Bool) {
        ...
        if an {
            ...
            melden("Ab jetzt öffnet sich die Planung auf diesem Mac über \(Tresor.freigabeName).")
        } else {
    ...
    private func enklaveWicklungAnlegen(_ tresor: Tresor) -> Bool {
        ...
        do { try tresor.enklaveAnlegen(auchImPruefstand: enklaveImPruefstandAnlegen) }
            ...
            melden(Planungsspeicher.enklaveHinweis(error), .warnung)
            return false
        }
