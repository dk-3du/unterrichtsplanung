...
/// Kern fett, damit er ins Auge fällt; Symbol Touch ID oder Schlüssel.
struct Starthinweiszeile: View {
    var body: some View {
        ...
        Label {
            ...
            Image(systemName: Tresor.biometrieEingerichtet ? Zeichen.touchID : Zeichen.schluessel)
        }
    }
}
...
struct Wiederherstellungsschritt: View {
    ...
    var body: some View {
        ...
        // Vor dem Einschalten, womit dieser Mac beim Start öffnet (B33) — oder,
        // wenn die Wicklung dieses Macs nicht zustande kam, warum und wie es
        // weitergeht (B32, E131). Mit eigener Bestätigung: Ein Missverständnis
        // hier kostet viel Frust (Nutzer, 16.09.2026).
        Section {
            if let hinweis = speicher.vorbereitungHinweis {
                Label(hinweis, systemImage: Zeichen.warnung)
                    .foregroundStyle(.orange)
                    .fixedSize(horizontal: false, vertical: true)
            } else {
