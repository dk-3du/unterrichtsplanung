...
final class Tresor: @unchecked Sendable {
    ...
    /// Fingern)? Einmal je Start bestimmt: Wer Touch ID später einrichtet,
    /// legt die Wicklung über den Schalter „Dieser Mac“ neu an (E135).
    static let biometrieEingerichtet: Bool = {
    ...
    /// allein. Gewickelt wird über den öffentlichen Teil — der ist ohne
    /// Freigabe zu haben (nachgemessen); Anlegen und Neuwickeln kosten keine
    /// Nachfrage. `auchImPruefstand` nur für den Prüfstein ohne Dialog.
    func enklaveAnlegen(auchImPruefstand: Bool = false) throws {
        ...
        guard let schutz = SecAccessControlCreateWithFlags(
            ...
            Tresor.zugriffsbedingung(biometrie: Tresor.biometrieEingerichtet), &fehler)
        else {
            throw Tresorfehler(art: .keineEnklave,
