...
#  Rechner denselben tragen (E127, N61-01).
# ──────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
if [ "$NURDMG" = "0" ]; then
  ...
  WERKZEUGSTAND="$(werkzeugkennung)" || { echo "Der Werkzeugstand ließ sich nicht bestimmen (bauumgebung.sh) — es wird nichts geschnürt."; exit 1; }
