...
struct DienstePruefungen {
    ...
    func sicherungInEigenerAblage() async throws {
        ...
        let dienst = Sicherungsdienst(ablage: ablage, pruefstand: true)
        ...
        dienst.jetztSichern()
        #expect(ablage.vorigeFassungLesen() == nil, "kein zweiter Schreibvorgang, keine Vorgängerfassung")
        ...
        #expect(ablage.vorigeFassungLesen() != nil, "der Klartext-Stand davor liegt daneben")
        ...
        dienst.jetztSichern()
        #expect(dienst.gesicherterStand == p.geaendert)
        #expect(Tresor.istBehaelter(try #require(ablage.vorigeFassungLesen())),
    ...
    func gesperrtSchreibtNicht() throws {
        ...
        let vorschau = Sicherungsdienst(ablage: ablage, vorschau: true, pruefstand: true)
    ...
    func stoerung() throws {
        ...
        let dienst = Sicherungsdienst(ablage: Ablage(ordner: versperrt), pruefstand: true)
        var gemeldet = 0
        dienst.beiStoerung = { gemeldet += 1 }
        var p = try planung()
        p.geaendert = "2026-09-01T10:00:00.000Z"
        dienst.sitzungsquelle = { .klartext(p) }
        ...
        dienst.lageMelden(geklappt: true)
        #expect(!dienst.gestoert)
        dienst.lageMelden(geklappt: false)
        #expect(gemeldet == 2)
    }
    ...
    func kopie() throws {
        ...
        let dienst = Sicherungsdienst(ablage: Ablage(ordner: ablageOrdner), pruefstand: true)
        ...
        let datei = try dienst.kopieSchreiben(.verschluesselt(p, t), name: "Kopie.json")
    ...
    @Test("Entsiegeln: Klartext geht durch; fremder Schlüssel, falscher Inhalt und fehlender Schlüssel werden benannt")
    func entsiegeln() throws {
        ...
        let ohne = try Statusabgleich.entsiegeln(klar, tresor: nil)
        #expect(ohne.daten == klar && !ohne.unverschluesseltTrotzTresor && ohne.uebernommeneWicklungen == 0)
        let trotz = try Statusabgleich.entsiegeln(klar, tresor: t)
        #expect(trotz.unverschluesseltTrotzTresor, "Klartext, obwohl die Ablage einen Schlüssel hat")
        ...
        let eigen = try Statusabgleich.entsiegeln(behaelter, tresor: t)
        #expect(eigen.daten == klar && !eigen.unverschluesseltTrotzTresor)
        ...
        // Eine Wicklung der Ansicht wandert in den eigenen Schlüssel.
        ...
        let mitWicklung = try Statusabgleich.entsiegeln(
            try traeger.versiegeln(klar, inhalt: .status, ziel: .kopie), tresor: t)
        #expect(mitWicklung.uebernommeneWicklungen == 1 && t.hat("passkey"))
        ...
        for hindernis: Statusabgleich.Hindernis in [.verschluesseltOhneTresor, .keinStatus("x"), .fremderSchluessel,
