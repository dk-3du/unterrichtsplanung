...
extension Planungsspeicher {
    ...
    func statusUebernehmen() {
        ...
        let nutzlast: Statusabgleich.Nutzlast
        do {
            nutzlast = try Statusabgleich.entsiegeln(roh, tresor: tresor)
        } catch {
        ...
        if nutzlast.uebernommeneWicklungen > 0 { neuVersiegeln() }
        if nutzlast.unverschluesseltTrotzTresor {
        ...
        do {
            let stand = try Statusabgleich.lesen(nutzlast.daten)
