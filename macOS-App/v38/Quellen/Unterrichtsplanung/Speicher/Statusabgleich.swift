...
enum Statusabgleich {
    ...
    /// Was aus der Datei herauskam, bevor sie gelesen wird.
    struct Nutzlast: Equatable {
        let daten: Data
        /// Klartext, obwohl die Ablage einen Schlüssel hat — eine ältere
        /// Ansichtsfassung oder eine ältere Kopie.
        let unverschluesseltTrotzTresor: Bool
        /// Wicklungen der Ansicht, die der eigene Schlüssel übernommen hat —
        /// die nächste Kopie trägt sie; die Ablage ist neu zu versiegeln.
        let uebernommeneWicklungen: Int
    }

    /// Ein Behälter unter fremdem Schlüssel wird benannt, nicht geraten;
    /// Klartext geht durch. Wicklungen einer Datei, die unter dem eigenen
    /// Schlüssel aufging, wandern in den Tresor.
    static func entsiegeln(_ roh: Data, tresor: Tresor?) throws(Hindernis) -> Nutzlast {
        guard Tresor.istBehaelter(roh) else {
            return Nutzlast(daten: roh, unverschluesseltTrotzTresor: tresor != nil, uebernommeneWicklungen: 0)
        }
        ...
        return Nutzlast(daten: daten, unverschluesseltTrotzTresor: false,
                        uebernommeneWicklungen: tresor.fremdeWicklungenUebernehmen(aus: kopf))
    }
