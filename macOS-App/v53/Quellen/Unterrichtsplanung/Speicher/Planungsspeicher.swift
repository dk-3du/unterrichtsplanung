...
final class Planungsspeicher {
    ...
    private func uebergangWiederanlaufen() -> Bool {
        switch sicherung.uebergangWiederanlaufen(stempel: Zeitrechnung.dateistempel()) {
        case .nichts:
            break
        ...
        case .verworfen(let anzahl):
            melden("Ein Übergang des Schutzes kam beim letzten Beenden nicht bis zur Übergabe — "
                   + "\(anzahl) vorbereitete \(anzahl == 1 ? "Datei wurde" : "Dateien wurden") verworfen; es gilt "
                   + "der bisherige Stand.")
        case .vorgaengerVerwahrt(let verwahrt):
            // Ohne Marke ist nicht zu sagen, welchen Stand sie ersetzten —
            // gelöscht wird deshalb keiner (E60).
            melden("Neben der Planung lagen \(verwahrt.count == 1 ? "die Vorgängerfassung einer Datei" : "Vorgängerfassungen von \(verwahrt.count) Dateien") "
                   + "aus einem Übergang des Schutzes, zu dem keine Marke mehr liegt. Es gilt, was in der Ablage "
                   + "steht; die Vorgänger liegen als \(verwahrt.map { "„\($0)“" }.joined(separator: ", ")) daneben "
                   + "und können von Hand verglichen werden.", .warnung)
