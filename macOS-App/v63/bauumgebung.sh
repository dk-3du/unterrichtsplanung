...
#     werkzeugstand            der Block: Xcode, Swift, SDK, Mindestsystem,
#                              Ziel, Linker — in fester Reihenfolge
#     werkzeugkennung          SHA-256 des Blocks, 64 Hexziffern oder Fehlschlag
...
werkzeugstand() {
  local xcode swiftzeile sdk sdkbau m
  ...
  printf 'Xcode: %s\nSwift: %s\nSDK: macOS %s (%s)\nMindestsystem: %s\nZiel: arm64, release\nLinker: -platform_version macos %s %s\n' \
    "$xcode" "$swiftzeile" "$sdk" "$sdkbau" "$m" "$m" "$sdk"
}

werkzeugkennung() {
  local block k
  block="$(werkzeugstand)" || return 1
