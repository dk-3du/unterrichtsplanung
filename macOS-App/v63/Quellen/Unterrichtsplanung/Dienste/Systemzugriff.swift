...
enum Systemzugriff {
    ...
    private static func imBereich(_ pfad: String, zugriff: Ordnerzugriff,
                                  _ arbeit: (URL) -> Ergebnis) -> Ergebnis {
        let roh = vereinheitlicht(pfad)
        if zugriff.zustaendig(fuer: roh.path) != nil {
            do {
                return try zugriff.mit(roh.path) { ziel in arbeit(vereinheitlicht(ziel.path)) }
    ...
    static func imFinderZeigen(_ pfad: String, zugriff: Ordnerzugriff) -> Ergebnis {
        imBereich(pfad, zugriff: zugriff) { ziel in
    ...
    static func dateiOeffnen(_ pfad: String, zugriff: Ordnerzugriff) -> Ergebnis {
        imBereich(pfad, zugriff: zugriff) { roh in dateiOeffnen(roh, name: Pfade.dateiName(pfad)) }
    }
    ...
    //
    // Jeder Dialog geht mit einem Abschluss (E115, v61): Aus einem Blatt
    // heraus ist er ein Bogen an diesem Blatt und der Abschluss kommt später;
    // aus dem Hauptfenster ist er ein freies Fenster wie bisher, und der
    // Abschluss kommt sofort — `Dialogort` entscheidet das am Schlüsselfenster.
    // Eine leere Liste bzw. `nil` heißt: abgebrochen.

    static func dateienWaehlen(start: String, zugriff: Ordnerzugriff,
                               _ gewaehlt: @escaping @MainActor ([String]) -> Void) {
        auswahl(ordner: false, mehrere: true, start: start,
                titel: "Material auswählen", knopf: "Verknüpfen", zugriff: zugriff, gewaehlt)
    }

    static func dateiWaehlen(start: String, titel: String, zugriff: Ordnerzugriff,
                             _ gewaehlt: @escaping @MainActor (String?) -> Void) {
        auswahl(ordner: false, mehrere: false, start: start,
                titel: titel, knopf: "Zuweisen", zugriff: zugriff) { gewaehlt($0.first) }
    }
    ...
    private static func auswahl(ordner: Bool, mehrere: Bool, start: String,
        ...
        Dialogort.zeigen(dialog) { antwort in
            guard antwort == .OK else { gewaehlt([]); return }
            gewaehlt(gemerkt(dialog.urls, in: zugriff).map(\.path))
        }
    }
    ...
    private static func gemerkt(_ urls: [URL], in zugriff: Ordnerzugriff) -> [URL] {
        zugriff.merken(alle: urls)
        return urls
    }
    ...
    static func zugriffWaehlen(fuer pfad: String, zugriff: Ordnerzugriff,
                               _ gewaehlt: @escaping @MainActor (URL?) -> Void) {
        let dialog = NSOpenPanel()
        ...
        Dialogort.zeigen(dialog) { antwort in
            guard antwort == .OK, let url = dialog.url else { gewaehlt(nil); return }
            gewaehlt(gemerkt([url], in: zugriff).first)
        }
    }

    static func zielWaehlen(name: String, _ gewaehlt: @escaping @MainActor (URL?) -> Void) {
        ...
        Dialogort.zeigen(dialog) { antwort in gewaehlt(antwort == .OK ? dialog.url : nil) }
    }

    static func quelleWaehlen(_ gewaehlt: @escaping @MainActor (URL?) -> Void) {
        let dialog = NSOpenPanel()
