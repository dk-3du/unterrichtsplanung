...
enum Widerrufprobe {
    ...
    static func laufenUndBeenden(_ speicher: Planungsspeicher) {
        Task { @MainActor in
            ...
            // ── Ein Blatt vor dem Schauplatz (E112) ───────────────────────
            // Das Vorhaben-Blatt puffert seinen Entwurf: Solange es steht, ruht
            // der Hauptverlauf — ⌘Z über das Menü ändert nichts dahinter, und
            // das Menü verspricht keinen Schritt; zu, und der Schritt ist wieder da.
            if let klasse = speicher.planung?.klassen.first {
                await vorndran()
                let titelDavor = speicher.planung?.titel ?? ""
                speicher.titelSetzen(titelDavor + " — vor dem Blatt")
                try? await Task.sleep(for: .milliseconds(300))
                ...
                await vorndran()
                pruefen(hauptfenster.attachedSheet != nil && speicher.blattVorDemSchauplatz,
                        "das Vorhaben-Blatt steht vor dem Schauplatz")
                pruefen(!speicher.kannWiderrufen && eintrag(umschalt: false)?.title == "Widerrufen",
                        "der Hauptverlauf ruht — im Menü steht „\(eintrag(umschalt: false)?.title ?? "—")“")
                kuerzel()
                speicher.widerrufen()
                try? await Task.sleep(for: .milliseconds(400))
                pruefen(speicher.planung?.titel == titelDavor + " — vor dem Blatt",
                        "⌘Z ändert nichts hinter dem Blatt")
                speicher.vorhabenDialog = nil
                for _ in 0..<40 where hauptfenster.attachedSheet != nil {
                    try? await Task.sleep(for: .milliseconds(150))
                }
                await vorndran()
                pruefen(speicher.kannWiderrufen
                        && eintrag(umschalt: false)?.title == "Widerrufen: " + Schrittname.planungstitelAendern,
                        "Blatt zu: der Schritt ist wieder da — „\(eintrag(umschalt: false)?.title ?? "—")“")
                kuerzel()
                try? await Task.sleep(for: .milliseconds(400))
                pruefen(speicher.planung?.titel == titelDavor, "und ⌘Z nimmt ihn zurück")
            }
