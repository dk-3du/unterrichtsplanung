...
/// Doppelklick oder ⌥⏎ benennt um, ⌫/⌦ entfernt die Auswahl, ⏎ übernimmt.
/// Pfeile verschieben um 8 Punkt, mit ⇧ um 1; Tab wechselt den Tisch; Escape
/// hebt die Auswahl auf. Fangen am unsichtbaren 8-Punkt-Raster, ⌥ beim
/// Ziehen hebt es auf (E26).
@MainActor
/// Was auf der Fläche geschehen ist — der Name fürs Menü und, wo Eingaben
/// fortgesetzt werden, die Kennung für die Schreibphase (S6, E88).
struct Sitzplanhandgriff: Sendable {
    let name: String
    var kennung: String?
}
...
final class Sitzplanansicht: NSView, NSTextFieldDelegate {
    ...
    var beiAenderung: (Sitzplan, Sitzplanhandgriff) -> Void = { _, _ in }
    var beiAuswahl: (Set<String>) -> Void = { _ in }
    /// ⏎ auf der Fläche: übernehmen — über diesen Haken, nicht über die
    /// Responder-Kette, die den Knopf des Blatts nicht verlässlich fand (B08).
    var beiUebernehmen: () -> Void = {}
    ...
    private func planAendern(_ neu: Sitzplan, _ handgriff: Sitzplanhandgriff) {
        guard neu != plan else { return }
        plan = neu
        beiAenderung(neu, handgriff)
    }
    ...
    override func mouseUp(with ereignis: NSEvent) {
        // Ein Zug ist ein Schritt: Gemeldet wird erst hier, nicht bei jedem
        // Rasterpunkt — das ist die gröbere Körnung, die der Nutzer wollte.
        if bewegt { beiAenderung(plan, Sitzplanhandgriff(name: Schrittname.tischeVerschieben)) }
        ziehstart = nil
        ziehanker = nil
    ...
    @objc private func elementEntfernen(_ sender: NSMenuItem) {
        ...
        let handgriff = Sitzplanansicht.entfernungsname(weg)
        auswaehlen([])
        planAendern(neu, handgriff)
    }

    /// Ein Tisch, mehrere Tische oder der Lehrertisch — das Menü sagt es
    /// hinterher genauso.
    static func entfernungsname(_ weg: Set<String>) -> Sitzplanhandgriff {
        if weg == [Sitzplan.lehrertischKennung] {
            return Sitzplanhandgriff(name: Schrittname.lehrertischEntfernen)
        }
        return Sitzplanhandgriff(name: weg.count > 1 ? Schrittname.tischeEntfernen
                                                     : Schrittname.tischEntfernen)
    }

    @objc private func lehrertischHinzufuegen(_ sender: NSMenuItem) {
        planAendern(plan.mitLehrertisch(), Sitzplanhandgriff(name: Schrittname.lehrertischHinzufuegen))
    }
    ...
    override func keyDown(with ereignis: NSEvent) {
        ...
        case 123, 124, 125, 126:
            ...
            // Mit Kennung: Wer die Pfeiltaste hält, schiebt einen Tisch, nicht zehn.
            planAendern(plan.verschoben(auswahl, um: weg, fangen: false),
                        Sitzplanhandgriff(name: Schrittname.tischeVerschieben,
                                          kennung: auswahl.sorted().joined(separator: ",")))
        case 48:
        ...
        case 53:
            guard !auswahl.isEmpty else { super.keyDown(with: ereignis); return }
            auswaehlen([])
        case 36 where ereignis.modifierFlags.contains(.option):
            // ⌥⏎ benennt den einen angewählten Tisch um (B06).
            guard auswahl.count == 1, let id = auswahl.first, id != Sitzplan.lehrertischKennung else {
        ...
        case 36:
            beiUebernehmen()
        case 51, 117:
            // ⌫ und ⌦ entfernen die Auswahl — wie „Tisch entfernen“ im Menü (B06).
            guard !auswahl.isEmpty else { super.keyDown(with: ereignis); return }
            var neu = plan
            for kennung in auswahl { neu = neu.ohne(kennung) }
            let handgriff = Sitzplanansicht.entfernungsname(auswahl)
            auswaehlen([])
            planAendern(neu, handgriff)
        default:
            super.keyDown(with: ereignis)
    ...
    private func umbenennenBeenden(uebernehmen: Bool) {
        ...
        if uebernehmen {
            planAendern(plan.umbenannt(id, name: neu),
                        Sitzplanhandgriff(name: Schrittname.tischUmbenennen, kennung: id))
        }
        needsDisplay = true
...
struct Sitzplanflaeche: NSViewRepresentable {
    ...
    let ton: Farbton
    /// ⏎ auf der Fläche — das Blatt verdrahtet sein „Übernehmen“ (B08).
    var beiUebernehmen: () -> Void = {}
    /// Jede Änderung mit ihrem Handgriff — das Blatt führt den Verlauf (S6).
    var beiAenderung: (Sitzplan, Sitzplanhandgriff) -> Void = { _, _ in }

    func makeNSView(context: Context) -> Sitzplanansicht {
        let ansicht = Sitzplanansicht(plan: plan, ton: ton)
        // Die Fläche schreibt den Entwurf nicht selbst: Das Blatt nimmt ihn
        // entgegen, meldet den Schritt an und setzt ihn dann (S6).
        ansicht.beiAenderung = { neu, handgriff in context.coordinator.aenderung(neu, handgriff) }
        ansicht.beiAuswahl = { neu in context.coordinator.auswahl.wrappedValue = neu }
        ansicht.beiUebernehmen = { context.coordinator.uebernehmen() }
        return ansicht
    }

    func updateNSView(_ ansicht: Sitzplanansicht, context: Context) {
        context.coordinator.uebernehmen = beiUebernehmen
        context.coordinator.aenderung = beiAenderung
    ...
    final class Koordinator {
        ...
        var aenderung: (Sitzplan, Sitzplanhandgriff) -> Void = { _, _ in }
