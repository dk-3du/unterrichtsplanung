...
def packziel(ziel, name):
    """Der Ordner, in den `--packen` schreibt. **ZIEL ist der Elternordner**;
    den Namen `Rundenpaket-<Fassung>-<Stufe>` hängt der Treiber selbst an.

    Wer das schon fertige Ziel übergibt, bekäme ein Rundenpaket im
    Rundenpaket — im Gast läge dann neben `runde.py --laufen` kein Manifest,
    und der Lauf startete nicht. Das wird abgewiesen statt stillschweigend
    verschachtelt (v77): Was ein Skript selbst abfangen kann, gehört nicht in
    die Aufmerksamkeit dessen, der es ruft (Lehre aus R75-02).

    **Geprüft wird der Ort, nicht die Schreibweise (R77-02, E243, v78).** Bis
    1.9.6 sah der Wächter nur den letzten Bestandteil des Aufrufs an, bevor er
    ihn absolut machte — `.` wurde als `.` geprüft, nicht als der Ordner, den
    es benennt. Durchs Netz fielen damit `.`, `..` aus einem Unterordner, jeder
    Unterordner innerhalb eines Rundenpakets und jeder Aliasname: Abgewiesen
    wurde nur, wer sein Ziel genau so buchstabierte, wie der Wächter es
    erwartete. Jetzt wird zuerst aufgelöst (auch über Aliasnamen), und
    abgewiesen wird, wenn **irgendein** Bestandteil des Ortes mit
    `Rundenpaket-` beginnt.

    Aufgelöst wird zum Prüfen, **gebaut wird aus der Schreibweise** des
    Aufrufs: Sonst stünde nach einem Aliasnamen ein anderer Ordner im
    Packprotokoll als der, den der Aufruf nannte.

    Rückgabe: (Ordner, None) oder (None, Grund)."""
    aufgeloest = os.path.realpath(ziel)
    teile = [t for t in aufgeloest.split(os.sep) if t]
    stelle = next((i for i, t in enumerate(teile) if t.startswith("Rundenpaket-")), None)
    if stelle is not None:
        fuehrend = os.sep if aufgeloest.startswith(os.sep) else ""
        darueber = fuehrend + os.sep.join(teile[:stelle]) if stelle else (fuehrend or ".")
        return None, (f"ZIEL „{ziel}“ liegt in „{teile[stelle]}“ und damit in einem Rundenpaket. "
                      f"ZIEL ist der Ordner darüber — den Namen hängt der Treiber an, sonst läge "
                      f"das Rundenpaket im Rundenpaket und --laufen fände im Gast kein Manifest "
                      f"neben sich. Gemeint war vermutlich: --packen {darueber}")
    return os.path.join(os.path.abspath(ziel), name), None
...
def packen(ziel):
    ...
    ordner, grund = packziel(ziel, name)
    if grund:
        sys.exit(grund)
...
def selbsttest():
    ...
    syntax()
    P = "Paket: org.3ducation.Unterrichtsplanung.probe 1.9.7 (78), Quellenstand abcabcabcabc…, Werkzeugstand d840422ba290…"
    F = "Paket: org.3ducation.Unterrichtsplanung 1.9.7 (78), Quellenstand abcabcabcabc…, Werkzeugstand d840422ba290…"
    ...
    # Das Ziel von --packen (v77): ZIEL ist der Elternordner. Ein ZIEL, das
    # schon ein Rundenpaket benennt, wird abgewiesen — sonst läge das
    # Rundenpaket im Rundenpaket, und im Gast fände --laufen kein Manifest.
    # Hier nur Schreibweisen; die Lage prüft `packzielSelbsttest` (v78).
    for ziel, sollGrund in (("Paket", False), ("Paket/", False), (".", False),
                            ("Rundenpakete", False),   # nur der genaue Präfix zählt
                            ("Paket/Rundenpaket-1.9.7-78", True),
                            ("Paket/Rundenpaket-1.9.7-78/", True),
                            ("/tmp/Rundenpaket-0.0.0-0", True)):
        ordner, grund = packziel(ziel, "Rundenpaket-1.9.7-78")
        gilt = (grund is not None) == sollGrund and (ordner is None) == sollGrund
        if gilt and sollGrund:
            # Die Meldung muss den richtigen Aufruf nennen, nicht nur abweisen (B65).
            gilt = "--packen" in grund and "Rundenpaket" in grund
        alle = alle and gilt
        print(f"  {'✓' if gilt else '✗'} Ziel von --packen „{ziel}“: "
              + ("abgewiesen" if sollGrund else "angenommen")
              + ("" if gilt else f" — der Treiber sagt {'abgewiesen' if grund else 'angenommen'}"))

    alle = packzielSelbsttest() and alle
    alle = manifestSelbsttest() and alle
    print("Selbsttest des Rundentreibers:", "bestanden" if alle else "mit Befund ✗")
    return alle


def packzielSelbsttest():
    """Das Ziel von `--packen` an einem echten Arbeitsverzeichnis (R77-02, E243,
    v78). Die Fälle in `selbsttest()` prüfen nur Schreibweisen; sie laufen im
    Fassungsordner und können darum nicht sehen, ob ein `.` in ein Rundenpaket
    zeigt. Hier wird die Lage hergestellt: ein Wegwerf-Rundenpaket im
    Temporärordner, hinein gewechselt, geprüft, zurück."""
    import tempfile
    alle = True
    ...
    NAME = "Rundenpaket-1.9.7-78"
    zurueck = os.getcwd()
    with tempfile.TemporaryDirectory() as wurzel:
        wurzel = os.path.realpath(wurzel)
        paket = os.path.join(wurzel, "Rundenpaket-0.0.0-0")
        drin = os.path.join(paket, "saat")
        gewoehnlich = os.path.join(wurzel, "Ausgabe")
        for d in (drin, gewoehnlich):
            os.makedirs(d)
        alias = os.path.join(wurzel, "Abkuerzung")
        os.symlink(paket, alias)
        aliasGewoehnlich = os.path.join(wurzel, "Ablage")
        os.symlink(gewoehnlich, aliasGewoehnlich)
        try:
            os.chdir(paket)
            ordner, grund = packziel(".", NAME)
            fall("„.“ im Rundenpaket wird abgewiesen", grund is not None and ordner is None,
                 f"angenommen: {ordner}")

            os.chdir(drin)
            ordner, grund = packziel("..", NAME)
            fall("„..“ aus einem Unterordner des Rundenpakets wird abgewiesen",
                 grund is not None and ordner is None, f"angenommen: {ordner}")
            ordner, grund = packziel(".", NAME)
            fall("ein Unterordner im Rundenpaket wird abgewiesen",
                 grund is not None and ordner is None, f"angenommen: {ordner}")

            os.chdir(wurzel)
            ordner, grund = packziel("Abkuerzung", NAME)
            fall("ein Aliasname auf ein Rundenpaket wird abgewiesen",
                 grund is not None and ordner is None, f"angenommen: {ordner}")

            os.chdir(gewoehnlich)
            ordner, grund = packziel(".", NAME)
            fall("„.“ in einem gewöhnlichen Ordner wird angenommen",
                 grund is None and ordner == os.path.join(gewoehnlich, NAME), grund or str(ordner))

            os.chdir(wurzel)
            ordner, grund = packziel("Ausgabe", NAME)
            fall("ein gewöhnlicher Unterordner wird angenommen",
                 grund is None and ordner == os.path.join(gewoehnlich, NAME), grund or str(ordner))

            # Aufgelöst wird zum Prüfen, gebaut wird wie bisher: Der Pfad
            # behält die Schreibweise des Aufrufs, sonst stünde plötzlich ein
            # anderer Ordnername im Packprotokoll.
            ordner, grund = packziel("Ablage", NAME)
            fall("gebaut wird aus der Schreibweise, nicht aus der Auflösung",
                 grund is None and ordner == os.path.join(aliasGewoehnlich, NAME), grund or str(ordner))
        finally:
            os.chdir(zurueck)
    return alle
