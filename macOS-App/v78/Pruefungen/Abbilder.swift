...
struct Abbilder {
    ...
    private func ablegen(_ name: String, _ bild: some View,
                         ...
                         rollen: CGPoint? = nil, ansEnde: Bool = false) throws {
        ...
        RunLoop.current.run(until: Date().addingTimeInterval(0.35))  // E145: Uhr — Anordnung für ein Abbild abwarten
        ansicht.layoutSubtreeIfNeeded()
        ...
        // Ans Ende gerollt — für die Abschnitte unter der Falz eines langen
        // Blatts (wie `ABBILD_ENDE=1` am Prüfstand).
        if ansEnde, let rollbereich = grosserRollbereich(in: ansicht),
           let dokument = rollbereich.documentView {
            let unten = max(0, dokument.bounds.height - rollbereich.contentView.bounds.height)
            rollbereich.contentView.scroll(to: NSPoint(x: 0, y: dokument.isFlipped ? unten : 0))
            rollbereich.reflectScrolledClipView(rollbereich.contentView)
            RunLoop.current.run(until: Date().addingTimeInterval(0.35))  // E145: Uhr — Anordnung für ein Abbild abwarten
            ansicht.layoutSubtreeIfNeeded()
        }
    ...
    func dialoge() throws {
        ...
        // Drei Fragen: erst die Verschlüsselung samt ihren zwei Schritten,
        // dann die Kopie, zuletzt die Updates. Alle hell — das dunkle Offscreen-
        ...
        try ablegen("04f-ersteinrichtung-updates",
                    ErsteinrichtungDialog().environment(speicher),
                    breite: 640, hoehe: 560, dunkel: false)
        // Die vierte Frage: die Materialliste von 3ducation.org (v71, E176).
        speicher.ersteinrichtungUpdates(erlauben: false)
        try ablegen("04h-ersteinrichtung-materialien",
                    ErsteinrichtungDialog().environment(speicher),
                    breite: 640, hoehe: 560, dunkel: false)
        // Die Nachwahl im Sandbox — mit eigenem Speicher, damit die Vorgabe die
        // übrigen Abbilder nicht verändert.
        let nachwahl = Planungsspeicher(vorschau: planung)
        nachwahl.nachwahlVorgeben(
            zielordner: "/Users/lehrkraft/Library/Mobile Documents/com~apple~CloudDocs/Unterrichtsplanung",
            basis: "/Users/lehrkraft/Unterricht")
        try ablegen("04g-nachwahl", NachwahlDialog().environment(nachwahl),
                    breite: 660, hoehe: 600, dunkel: false)
        ...
        try ablegen("07-einstellungen", EinstellungenDialog().environment(speicher),
                    breite: 760, hoehe: 820, dunkel: true)
        // Das Update-Blatt mit dem erfundenen Release der Probe und die
        // einmalige Frage für bestehende Planungen — beide hell.
        speicher.updateVorgeben(.probe)
        try ablegen("07b-update", UpdateDialog().environment(speicher),
                    breite: 640, hoehe: 640, dunkel: false)
        try ablegen("07c-update-nachfrage", UpdateNachfrageDialog().environment(speicher),
                    breite: 640, hoehe: 520, dunkel: false)
        // Die einmalige Frage nach der Materialliste für bestehende Planungen (v71).
        try ablegen("07d-material-nachfrage", MaterialNachfrageDialog().environment(speicher),
                    breite: 640, hoehe: 520, dunkel: false)
        ...
        // Der neue Abschnitt „Material von 3ducation.org“ steht am Ende des
        // Blatts — ans Ende gerollt (v71).
        try ablegen("11b-vorhaben-material", VorhabenDialog(start: entwurf).environment(speicher),
                    breite: 760, hoehe: 740, dunkel: false, ansEnde: true)
        // Das Material-Blatt in seinen Zuständen — mit der Vorlage des Tages,
        // ohne Netz (v71, E179, E181).
        let vorlage = URL(filePath: #filePath).deletingLastPathComponent()
            .appending(path: "Vorlagen/inhalte-2026-09-17.js")
        let katalog = try Materialkatalog.lesen(try Data(contentsOf: vorlage))
        var mitMaterial = entwurf
        if let erstes = katalog.kategorien.first?.kacheln.first {
            mitMaterial.links.append(Weblink(titel: erstes.titel, adresse: erstes.adresse))
        }
        // Hell wie 09c: Das dunkle Abbild eines freien Körpers bleibt abseits
        // des Bildschirms ohne Fensterfläche — weiß auf weiß.
        try ablegen("16-material", MaterialDialog(entwurf: .constant(mitMaterial), vorgabe: .geladen(katalog))
                        .environment(speicher), breite: 720, hoehe: 720, dunkel: false)
        try ablegen("16b-material-frage", MaterialDialog(entwurf: .constant(entwurf), vorgabe: .frage)
                        .environment(speicher), breite: 640, hoehe: 520, dunkel: false)
        try ablegen("16c-material-fehlt", MaterialDialog(entwurf: .constant(entwurf), vorgabe: .nichtErreichbar)
                        .environment(speicher), breite: 720, hoehe: 720, dunkel: false)
        // Unlesbar — mit dem Grund in einer Zeile.
        try ablegen("16e-material-unlesbar",
                    MaterialDialog(entwurf: .constant(entwurf),
                                   vorgabe: .unlesbar("Zeile 12, Spalte 5: eine Zahl mit führender Null — JavaScript läse sie anders"))
                        .environment(speicher), breite: 720, hoehe: 720, dunkel: false)
        try ablegen("16d-material-laedt", MaterialDialog(entwurf: .constant(entwurf), vorgabe: .laedt)
                        .environment(speicher), breite: 720, hoehe: 720, dunkel: false)
        ...
        tour.tourBeginnen()
        for _ in 0..<2 { tour.tourWeiter() }
        // Die siebte Karte (K3), an der Kurszelle — hell: dunkle Offscreen-
        // Abbilder einer Karte kommen weiß (siehe 14b).
        try ablegen("14c-tourkarte-kurszelle", Tourkarte().environment(tour),
                    breite: 420, hoehe: 340, dunkel: false)
        for _ in 0..<2 { tour.tourWeiter() }
    ...
    private func vergleichsfaelle(klasse: Klasse) -> [[Vorhaben]] {
        func machen(_ titel: String, _ text: String, _ mat: Int, _ links: Int,
                    ...
                    datum: Tag? = nil, dringend: Bool = false, korrektur: Bool = false) -> Vorhaben {
            Vorhaben(id: Kennung.neu("e"), klasseId: klasse.id, woche: 0,
                     ...
                     datum: datum, dringend: dringend, korrektur: korrektur)
        }
        ...
        return [
            ...
            [machen("Prüfung und Datum", "Kurz.", 1, 0, pruefung: true, datum: tag,
                    dringend: true)],
            [machen("Korrektur", "", 0, 0, korrektur: true)],
            [machen("Korrektur mit Datum, dringlich", lang, 1, 1, datum: tag, dringend: true, korrektur: true)],
        ]
    }
    ...
    func handgriffsdauer() throws {
        ...
        fenster.orderFrontRegardless()
        ansicht.layoutSubtreeIfNeeded()
        RunLoop.current.run(until: Date().addingTimeInterval(0.6))  // E145: Uhr — Anordnung vor der Messung der Rechenzeit
        ...
        for stelle in 0..<handgriffe {
            ...
            ansicht.layoutSubtreeIfNeeded()
            fenster.displayIfNeeded()
            RunLoop.current.run(until: Date().addingTimeInterval(0.05))  // E145: Uhr — Rechenzeit: ein Durchlauf der Ereignisschleife je Handgriff
        }
    ...
    /// Drei Zeilen je Klasse, immer — gesetzt oder als Angebot (E34); mit
    /// Sitzplan trägt die dritte das Stuhl-Symbol. Beide Darstellungen.
    @Test("Die Kurszelle zeigt drei Zeilen — gesetzt oder als Angebot, hell und dunkel")
    func kurszelle() throws {
        let ohne = Klasse(id: "k1", name: "5a", fach: "Biologie", notiz: "",
                          farbe: 0, farbeManuell: false)
        var eine = Klasse(id: "k2", name: "G6a", fach: "Informatik", notiz: "Raum 204",
                          farbe: 4, farbeManuell: false)
        eine.verwaltung = "Noten/G6a.numbers"
        var alle = Klasse(id: "k3", name: "10c", fach: "Chemie", notiz: "",
                          farbe: 8, farbeManuell: false)
        alle.verwaltung = "Noten/10c.numbers"
        alle.curriculum = "Chemie/Lehrplan-10.pdf"
        let planung = Planung.leer(titel: "Kurszelle", start: try #require(Tag(iso: "2026-08-10")),
                                   wochen: 4, basis: "", klassen: [ohne, eine, alle], fachfarben: [:])
        let speicher = Planungsspeicher(vorschau: planung)
        speicher.sitzplanUebernehmen(Sitzplan.anordnen(klasseId: "k3", namen: ["Amira Yilmaz", "Ben Fischer"]))
        #expect(speicher.sitzplan(fuer: "k3") != nil)

        let hoehen = planung.klassen.map { max(Masse.zelleMinHoehe, Zellenmass.kopfhoehe($0)) }
        let bild = VStack(spacing: 0) {
            ForEach(Array(planung.klassen.enumerated()), id: \.element.id) { stelle, klasse in
                Klassenkopf(klasse: klasse)
                    .frame(width: Masse.spalteKlasse, height: hoehen[stelle])
            }
        }
        .environment(speicher)
        for dunkel in [true, false] {
            try ablegen(dunkel ? "15-kurszelle-dunkel" : "15b-kurszelle-hell", bild,
                        breite: Masse.spalteKlasse, hoehe: hoehen.reduce(0, +), dunkel: dunkel)
        }
    }

    /// Eine Zeile, fünf Wochen: gewöhnlich, Prüfung, Korrektur, Korrektur und
    /// dringlich, Korrektur mit festem Datum — nebeneinander, damit sich Bezug
    /// und Unterschied auf einen Blick zeigen.
    private func korrekturplanung() throws -> Planung {
        var planung = Planung.leer(
            titel: "Merkmal Korrektur", start: Tag.heute.montagDerWoche.plus(tage: -7), wochen: 6,
            basis: "", klassen: Standardkurse.aufbauen([("8b", "Chemie"), ("9a", "Biologie")]), fachfarben: [:])
        let k = planung.klassen[0].id
        func vorhaben(_ woche: Int, _ titel: String, _ text: String = "") -> Vorhaben {
            Vorhaben(id: Kennung.neu("e"), klasseId: k, woche: woche, titel: titel, text: text,
                     erledigt: false, materialien: [], links: [])
        }
        var pruefung = vorhaben(1, "Klassenarbeit: Stoffe und ihre Eigenschaften")
        pruefung.pruefung = true
        pruefung.pruefungstag = Tag.heute
        var korrektur = vorhaben(2, "Klassenarbeit korrigieren", "Erwartungshorizont, Notenspiegel.")
        korrektur.korrektur = true
        var eilig = vorhaben(3, "Test korrigieren")
        eilig.korrektur = true
        eilig.dringend = true
        var mitDatum = vorhaben(4, "Rückgabe und Berichtigung")
        mitDatum.korrektur = true
        mitDatum.datum = Tag.heute.montagDerWoche.plus(tage: 23)
        var heute = vorhaben(1, "Lernstandserhebung korrigieren")
        heute.korrektur = true
        heute.datum = Tag.heute
        planung.eintraege = [vorhaben(0, "Dichte bestimmen", "Überlaufgefäß, Waage."), pruefung, heute,
                             korrektur, eilig, mitDatum]
        return planung
    }

    @Test("Das Merkmal Korrektur zeichnet sich — im Raster, im Dialog, in der Tagesliste, im Druck")
    func korrektur() throws {
        let planung = try korrekturplanung()
        for dunkel in [true, false] {
            let speicher = Planungsspeicher(vorschau: planung)
            speicher.erscheinung = dunkel ? .dunkel : .hell
            try ablegen(dunkel ? "17-korrektur-raster-dunkel" : "17b-korrektur-raster-hell",
                        Hauptansicht().environment(speicher), breite: 1480, hoehe: 520, dunkel: dunkel)
        }
        let speicher = Planungsspeicher(vorschau: planung)
        let korrektur = try #require(planung.eintraege.first { $0.korrektur && !$0.dringend && $0.datum == nil })
        let pruefung = try #require(planung.eintraege.first { $0.pruefung })
        try ablegen("17c-vorhaben-korrektur", VorhabenDialog(start: VorhabenEntwurf(korrektur)).environment(speicher),
                    breite: 760, hoehe: 740, dunkel: false)
        try ablegen("17d-vorhaben-pruefung-gesperrt", VorhabenDialog(start: VorhabenEntwurf(pruefung)).environment(speicher),
                    breite: 760, hoehe: 740, dunkel: false)
        try ablegen("17e-heute-korrektur", HeuteDialog().environment(speicher),
                    breite: 760, hoehe: 520, dunkel: false)
        try ablegen("17f-druck-korrektur", Druckansicht(planung: planung),
                    breite: Druckansicht.breite(planung), hoehe: 420, dunkel: false)
    }
