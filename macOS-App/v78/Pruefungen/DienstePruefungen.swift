...
struct DienstePruefungen {
    ...
    func sicherungInEigenerAblage() async throws {
        ...
        dienst.sichern()
        await dienst.entprelltJetzt()   // derselbe Hintergrundweg, ohne die 700 ms (E145)
        #expect(dienst.gesicherterStand == p.geaendert)
    ...
    @Test("Die Statusdatei aus dem Zielordner: keine, eine, eine zu große — ein fehlender Ordner wird benannt")
    func statusdatei() throws {
        ...
        let dienst = Sicherungsdienst(ablage: Ablage(ordner: ablageOrdner))
        dienst.kopieOrdner = ziel.path
        // Der Weg des Betriebs, seit v76 in drei Teilen und seit v77 der
        // einzige: Zugang holen, damit lesen, danach den Pfad nachführen.
        // Bis v77 gab es daneben eine Hülle `statusdateiLesen()` ohne
        // Argument, die nur noch diese Prüfung rief (AM1, v77).
        func lesen() throws(Sicherungsdienst.Statusdateilage) -> Data? {
            let zugang = try dienst.statuszugang()
            let (bereich, nachfuehrung) = try Sicherungsdienst.statuszugriff(zugang)
            do {
                let daten = try Sicherungsdienst.statusdateiLesen(bereich: bereich, ordner: zugang.ordner)
                dienst.nachfuehren(nachfuehrung)
                return daten
            } catch {
                if case .zielordner = error {} else { dienst.nachfuehren(nachfuehrung) }
                throw error
            }
        }
        #expect(try lesen() == nil)

        let datei = ziel.appending(component: Statusdatei.name)
        try Data("{}".utf8).write(to: datei)
        #expect(try lesen() == Data("{}".utf8))
        ...
        // Nicht „keine Datei“: Der Befehl „Stand der Web App abrufen“ nennt den
        // Ordner; still bleibt es beim Start und nach dem Aufwachen (B61, v75).
        dienst.kopieOrdner = ziel.appending(component: "weg").path
        #expect(throws: Sicherungsdienst.Statusdateilage.zielordner(.fehlt)) {
            try lesen()
        }
    }
    ...
    func entsiegeln() throws {
        ...
        for hindernis: Statusabgleich.Hindernis in [.klartextNichtErlaubt, .verschluesseltOhneTresor,
            ...
            #expect(hindernis.text.contains("Statusdatei der Web App"))
        }
    }
