...
struct DateiPruefungen {
    ...
    func zuVieleKurse() throws {
        ...
        let roh = #"{"typ":"unterrichtsplanung","version":2,"titel":"Viele","start":"2026-08-03","#
    ...
    func webDatei() throws {
        let text = """
        {
          ...
          "fachfarben": { "  Mathematik ": "rot-mittel", "Physik": "gibtsnicht" },
          "klassen": [
            { "id": "k1", "name": "G5", "fach": "Biologie", "farbe": 3, "farbeManuell": true },
        ...
        // Fachschlüssel normalisiert, unbekannte Farbe verworfen.
        ...
        #expect(planung.eintraege[0].text == "Ein Text")
    ...
    func fremdeDateien() throws {
        ...
        // Ohne Typ, ohne Fassung, mit älterer oder neuerer Fassung: benannt, nicht geraten.
        for (roh, erwartet) in [("{\"typ\":null,\"klassen\":[]}", "keine Unterrichtsplanung"),
                                ("{\"typ\":\"\",\"wochen\":4}", "keine Unterrichtsplanung"),
                                ("{\"typ\":\"unterrichtsplanung\",\"klassen\":[]}", "nicht mehr unterstützten"),
                                ("{\"typ\":\"unterrichtsplanung\",\"version\":1,\"klassen\":[]}", "nicht mehr unterstützten"),
                                ("{\"typ\":\"unterrichtsplanung\",\"version\":\"2\",\"klassen\":[]}", "nicht mehr unterstützten"),
                                ("{\"typ\":\"unterrichtsplanung\",\"version\":3,\"klassen\":[]}", "neueren Fassung (3)")] {
            do {
                _ = try Planungsdatei.lesen(Data(roh.utf8))
                Issue.record("nicht abgewiesen: \(roh)")
            } catch let fehler as Planungsfehler {
                #expect(fehler.text.contains(erwartet), "\(roh): \(fehler.text)")
            }
        }
        let mitLeer = try Planungsdatei.lesen(
            try #require("{\"typ\":\"unterrichtsplanung\",\"version\":2,\"wochen\":4}".data(using: .utf8)))
        #expect(mitLeer.wochen == 4)

        let knapp = try Planungsdatei.lesen(try #require("{\"typ\":\"unterrichtsplanung\",\"version\":2,\"wochen\":4}".data(using: .utf8)))
    ...
    func wochengrenzen() throws {
        func wochen(_ roh: String) throws -> Int {
            try Planungsdatei.lesen(try #require("{\"typ\":\"unterrichtsplanung\",\"version\":2,\"wochen\":\(roh)}".data(using: .utf8))).wochen
        }
        #expect(try wochen("0") == 52)
        #expect(try wochen("-3") == 1)
        #expect(try wochen("99") == 52)
        #expect(try wochen("\"12\"") == Kennwerte.wochenStandard, "eine Zeichenkette ist keine Zahl")
    ...
    func verlustbilanz() throws {
        ...
        let roh = #"{"typ":"unterrichtsplanung","version":2,"titel":"Grenzfall","start":"2026-08-10","#
    ...
    func gekuerzteTexte() throws {
        ...
        let roh = #"{"typ":"unterrichtsplanung","version":2,"titel":"Kurz","start":"2026-08-10","wochen":4,"#
        ...
        #expect(bilanz.hinweise == ["2 gekürzte Texte"])
    }
    ...
    func grenzenBeiderFassungen() throws {
        ...
        #expect(pfadbilanz.hinweise == ["2 gekürzte Texte"])

        // Die alten Feldnamen `basisordner` und `beschreibung` werden nicht
        // mehr gelesen — und zählen auch nicht als Verlust.
        let alteNamen = #"{"typ":"unterrichtsplanung","version":2,"start":"2026-08-10","#
            + #""wochen":4,"basis":"","basisordner":"\#(langerPfad)","#
            + #""klassen":[{"id":"k1","name":"5a"}],"#
            + #""eintraege":[{"id":"e1","klasseId":"k1","woche":0,"titel":"T","beschreibung":"alt"}]}"#
        let (ohneAlt, ohneVerlust) = try Planungsdatei.lesenMitBilanz(Data(alteNamen.utf8))
        #expect(ohneAlt.basis.isEmpty && ohneAlt.eintraege[0].text.isEmpty)
    ...
    func stutzenBeiderFassungen() throws {
        ...
        // 2) Kennungen werden an `.whitespaces` gestutzt — ein Zeilenumbruch
        //    bleibt stehen und macht die Kennung ungültig: Sie wird ersetzt,
        //    das Vorhaben findet seine Zeile nicht. Die Ansicht stutzte mit
        //    `String.trim()` und fand damit eine Zeile, die die App nicht fand:
        //    dasselbe Vorhaben stand dort im Raster und fehlte hier.
        ...
        #expect(mitUmbruch.klassen[0].id != "k1\n" && Kennung.istGueltig(mitUmbruch.klassen[0].id))
        #expect(mitUmbruch.eintraege.map(\.titel) == ["U"])
        #expect(mitUmbruch.eintraege[0].klasseId == mitUmbruch.klassen[0].id)
        #expect(umbruchbilanz.vorhaben == 1)
        #expect(umbruchbilanz.ersetzteKennungen == 1)
    ...
    /// Der Abzugvergleich (erzeugte Grenzfälle, Werkzeug `abzug_pruefen.py`
    /// in der Ansichtsfassung) fand Abweichungen mit einer Ursache:
    /// `JSONSerialization` verschluckt das führende U+FEFF **jeder**
    /// Zeichenkette — auch in einem Wahrheitswert und in einem Schlüssel; die
    /// Ansicht tat es nur an den Stellen, die durch `textwert` gehen. Bei zwei
    /// danach gleichen Schlüsseln behält `JSONSerialization` den **ersten**
    /// (nachgemessen). Zahlen nimmt keiner der Leser aus Zeichenketten.
    /// Derselbe Fall steht in `leser_pruefen.py`.
    @Test("U+FEFF in Wahrheitswerten und Schlüsseln wird gelesen wie in der Ansichtsfassung — Zahlen nur als Zahlen")
    func vorspannBeiderFassungen() throws {
        let bom = "\u{FEFF}"
        let roh = #"{"typ":"unterrichtsplanung","version":2,"start":"2026-08-10","#
            + #""wochen":6,"#
        ...
        #expect(gelesen.klassen[0].unterrichtstage == [.mittwoch], "„4“ als Zeichenkette ist kein Tag")
        #expect(gelesen.eintraege.count == 1, "die Woche als Zeichenkette ist keine Woche")
        #expect(gelesen.eintraege[0].woche == 1)
        #expect(!gelesen.eintraege[0].erledigt)
        #expect(gelesen.eintraege[0].dringend)
        #expect(!gelesen.eintraege[0].hausaufgabe, "ein bloßes U+FEFF ist die leere Zeichenkette")
        #expect(gelesen.eintraege[0].hausaufgabenText == "S. 42")
    ...
    func unterrichtstageBeiderFassungen() throws {
        let roh = #"{"typ":"unterrichtsplanung","version":2,"start":"2026-08-10","wochen":4,"#
            + #""klassen":[{"id":"k1","name":"5a","#
            + #""unterrichtstage":[5,"3",3,0,6,1.9,true,"x",null,-1,1]},"#
    ...
    func bilanzLeer() throws {
        ...
        #expect(bilanz.hinweise.isEmpty)
    }

    /// Kennungen sind Struktur: Was leer, doppelt, zu lang oder außerhalb des
    /// Musters ist, bekommt eine neue und wird gezählt — in allen vier Listen,
    /// nicht nur bei Zeilen und Vorhaben (Befund B16). Steuerzeichen fallen aus
    /// Texten heraus, ein Nicht-Objekt in `klassen` wird verworfen statt als
    /// leere Zeile nachgebildet, und zwei Fachfarben-Schlüssel, die sich nur
    /// in der Unicode-Form unterscheiden, sind einer. Derselbe Fall steht in
    /// `leser_pruefen.py`.
    @Test("Kennungen, Steuerzeichen und Nicht-Objekte werden gelesen wie in der Ansichtsfassung")
    func formatstrengeBeiderFassungen() throws {
        let lang = String(repeating: "x", count: Planungsdatei.maxKennungslaenge + 1)
        let roh = #"{"typ":"unterrichtsplanung","version":2,"start":"2026-08-10","wochen":4,"#
            + #""titel":"Ti\u0000tel\u202E","#
            ...
            + #"{"id":"\#(lang)","name":"5b"},{"id":"k 3","name":"5c"},{"id":"-k4","name":"5d"}],"#
            + #""eintraege":[{"id":"e1","klasseId":"K_1.a:b","woche":0,"titel":"T","#
            + #""kommentar":"links\u2066rechts\u2069"}]}"#
        let (gelesen, bilanz) = try Planungsdatei.lesenMitBilanz(Data(roh.utf8))
        #expect(gelesen.titel == "Titel")
        #expect(gelesen.eintraege[0].kommentar == "linksrechts")
        #expect(bilanz.bereinigteTexte == 2)

        #expect(gelesen.ferien.count == 2)
        #expect(gelesen.ferien[0].id == "f1")
        #expect(gelesen.ferien[1].id != "f1" && Kennung.istGueltig(gelesen.ferien[1].id))
        #expect(gelesen.sperrzeiten.count == 3)
        #expect(Set(gelesen.sperrzeiten.map(\.id)).count == 3)

        #expect(gelesen.klassen.count == 4, "das Nicht-Objekt ist keine Zeile")
        #expect(bilanz.klassen == 1)
        #expect(gelesen.klassen[0].id == "K_1.a:b")
        for zeile in gelesen.klassen.dropFirst() {
            #expect(Kennung.istGueltig(zeile.id) && !zeile.id.hasPrefix("-") && zeile.id != "k 3")
        }
        // Weihnachten, B, C, die überlange, die mit Leerzeichen, die mit Bindestrich vorn.
        #expect(bilanz.ersetzteKennungen == 6)
        #expect(bilanz.hinweise == ["2 bereinigte Texte", "6 ersetzte Kennungen"])

        // NFC: ein Schlüssel; von zweien gewinnt der in Codepunkt-Ordnung erste
        // (e + U+0301 vor é), und die Zeile mit Fach „é“ findet ihn.
        #expect(gelesen.fachfarben.count == 1)
        #expect(gelesen.fachfarben["\u{E9}"] == "rot-hell")
        #expect(gelesen.klassen[0].farbe == Farbwelt.tonNachSchluessel["rot-hell"]?.stelle)
    }

    /// Ein Verweis folgt einer ersetzten Kennung — genannt, wie die Datei sie
    /// bei der Zeile nannte; eine doppelte bleibt bei der ersten Zeile, die sie
    /// trug. Derselbe Fall in `leser_pruefen.py`.
    @Test("Verweise folgen einer ersetzten Kennung wie in der Ansichtsfassung")
    func verweiseBeiderFassungen() throws {
        let roh = #"{"typ":"unterrichtsplanung","version":2,"start":"2026-08-10","wochen":4,"#
            + #""klassen":[{"id":"k 1","name":"5a"},{"id":"k2","name":"5b"},"#
            + #"{"id":"k2","name":"5c"},{"id":" k4","name":"5d"}],"#
            + #""zellenfrei":[{"klasseId":"k 1","woche":"2026-08-10"},{"klasseId":"k2","woche":"2026-08-10"}],"#
        ...
        let (gelesen, bilanz) = try Planungsdatei.lesenMitBilanz(Data(roh.utf8))
        func zeile(_ id: String) -> Int? { gelesen.klassen.firstIndex { $0.id == id } }
        #expect(gelesen.klassen.count == 4)
        #expect(gelesen.eintraege.map { zeile($0.klasseId) } == [0, 1, 3, 3])
        #expect(Set(gelesen.zellenfrei.map { zeile($0.klasseId) }) == [0, 1])
        #expect(gelesen.sperrzeiten[0].kurse.map(zeile) == [0, 1, 3])
        #expect(bilanz.vorhaben == 0 && bilanz.freieZellen == 0)
        #expect(bilanz.hinweise == ["2 ersetzte Kennungen"], "„k 1“ und die doppelte „k2“; „ k4“ ist nur gestutzt")
    }

    /// Gemerkt wird ein Wert, der nach dem Stutzen nicht leer ist, nicht
    /// Kennung einer früheren Zeile ist und noch keiner Zeile gehört: Die
    /// leere Kennung wird nie Verweis, ein Duplikat mit Leerraum zieht seine
    /// Verweise nach. Derselbe Fall in `leser_pruefen.py`.
    @Test("Die leere Kennung wird kein Verweis, ein Duplikat mit Leerraum zieht seine Verweise nach — wie in der Ansichtsfassung")
    func verweisregelBeiderFassungen() throws {
        let roh = #"{"typ":"unterrichtsplanung","version":2,"start":"2026-08-10","wochen":4,"#
            + #""klassen":[{"id":"k1","name":"5a"},{"id":" k1","name":"5b"},{"id":"","name":"5c"},"#
            + #"{"name":"5d"},{"id":7,"name":"5e"}],"#
            + #""zellenfrei":[{"klasseId":"","woche":"2026-08-10"},{"klasseId":" k1","woche":"2026-08-10"}],"#
            + #""sperrzeiten":[{"id":"s1","name":"S","von":"2026-08-17","bis":"2026-08-18","kurse":[""," k1","k1"]}],"#
        ...
        let (gelesen, bilanz) = try Planungsdatei.lesenMitBilanz(Data(roh.utf8))
        func zeile(_ id: String) -> Int? { gelesen.klassen.firstIndex { $0.id == id } }
        #expect(gelesen.klassen.count == 5)
        #expect(gelesen.eintraege.map(\.titel) == ["A", "B"], "ohne Kennung hängt kein Vorhaben an einer Zeile")
        #expect(gelesen.eintraege.map { zeile($0.klasseId) } == [1, 0], "„ k1“ meint die zweite Zeile")
        #expect(gelesen.zellenfrei.map { zeile($0.klasseId) } == [1])
        #expect(gelesen.sperrzeiten[0].kurse.map(zeile) == [0, 1])
        #expect(bilanz.vorhaben == 3 && bilanz.freieZellen == 1 && bilanz.ersetzteKennungen == 4)
        #expect(bilanz.verworfenes + bilanz.hinweise == ["3 Vorhaben", "1 freie Zelle", "4 ersetzte Kennungen"])
    }

    /// Texte sind Zeichenketten: Zahl und Wahrheitswert im Textfeld gelten als
    /// leer — `NSNumber.stringValue` und `String(zahl)` schrieben 1e16 und 1e-7
    /// verschieden. Derselbe Fall in `leser_pruefen.py`.
    @Test("Zahl und Wahrheitswert im Textfeld gelten als leer — wie in der Ansichtsfassung")
    func texteBeiderFassungen() throws {
        let roh = #"{"typ":"unterrichtsplanung","version":2,"start":"2026-08-10","wochen":4,"titel":1e16,"#
            + #""klassen":[{"id":"k1","name":true,"fach":1.5,"notiz":12345678901234567890}],"#
            + #""eintraege":[{"id":"e1","klasseId":"k1","woche":0,"titel":1e-7,"text":7,"#
            + #""kommentar":false,"statusGeaendert":0}]}"#
        let (gelesen, bilanz) = try Planungsdatei.lesenMitBilanz(Data(roh.utf8))
        #expect(gelesen.titel == "Unterrichtsplanung")
        #expect(gelesen.klassen[0].name == "Klasse/Kurs 1" && gelesen.klassen[0].fach == "" && gelesen.klassen[0].notiz == "")
        #expect(gelesen.eintraege.count == 1)
        #expect(gelesen.eintraege[0].titel == "" && gelesen.eintraege[0].text == "")
        #expect(gelesen.eintraege[0].kommentar == "" && gelesen.eintraege[0].statusGeaendert == "")
        #expect(bilanz.istLeer)
        #expect(Planungsdatei.text(1e16) == "" && Planungsdatei.text(true) == "" && Planungsdatei.text(nil) == "")
        #expect(Planungsdatei.text("1e16") == "1e16")
    }
}
...
struct SicherungskopiePruefungen {
    ...
    func einstellungenBleibenUnberuehrt() {
        ...
        Einstellungen.setzen("nachher", schluessel)
        #expect(UserDefaults.standard.string(forKey: schluessel) == "vorher")

        Einstellungen.entfernen(schluessel)
        #expect(UserDefaults.standard.string(forKey: schluessel) == "vorher",
    ...
    func ersteinrichtungInSchritten() throws {
        ...
        // Die vierte Frage — die Materialliste von 3ducation.org (E176).
        #expect(speicher.ersteinrichtungsschritt == Schritt.materialien)
        #expect(!speicher.materialienGefragt)
        speicher.ersteinrichtungMaterialien(erlauben: true)
        #expect(speicher.materialienGefragt && speicher.materialienErlaubt)
    ...
    func ersteinrichtungUeberspringen() throws {
        ...
        #expect(speicher.ersteinrichtungsschritt == .materialien)
        speicher.ersteinrichtungMaterialien(erlauben: false)
        #expect(speicher.materialienGefragt && !speicher.materialienErlaubt,
                "auch die vierte Frage: „Nicht jetzt“ zählt")
    }

    /// Was die Hauptansicht tut, nachdem ein Blatt geschlossen ist — mit
    /// **ihrer** Folge, nicht einer nachgebauten: `angeboteUndFragenPruefen()`
    /// ist dieselbe Funktion, die `onChange(of: offenerDialog)` ruft. Die
    /// Nachwahl bleibt aus, sie braucht einen Sandbox.
    private func nachDemBlatt(_ s: Planungsspeicher) {
        s.naechstenDialogOeffnen()
        s.angeboteUndFragenPruefen()
    }

    /// B67 (v77, E239): Auf dem Weg „Verschlüsselung überspringen → Ordner
    /// wählen …“ gibt es noch keinen Schlüssel. Die Kopie gibt es nur
    /// verschlüsselt, also richtet die App sie ein — und ihr Blatt **ersetzt**
    /// das der Ersteinrichtung, nach der zweiten von vier Fragen. Bis 1.9.5
    /// fielen die Fragen drei und vier damit auf die beiden Ersatzblätter
    /// zurück, die eigentlich für ältere Planungen gedacht sind, und die Tour
    /// kam vor ihnen. Seit 1.9.6 kehrt die Ersteinrichtung zurück.
    ///
    /// Geprüft wird die Folge, die der Abschluss des Ordnerdialogs auslöst —
    /// der Dialog selbst ist ein `NSOpenPanel` und geht hier nicht auf.
    ///
    /// **Der dritte Ausgang, seit v78 (AN3):** Der Übergang kann auch
    /// *scheitern* — die dreiundzwanzigste Review hat ausdrücklich nach ihm
    /// gefragt, und geprüft war er nicht. Er trägt, weil die Rückkehr am
    /// Merker `ersteinrichtungFortsetzen` hängt und nicht am Rückruf
    /// `nachEinrichtung`: Der Rückruf schaltet die Sicherungskopie ein — das
    /// soll nach einem Fehlschlag gerade **nicht** geschehen —, der Merker
    /// führt die Befragung zu Ende, und das soll immer geschehen. Zwei
    /// getrennte Dinge, darum überlebt der Merker jeden Ausgang.
    @Test("Wird zwischendurch die Verschlüsselung eingerichtet, wird die Ersteinrichtung zu Ende geführt (B67)",
          arguments: ["eingerichtet", "abgebrochen", "gescheitert"])
    func ersteinrichtungUeberDieVerschluesselung(_ ausgang: String) throws {
        let ziel = try ordner()
        defer { try? FileManager.default.removeItem(at: ziel) }
        ...
        #expect(s.ersteinrichtungsschritt == .sicherung, "Frage zwei: die Sicherungskopie")

        // „Ordner wählen …“ — was `ErsteinrichtungDialog.waehlen()` und der
        // Abschluss des Ordnerdialogs ohne Schlüssel tun.
        s.ersteinrichtungBeantwortet()
        s.autoexportZielOhneTresor(ziel.path)
        s.ersteinrichtungZuUpdates()
        #expect(!s.ersteinrichtungFaellig, "die zweite Frage gilt als beantwortet")
        #expect(s.naechsterDialog == .verschluesselung, "das Blatt der Ersteinrichtung ist abgelöst")

        nachDemBlatt(s)
        #expect(s.offenerDialog == .verschluesselung)
        switch ausgang {
        case "eingerichtet":
            let fortsetzung = s.nachEinrichtung
            s.nachEinrichtung = nil
            fortsetzung?()
        case "gescheitert":
            // Schritt 2 ohne Vorbereitung: Der Übergang wird zurückgenommen,
            // und die Sicherungskopie bleibt aus. Der Rückruf bleibt hier für
            // einen zweiten Anlauf stehen — richtig so: Gelingt der Übergang
            // später, soll die Kopie angehen.
            //
            // **Was dieser Ausgang deckt und was nicht:** Er ist der einzige
            // Fehlweg, der an einem Vorschau-Speicher erreichbar ist — eine
            // gesperrte Ablage (`schutzuebergangGesperrt`) gibt es dort nicht,
            // `schreibsperrgrund` ist in der Vorschau immer nil. Geprüft wird
            // damit das zurückgenommene Ergebnis. Der andere Fehlweg löscht
            // zusätzlich den Rückruf — dass **das** den Merker nicht mitnimmt,
            // zeigt der Ausgang „abgebrochen“, der genau das tut. Zusammen
            // decken die drei Ausgänge den Mechanismus ab.
            let ergebnis = s.verschluesselungEinschalten()
            #expect(ergebnis.ablage == .zurueckgenommen("keine Vorbereitung"),
                    "der Übergang ist gescheitert")
            #expect(!s.autoexportAktiv, "nach einem Fehlschlag bleibt die Sicherungskopie aus")
        default:
            s.verschluesselungVerwerfen()
        }
        s.offenerDialog = nil

        // Frage drei kommt zurück — im Blatt der Ersteinrichtung, nicht als
        // Ersatzblatt, und vor der Tour.
        nachDemBlatt(s)
        #expect(s.offenerDialog == .ersteinrichtung,
                "\(ausgang): die Ersteinrichtung wird fortgesetzt")
        #expect(s.ersteinrichtungsschritt == .updates, "\(ausgang): bei der dritten Frage")

        s.ersteinrichtungUpdates(erlauben: false)
        #expect(s.ersteinrichtungsschritt == .materialien, "Frage vier im selben Blatt")
        s.ersteinrichtungMaterialien(erlauben: false)
        s.offenerDialog = nil

        // Und erst jetzt die Tour.
        nachDemBlatt(s)
        #expect(s.offenerDialog == .tour, "\(ausgang): die Tour kommt zuletzt")
        #expect(s.updatesGefragt && s.materialienGefragt, "alle vier Fragen sind beantwortet")
        #expect(!s.ersteinrichtungFortsetzen, "nichts bleibt vorgemerkt")
    }

    /// E239 (v77), zweiter Teil: Die Fragen kommen vor dem Angebot der Tour.
    ///
    /// **Was hier nicht geprüft werden kann:** Die beiden Ersatzblätter
    /// (`updateNachfrage`, `materialNachfrage`) sind im Prüflauf absichtlich
    /// stumm — `updateNachfrageFaellig` und `materialNachfrageFaellig` tragen
    /// `!pruefstand` in ihrer Bedingung, damit ein Lauf nicht an einem Blatt
    /// hängen bleibt. Ihre Stellung vor der Tour lässt sich darum nur an der
    /// Bedingung selbst zeigen, nicht am geöffneten Blatt; am Paket prüft es
    /// der Live-Test. Prüfbar ist der Teil, an dem B67 hängt: dass die
    /// fortgesetzte Ersteinrichtung das Fenster vor der Tour bekommt.
    @Test("Steht die Ersteinrichtung noch aus, bekommt sie das Fenster vor der Tour")
    func ersteinrichtungVorDerTour() throws {
        let s = Planungsspeicher(vorschau: try planung())
        s.tourAnbieten = true
        s.ersteinrichtungsschritt = .updates
        s.ersteinrichtungFortsetzen = true
        s.angeboteUndFragenPruefen()
        #expect(s.offenerDialog == .ersteinrichtung, "die offene Frage zuerst")
        #expect(s.tourAnbieten, "das Angebot der Tour steht noch aus")

        // Beantwortet, Blatt zu — jetzt ist die Tour an der Reihe.
        s.ersteinrichtungUpdates(erlauben: false)
        s.ersteinrichtungMaterialien(erlauben: false)
        s.offenerDialog = nil
        nachDemBlatt(s)
    ...
    /// Die Folge steht an **einer** Stelle, auch dort, wo die Tour nicht
    /// gehört (v77): Beim Erscheinen des Fensters ruft die Hauptansicht
    /// `angeboteUndFragenPruefen(mitTour: false)` — die Tour gehört zur neuen
    /// Planung, eine geladene ist keine neue. Bis dahin standen die drei
    /// Fragen dort einzeln; eine Folge, die zweimal geschrieben steht, läuft
    /// auseinander. Der Schalter sagt es ausdrücklich, statt sich darauf zu
    /// verlassen, dass `tourAnbieten` beim Start ohnehin falsch ist.
    @Test("Ohne Tour: Die Fragen kommen, das Angebot der Tour nicht — auch wenn es anstünde")
    func fragenOhneTour() throws {
        let s = Planungsspeicher(vorschau: try planung())
        s.tourAnbieten = true
        s.ersteinrichtungsschritt = .updates
        s.ersteinrichtungFortsetzen = true

        s.angeboteUndFragenPruefen(mitTour: false)
        #expect(s.offenerDialog == .ersteinrichtung, "die offene Frage kommt")
        s.ersteinrichtungUpdates(erlauben: false)
        s.ersteinrichtungMaterialien(erlauben: false)
        s.offenerDialog = nil

        s.angeboteUndFragenPruefen(mitTour: false)
        #expect(s.offenerDialog == nil, "ohne Tour bleibt es zu")
        #expect(s.tourAnbieten, "das Angebot steht weiterhin aus — nur nicht hier")

        // Mit Tour — der gewöhnliche Weg nach dem Schließen eines Blatts.
        s.angeboteUndFragenPruefen()
        #expect(s.offenerDialog == .tour)
    }

    /// Dieselbe Reihenfolge an den Bedingungen: Sind die Fragen drei und vier
    /// fällig (eine Planung, die älter ist als sie), stehen sie vor der Tour
    /// in `angeboteUndFragenPruefen()` — nachgewiesen an der Entscheidung
    /// ohne ihre drei Quellen, die es dafür gibt.
    @Test("Bei einer älteren Planung sind beide Ersatzblätter fällig — sie stehen vor dem Angebot der Tour")
    func ersatzblaetterVorDerTour() throws {
        // Ersteinrichtung beantwortet (Zielordner steht), Fragen drei und vier
        // offen: genau die Lage nach dem Weg über die Verschlüsselung, wenn
        // der Nutzer das fortgesetzte Blatt schließt, ohne zu antworten.
        #expect(Planungsspeicher.updateNachfrageFaellig(
            pruefstand: false, hatPlanung: true, ersteinrichtungFaellig: false, gefragt: false))
        #expect(Planungsspeicher.materialNachfrageFaellig(
            pruefstand: false, hatPlanung: true, ersteinrichtungFaellig: false,
            updateGefragt: true, gefragt: false))
        // Und im Prüflauf sind sie stumm — darum steht der Fall oben nur hier.
        #expect(!Planungsspeicher.updateNachfrageFaellig(
            pruefstand: true, hatPlanung: true, ersteinrichtungFaellig: false, gefragt: false))
    }

    /// Die Tour ist ein Angebot, keine Frage — sie kommt nach den Nachfragen,
    /// gleich welcher Weg dorthin führte (E239, v77). Hier der gewohnte Weg,
    /// auf dem die Ersteinrichtung ihre vier Fragen selbst stellt.
    @Test("Der gewohnte Weg stellt alle vier Fragen im Blatt — die Tour danach")
    func ersteinrichtungGewohnterWeg() throws {
        let s = Planungsspeicher(vorschau: try planung())
        s.tourAnbieten = true
        s.ersteinrichtungOeffnen()
        s.ersteinrichtungUeberspringen()
        s.ersteinrichtungBeantwortet()
        s.ersteinrichtungZuUpdates()
        #expect(s.offenerDialog == .ersteinrichtung && s.ersteinrichtungsschritt == .updates,
...
struct StatusPruefungen {
    ...
    func zeitstempelGrenzfaelle() {
        ...
        #expect(Statusabgleich.zeitstempelBrauchbar(gut))
        // Unmögliche Datums- und Uhrzeitfelder.
        #expect(!Statusabgleich.zeitstempelBrauchbar("2026-02-30T10:00:00.000Z"))
        #expect(!Statusabgleich.zeitstempelBrauchbar("2026-02-29T10:00:00.000Z"))
        #expect(Statusabgleich.zeitstempelBrauchbar("2024-02-29T10:00:00.000Z"))
        #expect(!Statusabgleich.zeitstempelBrauchbar("2026-08-10T24:00:00.000Z"))
        #expect(!Statusabgleich.zeitstempelBrauchbar("2026-08-10T10:60:00.000Z"))
        #expect(!Statusabgleich.zeitstempelBrauchbar("2026-08-10T10:00:60.000Z"))
        // Form und Zukunft wie bisher.
        #expect(!Statusabgleich.zeitstempelBrauchbar("2026-08-10T10:00:00Z"))
        #expect(!Statusabgleich.zeitstempelBrauchbar(""))
        #expect(!Statusabgleich.zeitstempelBrauchbar("2099-01-01T00:00:00.000Z"))
        // Dieselbe Jahresschranke wie `Tag(iso:)`.
        #expect(!Statusabgleich.zeitstempelBrauchbar("1899-12-31T23:59:59.999Z"))
    }
    ...
    func juengererStandGewinnt() throws {
        ...
        Thread.sleep(forTimeInterval: 0.005)  // E145: Uhr — Stempelvorschub: der nächste Stempel liegt sicher in einer späteren Millisekunde
    ...
    func lesen() throws {
        ...
        {"version":1,"gespeichert":"2026-08-18T18:00:00.000Z","eintraege":{"e-1":{"erledigt":true}}}
        """))
        #expect(ohneTyp.eintraege.count == 1)

        // Kennungen nach dem Muster der Planung, Kommentare ohne Steuerzeichen —
        // dieselben Regeln wie im Planungsleser (`eintraegeVereinen` der Ansicht).
        let streng = try Statusdatei.lesen(datei("""
        {"version":1,"eintraege":{"e-1":{"erledigt":true,"kommentar":"a\\u202Eb\\u0000c"},
         "e 2":{"erledigt":true},"":{"erledigt":true},"-e3":{"erledigt":true}}}
        """))
        #expect(streng.eintraege.count == 1)
        #expect(streng.eintraege["e-1"]?.kommentar == "abc")
    }
    ...
    @Test("Die Fassung wird geprüft — 1 gilt, fehlend und alles andere nicht")
    func fassung() throws {
        // Dieselben Fälle prüft `leser_pruefen.py` am Leser der Ansicht.
        func mit(_ version: String) -> Data {
            datei("{\"typ\":\"unterrichtsplanung-status\",\"version\":\(version),"
                  + "\"eintraege\":{\"e-1\":{\"erledigt\":true}}}")
        }
        #expect(try Statusdatei.lesen(mit("1")).eintraege.count == 1)
        #expect(try Statusdatei.lesen(mit("1.0")).eintraege.count == 1)
        #expect(throws: Planungsfehler.self, "ohne Fassung: keine unterstützte Ansicht") {
            try Statusdatei.lesen(datei("{\"eintraege\":{\"e-1\":{\"erledigt\":true}}}"))
        }
        for unzulaessig in ["0", "2", "9999", "\"1\"", "true", "null", "1.5", "1e100"] {
            #expect(throws: Planungsfehler.self, "\(unzulaessig)") { try Statusdatei.lesen(mit(unzulaessig)) }
        }
    }
    ...
    func ausDemOrdner() throws {
        ...
        #expect(speicher.meldungen.contains { $0.text.contains("Aus der Web App übernommen") })
    }
    ...
    /// Eine gescheiterte Größenabfrage ist ein Fehler, keine Null: Ein Ordner
    /// an der Stelle der Datei wird benannt, nicht als „keine“ übergangen.
    @Test("Ein Ordner statt der Statusdatei wird benannt")
    func ordnerStattDatei() throws {
        let ordner = URL.temporaryDirectory
            .appending(component: "status-\(UUID().uuidString)", directoryHint: .isDirectory)
        try FileManager.default.createDirectory(at: ordner.appending(component: Statusdatei.name),
                                                withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: ordner) }
        let speicher = Planungsspeicher(vorschau: try planung())
        speicher.autoexportZielSetzen(ordner.path)
        speicher.statusUebernehmen()
        #expect(speicher.meldungen.contains { $0.text.contains("keine reguläre Datei") },
                "\(speicher.meldungen.map(\.text))")
    }
...
// ── Sonderzeilen in der Datei ─────────────────────────────────────────────

@Suite("Planungsdatei: Sonderzeilen (E157)")
struct DateiSonderzeilenPruefungen {

    /// Derselbe Fall steht in `leser_pruefen.py`: Art, Titel und Ordnung liest
    /// die Ansicht wortgleich — eine Regel, die nur eine Seite kennt, ließe
    /// dieselbe Planung auf Mac und iPad verschieden aussehen.
    @Test("Art, Titel und Ordnung werden gelesen wie in der Ansichtsfassung")
    func sonderzeilenBeiderFassungen() throws {
        let roh = #"{"typ":"unterrichtsplanung","version":2,"start":"2026-08-10","wochen":4,"#
            + #""klassen":[{"id":"k1","name":"Vertretungen","art":"vertretungen","fach":"7a"},"#
            + #"{"id":"k2","name":"5a"},"#
            + #"{"id":"k3","name":"irgendwas","art":"klassenleitung","fach":"7a"},"#
            + #"{"id":"k4","name":"5b","art":"sonstiges"},"#
            + #"{"id":"k5","name":"5c","art":7},"#
            + #"{"id":"k6","name":"","art":"weiteres","notiz":"AG-Leitung"}],"#
            + #""eintraege":[{"id":"v1","klasseId":"k3","woche":0,"titel":"Elternabend"}]}"#
        let (gelesen, bilanz) = try Planungsdatei.lesenMitBilanz(Data(roh.utf8))
        #expect(gelesen.klassen.map(\.id) == ["k2", "k4", "k5", "k3", "k6", "k1"],
                "Unterricht in Dateireihenfolge, dann Klassenleitung, Weiteres, Vertretungen")
        #expect(gelesen.klassen.map(\.art) == [.unterricht, .unterricht, .unterricht, .klassenleitung, .weiteres, .vertretungen])
        #expect(gelesen.klassen.map(\.name) == ["5a", "5b", "5c", "Klassenleitung", "Weiteres", "Vertretungen"],
                "der Titel einer Sonderzeile ist fest, was die Datei nennt, zählt nicht")
        #expect(gelesen.klassen[3].fach == "7a", "die Klassenleitung trägt ihre Klasse als Fach")
        #expect(gelesen.klassen[5].fach == "" && gelesen.klassen[4].notiz == "AG-Leitung",
                "Vertretungen und Weiteres tragen kein Fach — der Leser lässt es fallen; die Notiz bleibt")
        #expect(gelesen.eintraege.first?.klasseId == "k3", "das Vorhaben hängt weiter an seiner Zeile")
        #expect(bilanz.istLeer, "eine unbekannte Art ist kein Verlust — sie heißt Unterricht")
    }

    @Test("Die Art überlebt Schreiben und Lesen; eine Datei ohne das Feld liest sich wie bisher")
    func rundreise() throws {
        var planung = try Planungsdatei.lesen(try Planungsdatei.schreiben(
            Planung.leer(titel: "P", start: .heute, wochen: 4, basis: "",
                         klassen: [Klasse.sonderzeile(.weiteres), Standardkurse.aufbauen([("7a", "Mathematik")])[0]],
                         fachfarben: [:])))
        #expect(planung.klassen.map(\.art) == [.unterricht, .weiteres])
        #expect(planung.klassen[1].name == "Weiteres")
        planung.klassen[1].fach = "Chor"
        let wieder = try Planungsdatei.lesen(try Planungsdatei.schreiben(planung))
        #expect(wieder.klassen[1].art == .weiteres && wieder.klassen[1].fach == "",
                "„Weiteres“ trägt kein Fach (E161) — der Leser lässt es fallen, die Art bleibt")
        // Die Datei nennt an jeder Zeile ihre Art — auch Unterricht, damit sie sich selbst erklärt.
        let roh = try JSONSerialization.jsonObject(with: try Planungsdatei.schreiben(planung)) as? [String: Any]
        let arten = (roh?["klassen"] as? [[String: Any]])?.map { $0["art"] as? String }
        #expect(arten == ["unterricht", "weiteres"])
    }

    /// Derselbe Fall steht in `leser_pruefen.py` („Dateiverweise, die die Art
    /// nicht vorsieht“). Bis v69 blieb ein solcher Verweis in der Planung — im
    /// Raster abgeblendet, im Blatt ohne Menü, im Speicher nicht entfernbar (B34).
    @Test("Ein Dateiverweis, den die Art nicht vorsieht, fällt beim Lesen und zählt in der Verlustbilanz (B34, E164)")
    func unerlaubteDateiverweiseFallen() throws {
        let roh = #"{"typ":"unterrichtsplanung","version":2,"start":"2026-08-10","wochen":4,"#
            + #""klassen":[{"id":"k1","name":"7a","verwaltung":"a.numbers","curriculum":"a.pdf"},"#
            + #"{"id":"k2","art":"klassenleitung","verwaltung":"b.numbers","curriculum":"b.pdf"},"#
            + #"{"id":"k3","art":"weiteres","curriculum":"c.pdf"},"#
            + #"{"id":"k4","art":"vertretungen","verwaltung":"d.numbers","curriculum":"d.pdf"}]}"#
        let (gelesen, bilanz) = try Planungsdatei.lesenMitBilanz(Data(roh.utf8))
        #expect(gelesen.klassen.map(\.verwaltung) == ["a.numbers", "b.numbers", "", ""],
                "Verwaltung: Unterricht und Klassenleitung ja, Vertretungen nein")
        #expect(gelesen.klassen.map(\.curriculum) == ["a.pdf", "", "", ""],
                "Curriculum: nur Unterricht")
        #expect(bilanz.dateiverweise == 4 && !bilanz.istLeer)
        #expect(bilanz.verworfenes == ["4 Dateiverweise"], "die Bilanz nennt sie — nichts fällt still")
        var einzeln = Planungsdatei.Verlustbilanz()
        einzeln.dateiverweise = 1
        #expect(einzeln.verworfenes == ["1 Dateiverweis"])
        // Einmal geschrieben, ist der Stand sauber: die nächste Lesung meldet nichts.
        let (_, wieder) = try Planungsdatei.lesenMitBilanz(try Planungsdatei.schreiben(gelesen))
        #expect(wieder.istLeer)
    }

    /// Politik E165 (B35): Der Leser verwirft nichts (E157). Zwei Zeilen derselben
    /// Sonderart aus einer fremden Datei laden beide — fester Titel, stabile
    /// Reihenfolge, ihre Vorhaben bleiben; die Eindeutigkeit „eine je Art“ gilt
    /// für das Anlegen in der Oberfläche. Wortgleich in `leser_pruefen.py`.
    @Test("Zwei Sonderzeilen derselben Art aus einer fremden Datei bleiben beide (B35, E165)")
    func doppelteSonderzeilenBleiben() throws {
        let roh = #"{"typ":"unterrichtsplanung","version":2,"start":"2026-08-10","wochen":4,"#
            + #""klassen":[{"id":"k1","name":"KL 7a","art":"klassenleitung","fach":"7a"},"#
            + #"{"id":"k2","name":"9c"},"#
            + #"{"id":"k3","name":"KL 9c","art":"klassenleitung","fach":"9c"}],"#
            + #""eintraege":[{"id":"v1","klasseId":"k1","woche":0,"titel":"A"},"#
            + #"{"id":"v2","klasseId":"k3","woche":1,"titel":"B"}]}"#
        let (gelesen, bilanz) = try Planungsdatei.lesenMitBilanz(Data(roh.utf8))
        #expect(gelesen.klassen.map(\.id) == ["k2", "k1", "k3"], "beide bleiben, in Dateireihenfolge unter dem Unterricht")
        #expect(gelesen.klassen.map(\.name) == ["9c", "Klassenleitung", "Klassenleitung"])
        #expect(gelesen.klassen.map(\.fach) == ["", "7a", "9c"], "jede trägt ihre Klasse")
        #expect(gelesen.eintraege.map(\.klasseId) == ["k1", "k3"], "die Vorhaben hängen an beiden")
        #expect(gelesen.hatSonderzeile(.klassenleitung) && gelesen.sonderzeilenzahl == 2)
        #expect(bilanz.istLeer, "kein Verlust — nichts wird verworfen oder umgedeutet")
        let wieder = try Planungsdatei.lesen(try Planungsdatei.schreiben(gelesen))
        #expect(wieder.klassen.map(\.id) == ["k2", "k1", "k3"], "und so bleibt es über die Rundreise")
    }
}

/// Die Zwillinge der Ansicht (E208, E209, B52, B53, v74). Dieselben Fälle fährt
/// `Web-App/vNN/leser_pruefen.py` gegen `kommentarKuerzen` und `nichtLeer` der
/// Ansichtsfassung — eine Regel, die nur eine Seite kennt, ließe denselben
/// Kommentar oder dieselbe Bezeichnung auf Mac und iPad Verschiedenes sein.
@Suite("Zwillinge: Status-Kommentar und Ersatzname wie in der Ansichtsfassung")
@MainActor
struct ZwillingePruefungen {

    /// Wortgleich mit `KOMMENTARFAELLE` in leser_pruefen.py.
    private static let kommentarfaelle: [(String, String, String)] = {
        let a = String(repeating: "a", count: Planungsdatei.maxTextlaenge - 1)
        return [
            ("Leerzeichen an der Grenze", a + " b", a),
            ("Zeilenumbruch an der Grenze", a + "\nb", a),
            ("CRLF an der Grenze", a + "\r\nb", a),
            ("NEL an der Grenze", a + "\u{85}b", a),
            ("U+2028 an der Grenze", a + "\u{2028}b", a),
            ("NBSP an der Grenze", a + "\u{A0}b", a),
            ("Leerraum innen bleibt", "  a  b  ", "a  b"),
            ("Steuerzeichen und Rand", "\u{7} x \u{202E}", "x"),
            ("kurz und ohne Rand", "Gut gelaufen.", "Gut gelaufen."),
        ]
    }()

    /// Ein Eintrag der Statusdatei mit diesem Kommentar, gelesen wie von der App.
    private func kommentarGelesen(_ roh: String) throws -> String {
        let objekt: [String: Any] = ["typ": Statusdatei.typ, "version": Statusdatei.version, "gespeichert": "",
                                     "eintraege": ["e-1": ["erledigt": false, "kommentar": roh]]]
        let stand = try Statusdatei.lesen(try JSONSerialization.data(withJSONObject: objekt))
        return try #require(stand.eintraege["e-1"]).kommentar
    }

    @Test("Der Kommentar der Statusdatei: dieselben Fälle wie in der Ansicht, zweimal gelesen ist einmal gelesen")
    func kommentar() throws {
        for (name, roh, erwartet) in Self.kommentarfaelle {
            let einmal = try kommentarGelesen(roh)
            #expect(einmal == erwartet, "\(name): \(einmal.count) statt \(erwartet.count) Zeichen")
            #expect(try kommentarGelesen(einmal) == einmal, "\(name): beim zweiten Lesen anders")
        }
    }

    @Test("Der Kommentar der Statusdatei: jedes Paar von Randzeichen an der Grenze übersteht das zweite Lesen")
    func kommentarJedesPaar() throws {
        let bausteine = [" ", "\t", "\n", "\r\n", "\r", "\u{A0}", "\u{2028}", "\u{3000}", "\u{85}",
                         "\u{301}", "\u{7}", "\u{202E}", "\u{FEFF}", "é", "b"]
        let a = String(repeating: "a", count: Planungsdatei.maxTextlaenge - 2)
        var abweichend = 0
        for x in bausteine {
            for y in bausteine {
                let einmal = try kommentarGelesen(a + x + y + "b")
                if try kommentarGelesen(einmal) != einmal { abweichend += 1 }
            }
        }
        #expect(abweichend == 0, "\(abweichend) von \(bausteine.count * bausteine.count) änderten sich beim zweiten Lesen")
    }

    /// Wortgleich mit dem Fall „Ersatzname bereinigt und gestutzt wie beim
    /// Schreiben“ in leser_pruefen.py.
    @Test("Der Ersatzname einer leeren Bezeichnung: bereinigt und gestutzt wie beim Schreiben")
    func ersatznameWieBeimSchreiben() throws {
        let objekt: [String: Any] = [
            "typ": "unterrichtsplanung", "version": 2, "start": "2026-08-10", "wochen": 4,
            "klassen": [["id": "k1", "name": "5a"]],
            "eintraege": [["id": "e1", "klasseId": "k1", "woche": 0, "titel": "T",
                           "materialien": [["titel": "", "pfad": "/tmp/ "],
                                           ["titel": "", "pfad": "/tmp/\u{202E}"],
                                           ["titel": "", "pfad": "/tmp/ blatt.pdf "]]]],
        ]
        let (planung, bilanz) = try Planungsdatei.lesenMitBilanz(try JSONSerialization.data(withJSONObject: objekt))
        #expect(planung.eintraege.first?.materialien.map(\.titel) == ["", "", "blatt.pdf"])
        #expect(bilanz.hinweise.isEmpty, "ein geliehener Name ist kein bereinigter Text")
    }

    @Test("Gesichert gleich gelesen, auch wenn der Dateiname nur aus Leerraum oder Steuerzeichen besteht")
    func ersatznameRundreise() throws {
        let planung = Planung.leer(titel: "Zwillinge", start: try #require(Tag(iso: "2026-08-10")),
                                   wochen: 6, basis: "",
                                   klassen: Standardkurse.aufbauen([("G6a", "Informatik")]),
                                   fachfarben: [:])
        ...
        entwurf.materialien = [Material(titel: "", pfad: "/tmp/ "), Material(titel: "", pfad: "/tmp/\u{202E}"),
                               Material(titel: "", pfad: "/tmp/ blatt.pdf ")]
        s.vorhabenSichern(entwurf)
        let gesichert = try #require(s.planung?.eintraege.first).materialien.map(\.titel)
        let (gelesen, bilanz) = try Planungsdatei.lesenMitBilanz(try Planungsdatei.schreiben(try #require(s.planung)))
        #expect(gesichert == ["", "", "blatt.pdf"])
        #expect(gelesen.eintraege.first?.materialien.map(\.titel) == gesichert, "gesichert gleich gelesen")
        #expect(bilanz.bereinigteTexte == 0 && bilanz.gekuerzteTexte == 0, "der Leser findet nichts zu tun")
    }
}
