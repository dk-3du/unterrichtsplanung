...
struct StatusAbrufPruefungen {
    ...
    // ── Die Politik des Abbruchs (R77-01, E242, v78) ─────────────────────

    /// **Eine zugelassene Übernahme läuft zu Ende.** Die Aufgabe, die
    /// `statusAbrufen()` und `statusNachAufwachen()` zurückgeben, ist eine
    /// Wartemarke für Prüfungen, kein Griff zum Abbrechen; wer sie abbricht,
    /// hält die Übernahme nicht auf.
    ///
    /// Die dreiundzwanzigste Review (R77-01) hat gefragt, ob das so gemeint
    /// ist. Es ist so gemeint, und zwar aus zwei Gründen: Erstens bricht im
    /// Betrieb niemand ab — der Menübefehl und der Anwendungsdelegat verwerfen
    /// die Aufgabe, und der Abbruch im `Aufwachabruf` trifft die Wartezeit von
    /// 15 Sekunden, nach der gar nicht mehr gelesen wird. Zweitens gäbe ein
    /// Abbruch den Leser nicht frei: Gelesen wird in einem Block auf einer
    /// GCD-Warteschlange hinter `withCheckedContinuation`, und der ist nicht
    /// abbruchfähig — ein Wächter auf `Task.isCancelled` unterdrückte also das
    /// Anwenden, während Faden und Sicherheitsbereich weiter hingen. Das wäre
    /// ein Versprechen, das die Stelle nicht halten kann.
    ///
    /// Diese Prüfung hält die Politik fest: Ohne sie fiele ein später
    /// eingefügter Abbruchwächter niemandem auf.
    @Test("Ein Abbruch der zurückgegebenen Aufgabe hält die Übernahme nicht auf",
          arguments: ["Befehl", "Aufwachen"])
    func abbruchHaeltDieUebernahmeNichtAuf(_ weg: String) async throws {
        let (s, ablage, ziel) = try speicher()
        defer { aufraeumen(ablage, ziel) }
        try statusSchreiben(ziel, ["e-1": .init(erledigt: true, kommentar: "", geaendert: Zeitrechnung.jetztAlsZeitstempel())])
        let tor = Lesetor()
        var aufgabe: Task<Planungsspeicher.Statusuebernahme?, Never>?
        Statusnaht.$halten.withValue(tor.halten) {
            aufgabe = weg == "Befehl" ? s.statusAbrufen() : s.statusNachAufwachen()
        }
        try await abwarten("das Lesen hängt (\(weg))") { tor.haengt }
        aufgabe?.cancel()
        tor.oeffnen()
        #expect(await aufgabe?.value == .some(.uebernommen), "\(weg): die Übernahme läuft zu Ende")
        #expect(erledigt(s, "e-1"), "\(weg): der Haken ist angekommen")
    }

    /// Dieselbe Politik an der Nachführung (E237): Auch das erneuerte
    /// Lesezeichen und der Pfad eines verlegten Ordners gehören zum Ergebnis
    /// und werden nach einem Abbruch angewandt. Ein Wächter, der hier
    /// aussteigt, verlöre beides — unschädlich, weil der nächste Lauf es
    /// wieder erzeugt, aber eben kein Ausstieg ohne Folgen.
    @Test("Ein Abbruch verwirft auch die Nachführung des Zielordners nicht")
    func abbruchVerwirftDieNachfuehrungNicht() async throws {
        let (s, ablage, ziel) = try speicher()
        let verlegt = ziel.deletingLastPathComponent()
            .appending(component: "abruf-verlegt-\(UUID().uuidString)", directoryHint: .isDirectory)
        defer { aufraeumen(ablage, ziel, verlegt) }
        try statusSchreiben(ziel, ["e-1": .init(erledigt: true, kommentar: "", geaendert: Zeitrechnung.jetztAlsZeitstempel())])
        // Ohne Lesezeichen gäbe es nichts zu erneuern — der Zugang liefe über
        // den gemerkten Pfad, und der zeigt nach dem Verlegen ins Leere.
        s.sicherung.lesezeichenAblegen(ziel)
        try FileManager.default.moveItem(at: ziel, to: verlegt)
        ...
        aufgabe?.cancel()
        tor.oeffnen()
        #expect(await aufgabe?.value == .some(.uebernommen))
        #expect(erledigt(s, "e-1"), "aus dem verlegten Ordner gelesen")
        // Verglichen wird aufgelöst: Der Pfad kommt aus dem erneuerten
        // Lesezeichen und kann /var gegen /private/var tauschen — die beiden
        // Ordnernamen unterscheiden sich trotzdem, die Prüfung bleibt scharf.
        #expect(URL(fileURLWithPath: s.autoexportOrdner).resolvingSymlinksInPath()
                == verlegt.resolvingSymlinksInPath(), "der Pfad ist nachgeführt, trotz Abbruch")
    }
