...
enum Zeichen {
    ...
    /// Der Ranzen — eindeutig schulisch, nicht mit Prüfung, Datei oder Ordner
    /// zu verwechseln (E17, 12.09.2026).
    static let hausaufgabe = "backpack.fill"
    /// Der Stuhl — wörtlich und frei neben Raster (`square.grid.3x2`) und
    /// Verwaltungsdatei (`tablecells`) (E36, 12.09.2026).
    static let sitzplan = "chair"
    static let sitzplanGesetzt = "chair.fill"
...
/// Die Korrektur: das Rot der Prüfung — der Bezug —, aber gestrichelt und
/// umrandet statt gefüllt — der Unterschied, der auch ohne Farbe trägt.
enum Korrekturmarke {
    static let wort = "Korrektur"
    static var text: Text { Text(wort).foregroundStyle(Systemfarben.pruefung) }
    /// Strich und Lücke des Rahmens, in Punkt.
    static let strichelung: [CGFloat] = [5, 3]
}
