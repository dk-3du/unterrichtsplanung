...
extension Planungsspeicher {
    ...
    func freigabenNachfuehren() {
        ...
        if !autoexportOrdner.isEmpty, zugriff.zugang(fuer: autoexportOrdner) == .nieGewaehlt {
        ...
        if let basis = planung?.basis, !basis.isEmpty, zugriff.zugang(fuer: basis) == .nieGewaehlt {
    ...
    func nachwahlWaehlen(_ freigabe: Ordnerfreigabe) {
        ...
        Systemzugriff.ordnerWaehlen(start: freigabe.pfad, mehrere: false,
                                    titel: titel, zugriff: zugriff) { [weak self] wahl in
            guard let self, let gewaehlt = wahl.first else { return }
