...
final class Sicherungsdienst {
    ...
    /// Der Zugang zur Statusdatei, auf dem Hauptstrang geholt — ohne eine
    /// Datei zu berühren (R75-01, v76) und seit 1.9.6 **ohne das Lesezeichen
    /// aufzulösen** (R76-02, E237, v77). Hier steht nur, was der Vorrat der
    /// Lesezeichen hergibt; aufgelöst und gelesen wird mit `statuszugriff` und
    /// `statusdateiLesen`, gleich oder abseits des Hauptstrangs.
    struct Statuszugang: Sendable {
        /// Die Daten des Lesezeichens; ohne sie (nur ohne Sandbox) der Pfad allein.
        let zugangsdaten: Ordnerzugriff.Zugangsdaten?
        let ordner: URL
    }
    ...
    func statuszugang() throws(Statusdateilage) -> Statuszugang {
        ...
        guard zugriff.zustaendig(fuer: gemerkt.path) != nil else {
            ...
            return Statuszugang(zugangsdaten: nil, ordner: gemerkt)
        }
        do {
            return Statuszugang(zugangsdaten: try zugriff.zugangsdaten(gemerkt.path), ordner: gemerkt)
        } catch let fehler as Ordnerzugriff.Fehler {
    ...
    /// Was nach dem Lesen auf dem Hauptstrang nachzuführen ist: der Pfad eines
    /// umbenannten Zielordners und ein erneuertes Lesezeichen. Beides wird
    /// erst übernommen, wenn die Lage dann noch gilt (E236).
    struct Nachfuehrung: Sendable {
        let ordner: URL
        let erneuert: Ordnerzugriff.Erneuerung?
    }

    /// Stufe 2a (R76-02, v77): das Lesezeichen auflösen. Das fasst den
    /// Zielordner an und dauert dort so lange, wie der Speicher braucht —
    /// darum läuft es, wo gelesen wird, nicht auf dem Hauptstrang.
    /// Ohne Lesezeichen (nur ohne Sandbox) gibt es nichts aufzulösen.
    nonisolated static func statuszugriff(_ zugang: Statuszugang)
        throws(Statusdateilage) -> (bereich: Ordnerzugriff.Bereich?, nachfuehrung: Nachfuehrung?) {
        guard let z = zugang.zugangsdaten else { return (nil, nil) }
        do {
            let (bereich, erneuert) = try Ordnerzugriff.aufloesen(z)
            return (bereich, Nachfuehrung(ordner: bereich.ordner, erneuert: erneuert))
        } catch let fehler as Ordnerzugriff.Fehler {
            throw .zielordner(fehler.art == .unaufloesbar ? .fehlt : .keinZugriff)
        } catch {
            throw .zielordner(.fehlt)
    ...
    /// Stufe 2b: Die Statusdatei im Bereich lesen — in einem Stück und ohne
    /// `await`: Bereich öffnen, nach dem Ordner sehen, gebunden lesen, Bereich
    /// schließen. Berührt weder die Sitzung noch die Oberfläche.
    nonisolated static func statusdateiLesen(bereich: Ordnerzugriff.Bereich?,
                                             ordner: URL) throws(Statusdateilage) -> Data? {
        ...
        do {
            if let bereich { return try bereich.mit(lesen) }
            return try lesen(ordner)
        } catch let lage as Statusdateilage {
            throw lage
        } catch let lage as Zielordnerlage {
            throw .zielordner(lage)
    ...
    /// Pfad wird nachgeführt — wie in `imZielordner`. Dazu seit 1.9.6 das
    /// erneuerte Lesezeichen, das der Arbeiter mitgebracht hat; abgelegt wird
    /// es hier, auf dem Hauptstrang, unter dem Schlüssel des Tresors.
    func nachfuehren(_ nachfuehrung: Nachfuehrung?) {
        guard let n = nachfuehrung else { return }
        if n.ordner.path != kopieOrdner { kopieOrdner = n.ordner.path }
        if let erneuert = n.erneuert { zugriff.lesezeichenErneuern(erneuert) }
    }
}
