...
struct Veroeffentlichung: Codable, Equatable, Sendable {
    ...
    static let probe = Veroeffentlichung(
        ...
        body: "Eine erfundene Fassung für den Prüfstand.\n\n**Neu:** ein Blatt, das dieses "
            ...
            + "**Unverändert:** Dateiformat, Tresor, Web App.",
...
struct Updatepruefer: Sendable {
    ...
    var zeitgrenze: TimeInterval = Updatepruefer.zeitueberschreitung

    /// Mehr als das ist keine Antwort, die diese App deuten will — das
    /// Release-JSON der Schnittstelle liegt unter 200 KB.
    static let antwortHoechstens = 1024 * 1024
    ...
    /// nichts mitbringen.
    static func sitzung(_ konfiguration: URLSessionConfiguration = .ephemeral,
                        zeitgrenze: TimeInterval = zeitueberschreitung) -> URLSession {
        konfiguration.timeoutIntervalForRequest = zeitgrenze
        konfiguration.timeoutIntervalForResource = zeitgrenze * 2
    ...
    func pruefen() async -> Updateergebnis {
        ...
        do {
            ...
            antwort = empfangen
            // Gebunden gelesen: an `Content-Length`, wo vorhanden, und an der
            // Bytezahl — was darüber liegt, wird weder gedeutet noch gemerkt.
            let grenze = Updatepruefer.antwortHoechstens
