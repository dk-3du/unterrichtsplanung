...
enum Planungsdatei {
    ...
    struct Verlustbilanz: Equatable, Sendable {
        ...
        /// Verwaltungs- und Curriculumverweise an Sonderzeilen, deren Art sie
        /// nicht vorsieht (E161) — verworfen und gezählt (B34, E164, v70): Behalten
        /// stünden sie unsichtbar und unentfernbar in der Datei.
        var dateiverweise = 0
        ...
        var istLeer: Bool {
            klassen == 0 && vorhaben == 0 && materialien == 0 && links == 0
                ...
                && bereinigteTexte == 0 && ersetzteKennungen == 0 && dateiverweise == 0
        }
        ...
        var verworfenes: [String] {
            ...
            if dateiverweise > 0 { teile.append(wort(dateiverweise, "Dateiverweis", "Dateiverweise")) }
            return teile
        }
    ...
    static func pruefenMitBilanz(_ roh: Any) throws
        ...
        func nichtLeer(_ wert: Any?, sonst ersatz: @autoclosure () -> String,
            ...
            // Den Ersatznamen so, wie der Schreiber ihn schriebe: bereinigt und
            // gestutzt (E209, B53, v74) — sonst käme ein Dateiname nur aus
            // Leerraum oder Steuerzeichen am Leser vorbei. Wortgleich in der Ansicht.
            let voll = t.isEmpty ? ohneSteuerzeichen(ersatz()).trimmingCharacters(in: .whitespacesAndNewlines) : t
        ...
        for (stelle, fall) in roheKlassen.prefix(Kennwerte.maxKlassen).enumerated() {
            ...
            // Die Art (E157): fehlend oder unbekannt heißt Unterricht; der Titel
            // einer Sonderzeile ist fest, was die Datei nennt, zählt nicht.
            let art = Zeilenart.lesen(k["art"])
            let name = art.istSonderzeile ? art.titel : nichtLeer(k["name"], sonst: "Klasse/Kurs \(stelle + 1)")
            // Ein Fach trägt nur, was eine Lerngruppe hat (E161) — die Ansicht liest gleich.
            let fach = art.erlaubtFach ? nichtLeer(k["fach"], sonst: "") : ""
            ...
            // Pfade, keine Namen: siehe `basis`. Ein Verweis, den die Art nicht
            // vorsieht (E161), fällt und zählt (B34, E164) — behalten stünde er
            // unsichtbar und unentfernbar in der Datei; die Ansicht liest gleich.
            let verwaltungRoh = nichtLeer(k["verwaltung"], sonst: "",
                                          hoechstens: maxPfadlaenge)
            let curriculumRoh = nichtLeer(k["curriculum"], sonst: "",
                                          hoechstens: maxPfadlaenge)
            let verwaltung = art.erlaubt(.verwaltung) ? verwaltungRoh : ""
            let curriculum = art.erlaubt(.curriculum) ? curriculumRoh : ""
            if !verwaltungRoh.isEmpty, !art.erlaubt(.verwaltung) { bilanz.dateiverweise += 1 }
            if !curriculumRoh.isEmpty, !art.erlaubt(.curriculum) { bilanz.dateiverweise += 1 }
        ...
        // Sonderzeilen ans Ende, in fester Folge — die Ansicht ordnet ebenso.
        planung.zeilenOrdnen()
        ...
        for fall in roheVorhaben.prefix(maxVorhaben) {
            ...
            planung.eintraege.append(Vorhaben(
                ...
                // Entweder Prüfung oder Korrektur: Trägt eine fremde Datei
                // beides, gilt die Prüfung. Wortgleich in der Ansicht.
                korrektur: wahrheit(e["korrektur"]) && !wahrheit(e["pruefung"]),
    ...
    /// Der Zuschnitt an der Schranke vor der Datei (E208, R73-02, v74):
    /// Steuerzeichen heraus, Leerraum an den Rändern weg, auf `grenze` Zeichen
    /// kappen — und den Leerraum, den das Kappen am Ende freilegt, auch weg.
    /// Erst so ist er idempotent: Ein zugeschnittener Text kommt beim nächsten
    /// Sichern unverändert heraus. `gekuerzt`: Die Grenze griff.
    static func zugeschnitten(_ wert: String, grenze: Int,
                              leerraum: CharacterSet = .whitespacesAndNewlines) -> (text: String, gekuerzt: Bool) {
        let sauber = ohneSteuerzeichen(wert).trimmingCharacters(in: leerraum)
        guard sauber.count > grenze else { return (sauber, false) }
        return (String(sauber.prefix(grenze)).trimmingCharacters(in: leerraum), true)
    }

    /// Der Titel, den eine neue Planung trägt: zugeschnitten wie jede Schranke,
    /// als Rand nur Leerzeichen und Tabulator; leer wird der Ersatzname. Das
    /// Anlegen und die Warnung vor dem gleichen Namen der Sicherungskopie
    /// rechnen mit dieser einen Funktion (E214, R74-01, v75) — bis 1.9.3 sah die
    /// Warnung die Eingabe, die Kopie aber den gekappten Titel.
    static func planungstitel(_ eingabe: String) -> String {
        let titel = zugeschnitten(eingabe, grenze: maxNamenslaenge, leerraum: .whitespaces).text
        return titel.isEmpty ? "Unterrichtsplanung" : titel
    }
    ...
    /// Zeilenumbrüche zu Leerzeichen, Mehrfache zu einem: eine Zeile, auch
    /// wenn ein Absatz eingesetzt wurde — Hausaufgabe, Name am Tisch.
    static func einzeilig(_ wert: String) -> String {
        wert.split(whereSeparator: \.isNewline)
