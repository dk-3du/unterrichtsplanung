...
extension Planungsspeicher {
    ...
    func neuVersiegeln() {
        ...
        guard !sicherung.istVorschau, let tresor else { return }
        let bilanz = sicherung.ablage.altbestaendeVersiegeln(mit: tresor)
        if !bilanz.vollstaendig {
    ...
    func passphraseAendernAsynchron(alt: String, neu: String) async throws -> Schutzergebnis? {
        ...
        }) { wicklung in passphraseUebernehmen(wicklung) }
    }

    /// Die neue Wicklung in den Tresor der Sitzung, dann jede Datei unter die
    /// neue Hülle.
    private func passphraseUebernehmen(_ wicklung: Wicklung) -> Schutzergebnis {
        guard let tresor else { return Schutzergebnis(ablage: .zurueckgenommen("nicht eingeschaltet")) }
