...
extension Planungsspeicher {
    ...
    /// über den Zugriff, die Sitzpläne über ihren Dienst, Kopie und
    /// Statusdatei im Zielordner.
    private func huelleNachziehen(alter: Tresor?, neu: Tresor, _ ergebnis: inout Schutzergebnis) {
        ...
        // Beim Ändern der Passphrase (derselbe Schlüssel, neue Hülle) sagt die
        // Meldung, was ein liegen gebliebener Behälter bedeutet: Die bisherige
        // Passphrase öffnet ihn noch, bis das Nachziehen gelingt — beim nächsten
        // Schreiben, beim Beenden oder beim nächsten Start (B09).
        let nachtrag = " — die App versucht es beim nächsten Schreiben, beim Beenden und beim nächsten Start erneut"
            + (alter === neu ? "; bis dahin öffnet auch die bisherige Passphrase diese Datei" : "")
        ergebnis.lesezeichen = zugriff.versiegeln(unter: neu).map { .offen($0.ohneSchlusspunkt + nachtrag) } ?? .erledigt
        ergebnis.sitzplaene = sitzplaeneVersiegeln(unter: neu, nachtrag: nachtrag)
        (ergebnis.kopie, ergebnis.statusdatei) = aussenstellenNachziehen(alter: alter, neu: neu)
    }

    /// Die Sitzpläne unter den Schlüssel, der schon gilt — erledigt nur, wenn
    /// welche liegen; sonst ist nichts nötig.
    private func sitzplaeneVersiegeln(unter neu: Tresor, nachtrag: String) -> Schutzergebnis.Aussenstelle {
        if let grund = sitzplaene.versiegeln(unter: neu) {
            return .offen(grund.ohneSchlusspunkt + nachtrag)
        }
        return sitzplaene.hatPlaene ? .erledigt : .nichtNoetig
    }
    ...
    func verschluesselungEinschalten() -> Schutzergebnis {
        ...
        var sitzplaeneStelle: Schutzergebnis.Aussenstelle = .nichtNoetig
        if !sicherung.istVorschau {
            // Aus dem Klartext ist ein gesperrter Vorrat oder Dienst kein Hindernis
            // (B03): Der nächste Start liest die Datei erneut und versiegelt, was
            // Klartext ist. Beim Erneuern bleibt die Abweisung — ein Behälter unter
            // dem alten Schlüssel wäre nach dem Wechsel fremd.
            if alter == nil, let grund = zugriff.sperrgrund {
                lesezeichen = .offen("die Datei bleibt liegen: \(grund.ohneSchlusspunkt) — beim nächsten "
                                     + "Start werden die Lesezeichen versiegelt")
            } else if let grund = zugriff.versiegeln(vorab: neu) {
            ...
            // Die Sitzpläne ebenso vorab — scheitern sie, gehen die Lesezeichen
            // zurück, und der Übergang findet nicht statt.
            if alter == nil, let grund = sitzplaene.sperrgrund {
                sitzplaeneStelle = .offen("die Datei bleibt liegen: \(grund.ohneSchlusspunkt) — beim nächsten "
                                          + "Start werden die Sitzpläne versiegelt")
            } else if let grund = sitzplaene.versiegeln(vorab: neu) {
                var ergebnis = Schutzergebnis(
                    ablage: .zurueckgenommen("die Sitzpläne ließen sich nicht versiegeln (\(grund.ohneSchlusspunkt))"))
                lesezeichenZuruecknehmen(auf: alter, &ergebnis)
                schutzMelden(ergebnis, getan: getan, nichtGetan: nichtGetan)
                nachEinrichtung = nil
                return ergebnis
            } else {
                sitzplaeneStelle = sitzplaene.hatPlaene ? .erledigt : .nichtNoetig
            }
        }
        tresorAnlegen(neu)
        ...
        if case .zurueckgenommen = ergebnis.ablage {
            // Die Behälter der Lesezeichen und der Sitzpläne gehen mit zurück: in
            // die Einstellungen bzw. den Klartext oder unter den alten Schlüssel —
            // gelingt das nicht, steht es in der Meldung.
            lesezeichenZuruecknehmen(auf: alter, &ergebnis)
            sitzplaeneZuruecknehmen(auf: alter, &ergebnis)
            schutzMelden(ergebnis, getan: getan, nichtGetan: nichtGetan)
            nachEinrichtung = nil
            return ergebnis
        }
        ergebnis.lesezeichen = lesezeichen
        ergebnis.sitzplaene = sitzplaeneStelle
        if !sicherung.istVorschau {
            ergebnis.nebendateien = sicherung.altbestaendeVersiegeln(mit: neu, alter: alter)
            (ergebnis.kopie, ergebnis.statusdatei) = aussenstellenNachziehen(alter: alter, neu: neu)
        }
    ...
    /// Der Behälter der Lesezeichen zurück auf den Stand vor dem Übergang.
    private func lesezeichenZuruecknehmen(auf alter: Tresor?, _ ergebnis: inout Schutzergebnis) {
        if let alter {
        ...
        } else if zugriff.sperrgrund == nil {
            zugriff.entsiegeln()
        }
    }

    /// Die Sitzpläne zurück auf den Stand vor dem Übergang: in den Klartext
    /// oder unter den alten Schlüssel. Was dabei nicht gelingt, steht in der
    /// Meldung (B02).
    private func sitzplaeneZuruecknehmen(auf alter: Tresor?, _ ergebnis: inout Schutzergebnis) {
        if let alter {
            if let grund = sitzplaene.versiegeln(unter: alter) {
                ergebnis.sitzplaene = .offen(grund.ohneSchlusspunkt
                    + " — die App versucht es beim nächsten Schreiben, beim Beenden und beim nächsten Start erneut")
            }
        } else {
            sitzplaene.entsiegeln()
            if let grund = sitzplaene.ungesichert {
                ergebnis.sitzplaene = .offen(grund.ohneSchlusspunkt
                    + " — die Sitzpläne gelten für diese Sitzung; die App versucht es beim nächsten Schreiben und beim Beenden erneut")
            }
        }
    }
    ...
    func verschluesselungAufheben() -> Schutzergebnis {
        ...
        // Sitzpläne, die gerade nicht lesbar sind, wären ohne Schlüssel für immer
        // zu: Das Aufheben wartet auf einen Neustart (E38 (a)).
        if !sicherung.istVorschau, let grund = sitzplaene.sperrgrund {
            return uebergangAbgewiesen("die Sitzpläne sind gerade nicht lesbar (\(grund.ohneSchlusspunkt)) — "
                                       + "nach einem Neustart erneut versuchen",
                                       nichtGetan: "Verschlüsselung nicht aufgehoben")
        }
        // Erst die Sitzpläne im Klartext hinlegen — scheitert das, bleibt alles,
        // wie es ist: der Schlüssel, der Behälter, die Sitzung (B02). Symmetrisch
        // zum Einschalten, das sie vorab versiegelt.
        let hattePlaene = sitzplaene.hatPlaene
        if !sicherung.istVorschau, let grund = sitzplaene.entsiegelnVorab() {
            return uebergangAbgewiesen("die Sitzpläne ließen sich nicht entsiegeln (\(grund.ohneSchlusspunkt))",
                                       nichtGetan: "Verschlüsselung nicht aufgehoben")
        }
        tresorAnlegen(nil)
        // Dann die Ablage im Klartext hinlegen — dabei wandert der versiegelte
        ...
        if case .zurueckgenommen = ergebnis.ablage {
            // Die Sitzpläne liegen schon im Klartext — zurück unter den Schlüssel,
            // der weiter gilt: ein Abgleich, kein Versuch (B17). Scheitert er,
            // bleibt der Dienst beim Behälter und holt nach; der Grund sagt, ob
            // die Klartextdatei entfernt wurde.
            if !sicherung.istVorschau, let grund = sitzplaene.versiegeln(zurueck: tresor) {
                ergebnis.sitzplaene = .offen(grund.ohneSchlusspunkt
                    + " — die App versiegelt sie beim nächsten Schreiben, beim Beenden und beim nächsten Start erneut")
            }
        ...
        if !sicherung.istVorschau {
            ...
            // Die Sitzpläne liegen seit dem ersten Schritt im Klartext (E22).
            ergebnis.sitzplaene = hattePlaene ? .erledigt : .nichtNoetig
            if let grund = sitzplaene.ungesichert { ergebnis.sitzplaene = .offen(grund) }
        }
        let alt = ergebnis.nebendateien.umgestellt.count
    ...
    private func lesezeichenOeffnen(_ tresor: Tresor) {
        ...
        case .geoeffnet:
            break
        case .nachgezogen:
            melden("Der Behälter der Lesezeichen trug noch eine ältere Hülle und ist neu versiegelt — "
                   + "die bisherige Passphrase öffnet ihn nicht mehr.")
        ...
        case .beiseitegelegt(let grund, let rettung, let versiegelt):
            // Unter einem anderen Schlüssel heißt meist: ein Übergang, der nicht zu
            // Ende kam — das Restrisiko aus E37.
            let uebergang = grund.contains("Schlüssel")
                ? " — womöglich aus einem Übergang der Verschlüsselung, der nicht zu Ende kam"
                : ""
            melden("Der Behälter der Lesezeichen galt nicht (\(grund)\(uebergang)) und liegt als „\(rettung)“ neben "
