...
extension Planungsspeicher {
    ...
    private func klartextFragen(_ anzahl: Int, bilanz: Sitzplandatei.Bilanz, kopie: String?) {
        var text = "\(anzahl) \(anzahl == 1 ? "Sitzplan liegt" : "Sitzpläne liegen") im Klartext neben der "
            + "versiegelten Planung — etwa aus einem Einschalten der Verschlüsselung, das nicht zu Ende kam. "
    ...
    private func sitzplanbefundMelden(_ befund: Sitzplandienst.Ladebefund) {
        ...
        case .geladen, .keine:
            break
        case .nachgezogen(let anzahl):
            melden("\(anzahl == 1 ? "Der Sitzplan trug" : "Die Sitzpläne trugen") noch eine ältere Hülle und "
                   + "\(anzahl == 1 ? "ist" : "sind") neu versiegelt — die bisherige Passphrase öffnet "
                   + "\(anzahl == 1 ? "ihn" : "sie") nicht mehr.")
        ...
        case .beiseitegelegt(let grund, let rettung):
            // Unter einem anderen Schlüssel heißt meist: ein Übergang (Einschalten,
            // Erneuern, Aufheben), der nicht zu Ende kam — das Restrisiko aus E37.
            let uebergang = grund.contains("Schlüssel")
                ? " — womöglich aus einem Übergang der Verschlüsselung, der nicht zu Ende kam"
                : ""
            melden("Die Sitzpläne galten nicht (\(grund)\(uebergang)) und liegen als „\(rettung)“ neben der "
