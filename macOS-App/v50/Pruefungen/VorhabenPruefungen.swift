...
struct KursdateiPruefungen {
    ...
    func alteDatei() throws {
        let roh = """
        {"typ":"unterrichtsplanung","version":2,"wochen":10,"start":"2026-08-10",
    ...
    @Test("Drei Verwaltungszeilen je Klasse, immer — gesetzt oder als Angebot (E34)")
    func kopfhoehe() throws {
        var klasse = Klasse(id: "k1", name: "5a", fach: "Biologie", notiz: "",
                            farbe: 0, farbeManuell: false)
        let leer = Zellenmass.kopfhoehe(klasse)
        #expect(Zellenmass.verwaltungszeilen == Kursdateiart.allCases.count + 1)
        #expect(leer >= Double(Zellenmass.verwaltungszeilen) * Zellenmass.hoeheVerwaltungszeile)

        // Eine gesetzte Datei ersetzt ihr Angebot — die Zeile bleibt, die Höhe auch.
        klasse.verwaltung = "Noten.numbers"
        #expect(Zellenmass.kopfhoehe(klasse) == leer)
        klasse.curriculum = "Lehrplan.pdf"
        #expect(Zellenmass.kopfhoehe(klasse) == leer)

        var ohneFach = klasse
        ohneFach.fach = ""
        #expect(Zellenmass.kopfhoehe(ohneFach) < leer, "nur Name, Fach und Notiz messen sich")

        // Die Angebote, wie sie in der Kurszelle stehen (E35 (b)).
        #expect(Kursdateiart.verwaltung.angebot == "+ Verwaltungsdatei")
        #expect(Kursdateiart.curriculum.angebot == "+ Curriculum")
        #expect(Kursdateiart.verwaltung.dateibeschriftung == "Verwaltungsdatei")
    }
}
