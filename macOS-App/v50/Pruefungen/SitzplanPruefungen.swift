...
struct SitzplanPruefungen {
    ...
    func dienstBehaelter() throws {
        ...
        let t = try tresor()
        #expect(dienst.versiegeln(unter: t) == nil)
    ...
    func dienstSonderfaelle() throws {
        ...
        #expect(gesperrt.versiegeln(unter: t) != nil && !gesperrt.entsiegeln())
    ...
    @Test("Dienst: Erneuern vorab — misslingt das Schreiben, gilt der bisherige Schlüssel weiter; Klartext bleibt Klartext")
    func dienstVorab() throws {
        let ort = try ordner("vorab")
        defer { aufraeumen(ort) }
        let ablage = Ablage(ordner: ort)
        let dienst = Sitzplandienst(ablage: ablage)
        _ = dienst.laden(stempel: "s")
        ...
        let alt = try tresor()
        #expect(dienst.versiegeln(vorab: alt) == nil)
        // Ein Ordner im Weg: Die Datei lässt sich nicht schreiben.
        ...
        let neu = try tresor()
        let grund = dienst.versiegeln(vorab: neu)
        #expect(grund != nil)
        #expect(dienst.quelle == .behaelter && dienst.plan(fuer: "k-eins") != nil, "die Pläne gelten weiter")
        try FileManager.default.removeItem(at: ablage.sitzplaene)
        // Der nächste Schreibanlass geht unter dem bisherigen Schlüssel.
        dienst.setzen(plan(4), fuer: "k-zwei")
        #expect(try kopf(ablage.sitzplaene).kennung == alt.kennung, "unter dem alten Schlüssel")
        #expect(dienst.ungesichert == nil)

        // Aus dem Klartext: Misslingt das Versiegeln, bleibt es Klartext.
        let ort2 = try ordner("vorab2")
        defer { aufraeumen(ort2) }
        let ablage2 = Ablage(ordner: ort2)
        let klartext = Sitzplandienst(ablage: ablage2)
        _ = klartext.laden(stempel: "s")
        klartext.setzen(plan(2), fuer: "k-eins")
        try FileManager.default.removeItem(at: ablage2.sitzplaene)
        try FileManager.default.createDirectory(at: ablage2.sitzplaene, withIntermediateDirectories: false)
        #expect(klartext.versiegeln(vorab: neu) != nil)
        #expect(klartext.quelle == .klartext && klartext.plan(fuer: "k-eins") != nil)
    }
    ...
    func vorschau() throws {
        ...
        #expect(!FileManager.default.fileExists(atPath: ablage.sitzplaene.path))
        #expect(dienst.versiegeln(unter: try tresor()) == nil)
        #expect(!FileManager.default.fileExists(atPath: ablage.sitzplaene.path))
    }
    ...
    @Test("Einschalten ohne Sitzpläne: nichts nötig, keine Datei; Erneuern scheitert am Sitzplan-Behälter ⇒ zurückgenommen, Lesezeichen und Ablage beim alten Schlüssel")
    func erneuernScheitert() throws {
        ...
        #expect(grund.contains("Sitzpläne"))
        ...
        #expect(try kopf(ablage.lesezeichen).kennung == alt.kennung, "die Lesezeichen kehren zurück")
    ...
    @Test("B02: Aufheben legt die Sitzpläne zuerst im Klartext hin — scheitert das, bleibt der Behälter unter dem Schlüssel")
    func entsiegelnVorab() throws {
        let ort = try ordner("aufheben")
        defer { aufraeumen(ort) }
        let ablage = Ablage(ordner: ort)
        let dienst = Sitzplandienst(ablage: ablage)
        _ = dienst.laden(stempel: "s")
        ...
        try FileManager.default.setAttributes([.posixPermissions: 0o500], ofItemAtPath: ort.path)
        defer { try? FileManager.default.setAttributes([.posixPermissions: 0o700], ofItemAtPath: ort.path) }
        #expect(dienst.entsiegelnVorab() != nil)
        #expect(dienst.quelle == .behaelter && dienst.plan(fuer: "k-eins")?.tische.count == 3)
        #expect(Tresor.istBehaelter(try Data(contentsOf: ablage.sitzplaene)), "der Behälter liegt unverändert")
        try FileManager.default.setAttributes([.posixPermissions: 0o700], ofItemAtPath: ort.path)
        #expect(dienst.entsiegelnVorab() == nil)
        let klartext = try Data(contentsOf: ablage.sitzplaene)
        #expect(dienst.quelle == .klartext && !Tresor.istBehaelter(klartext))
        #expect(dienst.entsiegelnVorab() == nil, "Klartext: nichts zu tun")
    }

    @Test("B02/E38: der Speicher nimmt das Aufheben zurück, wenn die Sitzpläne nicht in den Klartext kommen — und weist es bei gesperrtem Dienst ab")
    ...
    @Test("B09: eine Nebendatei unter der alten Hülle wird beim nächsten Start nachgezogen — die bisherige Passphrase öffnet sie nicht mehr")
    func huelleNachgezogen() async throws {
        ...
        let geaendert = try s.passphraseAendern(alt: passphrase, neu: neu)
        #expect(geaendert.ablage == .geschrieben)
        guard case .offen(let grund) = geaendert.sitzplaene else {
            Issue.record("erwartet offen, war \(geaendert.sitzplaene)")
            return
        }
        #expect(grund.contains("bisherige Passphrase"))
        // Noch trägt die Datei die alte Hülle.
        _ = try Tresor.oeffnen(kopf: try kopf(ablage.sitzplaene), passphrase: passphrase)
        ...
        #expect(zweiter.meldungen.contains { $0.text.contains("neu versiegelt") })
    }
    ...
    @Test("B02: Rücknahme beim Einschalten aus dem Klartext — die Sitzpläne liegen danach wieder im Klartext")
    ...
    /// Ein Stand, dessen Klartext unter der Lesegrenze liegt, dessen Behälter
    /// (Base64 im JSON, gut ein Drittel größer) aber darüber — das Versiegeln
    /// scheitert vor dem Schreiben, der Klartext liegt noch.
    private func grosserStand() -> [String: Sitzplan] {
        var plaene: [String: Sitzplan] = [:]
        let namen = (0..<35).map { String(repeating: "n", count: 96) + String(format: "%03d", $0) }
        for i in 0..<170 { plaene["k-\(i)"] = Sitzplan.anordnen(klasseId: "k-\(i)", namen: namen) }
        return plaene
    }

    @Test("B17: Rücknahme nach dem Aufheben — der Behälter bleibt maßgeblich; scheitert das Versiegeln vor dem Schreiben, wird die Klartextdatei entfernt, der Stand gilt für die Sitzung und wird nachgeholt")
    func versiegelnZurueck() throws {
        let ort = try ordner("zurueck")
        defer { aufraeumen(ort) }
        let ablage = Ablage(ordner: ort)
        let klartext = try Sitzplandatei.schreiben(grosserStand())
        #expect(klartext.count < Sitzplandienst.hoechstgroesse)
        try klartext.write(to: ablage.sitzplaene)
        let dienst = Sitzplandienst(ablage: ablage)
        #expect(dienst.laden(stempel: "s") == .geladen(170))
        let grund = try #require(dienst.versiegeln(zurueck: try tresor()))
        #expect(grund.contains("Lesegrenze") && grund.contains("Klartextdatei wurde entfernt"))
        #expect(dienst.quelle == .behaelter && dienst.schreibbar && dienst.ungesichert != nil)
        #expect(dienst.plan(fuer: "k-1")?.tische.count == 35, "die Pläne gelten für die Sitzung")
        #expect(!FileManager.default.fileExists(atPath: ablage.sitzplaene.path), "kein Klartext auf der Platte")
        // Das Hindernis fällt (weniger Pläne): das nächste Schreiben versiegelt.
        dienst.entfernen(klassen: Set((5..<170).map { "k-\($0)" }))
        #expect(dienst.ungesichert == nil)
        #expect(Tresor.istBehaelter(try Data(contentsOf: ablage.sitzplaene)))
        #expect(dienst.plan(fuer: "k-1")?.tische.count == 35)
    }

    @Test("B17: bleibt die Klartextdatei im Weg (Ordner gesperrt), sagt es der Grund — nachholen() versiegelt sie später")
    func versiegelnZurueckGesperrt() throws {
        let ort = try ordner("zurueck-gesperrt")
        defer { aufraeumen(ort) }
        let ablage = Ablage(ordner: ort)
        try Sitzplandatei.schreiben(["k-eins": plan(4)]).write(to: ablage.sitzplaene)
        let dienst = Sitzplandienst(ablage: ablage)
        #expect(dienst.laden(stempel: "s") == .geladen(1))
        try FileManager.default.setAttributes([.posixPermissions: 0o500], ofItemAtPath: ort.path)
        defer { try? FileManager.default.setAttributes([.posixPermissions: 0o700], ofItemAtPath: ort.path) }
        let grund = try #require(dienst.versiegeln(zurueck: try tresor()))
        #expect(grund.contains("ließ sich nicht entfernen"))
