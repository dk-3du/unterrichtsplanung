    ...
    private var fuss: some View {
        ...
        case .passphraseAendern:
            ...
            Button("Ändern") { aendern() }
                .buttonStyle(.glassProminent)
                .keyboardShortcut(.defaultAction)
                .disabled(!Passphrasenwahl.vollstaendig(passphrase, wiederholung, bisherige: bisherige))
        }
    }
    ...
    private func weiter(erneuern: Bool) {
        ...
        do {
            let schluessel = erneuern
                ? try speicher.schluesselErneuernVorbereiten(alt: bisherige, neu: passphrase)
