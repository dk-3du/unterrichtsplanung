...
extension Planungsspeicher {
    ...
    func verschluesselungAufheben() -> Schutzergebnis {
        ...
        if case .zurueckgenommen = ergebnis.ablage {
            ...
            if !sicherung.istVorschau, let grund = sitzplaene.versiegeln(unter: tresor) {
                ergebnis.sitzplaene = .offen(grund.ohneSchlusspunkt
                    + " — die Sitzpläne liegen im Klartext; die App versucht es beim nächsten Schreiben, "
                    + "beim Beenden und beim nächsten Start erneut")
            }
