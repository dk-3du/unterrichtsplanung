...
struct SitzplanPruefungen {
    ...
    func dienstSonderfaelle() throws {
        ...
        // Klartext neben der versiegelten Ablage wird beim Öffnen versiegelt.
        try Sitzplandatei.schreiben(["k-eins": plan(4)]).write(to: ablage.sitzplaene)
        let nachholer = Sitzplandienst(ablage: ablage)
        #expect(nachholer.oeffnen(mit: t, stempel: "c") == .nachgeholt(1))
    ...
    @Test("B05: Nachholen mit scheiterndem Schreiben meldet nicht „versiegelt“ — geladen, ungesichert; nachholen() holt es nach")
    func nachgeholtScheitert() throws {
        ...
        let dienst = Sitzplandienst(ablage: ablage)
        #expect(dienst.oeffnen(mit: try tresor(), stempel: "s") == .geladen(1))
