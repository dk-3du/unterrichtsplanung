...
struct AblagePruefungen {
    ...
    func nebendateienBilanz() throws {
        ...
        let zurueck = ablage.altbestaendeEntsiegeln(tresor, hoechstens: Planungsdatei.hoechstgroesse)
    ...
    @Test("Eine übergroße Nebendatei wird nicht gelesen, sondern benannt — auf Hin- und Rückweg")
    func nebendateiGebunden() throws {
        ...
        let zurueck = ablage.altbestaendeEntsiegeln(tresor, hoechstens: 4_096)
        #expect(zurueck.umgestellt == ["planung-beschaedigt-1.json"])
        #expect(zurueck.uebrig.map(\.name) == [gross.lastPathComponent])
    }
