...
if [ -d "$PAKET" ]; then
  ...
  # Der Prüfvermerk aus ./pruefen.sh (E39): ein Hinweis, keine Sperre — die
  # Prüfungen laufen vor dem Bau; der Vermerk gilt, wenn er zum Stand der
  # Quellen passt (nicht zur Uhr: gebaut wird nach dem Prüfen).
  ...
  elif [ "$(sed -n 's/^Quellenstand: //p' "$VERMERK")" != "$("$HIER/pruefen.sh" --stand 2>/dev/null || true)" ]; then
    echo "  · Prüfvermerk $(basename "$VERMERK") passt nicht zum Stand der Quellen — seit der Prüfung wurde geändert; ./pruefen.sh erneut"
  else
    ok "Prüfvermerk $(basename "$VERMERK") passt zum Stand der Quellen, ohne Befund"
  fi
