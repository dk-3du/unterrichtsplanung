...
struct UebergangPruefungen {
    ...
    func markeUnbrauchbarNachTeilweisemEinsetzen() throws {
        ...
        // Beide Marken sind nicht mehr zu lesen — welche Dateien zur Generation
        // gehören, ist damit unbekannt (seit E68 trägt sonst die Zweitschrift).
        try schreiben("{\"typ\":\"unterrichtsplanung-uebergang\",\"version\":9", Uebergangsdienst.markenname, in: ordner)
        try schreiben("{\"typ\":\"unterrichtsplanung-uebergang\",\"version\":9", Uebergangsdienst.zweitschriftname, in: ordner)
    ...
    @Test("Abbruch nach dem Vorbereiten: Zwillinge ohne Marke sperren, und nichts wird angerührt (E73)")
    func abbruchNachVorbereiten() throws {
        ...
        // Bis v54 galt: keine Marke, also hat nie etwas gegolten — fort mit den
        // Zwillingen. Ob die Marke nie geschrieben wurde oder nur nicht mehr
        // liegt, sagt der Ordner jedoch nicht; der Nutzer entscheidet über den
        // Ausweg im Menü (E73).
        let befund = Uebergangsdienst(ordner: ordner).wiederanlaufen(stempel: "w")
        guard case .blockiert = befund else {
            Issue.record("erwartet: blockiert — \(befund)")
            return
        }
        #expect(try namen(in: ordner) == vorher, "nichts wurde angerührt")
        try alterStandPruefen(in: ordner)
    }

    @Test("Beide Marken fort, eingesetzt war eine neu angelegte Datei: gesperrt, und der Zwilling bleibt (N53-02, E73)")
    func beideMarkenFortNeuAngelegteDatei() throws {
        let ordner = try ordner("ohne-marke-neu")
        defer { try? FileManager.default.removeItem(at: ordner) }
        // b.json liegt und wartet auf seinen Zwilling; c.json gab es vor dem
        // Übergang nicht und ist schon eingesetzt — einen Vorgänger hinterlässt
        // das nicht, an dem sich das erkennen ließe.
        try schreiben("b-alt", "b.json", in: ordner)
        let dienst = Uebergangsdienst(ordner: ordner)
        #expect(dienst.vorbereiten(.erneuern, kennung: Data([5]), stempel: "s",
                                   dateien: ["b.json": .daten(Data("b-neu".utf8)),
                                             "c.json": .daten(Data("c-neu".utf8))]) == nil)
        #expect(dienst.uebergeben() == nil)
        try FileManager.default.moveItem(at: Uebergangsdienst.zwilling("c.json", in: ordner),
                                         to: ordner.appending(component: "c.json"))
        for name in [Uebergangsdienst.markenname, Uebergangsdienst.zweitschriftname] {
            try FileManager.default.removeItem(at: ordner.appending(component: name))
        }
        let vorher = try namen(in: ordner)

        let befund = Uebergangsdienst(ordner: ordner).wiederanlaufen(stempel: "w")
        guard case .blockiert = befund else {
            Issue.record("zwei Stände nebeneinander, und der Zwilling wäre der einzige Weg zurück — \(befund)")
            return
        }
        #expect(try namen(in: ordner) == vorher, "nichts wurde angerührt")
        #expect(liegt("b.json.uebergang", in: ordner), "der Zwilling von b.json bleibt")
    }
    ...
    func zwillingWeichtAb() throws {
        ...
        #expect(befund == .beiseitegelegt(.einschalten, grund: "„c.json“ weicht ab",
                                          rettung: ["uebergang-w.json", "uebergang-zweitschrift-w.json", "a-uebergang-w.json", "c-uebergang-w.json"]))
        try alterStandPruefen(in: ordner)
        #expect(try namen(in: ordner) == ["a-uebergang-w.json", "a.json", "b.json", "c-uebergang-w.json", "uebergang-w.json", "uebergang-zweitschrift-w.json"])
    ...
    func zwillingFehlt() throws {
        ...
        #expect(art == .passphrase && grund == "„c.json“ fehlt" && rettung == ["uebergang-w.json", "uebergang-zweitschrift-w.json", "a-uebergang-w.json"])
        try alterStandPruefen(in: ordner)
    }

    @Test("Die Marke ist nicht lesbar: allein kommt sie beiseite, neben einem Zwilling blockiert sie (N53-02)")
    func markeUnlesbar() throws {
        ...
        // Ein Zwilling ist eine Spur: Ob schon eine neu angelegte Datei
        // eingesetzt wurde, ist ohne deutbare Marke nicht zu sagen.
        let vorher = try namen(in: ordner)
        guard case .blockiert(let art, let grund) = Uebergangsdienst(ordner: ordner).wiederanlaufen(stempel: "w") else {
            Issue.record("erwartet: blockiert")
            return
        }
        #expect(art == nil && grund.hasPrefix("die Marke ist nicht lesbar ("))
        #expect(try namen(in: ordner) == vorher, "nichts wurde angerührt")

        // Ohne Zwilling und ohne Vorgänger war nichts vorbereitet: Die Marke
        // allein geht ins Register.
        try FileManager.default.removeItem(at: ordner.appending(component: "a.json.uebergang"))
        guard case .beiseitegelegt(_, let grund2, let rettung) = Uebergangsdienst(ordner: ordner).wiederanlaufen(stempel: "w2") else {
            Issue.record("erwartet: beiseitegelegt, eingesetzt war nichts")
            return
        }
        #expect(grund2.hasPrefix("die Marke ist nicht lesbar (") && rettung == ["uebergang-w2.json"])
        try alterStandPruefen(in: ordner)
        #expect(try namen(in: ordner) == ["a.json", "b.json", "uebergang-w2.json"])
    }
    ...
    func markeUnzulaessig() throws {
        ...
        for (fall, text) in faelle {
            ...
            guard case .blockiert(let art, let grund) = Uebergangsdienst(ordner: ordner).wiederanlaufen(stempel: fall) else {
                Issue.record("\(fall): erwartet blockiert — die Marke ist nicht zu deuten, ein Zwilling liegt")
                continue
            }
            #expect(art == nil && grund.hasPrefix("die Marke ist nicht lesbar ("), "\(fall): \(grund)")
            try alterStandPruefen(in: ordner)
            try FileManager.default.removeItem(at: ordner.appending(component: Uebergangsdienst.markenname))
            try FileManager.default.removeItem(at: ordner.appending(component: "a.json.uebergang"))
        }
    ...
    func vorgaengerWaehrendDesEinsetzens() throws {
        ...
        let zweite = Uebergangsdienst.zweitschriftname
        #expect(staende.first == ["a.json", "a.json.vorgaenger", "b.json", "c.json.uebergang", zweite, "uebergang.json"], "\(staende)")
        #expect(staende.dropFirst().first == ["a.json", "a.json.vorgaenger", "b.json.vorgaenger", "c.json.uebergang", zweite, "uebergang.json"], "\(staende)")
        #expect(staende.last == ["a.json", "a.json.vorgaenger", "b.json.vorgaenger", "c.json", zweite, "uebergang.json"], "\(staende)")
        try neuerStandPruefen(in: ordner)
    }
    ...
    func rueckwegUeberVorgaenger() throws {
        ...
        let befund = Uebergangsdienst(ordner: ordner).wiederanlaufen(stempel: "w")
        #expect(befund == .zurueckgenommen(.erneuern, grund: "„c.json“ weicht ab", rettung: ["uebergang-w.json", "uebergang-zweitschrift-w.json", "c-uebergang-w.json"]), "\(befund)")
        try alterStandPruefen(in: ordner)
        #expect(try namen(in: ordner) == ["a.json", "b.json", "c-uebergang-w.json", "uebergang-w.json", "uebergang-zweitschrift-w.json"])
        #expect(Uebergangsdienst(ordner: ordner).wiederanlaufen(stempel: "w2") == .nichts)
    }
    ...
    func rueckwegGrenzen() throws {
        ...
        #expect(b1 == .zurueckgenommen(.passphrase, grund: "„a.json“ weicht ab", rettung: ["uebergang-w.json", "uebergang-zweitschrift-w.json", "a-uebergang-w.json"]), "\(b1)")
        try alterStandPruefen(in: eins)
        #expect(try namen(in: eins) == ["a-uebergang-w.json", "a.json", "b.json", "uebergang-w.json", "uebergang-zweitschrift-w.json"])
    ...
    // ── Die Beweislage (Phase O) ──────────────────────────────────────────

    @Test("Marke entfernt statt beschädigt, nach teilweisem Einsetzen: blockiert — nichts verwahrt, nichts geräumt (N53-02)")
    func markeFehltNachTeilweisemEinsetzen() throws {
        let ordner = try ordner("markefehlt")
        defer { try? FileManager.default.removeItem(at: ordner) }
        try teilweiseEingesetzt(in: ordner)
        // Erst die Marke allein: Die Zweitschrift trägt, der Wiederanlauf
        // vollendet nicht — er hat einen beschädigten Zwilling — aber er weiß,
        // wovon er spricht (E68).
        try FileManager.default.removeItem(at: ordner.appending(component: Uebergangsdienst.markenname))
        #expect(Uebergangsdienst.spur(in: ordner) != nil)
        // Und nun auch die Zweitschrift: Jetzt ist nicht mehr zu sagen, was zur
        // Generation gehörte.
        try FileManager.default.removeItem(at: ordner.appending(component: Uebergangsdienst.zweitschriftname))
        let vorher = try namen(in: ordner)

        let erster = Uebergangsdienst(ordner: ordner).wiederanlaufen(stempel: "w")
        guard case .blockiert = erster else {
            Issue.record("erwartet: blockiert — \(erster)")
            return
        }
        #expect(try namen(in: ordner) == vorher, "nichts verwahrt, nichts geräumt")
        #expect(inhalt("a.json.vorgaenger", in: ordner) == "a-alt")
        #expect(inhalt("b.json.uebergang", in: ordner) == "b-neu")

        let zweiter = Uebergangsdienst(ordner: ordner).wiederanlaufen(stempel: "w2")
        guard case .blockiert = zweiter else {
            Issue.record("auch der zweite Start bleibt blockiert — \(zweiter)")
            return
        }
        #expect(try namen(in: ordner) == vorher)
    }

    @Test("Eine Aufzählung, die scheitert, ist kein sauberer Ordner (N53-02)")
    func aufzaehlungScheitert() throws {
        let ordner = try ordner("aufzaehlung")
        defer {
            try? FileManager.default.setAttributes([.posixPermissions: 0o700], ofItemAtPath: ordner.path)
            try? FileManager.default.removeItem(at: ordner)
        }
        try alterStand(in: ordner)
        try schreiben("a-neu", "a.json.uebergang", in: ordner)
        try schreiben("a-alt", "a.json.vorgaenger", in: ordner)
        // Nur ausführbar: Bekannte Dateien lassen sich öffnen, der Ordner aber
        // nicht auflisten.
        try FileManager.default.setAttributes([.posixPermissions: 0o100], ofItemAtPath: ordner.path)

        let befund = Uebergangsdienst(ordner: ordner).wiederanlaufen(stempel: "w")
        try FileManager.default.setAttributes([.posixPermissions: 0o700], ofItemAtPath: ordner.path)
        if case .nichts = befund { Issue.record("ein unlesbarer Ordner ist kein leerer: \(befund)") }
        if case .vollendet = befund { Issue.record("\(befund)") }
        #expect(liegt("a.json.uebergang", in: ordner) && liegt("a.json.vorgaenger", in: ordner),
                "nichts wurde angerührt")
    }

    @Test("Eine neu angelegte Datei hinterlässt keinen Vorgänger — das Fehlen belegt nichts (N53-02)")
    func neuAngelegteDateiOhneVorgaenger() throws {
        let ordner = try ordner("neuangelegt")
        defer { try? FileManager.default.removeItem(at: ordner) }
        try schreiben("a-alt", "a.json", in: ordner)
        let dienst = Uebergangsdienst(ordner: ordner)
        // „neu.json“ gab es vorher nicht; sie steht in der Reihenfolge hinter
        // „a.json“, wird hier aber als erste eingesetzt.
        #expect(dienst.vorbereiten(.einschalten, kennung: Data([8]), stempel: "s",
                                   dateien: ["a.json": .daten(Data("a-neu".utf8)),
                                             "neu.json": .daten(Data("ganz neu".utf8))]) == nil)
        #expect(dienst.uebergeben() == nil)
        try FileManager.default.moveItem(at: ordner.appending(component: "neu.json.uebergang"),
                                         to: ordner.appending(component: "neu.json"))
        // Beide Marken unbrauchbar: Erst dann ist nicht mehr zu sagen, was zur
        // Generation gehörte — und ein Vorgänger liegt nicht, weil „neu.json“
        // vorher nicht da war.
        try schreiben("{\"typ\":\"unterrichtsplanung-uebergang\",\"version\":9", Uebergangsdienst.markenname, in: ordner)
        try schreiben("{\"typ\":\"unterrichtsplanung-uebergang\",\"version\":9", Uebergangsdienst.zweitschriftname, in: ordner)
        #expect(!liegt("neu.json.vorgaenger", in: ordner) && !liegt("a.json.vorgaenger", in: ordner),
                "kein Vorgänger im Ordner")
        let vorher = try namen(in: ordner)

        let befund = Uebergangsdienst(ordner: ordner).wiederanlaufen(stempel: "w")
        guard case .blockiert = befund else {
            Issue.record("erwartet: blockiert — es liegt eine Spur, auch ohne Vorgänger (\(befund))")
            return
        }
        #expect(try namen(in: ordner) == vorher, "nichts wurde angerührt")
        #expect(inhalt("neu.json", in: ordner) == "ganz neu", "die eingesetzte neue Datei liegt noch")
    }

    @Test("Die Zweitschrift der Marke: unlesbare Marke, heile Zweitschrift — der Wiederanlauf vollendet (E68)")
    func zweitschriftTraegt() throws {
        let ordner = try ordner("zweitschrift")
        defer { try? FileManager.default.removeItem(at: ordner) }
        try alterStand(in: ordner)
        let dienst = Uebergangsdienst(ordner: ordner)
        #expect(dienst.vorbereiten(.erneuern, kennung: Data([6]), stempel: "s", dateien: generation) == nil)
        #expect(dienst.uebergeben() == nil)
        #expect(liegt(Uebergangsdienst.zweitschriftname, in: ordner), "die Zweitschrift liegt")
        try schreiben("unbrauchbar", Uebergangsdienst.markenname, in: ordner)

        #expect(Uebergangsdienst(ordner: ordner).wiederanlaufen(stempel: "w") == .vollendet(.erneuern))
        try neuerStandPruefen(in: ordner)
        #expect(!liegt(Uebergangsdienst.zweitschriftname, in: ordner), "die Zweitschrift geht mit der Marke")
    }

    @Test("Beide Marken unbrauchbar: blockiert, wenn eine Spur liegt (E68)")
    func zweitschriftEbenfallsUnbrauchbar() throws {
        let ordner = try ordner("zweitschrift-fehler")
        defer { try? FileManager.default.removeItem(at: ordner) }
        try teilweiseEingesetzt(in: ordner)
        try schreiben("unbrauchbar", Uebergangsdienst.markenname, in: ordner)
        try schreiben("unbrauchbar", Uebergangsdienst.zweitschriftname, in: ordner)
        let vorher = try namen(in: ordner)
    ...
    // ── Zug 1 von Phase P: die beiden falschen Sperren ────────────────────

    @Test("Einen Ordner, den es noch nicht gibt, gibt es nicht zu lesen (N54-02)")
    func ordnerGibtEsNochNicht() throws {
        let ziel = URL.temporaryDirectory.appending(component: "erststart-\(UUID().uuidString)",
                                                    directoryHint: .isDirectory)
        #expect(!FileManager.default.fileExists(atPath: ziel.path), "der Ordner liegt bewusst nicht")
        let befund = Uebergangsdienst.wiederanlaufen(ordner: ziel, stempel: "w")
        #expect(befund == .nichts, Comment(rawValue: "erwartet: nichts — \(befund)"))
        #expect(!FileManager.default.fileExists(atPath: ziel.path), "der Wiederanlauf legt nichts an")
    }

    @Test("Der Ausweg scheitert nicht an einem Ordner, den es noch nicht gibt (N54-02, E67)")
    func auswegOhneOrdner() throws {
        let ziel = URL.temporaryDirectory.appending(component: "erststart-ausweg-\(UUID().uuidString)",
                                                    directoryHint: .isDirectory)
        #expect(try Uebergangsdienst.spuren(in: ziel).isEmpty)
        #expect(try Uebergangsdienst.blockadeRaeumen(in: ziel, stempel: "w").isEmpty)
        #expect(Uebergangsdienst.spur(in: ziel) == nil)
    }

    @Test("Nur die Marke fort, die Zweitschrift trägt: derselbe Start vollendet (N54-03)")
    func markeFehltZweitschriftLiegt() throws {
        let ordner = try ordner("marke-fehlt")
        defer { try? FileManager.default.removeItem(at: ordner) }
        try alterStand(in: ordner)
        let dienst = Uebergangsdienst(ordner: ordner)
        #expect(dienst.vorbereiten(.erneuern, kennung: Data([8]), stempel: "s", dateien: generation) == nil)
        #expect(dienst.uebergeben() == nil)
        // Nicht unbrauchbar gemacht, sondern fort — eine Wiederherstellung, ein
        // Abgleichdienst oder eine fremde Hand kann das hinterlassen.
        try FileManager.default.removeItem(at: Uebergangsdienst.marke(in: ordner))

        let befund = Uebergangsdienst(ordner: ordner).wiederanlaufen(stempel: "w")
        #expect(befund == .vollendet(.erneuern),
                Comment(rawValue: "eingesetzt wurde alles — das Aufräumen darf nicht sperren: \(befund)"))
        try neuerStandPruefen(in: ordner)
        #expect(!liegt(Uebergangsdienst.zweitschriftname, in: ordner))
    }

    @Test("Ein zweiter Start nach dem Vollenden ohne Marke findet nichts mehr (N54-03)")
    func zweiterStartNachMarkeFehlt() throws {
        let ordner = try ordner("marke-fehlt-zweimal")
        defer { try? FileManager.default.removeItem(at: ordner) }
        try alterStand(in: ordner)
        let dienst = Uebergangsdienst(ordner: ordner)
        #expect(dienst.vorbereiten(.erneuern, kennung: Data([8]), stempel: "s", dateien: generation) == nil)
        #expect(dienst.uebergeben() == nil)
        try FileManager.default.removeItem(at: Uebergangsdienst.marke(in: ordner))
        _ = Uebergangsdienst(ordner: ordner).wiederanlaufen(stempel: "w")

        #expect(Uebergangsdienst(ordner: ordner).wiederanlaufen(stempel: "x") == .nichts)
        try neuerStandPruefen(in: ordner)
    }

    @Test("Vorgänger ohne Marke: blockiert, und nichts wird angerührt (E60, E68)")
    func vorgaengerOhneMarke() throws {
        ...
        // womöglich der einzige Weg zurück. Seit E68 wird es deshalb nicht
        // mehr verwahrt, sondern gar nicht erst angefasst.
        let befund = Uebergangsdienst(ordner: ordner).wiederanlaufen(stempel: "w")
        guard case .blockiert = befund else {
            Issue.record("ein Vorgänger ohne Marke ist kein Nichts: \(befund)")
            return
        }
        #expect(try namen(in: ordner) == vorher, "nichts wurde angerührt")
        #expect(inhalt("a.json.vorgaenger", in: ordner) == "uralt")
        try alterStandPruefen(in: ordner)
        #expect(!Uebergangsdienst.nameZulaessig("a.json.vorgaenger"), "kein Name der Generation")
        #expect(!Uebergangsdienst.nameZulaessig(Uebergangsdienst.zweitschriftname), "kein Name der Generation")
    }
