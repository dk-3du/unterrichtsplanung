...
struct RuecknahmeBearbeitenPruefungen {
    ...
    static let faelle: [Fall] = [
        ...
        Fall(name: Schrittname.standardlisteErgaenzen) { s in s.standardkurseErgaenzen() },
        ...
        // ── Sperrzeiten ───────────────────────────────────────────────────
        Fall(name: Schrittname.sperrzeitHinzufuegen) { s in s.sperrzeitHinzufuegen() },
    ...
    @MainActor
    @Test("Ein Kurs ohne Sitzplan trägt kein Beiwerk")
    func kursOhneSitzplanOhneBeiwerk() throws {
        try Pruefuhr.angehalten {
            let (s, ordner) = try RuecknahmeBearbeitenPruefungen.speicher()
            defer { try? FileManager.default.removeItem(at: ordner) }
            ...
            s.klasseEntfernen(klasse)
            s.rueckfrageBeantworten(true)
            #expect(s.verlauf.zurueck.last?.beiwerk == nil, "es gab nichts mitzunehmen")
            s.widerrufen()
