...
enum Widerrufprobe {
    ...
    static func laufenUndBeenden(_ speicher: Planungsspeicher) {
        Task { @MainActor in
            ...
            func fenster() -> NSWindow? { NSApp.windows.first { $0.isVisible } }
            ...
            func suchen<T: NSView>(_ ansicht: NSView?, _ art: T.Type) -> T? {
            ...
            func freigegeben(_ eintrag: NSMenuItem?) -> Bool {
            ...
            if let klasse = speicher.planung?.klassen.first {
                ...
                if let blatt = hauptfenster.attachedSheet,
                   let flaeche = suchen(blatt.contentView, Sitzplanansicht.self) {
                    ...
                    func maus(_ art: NSEvent.EventType, _ punkt: NSPoint) {
