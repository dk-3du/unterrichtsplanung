...
final class Tresor: @unchecked Sendable {
    ...
    // Eine Abkürzung, die die Kennung aus dem Kopf liest, gibt es hier nicht
    // mehr (N57-01, E89). Sie belegte die *Herkunft* einer Datei, nicht ihre
    // *Brauchbarkeit*: Eine Fassung, die diese App nicht kennt, ein unbekanntes
    // Verfahren und ein abgeschnittener Rumpf beginnen genauso. Wer freigeben
    // will, fragt `kopfLesen` — den Leser selbst. Damit fällt auch der letzte
    // zweite Erkenner weg, der dieses Projekt dreimal getroffen hat (N54-02,
    // N55-01, N56-01).
