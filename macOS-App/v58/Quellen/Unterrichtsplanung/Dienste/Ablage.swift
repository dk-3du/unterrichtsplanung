...
final class Ablage: @unchecked Sendable {
    ...
    /// Was der Kopf **allein** belegt — und das ist nur noch das Abweisen: Wo
    /// gar kein Objekt beginnt, liegt Klartext. Alles andere entscheidet die
    /// Regel des Lesers an der ganzen Datei.
    ///
    /// **Erkennen ist nicht Erlauben (N57-01, E89).** Bis v57 sprach der Kopf
    /// auch die Freigabe aus, sobald er Wort für Wort der eigene war und die
    /// Kennung nannte. Das ging zu weit: Damit belegt er die *Herkunft*, nicht
    /// die *Brauchbarkeit* — eine Fassung, die diese App nicht kennt, ein
    /// unbekanntes Verfahren und ein abgeschnittener Rumpf sehen im Kopf
    /// genauso aus. Am Lauf gemessen: Ein Behälter mit `"version":9` unter der
    /// eigenen Kennung wurde ersetzt (234 → 246 Byte), obwohl `kopfLesen` ihn
    /// abweist und dabei zusagt, die Datei bleibe unangetastet; derselbe
    /// Behälter mit einem führenden Zeilenumbruch wurde gesperrt. Die Freigabe
    /// hing an der Schreibweise, nicht an der Sache.
    ///
    /// Das Abweisen bleibt billig, und es ist die Richtung, in der eine
    /// Abkürzung nichts anrichten kann. Die Freigabe kostet jetzt eine volle
    /// Deutung: gemessen 0,49 ms bei einer gut gefüllten Planung (285 KB
    /// Behälter), 8,2 ms bei 5,3 MB — gegen 700 ms Entprellung und ≈ 5 ms für
    /// das Schreiben selbst. Klartext kostet unverändert 0,07 ms; dort las der
    /// Wächter auch vorher schon die ganze Datei.
    private static func ausDemKopf(_ kopf: Data) -> Form? {
        Tresor.kopfKoennteBehaelter(kopf) ? nil : .klartext
    }

    /// Die Regel des Lesers, auf vollständige Daten angewandt — **die einzige
    /// Stelle, die eine Freigabe belegt.** Was `kopfLesen` annimmt, ist ein
    /// Behälter samt seiner Kennung; was es abweist, ist unbrauchbar und wird
    /// nicht ersetzt — mit dem Wortlaut, den der Leser dafür hat.
