...
extension Planungsspeicher {
    ...
    func sitzplanEntfernen(klasse id: String, ort: Rueckfrageort,
        ...
        fragen("Den Sitzplan von „\(klasse.name)“ endgültig löschen?\n\n"
            ...
            // Vor dem Entfernen greifen — danach ist er fort; er reist als
            // Beiwerk eines Schrittes mit, damit ⌘Z ihn zurückholt (S6).
            let gewesen = sitzplaene.plan(fuer: id)
            ...
            if let gewesen {
                nurBeiwerkAnmelden(Schrittname.sitzplanEntfernen, beiwerk: .init(
                    ...
                    zurueck: { [weak self] in self?.sitzplanZurueck(gewesen, fuer: id) },
                    vor: { [weak self] in _ = self?.sitzplaene.setzen(nil, fuer: id) }))
            }
            danach?()
        }
    }
